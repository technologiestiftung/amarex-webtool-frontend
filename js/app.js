...
import { initiateVueI18Next } from "./vueI18Next.js";
import {
    handleUrlParamsBeforeVueMount,
    readUrlParamEarly,
} from "../src/utils/parametricUrl/ParametricUrlBridge";
...
async function loadApp() {
    ...
    styleGetters = {
        ...
        highlightFeaturesPointStyleId:
            store.getters["HighlightFeatures/pointStyleId"],
        highlightFeaturesPolygonStyleId:
            store.getters["HighlightFeatures/polygonStyleId"],
        highlightFeaturesLineStyleId:
            store.getters["HighlightFeatures/lineStyleId"],
    };
    ...
    styleList
        .initializeStyleList(
            styleGetters,
            Config,
            Radio.request("Parser", "getItemsByAttributes", { type: "layer" }),
            Radio.request("Parser", "getItemsByAttributes", { type: "tool" }),
                ...
                if (error) {
                    Radio.trigger("Alert", "alert", {
                        text:
                            "<strong>Die Datei '" +
                            Config.styleConf +
                            "' konnte nicht geladen werden!</strong>",
    ...
    if (Object.prototype.hasOwnProperty.call(Config, "zoomToFeature")) {
        console.warn(
            "The configuration parameter 'zoomToFeature' is deprecated in v3.0.0. Please use 'zoomTo' instead."
        );
        store.commit("ZoomTo/setConfig", {
            zoomToFeature: Config.zoomToFeature,
        });
        store.commit("ZoomTo/setDeprecatedParameters", true);
    }
    if (Object.prototype.hasOwnProperty.call(Config, "zoomToGeometry")) {
        console.warn(
            "The configuration parameter 'zoomToGeometry' is deprecated in v3.0.0. Please use 'zoomTo' instead."
        );
        store.commit("ZoomTo/setConfig", {
            zoomToGeometry: Config.zoomToGeometry,
        });
        store.commit("ZoomTo/setDeprecatedParameters", true);
    }
    ...
    Radio.request("ModelList", "getModelsByAttributes", {
        type: "tool",
    }).forEach((tool) => {
        switch (tool.id) {
            case "parcelSearch": {
    ...
    if (!style || style !== "SIMPLE") {
        controls = Radio.request("Parser", "getItemsByAttributes", {
            type: "control",
        });
                ...
                case "button3d": {
                    if (control.attr === true) {
                        if (global.Cesium === null) {
                            console.warn(
                                "Cesium/3D-Functionality could not be loaded - Please check your Cesium configuration"
                            );
                        } else {
    ...
    searchbarAttributes = Radio.request("Parser", "getItemsByAttributes", {
        type: "searchBar",
    })[0].attr;
    sbconfig = Object.assign(
        {},
        { quickHelp: store.getters.portalConfig?.quickHelp } || {}
    );
    ...
    if (Config.addons !== undefined) {
        ...
        const i18nextLanguages = vueI18Next?.i18next?.options?.getLanguages()
            ? vueI18Next.i18next.options.getLanguages()
            : {};
        let initCounter = 0;
                ...
                Object.keys(i18nextLanguages).forEach((lng) => {
                    import(
                        ...
                        .then(({ default: additionalLocales }) => {
                            vueI18Next.i18next.addResourceBundle(
                                lng,
                                "additional",
                                additionalLocales,
                                true
                        ...
                        .catch((error) => {
                            ...
                            console.warn(error);
                            console.warn(
                                "Translation files of addon " +
                                    addonKey +
                                    " could not be loaded or does not exist. Addon is not translated."
                            );
                            checkInitCounter(initCounter, legacyAddons);
                        });
                });
            ...
            import(
                ...
                .then((module) => {
                    ...
                    if (addon.model) {
                        ...
                        if (!model) {
                            console.warn(
                                "wrong configuration: addon " +
                                    addonKey +
                                    " is not in tools menu or cannot be called from somewhere in the view! Defined this in config.json."
                            );
                        } else {
                            addon.model.set(
                                "i18nextTranslate",
                                model.get("i18nextTranslate")
                            );
                        }
                        Radio.trigger(
                            "ModelList",
                ...
                .catch((error) => {
                    ...
                    Radio.trigger(
                        "Alert",
                        "alert",
                        "Entschuldigung, diese Anwendung konnte nicht vollständig geladen werden. Bitte wenden sie sich an den Administrator."
                    );
                });
        }
    });

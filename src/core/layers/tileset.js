        ...
        if (hiddenObjects[id]) {
            hiddenObjects[id].forEach((f) => {
                if (
                    ...
                    f instanceof Cesium.Cesium3DTilePointFeature
                ) {
...
TileSetLayer.prototype.setIsSelected = function (newValue, attr) {
    ...
    if (map && map.mode === "3D") {
        let isVisibleInMap = this.attributes
        ...
        if (this.get("isSelected") === true && this.has("hiddenFeatures")) {
            const tileSetModels = Radio.request(
                "ModelList",
            ...
            tileSetModels.forEach((model) =>
                model.hideObjects(this.get("hiddenFeatures"))
            );
        } else if (
            this.get("isSelected") === false &&
            this.has("hiddenFeatures")
        ) {

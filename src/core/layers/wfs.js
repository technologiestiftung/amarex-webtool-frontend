...
WFSLayer.prototype.createLayer = function (attrs) {
    const rawLayerAttributes = {
        ...
        options = {
            ...
            loadingParams: {
                xhrParameters: attrs.isSecured
                    ? { credentials: "include" }
                    : undefined,
                propertyname: this.getPropertyname(attrs) || undefined,
                // only used if loading strategy is all
                bbox: attrs.bboxGeometry
                    ? attrs.bboxGeometry.getExtent().toString()
                    : undefined,
            },
    ...
    return function (features) {
        ...
        if (attrs.bboxGeometry) {
            filteredFeatures = filteredFeatures.filter((feature) =>
                attrs.bboxGeometry.intersectsCoordinate(
                ...
                (geometryTypes, error) => {
                    if (error) {
                        store.dispatch(
                            ...
                            "<strong>" +
                                i18next.t(
                                    "common:modules.vectorStyle.styleObject.getGeometryTypeFromWFSFetchfailed"
                                ) +
                                "</strong> <br>" +
                                "<small>" +
                                i18next.t(
                                    "common:modules.vectorStyle.styleObject.getGeometryTypeFromWFSFetchfailedMessage"
                                ) +
                                "</small>"
                        );
                    }
            ...
            ?.then((legendInfos) => {
                if (styleObject.styleId === "default") {
                    ...
                    if (rules[0].conditions !== undefined && features) {
                        const uniqueLegendInformation =
                            this.filterUniqueLegendInfo(
                                features,
                                rules,
                                legendInfos
                            );
                        ...
                        createStyle
                            ...
                            .then((legendInfoObject) => {
                                this.setLegend(
                                    legendInfoObject.legendInformation
                                );
                            });
                    } else {
                        this.setLegend(legendInfos);
 ...
 */
WFSLayer.prototype.filterUniqueLegendInfo = function (
    features,
    rules,
    legendInfos
) {

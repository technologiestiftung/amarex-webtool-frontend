...
import findWhereJs from "../../../../utils/findWhereJs.js";
import api from "@masterportal/masterportalapi/src/maps/api";
import parseCesiumParameters from "../../../../utils/parseCesiumParameters.js";
...
export default {
    ...
    initView({ dispatch, getters, commit }) {
        const mapView = getters.getView,
            params = findWhereJs(mapView.get("options"), {
                resolution: mapView.getConstrainedResolution(
        ...
        mapView.on("change:center", function () {
            ...
            Radio.trigger("RemoteInterface", "postMessage", {
                centerPosition: mapView.getCenter(),
            });
        });
    ...
    changedResolutionCallback({ commit, getters }, evt) {
        const mapViewTarget = evt.target,
            ...
            constrainResolution = mapView.getConstrainedResolution(
                mapViewTarget.get(evt.key)
            ),
            params = findWhereJs(mapView.get("options"), {
                resolution: constrainResolution,
            });

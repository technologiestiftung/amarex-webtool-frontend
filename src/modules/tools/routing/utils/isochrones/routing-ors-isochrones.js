...
function routingOrsOptimizationMultiplicator(optimization) {
    switch (optimization) {
        ...
        default:
            throw new Error(
                "Missing optimization to multiplicator translation"
            );
    }
}
...
async function fetchRoutingOrsIsochrones({
    ...
    const serviceUrl = store.getters.getRestServiceById(
        ...
        url = `${serviceUrl}/v2/isochrones/${routingOrsSpeedProfile(
            speedProfile
        )}/geojson`,
        rangeValue =
            optimization === "TIME"
                ? state.isochronesSettings.timeValue
                : state.isochronesSettings.distanceValue,
        optimizationMultiplicator =
            routingOrsOptimizationMultiplicator(optimization),
        range = rangeValue * optimizationMultiplicator,
        interval =
            state.isochronesSettings.intervalValue * optimizationMultiplicator;
    let result = null,
        first = null,
        second = null,
        isochrones = null,
        ...
        if (transformCoordinates) {
            for (const coordinate of feature.geometry.coordinates[0]) {
                localCoordinates.push(
                    await transformCoordinatesToLocal(coordinate)
                );
            }
        }
        isochrones.addArea(

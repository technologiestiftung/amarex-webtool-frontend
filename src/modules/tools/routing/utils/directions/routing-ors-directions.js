...
function routingOrsPreference(preference, speedProfile) {
    const portalConfig = store.state.configJson.Portalconfig,
        pathToRoutingConfig =
            portalConfig?.menu?.routing ||
            portalConfig?.menu?.tools?.children?.routing,
        preferenceConfigs =
            pathToRoutingConfig?.directionsSettings?.customPreferences;
...
async function fetchRoutingOrsDirections({
    ...
    const serviceUrl = store.getters.getRestServiceById(
        ...
        url = `${serviceUrl}/v2/directions/${routingOrsSpeedProfile(
            speedProfile
        )}/geojson`,
    ...
    try {
        ...
        if (e.response?.status === 404) {
            throw new Error(
                i18next.t("common:modules.tools.routing.errors.noRouteFound")
            );
        }
        if (e.response && e.response.data && e.response.data.error) {
            if (e.response.data.error.code === 2003) {
                throw new Error(
                    i18next.t(
                        "common:modules.tools.routing.errors.avoidAreaBig"
                    )
                );
            }
        }
        throw new Error(
            i18next.t("common:modules.tools.routing.errors.errorRouteFetch")
        );
    }

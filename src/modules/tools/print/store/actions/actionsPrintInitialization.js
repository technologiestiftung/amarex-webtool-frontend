...
export default {
    ...
    setDpiList: function ({ state, commit }) {
        state.currentLayout.attributes.forEach((attribute, idx) => {
            if (attribute.name === "map") {
                commit(
                    "setDpiList",
                    state.currentLayout.attributes[idx].clientInfo
                        ?.dpiSuggestions || []
                );
            }
        });
    ...
    togglePostrenderListener: function ({ state, dispatch, commit }) {
        ...
        if (foundVectorTileLayers.length && state.active) {
            dispatch(
                ...
                i18next.t("common:modules.tools.print.vtlWarning"),
                { root: true }
            );
        }
    ...
    setOriginalPrintLayer: function ({ state, rootGetters }) {
        const invisibleLayer = state.invisibleLayer,
            ...
            resoByMaxScale = rootGetters["Maps/getResolutionByScale"](
                mapScale,
                "max"
            ),
            // eslint-disable-next-line new-cap
            resoByMinScale = rootGetters["Maps/getResolutionByScale"](
                mapScale,
                "min"
            );

        invisibleLayer.forEach((layer) => {
            const layerModel = Radio.request(
                "ModelList",
            ...
            if (
                resoByMaxScale <= layer.getMaxResolution() &&
                resoByMinScale > layer.getMinResolution()
            ) {
    ...
    setPrintLayers: function ({ state, dispatch, commit, rootGetters }, scale) {
        ...
        hintInfo = i18next.t("common:modules.tools.print.invisibleLayer", {
        ...
        if (
            ...
            hintInfo !== state.hintInfo &&
            state.showInvisibleLayerInfo
        ) {
    ...
    drawMask: function (_, drawMaskOpt) {
        const mapSize = drawMaskOpt.frameState.size,
            ...
            ration =
                drawMaskOpt.context.canvas.width > mapSize[0]
                    ? DEVICE_PIXEL_RATIO
                    : 1,
    ...
    drawPrintPage: function ({ state }, canvasPrintOptions) {
        const ration =
                canvasPrintOptions.context.canvas.width >
                canvasPrintOptions.mapSize[0]
                    ? DEVICE_PIXEL_RATIO
                    : 1,
            center = [
    ...
    getPrintMapSize: function ({ state, commit, dispatch }) {
        if (state.printService === "plotservice") {
            ...
            commit("setLayoutMapInfo", [
                layoutMapInfo.width,
                layoutMapInfo.height,
            ]);
        }
    },

...
export default {
    ...
    sendRequest: function ({ state, dispatch }, serviceRequest) {
        ...
        const url = state.useProxy
            ? getProxyUrl(serviceRequest.serviceUrl)
    ...
    startPrint3d: async function ({ state, dispatch, commit }, print) {
        ...
        if (state.isLegendAvailable) {
            spec.buildLegend(
                state.isLegendSelected,
                state.isMetadataAvailable,
                print.getResponse,
                print.index
            );
        } else {
    ...
    getGfiForPrint: async function ({ rootGetters, commit }) {
        if (
            rootGetters["Tools/Gfi/currentFeature"] !== null &&
            typeof rootGetters["Tools/Gfi/currentFeature"]
                .getMappedProperties === "function" &&
            typeof rootGetters["Tools/Gfi/currentFeature"].getTitle ===
                "function"
        ) {
            commit("setGfiForPrint", [
                rootGetters["Tools/Gfi/currentFeature"].getMappedProperties(),
                rootGetters["Tools/Gfi/currentFeature"].getTitle(),
    ...
    getMetaDataForPrint: async function ({ rootGetters, dispatch }, cswObject) {
        ...
        if (
            cswObj.layer.get("datasets") &&
            Array.isArray(cswObj.layer.get("datasets")) &&
            cswObj.layer.get("datasets")[0] !== null &&
            typeof cswObj.layer.get("datasets")[0] === "object"
        ) {
            cswObj.cswUrl = Object.prototype.hasOwnProperty.call(
                cswObj.layer.get("datasets")[0],
                "csw_url"
            )
                ? cswObj.layer.get("datasets")[0].csw_url
                : null;
        }
        ...
        if (rootGetters.metadata.useProxy?.includes(cswObj.cswUrl)) {
            metadata = await getRecordById(
                getProxyUrl(cswObj.cswUrl),
                cswObj.metaId
            );
        } else {
        ...
        if (typeof metadata === "undefined") {
            dispatch(
                ...
                i18next.t("common:modules.layerInformation.errorMessage", {
                    cswObjCswUrl: cswObj.cswUrl,
                }),
                { root: true }
            );
        } else {
    ...
    createPrintJob: async function (
        ...
        if (
            ...
            !state.serviceUrl.includes("/print/")
        ) {
        ...
        if (state.serviceUrl === "") {
            ...
            if (state.mapfishServiceId !== "") {
                serviceUrl = rootGetters.getRestServiceById(
                    state.mapfishServiceId
                ).url;
            } else {
        ...
        url =
            state.printService === "plotservice"
                ? serviceUrlDefinition + "/create.json"
                : serviceUrlDefinition + printId + "/report." + printFormat;

        commit("setProgressWidth", "width: 50%");
        if (typeof printJob.getResponse === "function") {
            if (state.printService === "plotservice") {
                printJob.payload = await dispatch(
                    "migratePayload",
                    printJob.payload
                );
            }
    ...
    migratePayload: function ({ state, rootState }, payload) {
        const plotservicePayload = {},
            ...
            payloadString = decodeURIComponent(encodedPayload)
                .replace(/imageFormat/g, "format")
                .replace(/image\/[^"]*/g, "image/png")
                .replace(/"TRANSPARENT":"false"/gi, '"TRANSPARENT":"true"'),
            ...
            boundWidth =
                (((mapInfo[0] / state.DOTS_PER_INCH / state.INCHES_PER_METER) *
                    scale) /
                    resolution) *
                DEVICE_PIXEL_RATIO,
            boundHeight =
                (((mapInfo[1] / state.DOTS_PER_INCH / state.INCHES_PER_METER) *
                    scale) /
                    resolution) *
                DEVICE_PIXEL_RATIO,
        ...
        plotservicePayload.addparam.bboxwest =
            String(Math.round((center[0] - halfWidth) * 10) / 10.0) +
            " (" +
            epsgCode +
            ")";
        plotservicePayload.addparam.bboxsouth =
            String(Math.round((center[1] - halfHeight) * 10) / 10.0) +
            " (" +
            epsgCode +
            ")";
        plotservicePayload.addparam.bboxeast = String(
            Math.round((center[0] + halfWidth) * 10) / 10.0
        );
        plotservicePayload.addparam.bboxnorth = String(
            Math.round((center[1] + halfHeight) * 10) / 10.0
        );
    ...
    waitForPrintJobSuccess: async function (
        ...
        if (response.status === "error") {
            dispatch(
                "Alerting/addSingleAlert",
                i18next.t(
                    "common:modules.tools.print.waitForPrintErrorMessage"
                ),
                { root: true }
            );
    ...
    downloadFile: function ({ state, commit }, fileSpecs) {
         ...
         * getProxyUrl()
         */
        const fileUrl = state.useProxy
            ? getProxyUrl(fileSpecs.fileUrl)
            : fileSpecs.fileUrl;

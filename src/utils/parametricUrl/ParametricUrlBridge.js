...
function readFromUrlParams() {
    return (
        !Object.prototype.hasOwnProperty.call(Config, "allowParametricURL") ||
        Config.allowParametricURL === true
    );
}
/**
    ...
    if (readFromUrlParams()) {
        ...
        params.forEach(function (value, key) {
            if (
                ...
                key.toLowerCase() === "search/query"
            ) {
                store.state.urlParams["Search/query"] = parseQuery(value);
            } else if (
                key.toLocaleLowerCase() === "map/layerids" ||
                key.toLocaleLowerCase() === "layerids"
            ) {
                const visibility = params.has("visibility")
                        ? params.get("visibility")
                        : "",
                    transparency = params.has("transparency")
                        ? params.get("transparency")
                        : "";

                store.state.urlParams["Map/layerIds"] = parseLayerParams(
                    value,
                    visibility,
                    transparency
                );
...
export function translateToBackbone(urlParamsKey, urlParamsValue) {
    ...
    if (paramsKey.startsWith("tools") || paramsKey.indexOf("/active") > -1) {
        ...
        toolsNotInState.find(
            (toolName) =>
                toolName.toLowerCase() === paramsKey.toLocaleLowerCase()
        )
    ) {
    ...
    if (key === "Maps/mapMode") {
        ...
        store.dispatch("Maps/zoomToProjExtent", {
            data: {
                ...
                options: { duration: 0 },
                projection:
                    store.state.urlParams?.projection ||
                    store.state.Maps?.projection?.getCode(),
            },
        });
    } else if (key === "style") {
...
function getLayersUsingMetaId(values) {
    ...
    metaIds.forEach((metaId) => {
        const metaIDlayers = Radio.request(
            "Parser",
            "getItemsByMetaID",
            metaId
        );
    ...
    return {
        layerIdList: layersIds,
        visibilityList: layersIds.map(() => true),
        transparencyList: layersIds.map(() => 0),
    };
}
/**
...
function parseLayerParams(
    ...
    const visibilityListBooleans = store.state.urlParams?.visibility
            ? convert(store.state.urlParams.visibility)
            : convert(visibilityString),
        transparencyListNumbers = store.state.urlParams?.transparency
            ? convertTransparency(store.state.urlParams.transparency)
            : convertTransparency(transparencyString),
        layerParams = [],
        layerIdsNotFound = [],
    ...
    if (
        typeof transparencyListNumbers === "string" &&
        transparencyListNumbers !== ""
    ) {
        transparencyList = [transparencyListNumbers];
    ...
    if (
        layerIdList.length !== visibilityList.length ||
        visibilityList.length !== transparencyList.length
    ) {
        ...
        setTimeout(() => {
            store.dispatch(
                ...
                {
                    content: i18next.t(
                        "common:utils.parametricURL.alertWrongAmountVisibility"
                    ),
                    multipleAlert: true,
                },
                { root: true, category: "Warning" }
            );
        }, 500);
        return null;
    }
    layerIdList.forEach((id, index) => {
        const layerConfigured = Radio.request("Parser", "getItemByAttributes", {
                id: id,
            }),
        ...
        if (
            ...
            if (treeType === "light") {
                ...
                // treetype default
                const folderName =
                        layerExisting.datasets[0]?.kategorie_opendata[0],
                    folder = folderName
                ...
                Radio.trigger(
                    ...
                    layerToPush.url,
                    layerToPush.version,
                    transparencyList[index],
                    visibilityList[index]
                );
            }
        } else if (
            rawLayerList.getLayerWhere({ id: optionsOfLayer.id }) &&
            treeType === "custom"
        ) {
            console.warn(
                "The layer with id " +
                    optionsOfLayer.id +
                    "is loaded from the gdi."
            );
    ...
    if (layerParams) {
        layerParams.layerIdList.forEach((val, index) => {
            const id = String(val),
                ...
                layerFromParser = Radio.request(
                    "Parser",
                    "getItemByAttributes",
                    { id: id }
                );
            let layerToPush,
                layer = Radio.request("ModelList", "getModelByAttributes", {
                    id: id,
                });

            if (layerParams.transparencyList[index] !== null) {
                optionsOfLayer.transparency =
                    layerParams.transparencyList[index];
            }
...
function alertWrongLayerIds(layerIdsNotFound) {
    if (layerIdsNotFound.length > 0) {
        ...
        layerIdsNotFound.forEach((position) => {
            layerIdsNotFoundConcat =
                layerIdsNotFoundConcat + ", " + String(position);
        });

        console.warn(
            "The following Url-Param-LayerIds could not be found " +
                layerIdsNotFoundConcat
        );
        // timeout may be removed, if everything is migrated to vue. Now it is needed for portal/basic.
        setTimeout(() => {
            store.dispatch(
                ...
                {
                    content: i18next.t(
                        "common:utils.parametricURL.alertWrongLayerIds"
                    ),
                    multipleAlert: true,
                },
                { root: true }

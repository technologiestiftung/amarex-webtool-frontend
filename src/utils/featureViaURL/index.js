...
export function createGeoJSON(features, geometryType, epsg = 4326) {
    const geoJSON = {
        ...
        crs: {
            ...
            properties: {
                href:
                    "http://spatialreference.org/ref/epsg/" + epsg + "/proj4/",
                type: "proj4",
            },
        },
        features: [],
    ...
    if (flag) {
        console.warn(
            i18next.t("common:modules.featureViaURL.messages.featureParsing")
        );
    }
...
export function getFeatureIds(layerId) {
    ...
    if (typeof layer === "undefined") {
        console.warn(
            i18next.t("common:modules.featureViaURL.messages.layerNotFound")
        );
        return featureArray;
    }
    layer
...
export default function ({ layers, epsg, zoomTo }) {
    store.watch(
        ...
        (params) => {
            const urlLayers = params.featureViaURL
                    ? JSON.parse(params.featureViaURL)
                    : [],
                treeType = Radio.request("Parser", "getTreeType"),
                parentId =
                    treeType === "custom" || treeType === "light"
                        ? "featureViaURLFolder"
                        : undefined;
            let features, geoJSON, geometryType, layerId, pos;

            if (urlLayers.length === 0) {
                if (params.featureViaURL) {
                    console.warn(
                        i18next.t(
                            "common:modules.featureViaURL.messages.featureParsing"
                        )
                    );
                }
                return;
            ...
            if (treeType === "custom" || treeType === "light") {
                Radio.trigger(
                    "Parser",
                    "addFolder",
                    gfiAttributes.folderName,
                    "featureViaURLFolder",
                    "Overlayer",
                    0,
                    true,
                    "modules.featureViaURL.folderName"
                );
            }
            ...
            urlLayers.forEach((layer) => {
                ...
                if (pos === -1) {
                    console.error(
                        i18next.t(
                            "common:modules.featureViaURL.messages.layerNotFound",
                            { layerId }
                        )
                    );
                    return;
                ...
                if (!layers[pos].name) {
                    console.error(
                        i18next.t(
                            "common:modules.featureViaURL.messages.noNameDefined",
                            { layerId }
                        )
                    );
                    return;
                ...
                if (
                    ...
                    console.error(
                        i18next.t(
                            "common:modules.featureViaURL.messages.geometryNotSupported"
                        ),
                ...
                if (
                    ...
                    store.dispatch("Alerting/addSingleAlert", {
                        content: i18next.t(
                            "common:modules.featureViaURL.messages.featureParsingAll"
                        ),
                        multipleAlert: true,
                    });
                    return;
                ...
                if (geoJSON.features.length === 0) {
                    store.dispatch("Alerting/addSingleAlert", {
                        content: i18next.t(
                            "common:modules.featureViaURL.messages.featureParsingNoneAdded"
                        ),
                        multipleAlert: true,
                    });
                }
                ...
                if (parentId !== undefined) {
                    ...
                    store.dispatch("Alerting/addSingleAlert", {
                        content: i18next.t(
                            "common:modules.featureViaURL.messages.defaultTreeNotSupported"
                        ),
                        multipleAlert: true,
                    });
                    return;
                ...
                if (
                    typeof zoomTo !== "undefined" &&
                    (zoomTo === layerId || zoomTo.indexOf(layerId) !== -1)
                ) {
                    store.dispatch("Maps/zoomToFilteredFeatures", {
                        ids: getFeatureIds(layerId),
                        layerId: layerId,
                    });
                }
            });

...
const ModelList = Backbone.Collection.extend(
    /** @lends ModelList.prototype */ {
        ...
        initialize: function () {
            ...
            channel.reply(
                {
                    ...
                    getModelByAttributes: function (attributes) {
                        return this.findWhere(attributes) !== undefined
                            ? this.findWhere(attributes)
                            : this.retrieveGroupModel(attributes);
                    },
                },
                this
            );
            ...
            channel.on(
                {
                    ...
                    addAndExpandModelsRecursive:
                        this.addAndExpandModelsRecursive,
                    ...
                    transparencyChanged: function () {
                        channel.trigger(
                            ...
                            this.where({ isSelected: true, type: "layer" })
                        );
                        store.dispatch(
                            "Tools/SaveSelection/createUrlParams",
                            filterAndReduceLayerList(
            ...
            this.defaultToolId = Object.prototype.hasOwnProperty.call(
                Config,
                "defaultToolId"
            )
        ...
        model: function (attrs, options) {
            if (attrs.type === "layer") {
                 ...
                 */
                    console.warn(
                        "Tool: 'wfsFeatureFilter' is deprecated. Please use 'filter' instead."
                    );
                 ...
                 */
                    console.warn(
                        "Tool: 'extendedFilter' is deprecated. Please use 'filter' instead."
                    );
        ...
        setVisibleByParentIsExpanded: function (parentId) {
            ...
            if (!parent?.get("isExpanded")) {
                this.setAllDescendantsInvisible(
                    parentId,
        ...
        setAllDescendantsInvisible: function (parentId, isMobile) {
            const additionalChildren = this.where({
        ...
        setIsSelectedOnChildModels: function (model) {
            const folder = Radio.request("Parser", "getItemsByAttributes", {
                id: model.get("id"),
            });
            let descendantModels = this.add(
                Radio.request("Parser", "getItemsByAttributes", {
        ...
        setIsSelectedOnParent: function (model) {
            const layers = this.where({ parentId: model.get("parentId") }),
                ...
                allLayersSelected = layers.every(
                    (layer) => layer.get("isSelected") === true
                );
        ...
        setActiveToolsToFalse: function (activatedToolModel) {
            const legendModel = this.findWhere({ id: "legend" }),
                ...
                activatedToolModels = Array.isArray(activatedToolModel)
                    ? activatedToolModel
                    : [activatedToolModel],
                alwaysActiveTools = [
                    ...activatedToolModels,
                ...
                deactivateGFI =
                    typeof activatedToolModel.get === "function"
            ...
            activeToolsToDeactivate = activeTools.filter(
                (tool) => !alwaysActiveTools.includes(tool)
            );
            activeToolsToDeactivate.forEach((tool) => {
                tool.setIsActive(false);
                store.dispatch("Tools/setToolActive", {
                    id: tool.id,
                    active: false,
                });
            });
        ...
        moveModelInTree: function (model, movement) {
            const currentSelectionIdx = model.get("selectionIDX"),
                ...
                isTreeMove =
                    Config?.layerSequence?.moveModelInTree !== undefined
                        ? Config?.layerSequence?.moveModelInTree
                        : true;
        ...
        initLayerIndeces: function (paramLayers) {
            const allLayerModels = this.getTreeLayers(),
                baseLayerModels = allLayerModels.filter((layerModel) => {
                    return (
                        layerModel.get("isBaseLayer") === true &&
                        paramLayers.find((model) => {
            ...
            if (
                treeType === "custom" &&
                layerModels.find((layer) => layer.get("layerSequence"))
            ) {
                ...
                baseLayerModels.sort(function (a, b) {
                    const indexA = paramLayers.findIndex(
                            (param) => a.id === param.id
                        ),
                        indexB = paramLayers.findIndex(
                            (param) => b.id === param.id
                        );
        ...
        addNewLayerToSequence: function (layers, newLayer) {
            ...
            if (
                ...
                layers.find(
                    (layer) =>
                        layer.get("layerSequence") <
                        newLayer.get("layerSequence")
                ) !== undefined
            ) {
        ...
        getTreeLayers: function () {
            ...
            allLayerModels = allLayerModels.filter((layerModel) => {
                return (
                    ["Oblique", "TileSet3D", "Terrain3D", "Entities3D"].indexOf(
                        layerModel.get("typ")
                    ) === -1
                );
            });
        ...
        getSortedTreeLayers: function (isTreeMove) {
            const combinedLayers = this.getTreeLayers(),
                newLayers = combinedLayers.filter(
                    (layer) => layer.get("selectionIDX") === 0
                ),
                treeType = Radio.request("Parser", "getTreeType");

            // we need to devide current layers from newly added ones to be able to put the latter ones in
            // at a nice position
            let currentLayers = combinedLayers.filter(
                    (layer) => layer.get("selectionIDX") !== 0
                ),
                firstBaseLayerIndex;
            ...
            currentLayers.sort(function (layer1, layer2) {
                return layer1.get("selectionIDX") > layer2.get("selectionIDX")
                    ? 1
                    : -1;
            });
            ...
            if (
                treeType === "custom" &&
                !isTreeMove &&
                combinedLayers.find((layer) => layer.get("layerSequence"))
            ) {
                currentLayers = this.handleLayerSequence(
                    combinedLayers,
                    currentLayers,
                    newLayers
                );
        ...
        handleLayerSequence: function (
            ...
            const layerWithSequence = combinedLayers.find((layer) => {
                ...
                previousLayers =
                    currentLayers !== undefined ? currentLayers : [];
            ...
            if (
                ...
                numberOfLayerWithSequence !== previousLayers.length
            ) {
                if (
                    layerWithSequence !== undefined &&
                    typeof layerWithSequence === "object"
                ) {
                    sortedLayers = this.sortLayerSequence(
                        this.addLayerSequence(combinedLayers)
                    );
                }
            }
            // add new layer
                ...
                layersToAdd.forEach((newLayer) => {
                    sortedLayers = this.addNewLayerToSequence(
                        previousLayers,
                        newLayer
                    );
                });
        ...
        addLayerSequence: function (layers) {
            ...
            layers.forEach((layer) => {
                if (
                    ...
                    layer.get("layerSequence") === undefined
                ) {
        ...
        updateLayerView: function (isTreeMove) {
            ...
            sortedLayers
                ...
                .forEach((layer) => {
                    Radio.trigger("Map", "addLayerToIndex", [
                        layer.get("layer"),
                        layer.get("selectionIDX"),
                        layer.get("layerSequence"),
                    ]);
                });
        },
        ...
        addInitiallyNeededModels: function () {
            ...
            const paramLayers =
                    ...
                    store.state.urlParams["Map/layerIds"]
                        ? store.state.urlParams["Map/layerIds"]
                        : [],
                treeType = Radio.request("Parser", "getTreeType");

            let lightModels, itemIsVisibleInMap, lightModel;

            if (treeType === "light") {
                lightModels = Radio.request("Parser", "getItemsByAttributes", {
                    type: "layer",
                });
                lightModels = this.mergeParamsToLightModels(
                    lightModels,
                    paramLayers
                );
                ...
                lightModels.forEach((model) => {
                    if (
                        Object.prototype.hasOwnProperty.call(model, "children")
                    ) {
                ...
                itemIsVisibleInMap = Radio.request(
                    "Parser",
                ...
                paramLayers.forEach((paramLayer) => {
                    ...
                    if (lightModel !== undefined) {
                        this.add(lightModel);
                        this.setModelAttributesById(paramLayer.id, {
                            isSelected: true,
                            transparency: paramLayer.transparency,
                        });
                        ...
                        if (
                            paramLayer.visibility === false &&
                            this.get(paramLayer.id) !== undefined
                        ) {
                        ...
                        const source = rawLayerList.getLayerWhere({
                            id: paramLayer.id,
                        });

                        if (source) {
                            Radio.trigger("Parser", "addGdiLayer", {
                                id: paramLayer.id,
                                source: source,
                                name: source.name,
                            });
        ...
        checkTimeLayerHandling: function (model) {
            const timeLayers = Radio.request("Parser", "getItemsByAttributes", {
                    type: "layer",
                    typ: "WMS",
            ...
            if (timeLayers.length > 0) {
                if (treeType === "light") {
                    ...
                    const selectedTimeLayer = timeLayers.filter(
                    ...
                    if (selectedTimeLayer.length > 0) {
                        if (
                            model.id ===
                            selectedTimeLayer[selectedTimeLayer.length - 1].id
                        ) {
                            setTimeout(function () {
                        ...
                        if (selectedTimeLayer.length > 1) {
                            Radio.trigger(
                                ...
                                "alert",
                                i18next.t(
                                    "common:modules.core.modelList.layer.wms.warningTimeLayerQuantity",
                                    {
                                        name: selectedTimeLayer[
                                            selectedTimeLayer.length - 1
                                        ].name,
                                    }
                                )
                            );
        ...
        mergeParamsToLightModels: function (lightModels, paramLayers) {
            ...
            if (paramLayers !== undefined && paramLayers.length !== 0) {
                lightModels.forEach((lightModel) => {
                    const hit = paramLayers.find(
                        (paramLayer) => paramLayer.id === lightModel.id
                    );
        ...
        addModelsByAttributes: function (attrs) {
            const lightModels = Radio.request(
                "Parser",
                "getItemsByAttributes",
                attrs
            );
        ...
        showModelInTree: function (modelId) {
            const mode = Radio.request("Map", "getMapMode"),
                lightModel = Radio.request("Parser", "getItemByAttributes", {
                    id: modelId,
                }),
            ...
            if (!isMobile && !isTable) {
                dropdown = Dropdown.getInstance(
                    "#root li:first-child > .dropdown-toggle"
                );
                // open the layerTree
                dropdown?.show();
            }
            ...
            if (uiStyle.getUiStyle() === "TABLE") {
                Radio.request("ModelList", "getModelByAttributes", {
                    id: modelId,
                }).setIsJustAdded(true);
        ...
        openFolderInTree: function (id) {
            const dropdown = Dropdown.getInstance(
                "#root li:first-child > .dropdown-toggle"
            );
        ...
        scrollToLayer: function (overlayerName) {
            const $Overlayer = $("#Overlayer"),
                element = $Overlayer
                    .find("span")
                    .toArray()
                    .find((layer) => layer.title === overlayerName);
        ...
        addAndExpandModelsRecursive: function (parentId) {
            const lightSiblingsModels = Radio.request(
                    "Parser",
                    "getItemsByAttributes",
                    { parentId: parentId }
                ),
                parentModel = Radio.request("Parser", "getItemByAttributes", {
                    id: lightSiblingsModels[0].parentId,
                });
        ...
        removeModelsByParentId: function (parentId) {
            this.where({ parentId: parentId }).forEach((model) => {
                if (
                    model.get("type") === "layer" &&
                    model.get("isVisibleInMap") === true
                ) {
        ...
        retrieveGroupModel: function (attributes) {
            const layerId =
                    typeof attributes === "object" ? attributes.id : attributes,
        ...
        getModelById: function (id) {
            ...
            if (model === undefined || model instanceof GroupedLayers) {
                model = this.retrieveGroupModel(id)
                    .get("layerSource")
        ...
        selectedChanged: function (model, value) {
            ...
            Radio.channel("ModelList").trigger(
                "updatedSelectedLayerList",
                this.where({ isSelected: true, type: "layer" })
            );

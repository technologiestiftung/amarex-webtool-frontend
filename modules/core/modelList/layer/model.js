    ...
    /** @lends Layer.prototype */ {
        ...
        initialize: function () {
            ...
            if (
                ...
                this.setIsRemovable(
                    Radio.request("Parser", "getPortalConfig").layersRemovable
                );
        ...
        changeLang: function (lng) {
            ...
            if (
                this.has("i18nextTranslate") &&
                typeof this.get("i18nextTranslate") === "function"
            ) {
        ...
        prepareFeaturesFor3D: function (features) {
            ...
            features.forEach((feature) => {
                ...
                if (altitude || altitudeOffset) {
                    geometry = this.setAltitudeOnGeometry(
                        geometry,
                        altitude,
                        altitudeOffset
                    );
        ...
        setAltitudeOnGeometry: function (geometry, altitude, altitudeOffset) {
            ...
            if (type === "Point") {
                overwrittenCoords = this.setAltitudeOnPoint(
                    coords,
                    altitude,
                    altitudeOffset
                );
            } else if (type === "MultiPoint") {
                overwrittenCoords = this.setAltitudeOnMultiPoint(
                    coords,
                    altitude,
                    altitudeOffset
                );
        ...
        setAltitudeOnMultiPoint: function (coords, altitude, altitudeOffset) {
            ...
            coords.forEach((coord) => {
                overwrittenCoords.push(
                    this.setAltitudeOnPoint(coord, altitude, altitudeOffset)
                );
            });
        ...
        setAltitudeOnPoint: function (coords, altitude, altitudeOffset) {
            ...
            if (!isNaN(altitudeOffsetAsFloat)) {
                if (overwrittenCoords.length === 2) {
                    ...
                    overwrittenCoords[2] =
                        overwrittenCoords[2] + altitudeOffsetAsFloat;
                }
            }
            return overwrittenCoords;
        },
        ...
        setWindowsInterval: function (func, autorefreshInterval) {
            this.set(
                "windowsInterval",
                setInterval(func.bind(this), autorefreshInterval)
            );
        },
        ...
        getResolutions: function () {
            const resoByMaxScale = Radio.request(
                    "MapView",
                    "getResolutionByScale",
                    this.get("maxScale"),
                    "max"
                ),
                resoByMinScale = Radio.request(
                    "MapView",
                    "getResolutionByScale",
                    this.get("minScale"),
                    "min"
        ...
        toggleIsSelected: function () {
            const id = this.get("id"),
                layerGroup = Radio.request(
                    "ModelList",
                    "getModelsByAttributes",
                    { parentId: this.get("parentId") }
                ),
                singleBaselayer =
                    this.get("singleBaselayer") &&
                    this.get("parentId") === "Baselayer";
            ...
            if (this.get("isSelected")) {
                ...
                if (singleBaselayer) {
                    layerGroup.forEach((layer) => {
                        ...
                        if (
                            layer.get("id") !== id &&
                            typeof layer.checkForScale === "function"
                        ) {
        ...
        toggleAttributionsInterval: function () {
            ...
            if (
                this.has("layerAttribution") &&
                typeof this.get("layerAttribution") === "object"
            ) {
                ...
                if (this.get("isVisibleInMap") === true) {
                    ...
                    this.get("layerAttribution").interval = setInterval(
                        function (model) {

...
Layer2dVectorSensorThings.prototype.createLayer = function (attributes) {
  ...
  this.setLayer(
    this.createVectorLayer(rawLayerAttributes, { layerParams, options }),
  );
};
 ...
 */
Layer2dVectorSensorThings.prototype.getRawLayerAttributes = function (
  attributes,
) {
  return {
 ...
 */
Layer2dVectorSensorThings.prototype.createVectorLayer = function (
  rawLayer = {},
  { layerParams = {}, options = {} } = {},
) {
  const source = new VectorSource(),
    layer = new VectorLayer(
      Object.assign(
        {
          source: rawLayer.clusterDistance
            ? new Cluster({
                source,
...
Layer2dVectorSensorThings.prototype.updateLayerValues = function (values) {
  const state = this.getStateOfSTALayer(
    values.visibility,
...
Layer2dVectorSensorThings.prototype.createLegend = async function () {
  ...
  if (!Array.isArray(legend)) {
    if (styleObject && legend === true) {
      const legendInfos = await createStyle.returnLegendByStyleId(
        styleObject.styleId,
      );
  ...
  if (typeof styleObject !== "undefined") {
    ...
    styleFunction = function (feature, resolution) {
      const feat = typeof feature !== "undefined" ? feature : this,
        isClusterFeature =
          typeof feat.get("features") === "function" ||
          (typeof feat.get("features") === "object" &&
            Boolean(feat.get("features").length > 1)),
      ...
      if (styleElement?.getImage() !== null && attrs.scaleStyleByZoom) {
        styleElement
          .getImage()
          .setScale(
            (styleElement.getImage().getScale() * zoomLevel) / zoomLevelCount,
          );
      }
      return style;
    };
    ...
    this.setStyle(null);
    console.warn(
      i18next.t("common:core.layers.errorHandling.wrongStyleId", {
...
Layer2dVectorSensorThings.prototype.initializeSensorThings = function () {
  try {
    this.createMqttConnectionToSensorThings(
      ...
      this.get("timezone"),
      this.get("showNoDataValue"),
      this.get("noDataValue"),
    );
  } catch (err) {
    console.error(
      "Connecting to mqtt-broker failed. Won't receive live updates. Reason:",
      err,
    );
  }
  ...
  store.watch(
    (_, getters) => getters.visibleSubjectDataLayerConfigs,
    (layerConfigs) => {
 ...
 * @returns {void}
 */
Layer2dVectorSensorThings.prototype.createMqttConnectionToSensorThings =
  function (url, mqttOptions, timezone, showNoDataValue, noDataValue) {
    ...
    this.mqttClient.on("message", (topic, observation) => {
      ...
      if (
        this.get("observeLocation") &&
        isObject(feature) &&
        isObject(observation?.location) &&
        this.subscribedDataStreamIds[datastreamId]?.subscribed
      ) {
        ...
        if (
          this.showHistoricalFeatures &&
          Array.isArray(feature.get("historicalFeatureIds")) &&
 ...
 * @returns {void}
 */
Layer2dVectorSensorThings.prototype.updateHistoricalFeatures = function (
  feature,
  clonedFeature,
  layerSource,
) {
  if (
    ...
    !Array.isArray(feature.get("historicalFeatureIds")) ||
    typeof clonedFeature?.set !== "function"
  ) {
    return;
  }
  ...
  feature.get("historicalFeatureIds").forEach((id, index) => {
    ...
    layerSource
      ...
      .set(
        "originScale",
        this.getScale(index, feature.get("historicalFeatureIds").length),
      );
    this.setStyleOfHistoricalFeature(
 ...
 */
Layer2dVectorSensorThings.prototype.getMqttHostFromUrl = function (
  url,
  onError,
) {
  ...
  if (typeof mqttHost !== "string") {
    if (typeof onError === "function") {
      onError(
        new Error(
          "getMqttHostFromUrl: the given url does not include the host name.",
        ),
      );
    }
    return "";
 ...
 */
Layer2dVectorSensorThings.prototype.getDatastreamIdFromMqttTopic = function (
  topic,
) {
  ...
  return topic.substring(
    datastreamIdx + 12,
    topic.indexOf(")", datastreamIdx + 12),
  );
};
 ...
 */
Layer2dVectorSensorThings.prototype.getFeatureByDatastreamId = function (
  features,
  id,
) {
 ...
 * @returns {void}
 */
Layer2dVectorSensorThings.prototype.initializeConnection = function (
  onsuccess,
  updateOnly = false,
) {
  const url = this.get("url"),
    ...
    currentExtent = {
      ...
      sourceProjection: mapCollection
        .getMapView("2D")
        .getProjection()
        .getCode(),
    ...
    intersect =
      typeof this.get("intersect") === "boolean" ? this.get("intersect") : true,
    mapProjection = mapCollection.getMapView("2D").getProjection(),
    epsg = this.get("epsg"),
    gfiTheme = this.get("gfiTheme"),
    utc = this.get("utc"),
    datastreamIds = this.getDatastreamIdsInCurrentExtent(
      this.getLayerSource().getFeatures(),
      store.getters["Maps/extent"],
    );

  this.callSensorThingsAPI(
    url,
    version,
    urlParams,
    currentExtent,
    intersect,
    (sensorData) => {
      const filteredSensorData = !updateOnly
          ? sensorData
          : sensorData.filter(
              (data) => !datastreamIds.includes(data?.properties?.dataStreamId),
            ),
        features = this.createFeaturesFromSensorData(
          filteredSensorData,
          mapProjection,
          epsg,
          gfiTheme,
        ...
        layerSource =
          this.getLayerSource() instanceof Cluster
            ? this.getLayerSource().getSource()
            : this.getLayerSource(),
        oldHistoricalFeatures = layerSource
          .getFeatures()
          .filter((f) => typeof f.get("dataStreamId") === "undefined"),
        copyFeatures = {};
      ...
      if (Array.isArray(features) && features.length) {
        if (
          ...
          (features[0].getGeometry().getType() === "Point" ||
            features[0].getGeometry().getType() === "MultiPoint")
        ) {
      ...
      if (
        typeof this.get("historicalLocations") === "number" &&
        this.showHistoricalFeatures
      ) {
        if (
          this.get("loadThingsOnlyInCurrentExtent") &&
          Object.entries(copyFeatures).length
        ) {
          features.forEach((feature) => {
            const dataStreamIdFeature = feature.get("dataStreamId");

            if (
              this.subscribedDataStreamIds[dataStreamIdFeature]?.subscribed &&
              Array.isArray(
                this.subscribedDataStreamIds[dataStreamIdFeature]
 ...
 * @returns {void}
 */
Layer2dVectorSensorThings.prototype.callSensorThingsAPI = function (
  url,
  version,
  urlParams,
  currentExtent,
 ...
 */
Layer2dVectorSensorThings.prototype.buildSensorThingsUrl = function (
  url,
  version,
  urlParams,
) {
  const root = urlParams?.root || "Things",
    versionAsString =
      typeof version === "number" ? version.toFixed(1) : version;
  let query = "";
 ...
 */
Layer2dVectorSensorThings.prototype.getAllThings = function (
  sensordata,
  urlParams,
  url,
  version,
  ...
  if (urlParams?.root === "Datastreams") {
    allThings = this.changeSensordataRootToThings(
      sensordata,
      this.get("datastreamAttributes"),
      this.get("thingAttributes"),
    );
  ...
  allThings = this.createPropertiesOfDatastreams(
    allThings,
    this.get("showNoDataValue"),
    this.get("noDataValue"),
    this.get("utc"),
 ...
 */
Layer2dVectorSensorThings.prototype.changeSensordataRootToThings = function (
  sensordata,
  datastreamAttributes,
  thingAttributes,
) {
  ...
  const result = [],
    datastreamAttributesAssociation =
      this.createAssociationObject(datastreamAttributes),
  ...
  sensordata.forEach((stream) => {
    ...
    Object.keys(stream.Thing).forEach((key) => {
      if (
        Object.prototype.hasOwnProperty.call(thingAttributesAssociation, key)
      ) {
...
Layer2dVectorSensorThings.prototype.unifyThingsByIds = function (allThings) {
  ...
  allThings.forEach((thing) => {
    ...
    allThingsAssoc[thing["@iot.id"]].Datastreams = allThingsAssoc[
      thing["@iot.id"]
    ].Datastreams.concat(thing.Datastreams);
  });
 ...
 */
Layer2dVectorSensorThings.prototype.createPropertiesOfDatastreams = function (
  allThings,
  showNoDataValue,
  noDataValue,
  utc,
  ...
  allThings.forEach((thing) => {
    ...
    this.createPropertiesOfDatastreamsHelper(
      thing.Datastreams,
      thing.properties,
      showNoDataValue,
      noDataValue,
    ...
    thing.properties.dataStreamValue =
      thing.properties.dataStreamValue.join(" | ");
    thing.properties.dataStreamName =
      thing.properties.dataStreamName.join(" | ");
    thing.properties.dataPhenomenonTime =
      thing.properties.dataPhenomenonTime.join(" | ");
  });
 ...
 */
Layer2dVectorSensorThings.prototype.createPropertiesOfDatastreamsHelper =
  function (
    dataStreams,
    properties,
    showNoDataValue,
    ...
    dataStreams.forEach((dataStream) => {
      ...
      const dataStreamId = String(dataStream["@iot.id"]),
        ...
        dataStreamValue = Array.isArray(dataStream.Observations)
          ? dataStream.Observations[0]?.result
          : "",
        dataStreamUnit = dataStream.unitOfMeasurement?.name,
        key = "dataStream_" + dataStreamId + "_" + dataStreamName;
      let phenomenonTime = Array.isArray(dataStream.Observations)
        ? dataStream.Observations[0]?.phenomenonTime
        : "";
      ...
      if (dataStreamValue !== "") {
        ...
        properties[key + "_phenomenonTime"] = this.getLocalTimeFormat(
          phenomenonTime,
          timezone,
        );
      ...
      if (
        typeof dataStreamUnit !== "undefined" &&
        typeof this.get("rotationUnit") !== "undefined" &&
        dataStreamUnit === this.get("rotationUnit")
      ) {
        properties.rotation = {
          isDegree: true,
 ...
 */
Layer2dVectorSensorThings.prototype.moveDatastreamPropertiesToThing = function (
  thingProperties,
 ...
 */
Layer2dVectorSensorThings.prototype.getLocalTimeFormat = function (
  phenomenonTime,
  timezone,
) {
 ...
 */
Layer2dVectorSensorThings.prototype.getFirstPhenomenonTime = function (
  phenomenonTime,
) {
 ...
 */
Layer2dVectorSensorThings.prototype.aggregatePropertiesOfThings = function (
  allThings,
  url,
  version,
) {
 ...
 */
Layer2dVectorSensorThings.prototype.aggregatePropertiesOfThingAsArray =
  function (arrayOfThings, result) {
    ...
    result.properties = Object.assign(
      {},
      props,
      this.aggregateProperties(arrayOfThings, keys),
    );
 ...
 */
Layer2dVectorSensorThings.prototype.getThingsGeometry = function (
  thing,
  index,
) {
  const locations = thing?.Locations || thing?.Thing?.Locations,
    location =
      locations &&
      Object.prototype.hasOwnProperty.call(locations, index) &&
 ...
 */
Layer2dVectorSensorThings.prototype.aggregateProperties = function (
  things,
  keys,
) {
  const result = {};
 ...
 * @returns {Boolean} returns true on success and false if something went wrong.
 */
Layer2dVectorSensorThings.prototype.aggregatePropertiesOfOneThing = function (
  thing,
  result,
) {
 ...
 */
Layer2dVectorSensorThings.prototype.createFeaturesFromSensorData = function (
  sensorData,
  mapProjection,
  epsg,
  gfiTheme,
  ...
  sensorData.forEach((data, index) => {
    ...
    if (isHistorical) {
      ...
      feature.set(
        "originScale",
        this.getScale(index - 1, sensorData.length - 1),
      );
    }
  ...
  return features.filter(
    (subFeature) => typeof subFeature.getGeometry() !== "undefined",
  );
};
 ...
 */
Layer2dVectorSensorThings.prototype.createFeatureByLocation = function (
  data,
  featureProjection,
  dataProjection,
  onerror,
  ...
  try {
    ...
    if (typeof onerror === "function") {
      onerror(
        new Error(
          "createFeatureByLocation: JSON structure in sensor thing location can't be parsed.",
        ),
      );
    }
  }
 ...
 */
Layer2dVectorSensorThings.prototype.aggregateDataStreamValue = function (
  feature,
) {
 ...
 */
Layer2dVectorSensorThings.prototype.aggregateDataStreamPhenomenonTime =
  function (feature) {
    const modifiedFeature = feature,
    ...
    if (feature && feature.get("dataStreamId")) {
      feature
        ...
        .forEach((id, i) => {
          ...
          if (
            this.get("showNoDataValue") &&
            !feature.get(
              "dataStream_" + id + "_" + dataStreamName + "_phenomenonTime",
            )
          ) {
            ...
            dataStreamPhenomenonTimes.push(
              feature.get(
                "dataStream_" + id + "_" + dataStreamName + "_phenomenonTime",
              ),
            );
      ...
      modifiedFeature.set(
        "dataStreamPhenomenonTime",
        dataStreamPhenomenonTimes.join(" | "),
        true,
      );
    }
    return modifiedFeature;
 ...
 * @returns {void}
 */
Layer2dVectorSensorThings.prototype.toggleSubscriptionsOnMapChanges =
  function () {
    const state = this.getStateOfSTALayer(
      this.getLayer().getVisible(),
      this.get("isSubscribed"),
    );
...
Layer2dVectorSensorThings.prototype.stopSubscription = function () {
  ...
  store.dispatch("Maps/unregisterListener", {
    type: "moveend",
    listener: this.updateSubscription.bind(this),
  });
...
Layer2dVectorSensorThings.prototype.startIntervalUpdate = function (timeout) {
  ...
  this.intervallRequest = setTimeout(() => {
    const datastreamIds = this.getDatastreamIdsInCurrentExtent(
        this.getLayerSource().getFeatures(),
        store.getters["Maps/extent"],
      ),
    ...
    this.initializeConnection(async (features) => {
      await this.subscribeToSensorThings(
        this.getDatastreamIdsInCurrentExtent(
          features,
          store.getters["Maps/extent"],
        ),
        subscriptionTopics,
 ...
 */
Layer2dVectorSensorThings.prototype.getStateOfSTALayer = function (
  visibility,
  isSubscribed,
) {
      ...
      function () {
        ...
        if (this.get("enableContinuousRequest")) {
          this.startIntervalUpdate(
            typeof this.get("factor") === "number"
              ? this.get("factor") * 1000
              : undefined,
          );
        }
      }.bind(this),
...
Layer2dVectorSensorThings.prototype.updateSubscription = async function () {
  const datastreamIds = this.getDatastreamIdsInCurrentExtent(
      this.getLayer().getSource().getFeatures(),
      store.getters["Maps/extent"],
    ),
    subscriptionTopics = this.get("subscriptionTopics"),
    version = this.get("version"),
 ...
 */
Layer2dVectorSensorThings.prototype.getDatastreamIdsInCurrentExtent = function (
  features,
  currentExtent,
) {
 ...
 */
Layer2dVectorSensorThings.prototype.getFeaturesInExtent = function (
  features,
  currentExtent,
) {
  ...
  if (typeof this.get("maxSpeedKmh") !== "undefined") {
    enlargedExtent = this.enlargeExtentForMovableFeatures(
      currentExtent,
      this.get("maxSpeedKmh"),
      this.get("factor"),
    );
  } else {
 ...
 */
Layer2dVectorSensorThings.prototype.enlargeExtentForMovableFeatures = function (
  extent,
  maxSpeed,
  factor = 10,
) {
  ...
  if (!Array.isArray(extent) || extent.length !== 4) {
    console.error(
      "sta.enlargeExtentForMovableFeatures: The first parameter must be an array with the length of 4, but got " +
        typeof extent,
    );
    return false;
  }
  if (typeof maxSpeed !== "number") {
    console.error(
      "sta.enlargeExtentForMovableFeatures: The second parameter must be a number, but got " +
        typeof maxSpeed,
    );
    return false;
  }
  if (typeof factor !== "number") {
    console.error(
      "sta.enlargeExtentForMovableFeatures: The third parameter must be a number, but got " +
        typeof factor +
        ". Use the default 10 instead.",
    );
    defaultFactor = 10;
  }
...
Layer2dVectorSensorThings.prototype.getDatastreamIds = function (features) {
  ...
  features.forEach((feature) => {
    if (
      typeof feature?.get === "function" &&
      Array.isArray(feature.get("features"))
    ) {
 ...
 */
Layer2dVectorSensorThings.prototype.getDatastreamIdsHelper = function (
  feature,
  dataStreamIdsArray,
) {
  if (
    ...
    !Array.isArray(dataStreamIdsArray)
  ) {
    return false;
  }
 ...
 * @returns {Boolean} returns true on success and false if something went wrong
 */
Layer2dVectorSensorThings.prototype.unsubscribeFromSensorThings = function (
  datastreamIdsNotToUnsubscribe,
  subscriptionTopics,
  version,
  mqttClient,
) {
  if (
    !Array.isArray(datastreamIdsNotToUnsubscribe) ||
    !isObject(subscriptionTopics) ||
    !isObject(mqttClient)
  ) {
    return false;
  }
  ...
  Object.entries(subscriptionTopics).forEach(([id, isTopicSubscribed]) => {
    if (
      ...
      !Object.prototype.hasOwnProperty.call(datastreamIdsAssoc, id)
    ) {
      mqttClient.unsubscribe(
        "v" + version + "/Datastreams(" + id + ")/Observations",
      );
      if (this.get("observeLocation")) {
        mqttClient.unsubscribe(
          "v" + version + "/Datastreams(" + id + ")/Thing/Locations",
        );
        if (
          typeof this.get("historicalLocations") === "number" &&
          this.showHistoricalFeatures
        ...
        const layerSource =
            this.getLayerSource() instanceof Cluster
              ? this.getLayerSource().getSource()
              : this.getLayerSource(),
          feature = this.getFeatureByDatastreamId(
            layerSource.getFeatures(),
            id,
          );
 ...
 * @returns {Boolean} returns true on success and false if something went wrong
 */
Layer2dVectorSensorThings.prototype.subscribeToSensorThings = async function (
  dataStreamIds,
  subscriptionTopics,
  version,
  mqttClient,
  ...
  for (let i = 0; i < dataStreamIds.length; i++) {
    ...
    if (id && !subscriptionTopics[id]) {
      ...
      const feature = this.getFeatureByDatastreamId(
        layerSource.getFeatures(),
        id,
      );
 ...
 * @returns {void}
 */
Layer2dVectorSensorThings.prototype.updateObservationForDatastreams = function (
  feature,
  dataStreamId,
  observation,
) {
  if (
    typeof feature?.get !== "function" ||
    !Array.isArray(feature.get("Datastreams"))
  ) {
    return;
  }
  ...
  feature.get("Datastreams").forEach((datastream) => {
    if (
      typeof datastream["@iot.id"] !== "undefined" &&
      String(datastream["@iot.id"]) === String(dataStreamId)
    ) {
 ...
 * @returns {void}
 */
Layer2dVectorSensorThings.prototype.updateFeatureLocation = function (
  feature,
  observation,
  onsuccess,
) {
  if (
    ...
    !Array.isArray(observation?.location?.geometry?.coordinates) ||
    !observation.location.geometry.coordinates.length
  ) {
    return;
  }
  const mapProjection = this.get("crs"),
    coordinates =
      this.get("epsg") !== mapProjection
        ? crs.transform(
            this.get("epsg"),
            mapProjection,
            observation.location.geometry.coordinates,
          )
        : observation.location.geometry.coordinates;

  if (typeof onsuccess === "function") {
    this.locationUpdating[feature.get("dataStreamId")] = true;
    this.eventKeys[feature.get("dataStreamId")] = feature
      .getGeometry()
      .once("change", () => {
        onsuccess();
      });
 ...
 * @returns {Boolean} true on success, false if something went wrong
 */
Layer2dVectorSensorThings.prototype.updateFeatureProperties = function (
  feature,
  dataStreamId,
  result,
  phenomenonTime,
  ...
  const dataStreamIdIdx = feature
      .get("dataStreamId")
      .split(" | ")
      .indexOf(String(dataStreamId)),
    dataStreamNameArray = feature.get("dataStreamName").split(" | "),
    dataStreamName = Object.prototype.hasOwnProperty.call(
      dataStreamNameArray,
      dataStreamIdIdx,
    )
      ? dataStreamNameArray[dataStreamIdIdx]
      : "",
    preparedResult = result === "" && showNoDataValue ? noDataValue : result;

  feature.set(
    "dataStream_" + dataStreamId + "_" + dataStreamName,
    preparedResult,
    true,
  );
  feature.set(
  ...
  feature.set(
    "dataStreamValue",
    this.replaceValueInPipedProperty(
      feature,
      "dataStreamValue",
      dataStreamId,
      preparedResult,
  ...
  if (
    typeof feature.get("rotation") !== "undefined" &&
    typeof preparedResult !== "undefined"
  ) {
 ...
 * @returns {void}
 */
Layer2dVectorSensorThings.prototype.registerInteractionMapScaleListeners =
  function () {
    store.watch(
      ...
      (scale) => {
        ...
        this.showHistoricalFeatures = true;
        if (
          scale <= this.get("maxScaleForHistoricalFeatures") &&
          this.lastScale >= this.get("maxScaleForHistoricalFeatures")
        ) {
          this.getHistoricalLocationsOfFeatures();
          this.lastScale = scale;
        }
...
Layer2dVectorSensorThings.prototype.removeHistoricalFeatures = function () {
  const layerSource =
    ...
    featuresWithHistoricalIds = allFeatures.filter(
      (feature) =>
        typeof feature.get("dataStreamId") !== "undefined" &&
        Array.isArray(feature.get("historicalFeatureIds")),
    );
  ...
  featuresWithHistoricalIds.forEach((feature) => {
    if (typeof feature?.get === "function") {
      feature
        .get("historicalFeatureIds")
        .forEach((featureId) =>
          layerSource.removeFeature(layerSource.getFeatureById(featureId)),
        );
      feature.unset("historicalFeatureIds");
    }
  });
 ...
 */
Layer2dVectorSensorThings.prototype.replaceValueInPipedProperty = function (
  feature,
  property,
  dataStreamId,
  value,
  ...
  this.replaceValueInArrayByReference(
    dataStreamProperty,
    dataStreamIds,
    dataStreamId,
    value,
 ...
 * @returns {Boolean} true if the function ran successfull, false if not
 */
Layer2dVectorSensorThings.prototype.replaceValueInArrayByReference = function (
  result,
  referenceArray,
  reference,
  value,
 ...
 * @returns {void}
 */
Layer2dVectorSensorThings.prototype.fetchHistoricalLocations = function (
  url,
  urlParams,
  version,
  onsuccess,
 ...
 * @returns {void}
 */
Layer2dVectorSensorThings.prototype.getHistoricalLocationsOfFeatures =
  function () {
    const allFeatures = (
        this.getLayerSource() instanceof Cluster
          ? this.getLayerSource().getSource()
      ...
      featuresWithoutHistoricalIds = allFeatures.filter(
        (feature) =>
          typeof feature.get("dataStreamId") !== "undefined" &&
          !Array.isArray(feature.get("historicalFeatureIds")),
      ),
      datastreamIds = this.getDatastreamIdsInCurrentExtent(
        featuresWithoutHistoricalIds,
        store.getters["Maps/extent"],
      ),
      url = this.get("url"),
      urlParam = {
 ...
 * @returns {void}
 */
Layer2dVectorSensorThings.prototype.fetchHistoricalLocationsByDatastreamId =
  function (
    allFeatures,
    datastreamId,
    amount,
    ...
    if (
      this.subscribedDataStreamIds[datastreamId]?.subscribed &&
      feature.get("historicalFeatureIds")
    ) {
      return;
    }
    this.fetchHistoricalLocations(
      url,
      urlParam,
      version,
      (historicalSensorData) => {
        ...
        if (
          !Array.isArray(historicalSensorData) ||
          !Array.isArray(historicalSensorData[0]?.Locations) ||
          !historicalSensorData[0].Locations.length
        ) {
          return;
        }
        this.updateFeatureLocation(
          feature,
          historicalSensorData[0].Locations[0],
          () => {
            this.parseSensorDataToFeature(
              feature,
              historicalSensorData,
 ...
 * @returns {void}
 */
Layer2dVectorSensorThings.prototype.parseSensorDataToFeature = function (
  feature,
  sensorData,
  urlParam,
  url,
  ...
  const layerSource =
    ...
    historicalFeatures = this.createFeaturesFromSensorData(
      things,
      mapProjection,
      epsg,
      gfiTheme,
 ...
 * @returns {void}
 */
Layer2dVectorSensorThings.prototype.resetHistoricalLocations = function (
  datastreamId,
) {
  const layerSource =
      this.getLayerSource() instanceof Cluster
    ...
    feature = layerSource
      ...
      .filter(
        (layerFeature) =>
          typeof layerFeature?.get === "function" &&
          layerFeature.get("dataStreamId") === datastreamId,
      )[0];
 ...
 */
Layer2dVectorSensorThings.prototype.getScale = function (
  index,
  amount,
  scaleStyleByZoom = false,
  zoomLevel = 1,
  zoomLevelCount = 1,
) {
  if (scaleStyleByZoom) {
 ...
 * @returns {void}
 */
Layer2dVectorSensorThings.prototype.setStyleOfHistoricalFeature = function (
  feature,
  scale,
  styleRule,
) {
  if (!Array.isArray(styleRule) || !styleRule.length || !styleRule[0].style) {
    console.error(
      "The style rule has wrong type, must be an array with at least one entry with prop style!",
      styleRule,
    );
    return;
  }
  feature.setStyle(() =>
    this.getStyleOfHistoricalFeature(styleRule[0].style, scale),
  );
};
 ...
 * @returns {void}
 */
Layer2dVectorSensorThings.prototype.getStyleOfHistoricalFeature = function (
  style,
  scale,
) {
  let circleRadius, circleFillColor, circleStrokeColor, circleStrokeWidth;
 ...
 * @returns {void}
 */
Layer2dVectorSensorThings.prototype.setDynamicalScaleOfHistoricalFeatures =
  function (
    features,
    zoomLevel,
    zoomLevelCount,
      ...
      if (feature.getStyle()) {
        ...
        if (typeof style.getImage === "function" && style.getImage() !== null) {
          style
            .getImage()
            .setScale(
              (feature.get("originScale") * zoomLevel) / zoomLevelCount,
            );
 ...
 * @returns {void}
 */
Layer2dVectorSensorThings.prototype.registerInteractionMapResolutionListeners =
  function (scaleStyleByZoom) {
    if (!scaleStyleByZoom) {
      return;
    }
    store.watch(
      (state, getters) => getters["Maps/resolution"],
      (resolution) => {
        const layerSource =
            this.getLayerSource() instanceof Cluster
              ? this.getLayerSource().getSource()
              : this.getLayerSource(),
          allFeatures = layerSource.getFeatures(),
          historicalFeatures = allFeatures.filter(
            (feature) =>
              typeof feature.get("dataStreamId") === "undefined" &&
              typeof feature.get("originScale") !== "undefined",
          ),
          zoomLevel =
            mapCollection.getMapView("2D").getZoomForResolution(resolution) + 1,
          zoomLevelCount = mapCollection
            .getMapView("2D")
            .getResolutions().length;

        this.setDynamicalScaleOfHistoricalFeatures(
          historicalFeatures,
          zoomLevel,
          zoomLevelCount,
          this.get("observeLocation"),

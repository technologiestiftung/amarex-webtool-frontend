...
Layer2dVectorOaf.prototype.getRawLayerAttributes = function (attributes) {
  return {
    ...
    crs:
      attributes.crs === false || attributes.crs
        ? attributes.crs
        ...
        (error) => {
          if (error) {
            store.dispatch(
              ...
              "<strong>" +
                i18next.t(
                  "common:core.layers.errorHandling.getGeometryTypeFromOAFFetchfailed",
                ) +
                "</strong> <br>" +
                "<small>" +
                i18next.t(
                  "common:core.layers.errorHandling.getGeometryTypeFromOAFFetchfailedMessage",
                ) +
                "</small>",
            );
          }

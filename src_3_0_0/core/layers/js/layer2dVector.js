...
Layer2d.prototype.updateLayerValues = function (attributes) {
  ...
  if (
    ...
    this.layerSource.getFeatures().length === 0
  ) {
...
Layer2dVector.prototype.featuresFilter = function (attributes, features) {
  let filteredFeatures = features.filter(
    (feature) => feature.getGeometry() !== undefined,
  );

  if (attributes.bboxGeometry) {
    filteredFeatures = filteredFeatures.filter((feature) =>
      attributes.bboxGeometry.intersectsCoordinate(
...
Layer2dVector.prototype.loadingParams = function (attributes) {
  const loadingParams = {
    xhrParameters: attributes.isSecured
      ? { credentials: "include" }
      : undefined,
    propertyname: this.propertyNames(attributes) || undefined,
    // only used if loading strategy is all
    bbox: attributes.bboxGeometry
      ? attributes.bboxGeometry.getExtent().toString()
      : undefined,
  };
...
Layer2dVector.prototype.createStyle = async function (attrs) {
  ...
  if (styleObject !== undefined) {
      ...
      const feat = feature !== undefined ? feature : this,
        isClusterFeature =
          ...
          (typeof feat.get("features") === "object" &&
            Boolean(feat.get("features")));

      return createStyle.createStyle(
        styleObject,
        feat,
        isClusterFeature,
        Config.wfsImgPath,
    ...
    console.warn(
      i18next.t("common:core.layers.errorHandling.wrongStyleId", { styleId }),
    );
  }
};
...
Layer2dVector.prototype.createLegend = async function () {
  const styleObject = styleList.returnStyleObject(this.attributes.styleId),
    ...
    isSecured =
      typeof this.attributes.isSecured === "boolean"
        ? this.attributes.isSecured
        : false;
    ...
    if (styleObject && legend === true) {
      ...
      if (styleObject.styleId === "default") {
        ...
        if (!this.geometryTypeRequestLayers.includes(this.get("id"))) {
          ...
          getGeometryTypeFromService.getGeometryTypeFromWFS(
            rules,
            this.get("url"),
            this.get("version"),
            ...
            (geometryTypes, error) => {
              if (error) {
                store.dispatch(
                  ...
                  "<strong>" +
                    i18next.t(
                      "common:core.layers.errorHandling.getGeometryTypeFromWFSFetchfailed",
                    ) +
                    "</strong> <br>" +
                    "<small>" +
                    i18next.t(
                      "common:core.layers.errorHandling.getGeometryTypeFromWFSFetchfailedMessage",
                    ) +
                    "</small>",
                );
              }
        ...
        if (
          ...
          legend = this.filterUniqueLegendInfo(
            this.layer.getSource().getFeatures(),
            rules,
 ...
 */
Layer2dVector.prototype.filterUniqueLegendInfo = function (
  features,
  rules,
  legendInfos,
) {
  const rulesKey = Object.keys(rules[0].conditions.properties)[0],
    conditionProperties = [
      ...new Set(features.map((feature) => feature.get(rulesKey))),
    ];
  ...
  rules.forEach((rule) => {
    ...
    if (conditionProperties.includes(value)) {
      const legendInformation = legendInfos.find(
        (legendInfo) =>
          legendInfo?.label === (rule.style?.legendValue || value),
      );

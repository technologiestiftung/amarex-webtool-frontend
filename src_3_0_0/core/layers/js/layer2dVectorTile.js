...
Layer2dVectorTile.prototype.createLayer = function (attributes) {
  ...
  store.dispatch("Maps/registerListener", {
    ...
    listener: () => {
      if (
        typeof this.layer.getSource !== "function" ||
...
Layer2dVectorTile.prototype.setConfiguredLayerStyle = function () {
  ...
  if (this.get("styleId") && this.get("styleId") !== "default") {
    ...
    if (typeof this.get("vtStyles") !== "undefined") {
      const style =
        this.get("vtStyles").find(({ defaultStyle }) => defaultStyle) ||
        this.get("vtStyles")[0];
    ...
    if (!stylingPromise) {
      console.warn(
        `Rendering VT layer ${this.get("name")} without style; falls back to OL default styles.`,
      );
      return;
    }
  }
...
Layer2dVectorTile.prototype.setStyleById = function (styleID) {
  const styleDefinition = this.get("vtStyles")?.find(
    ({ id }) => id === styleID,
  );

  if (!styleDefinition) {
    return Promise.reject(
      `No style found with ID ${styleID} for layer ${this.get("name")}.`,
    );
  }
 ...
 */
Layer2dVectorTile.prototype.setStyleByDefinition = function ({
  id,
  url,
  resolutions,
}) {
  return axios
    ...
    .then((style) => {
      ...
      if (style.sprite) {
        ...
        this.fetchSpriteData(spriteDataUrl).then((spriteData) => {
          vectorTile.setStyle(
            this.getLayer(),
            style,
            {
        ...
        vectorTile.setStyle(
          this.getLayer(),
          style,
          { resolutions: resolutions, getFonts: addMpFonts },
          url,
        );
        this.set("selectedStyleID", id);
      }
...
Layer2dVectorTile.prototype.fetchSpriteData = function (spriteUrl) {
  return axios.get(spriteUrl).then((resp) => resp.data);
};

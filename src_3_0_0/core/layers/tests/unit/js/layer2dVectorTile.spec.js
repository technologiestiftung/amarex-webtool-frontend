...
describe("src_3_0_0/core/js/layers/layer2dVectorTile.js", () => {
  const attrs = {
      epsg: "EPSG:3857",
      extent: [
        902186.6748764697, 7054472.604709217, 1161598.3542590786,
        7175683.411718197,
      ],
  ...
  describe("getRawLayerAttributes", () => {
    ...
    it("should return the raw layer attributes", () => {
      ...
      expect(
        vectorTileLayer.getRawLayerAttributes(localAttributes),
      ).to.deep.equals({
        id: "911",
        maxScale: "1000000",
        minScale: "0",
  ...
  describe("vector tile layer config", () => {
    it("should create a layer with minimal config", () => {
      ...
      expect(source.getUrls()[0]).to.equal(
        "https://doesthisurlexist.de/vt/tiles/esri/Test_VT_3857/p12/tile/{z}/{y}/{x}.pbf",
      );
    });

    it("should apply in services.json.md defined defaults", () => {
      const defaultValues = {
          // defaults defined in services.json.md
          zDirection: 1,
          epsg: "EPSG:25832", // default value from config.json.md/MapView. Should be the default CRS
          extent: [
            // If not set, the portal's coordinate reference system's extent is used
            -1877994.66,
            3932281.56, 836715.13, 9440581.95,
          ],
          origin: [
            // if not set, the portal's coordinate reference system's top-left corner is used.
            -1877994.66,
            836715.13,
          ],
          resolutions: [
            // If not used, the portal's resolutions are used. (Missing default resolution definition? used default resolutions from masterportal-api)
            66.14579761460263,
            26.458319045841044, 15.874991427504629, 10.583327618336419,
            5.2916638091682096, 2.6458319045841048, 1.3229159522920524,
            0.6614579761460262, 0.2645831904584105, 0.1322915952292052,
          ],
          tileSize: 512,
      ...
      expect(tileGrid.getExtent()).to.deep.equal([
        -20015077.371242613, -20015077.371242613, 20015077.371242613,
        20015077.371242613,
      ]);
      ...
      expect(tileGrid.getOrigin()).to.deep.equal([
        -20015077.371242613, 20015077.371242613,
      ]);
      expect(tileGrid.getResolutions()).to.deep.equal(
        defaultValues.resolutions,
      );
      expect(tileGrid.getTileSize()).to.deep.equal(
        new Array(2).fill(defaultValues.tileSize),
      );
    });
    ...
    it("should apply given attributes correct", () => {
      ...
      expect(vectorTileLayer.get("gfiAttributes")).to.equal(
        attrs.gfiAttributes,
      );
  ...
  describe("setConfiguredLayerStyle", () => {
    ...
    it("uses config.json style first", (done) => {
      const context = makeContext({
          styleId: "lConfigJson",
          givenVtStyles: vtStylesDefaultL2,
          done,
        }),
        { set } = context;
  ...
  describe("addMpFonts", () => {
    it("returns Masterportal italic font if style is italic", () => {
      const italicFont1 = "Font italic",
        ...
        returnedItalicFont1 =
          Layer2dVectorTile.prototype.addMpFonts(italicFont1),
        returnedItalicFont2 =
          Layer2dVectorTile.prototype.addMpFonts(italicFont2),
        returnedItalicFont3 =
          Layer2dVectorTile.prototype.addMpFonts(italicFont3);
  ...
  describe("showFeaturesByIds", function () {
    ...
    it("should set tileLoadFunction", () => {
      ...
      vtLayer.showFeaturesByIds([]);
      expect(source.getTileLoadFunction()).to.not.be.equal(
        "defaultLoadFunction",
      );
    });
  });
  ...
  describe("fetchSpriteData", () => {
    it("Creates a VectorTileLayer", async () => {
      const url =
          "https://testemich.de/vt/tiles/esri/Test_VT_3857/p12/resources/sprites/sprite.json",
        resp = {
          config: {
            transitional: {},
  ...
  describe("Use WebGL renderer", () => {
    it('Should create the layer with WebGL methods, if renderer: "webgl" is set', function () {
      const vectorLayer = new Layer2dVectorTile({
          ...attributes,
          renderer: "webgl",
          isPointLayer: false,
        }),

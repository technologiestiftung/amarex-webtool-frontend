...
describe("src_3_0_0/core/js/layers/layer2dVector.js", () => {
  let attributes, error, warn, styleListStub;
  ...
  beforeEach(() => {
    ...
    styleListStub = sinon
      .stub(styleList, "returnStyleObject")
      .returns(styleObj);
  });
  ...
  describe("featuresFilter", () => {
    it("featuresFilter shall filter getGeometry", function () {
      ...
      expect(layer2d.featuresFilter(attributes, features).length).to.be.equals(
        1,
      );
    });
  ...
  describe("filterUniqueLegendInfo", () => {
    it("returns uniqueLegendInfo for several features with same condition property (same style)", () => {
      ...
      expect(
        vectorLayer.filterUniqueLegendInfo(
          features,
          rules,
          legendInfos.legendInformation,
        ),
      ).to.deep.equal(expectedUniqueLegendInfo);
      expect(
        vectorLayer.filterUniqueLegendInfo(
          features,
          rules,
      ...
      ).to.not.deep.equal(features.length);
    });
    ...
    it("should not fail if some rules have no condition", () => {
      ...
      expect(
        vectorLayer.filterUniqueLegendInfo(features, rules, legendInfos),
      ).to.deep.equal(expectedUniqueLegendInfo);
    });
  });
});

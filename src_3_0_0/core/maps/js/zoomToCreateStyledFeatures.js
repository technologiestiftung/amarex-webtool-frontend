...
export default function (features, styleId) {
  return features
    .map((feature) =>
      calculateCenterOfExtent(feature.getGeometry().getExtent()),
    )
    .map(
      (centerCoordinates, index) =>
        new Feature({
    ...
    .map((feature) => {
      ...
      if (isUrl(styleId)) {
        console.warn(
          "zoomTo: The usage of the configuration parameter 'imgLink' will be deprecated in v3.0.0. Please use 'styleId' instead.",
        );
        style = new Style({
          image: new Icon({

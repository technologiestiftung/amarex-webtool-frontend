...
export default {
   ...
   */
  createVectorLayer: async function (
    styleId,
    layerId,
    name,
    gfiAttributes,
   ...
   * @returns {void}
   */
  highlightPointFeature: function (
    styleId,
    layerId,
    name,
    rawLayer,
  ...
  showLayer: async function (
    ...
    const highlightLayer = await this.createVectorLayer(
      styleId,
      layerId,
      rootGetters.treeHighlightedFeatures?.layerName || name,
      rawLayer.gfiAttributes,
      dispatch,
    );
    ...
    dispatch(
      "Maps/zoomToExtent",
      { extent: highlightLayer.getSource().getExtent() },
      { root: true },
    );
  },
   ...
   * @returns {void}
   */
  highlightLineOrPolygonFeature: async function (
    styleId,
    layerId,
    name,
    geometryRequested,
  ...
  handleGetFeatureError: function (dispatch, error) {
    ...
    dispatch(
      ...
      i18next.t(
        "common:core.maps.highlightFeaturesByAttribute.messages.requestFailed",
      ),
      { root: true },
    );
  },
   ...
   * @returns {void}
   */
  handleGetFeatureResponse: function (
    dispatch,
    rootGetters,
    response,
    ...
    const features = new WFS({
      version: highlightFeaturesLayer.version,
    ...
    if (features.length === 0) {
      const parser = new DOMParser(),
        ...
        exceptionText =
          xmlDoc.getElementsByTagNameNS("*", "ExceptionText").length > 0
            ? xmlDoc.getElementsByTagNameNS("*", "ExceptionText")[0]
                .childNodes[0].nodeValue
            : "";

      if (exceptionText) {
        console.error(
          "highlightFeaturesByAttribute: service exception: " + exceptionText,
        );
        dispatch(
          "Alerting/addSingleAlert",
          i18next.t(
    ...
    this.highlightPointFeature(
      this.settings.pointStyleId,
      "highlight_point_layer",
      "highlightPoint",
      highlightFeaturesLayer,
      features,
      dispatch,
    ...
    this.highlightLineOrPolygonFeature(
      this.settings.polygonStyleId,
      "highlight_polygon_layer",
      "highlightPolygon",
      "Polygon",
      highlightFeaturesLayer,
      features,
   ...
   * @returns {String} query snippet
   */
  getOGCFilterSnippetIn: function (
    valueDelimiter,
    wildCard,
    singleChar,
    escapeChar,
  ...
  configHasErrors: function (layer, wfsId) {
    if (!layer) {
      console.error(
        "highlightFeaturesByAttribute: Layer with ID " +
          wfsId +
          " not found in Config",
      );
      return true;
    ...
    if (!layer.wildCard || layer.wildCard?.length !== 1) {
      console.error(
        "highlightFeaturesByAttribute: wildCard config setting must exist and be one character",
      );
      return true;
    }
    if (!layer.singleChar || layer.singleChar?.length !== 1) {
      console.error(
        "highlightFeaturesByAttribute: singleChar config setting must exist and be one character",
      );
      return true;
    }
    if (!layer.escapeChar || layer.escapeChar?.length !== 1) {
      console.error(
        "highlightFeaturesByAttribute: escapeChar config setting must exist and be one character",
      );
      return true;
    }
  ...
  highlightFeaturesByAttribute: function (
    ...
    const layer = rootGetters.layerConfigById(wfsId),
      ...
      filterSnippet = this.getOGCFilterSnippet(
        isEqual,
        layer?.wildCard,
        layer?.singleChar,
        layer?.escapeChar,
    ...
    let requestBody = this.getWFSQuery(
      layer?.featureType,
      layer?.version,
      filterSnippet,
    );
    ...
    if (isIn) {
      const filterSnippetIn = this.getOGCFilterSnippetIn(
        layer?.valueDelimiter,
        layer?.wildCard,
        layer?.singleChar,
        layer?.escapeChar,
        layer?.featurePrefix,
    ...
    if (this.configHasErrors(layer, wfsId)) {
      dispatch(
        "Alerting/addSingleAlert",
        i18next.t(
          "common:core.maps.highlightFeaturesByAttribute.messages.configurationError",
        ),
        { root: true },
      );
      return;

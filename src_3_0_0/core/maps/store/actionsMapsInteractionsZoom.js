...
export default {
  ...
  zoomToFilteredFeatures({ getters, dispatch }, { ids, layerId, zoomOptions }) {
    ...
    if (layer?.getSource()) {
      const source =
        layerSource instanceof Cluster ? layerSource.getSource() : layerSource;
      let filteredFeatures = source
          .getFeatures()
          .filter((feature) => ids.indexOf(feature.getId()) > -1),
    ...
    if (Object.values(zoomParams).every((val) => val !== undefined)) {
      ...
      if (projection !== getters["Maps/projectionCode"]) {
        const map2d = mapCollection.getMap("2D"),
          ...
          transformedLeftBottom = crs.transformToMapProjection(
            map2d,
            zoomParams.projection,
            leftBottom,
          ),
          transformedTopRight = crs.transformToMapProjection(
            map2d,
            zoomParams.projection,
            topRight,
          );

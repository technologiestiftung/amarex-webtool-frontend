...
export default {
  ...
  registerListener(
    { commit, dispatch },
    { type, listener, listenerType = "function", root = false },
  ) {
    registeredActions[type] = registeredActions[type] || {};
    registeredActions[type][listenerType] =
      registeredActions[type][listenerType] || {};
    ...
    mapCollection
      .getMap("2D")
      .on(type, registeredActions[type][listenerType][listener]);
  },
  ...
  setCamera({ rootGetters }, cameraParams) {
    ...
    if (map3d) {
      api.map.olcsMap.setCameraParameter(
        cameraParams,
  ...
  setCenter({ commit }, coords) {
    ...
    if (first2Coords.some((coord) => typeof coord !== "number")) {
      first2Coords = first2Coords.map((singleCoord) =>
        parseInt(singleCoord, 10),
      );
    }
    if (Array.isArray(first2Coords) && first2Coords.length === 2) {
      ...
      console.warn(
        "Center was not set. Probably there is a data type error. The format of the coordinate must be an array with two numbers.",
      );
    }
  },
  ...
  unregisterListener(context, { type, listener, listenerType = "function" }) {
    if (typeof type === "string") {
      if (
        ...
        registeredActions[type][listenerType] &&
        registeredActions[type][listenerType][String(listener)]
      ) {
        mapCollection
          .getMap("2D")
          .un(type, registeredActions[type][listenerType][String(listener)]);

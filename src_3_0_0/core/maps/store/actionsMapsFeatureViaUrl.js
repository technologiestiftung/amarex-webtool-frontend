...
export function createGeoJSON(features, geometryType, epsg = 4326) {
  ...
  features.forEach((feature) => {
    coordinates =
      epsg === 4326
        ? feature.coordinates
        : crs.transform("EPSG:" + epsg, "EPSG:4326", feature.coordinates);
    if (
      !coordinates ||
...
export function getFeatureIds(layerId) {
  ...
  if (typeof layer === "undefined") {
    console.warn(
      i18next.t("common:core.maps.featureViaURL.messages.layerNotFound"),
    );
    return featureArray;
  }
  layer
  ...
  featureViaUrl({ dispatch, rootGetters }, urlLayers) {
    ...
    if (layers) {
      urlLayers.forEach((layer) => {
        ...
        if (!layers[pos].name) {
          console.error(
            i18next.t("common:core.maps.featureViaURL.messages.noNameDefined", {
              layerId,
            }),
          );
          return;
        ...
        geometryType =
          layer.type !== undefined ? layer.type : layers[pos].geometryType;
        if (
          ...
          console.error(
            i18next.t(
              "common:core.maps.featureViaURL.messages.geometryNotSupported",
            ),
        ...
        if (!features || !Array.isArray(features) || features.length === 0) {
          dispatch(
            ...
            {
              content: i18next.t(
                "common:core.maps.featureViaURL.messages.featureParsingAll",
              ),
              multipleAlert: true,
            },
            { root: true },
        ...
        if (geoJSON.features.length === 0) {
          dispatch(
            ...
            {
              content: i18next.t(
                "common:core.maps.featureViaURL.messages.featureParsingNoneAdded",
              ),
              multipleAlert: true,
            },
            { root: true },
        ...
        if (
          ...
          dispatch("zoomToFilteredFeatures", {

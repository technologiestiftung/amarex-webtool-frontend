...
export default {
  ...
  changeMarkerStyle(context, { markerId, styleId }) {
    mapMarker
      .getMapmarkerLayerById(markerId)
      ?.set("styleId", styleId || "defaultMapMarkerPoint");
  },
  ...
  rotatePointMarker({ dispatch, state }, { feature, position }) {
    feature
      .getStyle()
      ?.getImage()
      ?.setRotation((position.rotation * Math.PI) / 180);
  ...
  rotatePointMarkerIn3D({ rootGetters }, position) {
    ...
    if (clickPixel) {
      mapCollection
        .getMap("3D")
        .getCesiumScene()
        .drillPick({ x: clickPixel[0], y: clickPixel[1] }, 100, 200, 200)
        .forEach((primitiveObject) => {
          if (
            primitiveObject?.primitive?.olLayer?.get("id") ===
            "marker_point_layer"
          ) {
            switch (angle) {
              case 0: {
                pixelOffset = {
                  x:
                    (primitiveObject.primitive.olFeature
                      .getStyle()
                      .getImage()
                      .getAnchor()[0] *
                      primitiveObject.primitive.scale -
                      (primitiveObject.primitive.width -
                        primitiveObject.primitive.olFeature
                          .getStyle()
                          .getImage()
                          .getAnchor()[0]) *
                  ...
                  y:
                    -(
                      primitiveObject.primitive.olFeature
                        ...
                        .getAnchor()[1] *
                        primitiveObject.primitive.scale -
                      (primitiveObject.primitive.height -
                        primitiveObject.primitive.olFeature
                          .getStyle()
                          .getImage()
                          .getAnchor()[1]) *

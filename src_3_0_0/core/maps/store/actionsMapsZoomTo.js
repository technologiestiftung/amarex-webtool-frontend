  ...
  zoomToFeatures({ dispatch, rootGetters }, param) {
    ...
    featurePromises = config.map((conf) => {
      ...
      if (!urlValues) {
        ...
        urlValues = (
          Array.isArray(urlValues) ? urlValues : urlValues.split(",")
        ...
        if (
          ...
          !allowedValues.includes(urlValues[0]) &&
          parseInt(urlValues[0], 10) < allowedValues.length &&
          parseInt(urlValues[0], 10) > 0
        ) {
      ...
      return getAndFilterFeatures(layerId, property, urlValues).then(
        (featureCollection) => {
          ...
          if (allowedValues !== undefined) {
            filteredFeatures = filteredFeatures.filter((feature) =>
              allowedValues.includes(
                feature.get(property).toUpperCase().trim(),
              ),
            );
          }
          if (styleId) {
    ...
    return Promise.allSettled(featurePromises)
      .then((results) => {
        ...
        if (features.length > 0) {
          return dispatch(
            "Maps/zoomToExtent",
            { extent: calculateExtent(features) },
            { root: true },
          );
        }
        return console.warn(
      ...
      .catch((error) => {
        console.error(
          "zoomTo: An error occurred while trying to fetch features from one of the given services.",
          error,
        );
      });
  },

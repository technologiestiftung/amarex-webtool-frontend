...
export default {
  ...
  registerCesiumListener({ dispatch, rootGetters }) {
    ...
    if (
      rootGetters["Modules/GetFeatureInfo/coloredHighlighting3D"] &&
      rootGetters["Modules/GetFeatureInfo/coloredHighlighting3D"]?.enabled !==
        false
    ) {
    ...
    api.map.olcsMap.handle3DEvents({
      scene: scene3d,
      map3D: map3d,
      callback: (clickObject) =>
        dispatch("updateAttributesByClick", Object.freeze(clickObject)),
    });

    eventHandler.setInputAction(
      (evt) => dispatch("updateAttributesByPointer", evt),
  ...
  setInitialAttributes({ commit }) {
    ...
    commit(
      "setScales",
      mapView.get("options").map((option) => option.scale),
    );

...
describe("src_3_0_0/core/maps/js/highlightFeaturesByAttribute", () => {
  ...
  describe("getOGCFilterSnippet for isEqual", () => {
    it("should return isEqual XML Snippet", function () {
      ...
      expect(
        highlightFeaturesByAttribute.getOGCFilterSnippet(
          isEqual,
          wildCard,
          singleChar,
          escapeChar,
      ...
      ).to.have.string(expectedEqualToOGC);
    });
  });
  ...
  describe("getOGCFilterSnippet for isLike", () => {
    it("should return isLike XML Snippet", function () {
      ...
      ).to.have.string(expectedIsLikeOGC);
    });
  });
  ...
  describe("getWFSQuery for P_TIERARTEN_INVASIV", () => {
    it("should return full query Snippet for equalTo", function () {
      ...
      expect(
        highlightFeaturesByAttribute.getWFSQuery(
          featureType,
          version,
          expectedEqualToOGC,
        ),
      ).to.have.string(expectedWFSQueryEqualTo);
    });
    ...
    it("should return full query Snippet for isLike", function () {
      ...
      ).to.have.string(expectedWFSQueryIsLike);
    });
  });
  ...
  describe("createLayer", () => {
    it("createVectorLayer should start the action addLayerToLayerConfig", () => {
      ...
      highlightFeaturesByAttribute.createVectorLayer(
        styleId,
        layerId,
        layerName,
        gfiAttributes,
  ...
  describe("highlight(Point/Polygon/LineString)Feature", () => {
    ...
    let highlightVector, showLayerSpy;
    ...
    beforeEach(() => {
      ...
      sinon
        .stub(highlightFeaturesByAttribute, "createVectorLayer")
        .returns(highlightVector);
    ...
    it("should call showLayer and return 2 features for Points", async () => {
      ...
      highlightFeaturesByAttribute.highlightPointFeature(
      ...
      expect(showLayerSpy.firstCall.args[2]).to.equals(
        "defaultHighlightFeaturesPoint",
      );
    ...
    it("should call showLayer and return 2 features for Polygons", async () => {
      const layer = {
        ...
        dispatch = sinon.spy(),
        rootGetters = {
          treeHighlightedFeatures: { active: false },
          treeType: undefined,
        };

      highlightFeaturesByAttribute.highlightLineOrPolygonFeature(
      ...
      expect(showLayerSpy.calledOnce).to.be.true;
      expect(showLayerSpy.firstCall.args[0]).to.equals(
        "highlight_polygon_layer",
      );
      expect(showLayerSpy.firstCall.args[1].length).to.equals(2);
      expect(showLayerSpy.firstCall.args[2]).to.equals(
  ...
  describe("configHasErrors", () => {
    ...
    it("configHasErrors should return false", () => {
      expect(highlightFeaturesByAttribute.configHasErrors(layer, wfsId)).to.be
        .false;
    });

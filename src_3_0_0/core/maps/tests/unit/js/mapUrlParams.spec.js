...
describe("src_3_0_0/core/maps/js/mapUrlParams.js", () => {
  ...
  describe("featureViaUrl", () => {
    it("should add a feature to map via url params", () => {
      const params = {
        FEATUREVIAURL:
          '[{"layerId":"42","features":[{"coordinates":[10,53.5],"label":"TestPunkt"}]}]',
      };
  ...
  describe("highlightFeature", () => {
    ...
    it("should highlight a list of features with HIGHLIGHTFEATURE", () => {
      const params = {
        HIGHLIGHTFEATURE:
          "1711,DE.HH.UP_GESUNDHEIT_KRANKENHAEUSER_6,DE.HH.UP_GESUNDHEIT_KRANKENHAEUSER_18,DE.HH.UP_GESUNDHEIT_KRANKENHAEUSER_29",
      };
      ...
      expect(dispatchCalls["Maps/zoomToFilteredFeatures"]).to.deep.equals({
        ids: [
          "DE.HH.UP_GESUNDHEIT_KRANKENHAEUSER_6",
          "DE.HH.UP_GESUNDHEIT_KRANKENHAEUSER_18",
          "DE.HH.UP_GESUNDHEIT_KRANKENHAEUSER_29",
        ],
        layerId: "1711",
        zoomOptions: { duration: 0 },
      });
  ...
  describe("highlightFeaturesByAttributes", () => {
    ...
    beforeEach(() => {
      highlightFeaturesByAttributeStub = sinon.stub(
    ...
    it("should highlight a feature with HIGHLIGHTFEATURESBYATTRIBUTE", () => {
      ...
      expect(highlightFeaturesByAttributeStub.firstCall.args[0]).to.equals(
        store.dispatch,
      );
      expect(highlightFeaturesByAttributeStub.firstCall.args[1]).to.equals(
        store.getters,
      );
      expect(highlightFeaturesByAttributeStub.firstCall.args[2]).to.equals(
        "8712",
      );
      expect(highlightFeaturesByAttributeStub.firstCall.args[3]).to.equals(
        "bezirk",
      );
      expect(highlightFeaturesByAttributeStub.firstCall.args[4]).to.equals(
        "Altona",
      );
      expect(highlightFeaturesByAttributeStub.firstCall.args[5]).to.equals(
        "IsLike",
      );
    });
  ...
  describe("setMapMarker", () => {
    it("should set coordinates to mapMarker with MARKER", () => {
      ...
      expect(Object.keys(dispatchCalls).length).to.equals(1);
      expect(dispatchCalls["Maps/placingPointMarker"]).to.deep.equals([
        565874, 5934140,
      ]);
    });

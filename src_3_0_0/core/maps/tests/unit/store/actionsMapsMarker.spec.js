...
describe("src_3_0_0/core/maps/store/actionsMapsMarker.js", () => {
  ...
  describe("changeMarkerStyle", () => {
    it("change the styleId of the marker", () => {
      ...
      expect(mapMarker.getMapmarkerLayerById.firstCall.args[0]).to.equals(
        markerId,
      );
    });
  });
  ...
  describe("placingPointMarker", () => {
    it("place a point marker, if position is an array", () => {
      ...
      expect(mapMarker.addFeatureToMapMarkerLayer.firstCall.args[0]).to.equals(
        "marker_point_layer",
      );
      expect(
        mapMarker.addFeatureToMapMarkerLayer.firstCall.args[1] instanceof
          Feature,
      ).to.be.true;
    });
  ...
  describe("removePointMarker", () => {
    it("remove a point marker", () => {
      ...
      expect(mapMarker.removeMapMarker.firstCall.args[0]).to.equals(
        "marker_point_layer",
      );
    });
  });
  ...
  describe("rotatePointMarkerIn3D", () => {
    ...
    it("rotatePointMarkerIn3D with angle 0", () => {
      ...
      expect(
        mapCollection.getMap("3D").getCesiumScene().drillPick()[0].primitive
          .rotation,
      ).to.equal(rotation);
      expect(
        mapCollection.getMap("3D").getCesiumScene().drillPick()[0].primitive
          .pixelOffset,
      ).to.deep.equal(pixelOffset);
    });

...
describe("src_3_0_0/core/maps/store/actionsMapsAttributesMapper.js", () => {
  const options = [
    ...
    resolutions = [
      66.14579761460263, 26.458319045841044, 15.874991427504629,
      10.583327618336419, 5.2916638091682096, 2.6458319045841048,
    ];
  let commit, dispatch, getters, map2d;
  ...
  describe("setMapAttributes", () => {
    it("Should dispatch map attributes", () => {
      ...
      expect(dispatch.secondCall.args[0]).to.deep.equals(
        "registerMapViewListener",
      );
      expect(dispatch.thirdCall.args[0]).to.deep.equals("setInitialAttributes");
      expect(dispatch.getCall(3).args[0]).to.deep.equals(
        "updateAttributesByMoveend",
      );
      expect(dispatch.getCall(4).args[0]).to.deep.equals(
        "updateAttributesByChangeResolution",
      );
    });
  });
  ...
  describe("registerMapListener", () => {
    it("Should register listeners", () => {
      ...
      expect(dispatch.firstCall.args).to.deep.equals([
        "registerListener",
        {
          type: "change:size",
      ...
      expect(dispatch.secondCall.args).to.deep.equals([
        "registerListener",
        {
          type: "click",
      ...
      expect(dispatch.thirdCall.args).to.deep.equals([
        "registerListener",
        {
          type: "moveend",
      ...
      expect(dispatch.getCall(3).args).to.deep.equals([
        "registerListener",
        {
          type: "pointermove",
  ...
  describe("setInitialAttributes", () => {
    it("Should set initial attributes", () => {
      ...
      expect(commit.firstCall.args).to.deep.equals([
        "setInitialCenter",
        [10, 20],
      ]);
      ...
      expect(commit.getCall(5).args).to.deep.equals([
        "setResolutions",
        [
          66.14579761460263, 26.458319045841044, 15.874991427504629,
          10.583327618336419, 5.2916638091682096, 2.6458319045841048,
      ...
      expect(commit.getCall(6).args).to.deep.equals([
        "setScales",
        [250000, 100000, 60000, 40000, 20000, 10000],
      ]);
      expect(commit.getCall(7).args).to.deep.equals(["setSize", [50, 50]]);
    });
  });
  ...
  describe("updateAttributesByMoveend", () => {
    it("Should update attributes by moveend event", () => {
      ...
      expect(commit.secondCall.args).to.deep.equals([
        "setExtent",
        [
          -56.145797614602614, -46.145797614602614, 76.14579761460261,
          86.14579761460261,
        ],
      ]);
    });
  ...
  describe("updateAttributesByChangeResolution", () => {
    it("Should update attributes by change resolution event", () => {
      ...
      expect(commit.thirdCall.args).to.deep.equals([
        "setResolution",
        2.6458319045841048,
      ]);

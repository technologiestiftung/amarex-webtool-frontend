...
function getFeatureGET(url, payload, onerror) {
  ...
  if (typeof url !== "string") {
    error = new Error(
      `api/wfs/getFeatureGET: Url is ${url}. Url has to be defined and a string.`,
    );
  } else if (!isObject(payload)) {
    error = new Error(
      `api/wfs/getFeatureGET: Payload is ${payload}. Payload has to be defined and an object (not null).`,
    );
  } else if (typeof payload.version !== "string") {
    error = new Error(
      `api/wfs/getFeatureGET: Version is ${payload.version}. Payload has to have the version attribute and it has to be a string.`,
    );
  } else if (
    typeof payload.featureType !== "string" &&
    !Array.isArray(payload.featureType)
  ) {
    error = new Error(
      `api/wfs/getFeatureGET: FeatureType is ${payload.featureType}. Payload has to have the featureType attribute and it has to be defined and a string or an array.`,
    );
  }
  ...
  const { featureType, version, propertyNames, bbox, srsName } = payload,
    options = {
      ...
      params: {
        ...
        request: "GetFeature",
        version,
        typeName: Array.isArray(featureType)
          ? featureType.join(",")
          : featureType, // WFS 1.x.x
        typeNames: Array.isArray(featureType)
          ? featureType.join(",")
          : featureType, // WFS 2.x.x
        // optional parameters
  ...
  return axios(options)
    ...
    .catch((axiosError) =>
      handleAxiosError(axiosError, "api/wfs/getFeatureGET", onerror),
    );
}
...
function getFeaturePOST(url, payload, onerror) {
  ...
  if (typeof url !== "string") {
    error = new Error(
      `api/wfs/getFeaturePOST: Url is ${url}. Url has to be defined and a string.`,
    );
  } else if (!isObject(payload)) {
    error = new Error(
      `api/wfs/getFeaturePOST: Payload is ${payload}. Payload has to be defined and an object (not null).`,
    );
  } else if (!Array.isArray(payload.featureTypes)) {
    error = new Error(
      `api/wfs/getFeaturePOST: FeatureTypes is ${payload.featureTypes}. FeatureTypes has to be defined and an array.`,
    );
  }
  ...
  return axios(options)
    .then((response) => handleWfsResponse(response, onerror))
    .catch((axiosError) =>
      handleAxiosError(axiosError, "api/wfs/getFeaturePOST", onerror),
    );
}

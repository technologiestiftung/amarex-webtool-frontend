...
class SensorThingsHttp {
  ...
  get(url, onsuccess, onstart, oncomplete, onerror, callNextLinkOpt) {
    ...
    (typeof callNextLinkOpt === "function"
      ? callNextLinkOpt
      : this.callNextLink
    ).bind(this)(
      url,
  ...
  getInExtent(
    ...
    const extent = extentObj && extentObj.extent ? extentObj.extent : false,
      sourceProjection =
        extentObj && extentObj.sourceProjection
          ? extentObj.sourceProjection
          : false,
      targetProjection =
        extentObj && extentObj.targetProjection
          ? extentObj.targetProjection
          : false,
      points = this.convertExtentIntoPoints(
        extent,
        sourceProjection,
        targetProjection,
        (error) => {
    ...
    (typeof getOpt === "function" ? getOpt : this.get).bind(this)(
      requestUrl,
      onsuccess,
      onstart,
      oncomplete,
  ...
  getPolygonQueryWithPoints(points, intersect, onerror) {
    ...
    if (!Array.isArray(points)) {
      if (typeof onerror === "function") {
        onerror(
          "SensorThingsHttp - getPolygonQueryWithPoints: the given points should be an array",
        );
      }
      return false;
    }
    ...
    if (this.rootNode === "Things") {
      return (
        loadRange + "(Locations/location,geography'POLYGON ((" + query + "))')"
      );
    }
    return (
      loadRange +
      "(Thing/Locations/location,geography'POLYGON ((" +
      query +
      "))')"
    );
  }
  ...
  convertExtentIntoPoints(extent, sourceProjection, targetProjection, onerror) {
    ...
    if (!Array.isArray(extent) || extent.length !== 4) {
      if (typeof onerror === "function") {
        onerror(
          "SensorThingsHttp - convertExtentToPoints: the given extent must be an array with 4 entries",
        );
      }
      return false;
      ...
      if (typeof onerror === "function") {
        onerror(
          "SensorThingsHttp - convertExtentToPoints: the sourceProjection must be a string describing a projection (e.g. 'EPSG:4326')",
        );
      }
      return false;
      ...
      if (typeof onerror === "function") {
        onerror(
          "SensorThingsHttp - convertExtentToPoints: the targetProjection must be a string describing a projection (e.g. 'EPSG:4326')",
        );
      }
      return false;
    }
  ...
  addPointsToUrl(url, points, intersect, onerror) {
    ...
    if (!polygonQuery) {
      ...
      if (typeof onerror === "function") {
        onerror(
          "SensorThingsHttp - addPointsToUrl: an external url begining with http is expected",
        );
      }
      return false;
    }
    ...
    if (
      polygonQuery &&
      !Object.prototype.hasOwnProperty.call(parsedUrl.query, "$filter")
    ) {
      parsedUrl.query.$filter = polygonQuery;
    } else if (
      polygonQuery &&
      parsedUrl.query.$filter.indexOf("geography'POLYGON") === -1 &&
      parsedUrl.query.$filter.indexOf("geography%27POLYGON") === -1
    ) {
  ...
  callNextLink(
    ...
    if (!Array.isArray(resultRef)) {
      if (typeof onerror === "function") {
        onerror(
          "SensorThingsHttp - callNextLink: given resultRef is not an array - nextLink: " +
            nextLink +
            " - resultRef: " +
            resultRef,
        );
    ...
    this.callHttpClient(
      ...
      (response) => {
        if (
          ...
          if (typeof onerror === "function") {
            onerror(
              "SensorThingsHttp - callNextLink: the response from the SensorThingsApi is not an object - check nextLink: " +
                nextLink,
            );
          }
          return;
        ...
        (typeof collectNextLinksOpt === "function"
          ? collectNextLinksOpt
          : this.collectNextLinks
        ).bind(this)(resultRef, nextLinkFiFo, () => {
  ...
  collectNextLinks(resultRef, nextLinkFiFo, onfinish) {
    ...
    for (key in resultRef) {
      if (
        this.removeIotLinks &&
        (key.indexOf("@iot.navigationLink") !== -1 ||
          key.indexOf("@iot.selfLink") !== -1)
      ) {
  ...
  getNextFiFoObj(nextLinkFiFo) {
    if (
      !Array.isArray(nextLinkFiFo) ||
      nextLinkFiFo.length === 0 ||
      !nextLinkFiFo[0]?.nextLink
    ) {
      return false;
    }

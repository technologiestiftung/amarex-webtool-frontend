...
describe("src_3_0_0/shared/js/api/sensorThingsHttp", () => {
  ...
  describe("getPolygonQueryWithPoints", () => {
    it("should return an url query with the given points in a correct format with an intersect range", () => {
      ...
      expect(http.getPolygonQueryWithPoints(points, intersect)).to.equal(
        "st_intersects(Locations/location,geography'POLYGON ((foo bar,baz qux))')",
      );
      expect(http.getPolygonQueryWithPoints([{}, {}], intersect)).to.equal(
        "st_intersects(Locations/location,geography'POLYGON (())')",
      );
      expect(http.getPolygonQueryWithPoints([], intersect)).to.equal(
        "st_intersects(Locations/location,geography'POLYGON (())')",
      );
    ...
    it("should return an url query with the given points in a correct format with an intersect range if root node is Datastreams", () => {
      ...
      expect(http.getPolygonQueryWithPoints(points, intersect)).to.equal(
        "st_intersects(Thing/Locations/location,geography'POLYGON ((foo bar,baz qux))')",
      );
      expect(http.getPolygonQueryWithPoints([{}, {}], intersect)).to.equal(
        "st_intersects(Thing/Locations/location,geography'POLYGON (())')",
      );
      expect(http.getPolygonQueryWithPoints([], intersect)).to.equal(
        "st_intersects(Thing/Locations/location,geography'POLYGON (())')",
      );
    ...
    it("should return an url query with the given points in a correct format with a within range", () => {
      ...
      expect(http.getPolygonQueryWithPoints(points, intersect)).to.equal(
        "st_within(Locations/location,geography'POLYGON ((foo bar,baz qux))')",
      );
      expect(http.getPolygonQueryWithPoints([{}, {}], intersect)).to.equal(
    ...
    it("should return an url query with the given points in a correct format with a within range if root node is Datastreams", () => {
      ...
      http.setRootNode("Datastreams");
      expect(http.getPolygonQueryWithPoints(points, intersect)).to.equal(
        "st_within(Thing/Locations/location,geography'POLYGON ((foo bar,baz qux))')",
      );
      expect(http.getPolygonQueryWithPoints([{}, {}], intersect)).to.equal(
    ...
    it("should return false if some damaged points are given", () => {
      ...
      expect(http.getPolygonQueryWithPoints(points, intersect)).to.equal(
        "st_intersects(Locations/location,geography'POLYGON ((foo bar))')",
      );
  ...
  describe("convertExtentIntoPoints", () => {
    const extent = [
        557698.8791748052, 5925961.066824824, 573161.1208251948,
        5941978.933175176,
      ],
    ...
    it("should return false and send an error if anything unexpected is given as sourceProjection", () => {
      expect(
        http.convertExtentIntoPoints(extent, false, targetProjection, onerror),
      ).to.be.false;
      expect(lastError).to.be.a("string");
      expect(
        http.convertExtentIntoPoints(
      ...
      expect(http.convertExtentIntoPoints(extent, 1, targetProjection, onerror))
        .to.be.false;
      expect(
        http.convertExtentIntoPoints(extent, null, targetProjection, onerror),
      ).to.be.false;
    });

    it("should return false and send an error if anything unexpected is given as targetProjection", () => {
      expect(
        http.convertExtentIntoPoints(extent, sourceProjection, false, onerror),
      ).to.be.false;
      expect(lastError).to.be.a("string");
      expect(
        http.convertExtentIntoPoints(
      ...
      expect(http.convertExtentIntoPoints(extent, sourceProjection, 1, onerror))
        .to.be.false;
      expect(
        http.convertExtentIntoPoints(extent, sourceProjection, null, onerror),
      ).to.be.false;
    });

    it("should not call the error callback function on error if anything but a function is given as error handler", () => {
      expect(http.convertExtentIntoPoints(false, false, false, undefined)).to.be
        .false;
  ...
  describe("addPointsToUrl", () => {
    const polygon = [
      ...
      intersect = false,
      expectedOutcome =
        "https://iot.hamburg.de/v1.1/Things?%24filter=st_within(Locations%2Flocation%2Cgeography'POLYGON%20((9.869432803790303%2053.47946522163486%2C10.102382514144907%2053.47754336682167%2C10.10613018673993%2053.62149474831524%2C9.872388814958066%2053.623426671455626%2C9.869432803790303%2053.47946522163486))')";
    ...
    it("should return the expected url with a well formed input", () => {
      ...
      ).to.equal(expectedOutcome);
    });

    it("should return false and call an error if a funny url is given", () => {
      expect(http.addPointsToUrl(false, polygon, intersect, onerror)).to.be
        .false;
      expect(lastError).to.be.a("string");
      expect(http.addPointsToUrl(undefined, polygon, intersect, onerror)).to.be
        .false;
      expect(http.addPointsToUrl(null, polygon, intersect, onerror)).to.be
        .false;
      expect(http.addPointsToUrl("foo", polygon, intersect, onerror)).to.be
        .false;
      expect(http.addPointsToUrl(123456, polygon, intersect, onerror)).to.be
        .false;
    ...
    it("should return false and call an error if a broken polygon is given", () => {
      ...
      expect(lastError).to.be.a("string");
      expect(http.addPointsToUrl(url, undefined, intersect, onerror)).to.be
        .false;
    ...
    it("should not call the error callback function on error if anything but a function is given", () => {
      expect(http.addPointsToUrl("foo", "bar", intersect, undefined)).to.be
        .false;
      expect(lastError).to.be.false;
    });
  });
  ...
  describe("getNextFiFoObj", () => {
    ...
    it("should return false if the next item in the given list has no key nextLink", () => {
      const nextLinkFiFo = [{ key: "nextLink" }];
    ...
    it("should shift the first item of an array of objects and reduce the list", () => {
      const nextLinkFiFo = [
        ...
        expectedList = [{ nextLink: "nextLinkB" }];
  ...
  describe("handling of response", () => {
    ...
    it("should call collectNextLinks with resultRef and nextLinkFiFo", () => {
      ...
      http.callNextLink(
        ...
        "onerror",
        resultRef,
        (resultRefShadow, nextLinkFiFoShadow) => {
  ...
  describe("get", () => {
    it("should call this.callNextLink with specific parameters", () => {
      ...
      http.get(
        ...
        "onerror",
        (nextLink, nextLinkFiFo, onfinish, onerrorShadow, resultRef) => {
    ...
    it("should call oncomplete if an error occurs", () => {
      ...
      http.get(
        ...
        "onerror",
        (nextLink, nextLinkFiFo, onfinish, onerrorShadow) => {
          onerrorShadow();
        },
      );
  ...
  describe("getInExtent", () => {
    it("should add the extent to the url and call this.get", () => {
      const extentObj = {
        ...
        lastUrlExpected =
          "https://iot.hamburg.de/v1.0/Things?%24filter=st_within(Locations%2Flocation%2Cgeography'POLYGON%20((9.869432803790303%2053.47946522163486%2C10.102382514144907%2053.47754336682167%2C10.10613018673993%2053.62149474831524%2C9.872388814958066%2053.623426671455626%2C9.869432803790303%2053.47946522163486))')";
      let lastUrl = null;

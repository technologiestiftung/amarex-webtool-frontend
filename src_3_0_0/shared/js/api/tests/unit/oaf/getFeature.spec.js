...
describe("src_3_0_0/shared/js/api/oaf", () => {
  describe("getOAFFeatureGet", () => {
    it("should returns a promise which rejects if first param is not a string", async () => {
      ...
      expect(catchError).to.deep.equal(
        new Error(`Please provide a valid base url! Got ${undefined}`),
      );
    });
    it("should returns a promise which rejects if second param is not a string", async () => {
      ...
      expect(catchError).to.not.be.null;
      expect(catchError).to.deep.equal(
        new Error(`Please provide a collection! Got ${undefined}`),
      );
    });
    it("should returns a promise which rejects if last param is undefined and fourth param is set", async () => {
      ...
      await getOAFFeature
        .getOAFFeatureGet("", "", undefined, {}, undefined)
        .catch((error) => {
          catchError = error;
        });
      ...
      expect(catchError).to.deep.equal(
        new Error(
          `Please provide a valid crs for the oaf filter! Got ${undefined}`,
        ),
      );
    });
    it("should call oafRecursionHelper if first and second param are strings", async () => {
      const oafRecursionHelperStub = sinon.stub(
          getOAFFeature,
          "oafRecursionHelper",
        ),
        param1 = "foo",
      ...
      expect(
        oafRecursionHelperStub.calledWith(
          [],
          `${param1}/collections/${param2}/items?limit=${defaultLimit}`,
        ),
      ).to.be.true;
      sinon.restore();
    });
  ...
  describe("readAllOAFToGeoJSON", () => {
    it("should return the given param if it is not an array", () => {
      ...
      expect(getOAFFeature.readAllOAFToGeoJSON({})).to.be.an("object").that.is
        .empty;
  ...
  describe("oafRecursionHelper", () => {
    it("should call given onerror function if request fails", async () => {
      ...
      await getOAFFeature
        .oafRecursionHelper(undefined, undefined)
        .catch((stubError) => {
  ...
  describe("getNextLinkFromFeatureCollection", () => {
    it("should return false if featureCollection is not an object", () => {
      expect(getOAFFeature.getNextLinkFromFeatureCollection(undefined)).to.be
        .false;
      ...
      expect(getOAFFeature.getNextLinkFromFeatureCollection("string")).to.be
        .false;
    ...
    it("should return false if featureCollection.links is not an array", () => {
      expect(getOAFFeature.getNextLinkFromFeatureCollection({})).to.be.false;
      expect(getOAFFeature.getNextLinkFromFeatureCollection({ links: null })).to
        .be.false;
      expect(
        getOAFFeature.getNextLinkFromFeatureCollection({ links: undefined }),
      ).to.be.false;
      expect(getOAFFeature.getNextLinkFromFeatureCollection({ links: "" })).to
        .be.false;
      expect(
        getOAFFeature.getNextLinkFromFeatureCollection({ links: "string" }),
      ).to.be.false;
      expect(getOAFFeature.getNextLinkFromFeatureCollection({ links: 1234 })).to
        .be.false;
      expect(getOAFFeature.getNextLinkFromFeatureCollection({ links: true })).to
        .be.false;
      expect(getOAFFeature.getNextLinkFromFeatureCollection({ links: false }))
        .to.be.false;
      expect(getOAFFeature.getNextLinkFromFeatureCollection({ links: {} })).to
        .be.false;
    });
    it("should return false if featureCollection.links contains no objects", () => {
      expect(
        getOAFFeature.getNextLinkFromFeatureCollection({
          links: [undefined, null, "string", 1234, true, false, []],
        }),
      ).to.be.false;
    });
    ...
    it("should return false if featureCollection.links are objects with href string but rel is not equal 'next'", () => {
      expect(
        getOAFFeature.getNextLinkFromFeatureCollection({
          links: [{ href: "string", rel: "this is not a next" }],
        }),
      ).to.be.false;
    });

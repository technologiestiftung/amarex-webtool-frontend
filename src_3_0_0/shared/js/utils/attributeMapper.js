...
function mapAttributes(properties, mappingObject, isNested = true) {
  ...
  if (!isNested) {
    if (typeof mappingObject === "string") {
      ...
      mappedProperties = prepareValueFromObject(
        mappingObject.name,
        mappingObject,
        properties,
      );
  ...
  let value =
    properties[
      Object.keys(properties).find(
        (propertiesKey) => propertiesKey.toLowerCase() === key.toLowerCase(),
      )
    ];
...
export { mapAttributes, isObjectPath, prepareValue };

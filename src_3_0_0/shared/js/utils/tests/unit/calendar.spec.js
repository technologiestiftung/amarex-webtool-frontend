...
describe("src_3_0_0/utils/calendar.js", () => {
  ...
  describe("getPublicHolidays", () => {
    it("should return a list of CalendarMoment for the given year and holiday keys", () => {
      const publicHolidays = getPublicHolidays(2021, [
        "epiphany",
        "ascensionDay",
        "penanceDay",
        "newYearsEve",
      ...
      expect(publicHolidays[0].moment.format("YYYY-MM-DD")).to.equal(
        "2021-01-06",
      );
      expect(publicHolidays[0].holidayKey).to.equal("epiphany");
      expect(publicHolidays[0].translationKey).to.equal(
        "common:utils.calendar.epiphany",
      );
      ...
      expect(publicHolidays[1].moment.format("YYYY-MM-DD")).to.equal(
        "2021-05-13",
      );
      expect(publicHolidays[1].holidayKey).to.equal("ascensionDay");
      expect(publicHolidays[1].translationKey).to.equal(
        "common:utils.calendar.ascensionDay",
      );
      ...
      expect(publicHolidays[2].moment.format("YYYY-MM-DD")).to.equal(
        "2021-11-17",
      );
      expect(publicHolidays[2].holidayKey).to.equal("penanceDay");
      expect(publicHolidays[2].translationKey).to.equal(
        "common:utils.calendar.penanceDay",
      );
      ...
      expect(publicHolidays[3].moment.format("YYYY-MM-DD")).to.equal(
        "2021-12-31",
      );
      expect(publicHolidays[3].holidayKey).to.equal("newYearsEve");
      expect(publicHolidays[3].translationKey).to.equal(
        "common:utils.calendar.newYearsEve",
      );
    });
  });
  describe("hasHolidayInWeek", () => {
    ...
    it("should return false if no holiday is in the given week and the date is a monday", () => {
      expect(hasHolidayInWeek("2021-01-11", ["epiphany"], "YYYY-MM-DD")).to.be
        .false;
    });
    it("should return true if a holiday is in the given week and the date is a monday", () => {
      expect(hasHolidayInWeek("2021-01-04", ["epiphany"], "YYYY-MM-DD")).to.be
        .true;
    });
    it("should check a week with a holiday if the given date is not a monday", () => {
      expect(hasHolidayInWeek("2021-01-07", ["epiphany"], "YYYY-MM-DD")).to.be
        .true;
    });
  });
});

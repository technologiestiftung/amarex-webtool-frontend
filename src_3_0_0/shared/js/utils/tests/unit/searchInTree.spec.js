...
describe("src_3_0_0/utils/searchInTree.js", () => {
  ...
  describe("searchInTree", () => {
    it("should return a string no matter what no-string input is given", () => {
      ...
      expect(searchInTree({}, "elements", "key", "value")).to.be.an("array").and
        .to.be.empty;
      expect(searchInTree([], "elements", "key", "value")).to.be.an("array").and
        .to.be.empty;
    });
    ...
    it("should return all folder elements", () => {
      ...
      expect(result[2]).to.be.deep.equals(
        layersWithFolder.elements[0].elements[2],
      );
    });
    ...
    it("should return all elements with parentId 'folder-2'", () => {
      ...
      expect(result[0]).to.be.deep.equals(
        layersWithFolder.elements[0].elements[0],
      );
      expect(result[1]).to.be.deep.equals(
        layersWithFolder.elements[0].elements[1],
      );
      expect(result[2]).to.be.deep.equals(
        layersWithFolder.elements[0].elements[2],
      );

...
describe("src_3_0_0/utils/localeCompare.js", () => {
  ...
  describe("Testing localeCompare with options", () => {
    it("should return -1 if ignorePunctuation is true", () => {
      expect(
        localeCompare("a", "(b)", "de", { ignorePunctuation: true }),
      ).to.equal(-1);
    });
    it("should return 1 if ignorePunctuation is false", () => {
      expect(
        localeCompare("a", "(b)", "de", { ignorePunctuation: false }),
      ).to.equal(1);
    });
    it("should return 0 if sensitivity is 'base' in German", () => {
      expect(localeCompare("a", "ä", "de", { sensitivity: "base" })).to.equal(
        0,
      );
    });
    it("should return -1 if sensitivity is 'base' in Swedish", () => {
      expect(localeCompare("a", "ä", "sv", { sensitivity: "base" })).to.equal(
        -1,
      );
    });

...
describe("src_3_0_0/utils/isHTML.js", function () {
  ...
  it("should return true for valid html string with attributes, img", function () {
    expect(isHTML('<img style="max-width:100%" src="https://foo.bar"/>')).to.be
      .true;
  });
  it("should return true for valid html string with attributes, iframe", function () {
    expect(
      isHTML(
        '<iframe width="250px" height="250px" src="https://foo.bar" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen="true"></iframe>',
      ),
    ).to.be.true;
  });
});

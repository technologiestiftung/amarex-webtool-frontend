...
describe("src_3_0_0/shared/js/utils/getGfiFeaturesByTileFeature.js", () => {
  describe("getGfiFeature", () => {
    ...
    it("should get attributes and properties according to given parameters", () => {
      const layerAttributes = {
        ...
        result = getGfiFeatureProvider.getGfiFeature(
          layerAttributes,
          properties,
        );
      ...
      expect(result.getAttributesToShow()).to.deep.equal(
  ...
  describe("getLayerModelFromTileFeature", () => {
    ...
    it("should return undefined if no tileFeature is given", () => {
      expect(getGfiFeatureProvider.getLayerModelFromTileFeature(undefined)).to
        .be.undefined;
      expect(getGfiFeatureProvider.getLayerModelFromTileFeature(null)).to.be
        .undefined;
      expect(getGfiFeatureProvider.getLayerModelFromTileFeature("")).to.be
        .undefined;
      expect(getGfiFeatureProvider.getLayerModelFromTileFeature(0)).to.be
        .undefined;
      expect(getGfiFeatureProvider.getLayerModelFromTileFeature(false)).to.be
        .undefined;
      expect(getGfiFeatureProvider.getLayerModelFromTileFeature("test")).to.be
        .undefined;
      expect(getGfiFeatureProvider.getLayerModelFromTileFeature(1234)).to.be
        .undefined;
      expect(getGfiFeatureProvider.getLayerModelFromTileFeature(true)).to.be
        .undefined;
    });
    it("should create a filter with the Cesium3DTileFeature and apply the filter to get the model", () => {
      const tileFeature = new Cesium.Cesium3DTileFeature(
          "Cesium3DTileFeature.layerReferenceId",
        ),
        result = getGfiFeatureProvider.getLayerModelFromTileFeature(
          tileFeature,
          dummy.getModelByAttributesOpt,
          dummy.isCesium3dTileFeature,
          dummy.isCesiumEntity,
        ),
    ...
    it("should return undefined if tileFeature is no Cesium3DTileFeature nor Cesium3DTilePointFeature and has no valid primitive option", () => {
      expect(
        getGfiFeatureProvider.getLayerModelFromTileFeature(
          {},
          dummy.getModelByAttributesOpt,
          dummy.isCesium3dTileFeature,
          dummy.isCesiumEntity,
    ...
    it("should use Cesium3DTileFeature before using primitive.olLayer or Cesium.Entity", () => {
      const tileFeature = Object.assign(
  ...
  describe("getGfiFeatureByCesium3DTileFeature", () => {
    it("should return undefined if tileFeature is no object", () => {
      expect(getGfiFeatureProvider.getGfiFeatureByCesium3DTileFeature()).to.be
        .undefined;
      expect(getGfiFeatureProvider.getGfiFeatureByCesium3DTileFeature(null)).to
        .be.undefined;
      expect(getGfiFeatureProvider.getGfiFeatureByCesium3DTileFeature(1234)).to
        .be.undefined;
      expect(getGfiFeatureProvider.getGfiFeatureByCesium3DTileFeature("string"))
        .to.be.undefined;
      expect(getGfiFeatureProvider.getGfiFeatureByCesium3DTileFeature(true)).to
        .be.undefined;
    });
    it("should return undefined if tileFeature has no functions getPropertyIds or getProperty", () => {
      ...
      expect(getGfiFeatureProvider.getGfiFeatureByCesium3DTileFeature([])).to.be
        .undefined;
      expect(
        getGfiFeatureProvider.getGfiFeatureByCesium3DTileFeature({
          getPropertyIds: () => {
            return false;
          },
        }),
  ...
  describe("getGfiFeatureByCesiumEntity", () => {
    it("should return undefined if tileFeature.primitive.id is not an object", () => {
      expect(getGfiFeatureProvider.getGfiFeatureByCesiumEntity()).to.be
        .undefined;
      expect(getGfiFeatureProvider.getGfiFeatureByCesiumEntity(null)).to.be
        .undefined;
      expect(getGfiFeatureProvider.getGfiFeatureByCesiumEntity(1234)).to.be
        .undefined;
      expect(getGfiFeatureProvider.getGfiFeatureByCesiumEntity("string")).to.be
        .undefined;
      expect(getGfiFeatureProvider.getGfiFeatureByCesiumEntity(true)).to.be
        .undefined;
      expect(getGfiFeatureProvider.getGfiFeatureByCesiumEntity([])).to.be
        .undefined;
      expect(getGfiFeatureProvider.getGfiFeatureByCesiumEntity({})).to.be
      ...
      expect(
        getGfiFeatureProvider.getGfiFeatureByCesiumEntity({
          primitive: undefined,
        }),
      ).to.be.undefined;
      expect(
        getGfiFeatureProvider.getGfiFeatureByCesiumEntity({ primitive: null }),
      ).to.be.undefined;
      expect(
        getGfiFeatureProvider.getGfiFeatureByCesiumEntity({ primitive: 1234 }),
      ).to.be.undefined;
      expect(
        getGfiFeatureProvider.getGfiFeatureByCesiumEntity({
          primitive: "string",
      ...
      expect(
        getGfiFeatureProvider.getGfiFeatureByCesiumEntity({ primitive: true }),
      ).to.be.undefined;
      expect(
        getGfiFeatureProvider.getGfiFeatureByCesiumEntity({ primitive: [] }),
      ).to.be.undefined;
      expect(
        getGfiFeatureProvider.getGfiFeatureByCesiumEntity({ primitive: {} }),
      ).to.be.undefined;
  ...
  describe("getGfiFeatureByOlFeature", () => {
    it("should return undefined if olFeature is no object", () => {
      ...
      expect(getGfiFeatureProvider.getGfiFeatureByOlFeature(null)).to.be
        .undefined;
      expect(getGfiFeatureProvider.getGfiFeatureByOlFeature(1234)).to.be
        .undefined;
      expect(getGfiFeatureProvider.getGfiFeatureByOlFeature("string")).to.be
        .undefined;
      expect(getGfiFeatureProvider.getGfiFeatureByOlFeature(true)).to.be
        .undefined;
    });
    it("should return undefined if olFeature has no functions getProperties or getProperty", () => {
      expect(getGfiFeatureProvider.getGfiFeatureByOlFeature({})).to.be
        .undefined;
      expect(getGfiFeatureProvider.getGfiFeatureByOlFeature([])).to.be
        .undefined;
      expect(
        getGfiFeatureProvider.getGfiFeatureByOlFeature({
          getProperties: () => {
            return false;
          },
        }),
  ...
  describe("getGfiFeaturesByOlFeature", () => {
    it("should return undefined if olFeature is no object", () => {
      ...
      expect(getGfiFeatureProvider.getGfiFeaturesByOlFeature(null)).to.be
        .undefined;
      expect(getGfiFeatureProvider.getGfiFeaturesByOlFeature(1234)).to.be
        .undefined;
      expect(getGfiFeatureProvider.getGfiFeaturesByOlFeature("string")).to.be
        .undefined;
      expect(getGfiFeatureProvider.getGfiFeaturesByOlFeature(true)).to.be
        .undefined;
    });
    it("should return undefined if olFeature has no functions getProperties or getProperty", () => {
      expect(getGfiFeatureProvider.getGfiFeaturesByOlFeature({})).to.be
        .undefined;
      expect(getGfiFeatureProvider.getGfiFeaturesByOlFeature([])).to.be
        .undefined;
      expect(
        getGfiFeatureProvider.getGfiFeaturesByOlFeature({
          getProperties: () => {
            return false;
          },
        }),
    ...
    it("should create an array of one gfiFeature if olFeature is valid and is not clustered (has no features property)", () => {
      const olFeature = {
        ...
        result = getGfiFeatureProvider.getGfiFeaturesByOlFeature(
          olFeature,
          "attributes",
          (feature) => {
    ...
    it("should pass through attributes if olFeature is valid and is clustered (has feature property)", () => {
      const props = {
        ...
        result = getGfiFeatureProvider.getGfiFeaturesByOlFeature(
          olFeature,
          "attributes",
          testFunctions.getGfiFeatureByOlFeatureOpt,
        );

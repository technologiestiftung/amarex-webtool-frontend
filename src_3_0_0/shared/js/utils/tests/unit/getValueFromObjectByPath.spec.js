import {
  ...
  getPathPartsFromPath,
} from "../../getValueFromObjectByPath.js";
import { expect } from "chai";

describe("src_3_0_0/shared/js/utils/getValueFromObjectByPath.js", () => {
  ...
  describe("getValueFromObjectByPath", () => {
    ...
    it("should return undefined if the given path can't be followed through the given object", () => {
      ...
      expect(getValueFromObjectByPath({ test: null }, "@test.test")).to.be
        .undefined;
    });
    it("should return the found value if the given path was successfully followed through the object", () => {
      ...
      expect(
        getValueFromObjectByPath({ test: { test: 1 } }, "@test"),
      ).to.deep.equal({ test: 1 });
      expect(
        getValueFromObjectByPath({ test: { test: { test: 1 } } }, "@test.test"),
      ).to.deep.equal({ test: 1 });
    });
    ...
    it("should be able to handle a path with a complex prefix", () => {
      expect(
        getValueFromObjectByPath({ test: 1 }, "1234test", "1234"),
      ).to.equal(1);
    });
  });
});

...
function getPublicHoliday(date, holidayKeys = false, format = false) {
  const givenMoment = dayjs(date, format),
    ...
    keys = Array.isArray(holidayKeys)
      ? holidayKeys
      : Object.keys(publicHolidayMatrix),
    len = keys.length;
  let holidayKey = "",
    holidayMoment = null;
  ...
  for (let i = 0; i < len; i++) {
    ...
    holidayMoment = publicHolidayMatrix[holidayKey].getMoment(
      year,
      easterMoment,
      adventMoment,
    );
    if (
      givenMoment.format("YYYY-MM-DD") !== holidayMoment.format("YYYY-MM-DD")
    ) {
      continue;
    }
...
function getPublicHolidays(year, holidayKeys = false) {
  ...
  keys.forEach((holidayKey) => {
    if (isCalendarMoment(publicHolidayMatrix[holidayKey])) {
      result.push({
        moment: publicHolidayMatrix[holidayKey].getMoment(
          year,
          easterMoment,
          adventMoment,
        ),
...
function isCalendarMoment(calendarMoment) {
  return (
    typeof calendarMoment === "object" &&
    calendarMoment !== null &&
    typeof calendarMoment.getMoment === "function" &&
    typeof calendarMoment.translationKey === "string"
  );
}

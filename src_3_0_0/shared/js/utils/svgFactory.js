...
function createCircle(style) {
  ...
  const strokeColor =
      ...
      (style.attributes && style.attributes.circleStrokeColor),
    strokeWidth =
      style.circleStrokeWidth ||
      (style.attributes && style.attributes.circleStrokeWidth),
    fillColor =
      style.circleFillColor ||
      (style.attributes && style.attributes.circleFillColor),
    circleStrokeColor = strokeColor ? returnColor(strokeColor, "hex") : "black",
    circleStrokeOpacity =
      Array.isArray(strokeColor) && strokeColor.length > 3
        ? strokeColor[3].toString()
        : 0,
    circleStrokeWidth = strokeWidth ? strokeWidth : "auto",
    circleFillColor = fillColor ? returnColor(fillColor, "hex") : "black",
    circleFillOpacity =
      Array.isArray(fillColor) && fillColor.length > 3 ? fillColor[3] : 0;
...
function createLine(style) {
  let svg = "";
  const strokeColor =
      style.lineStrokeColor ||
      (style.attributes && style.attributes.lineStrokeColor),
    strokeWidth =
      style.lineStrokeWidth ||
      (style.attributes && style.attributes.lineStrokeWidth),
    strokeDash =
      style.lineStrokeDash ||
      (style.attributes && style.attributes.lineStrokeDash),
    ...
    lineStrokeOpacity =
      Array.isArray(strokeColor) && strokeColor.length > 3
        ? strokeColor[3].toString()
        : 0,
    lineStrokeDash = Array.isArray(strokeDash)
      ? strokeDash.join(" ")
      : undefined;
...
function createPolygon(style) {
  ...
  const fillColor =
      ...
      (style.attributes && style.attributes.polygonFillColor),
    strokeColor =
      style.polygonStrokeColor ||
      (style.attributes && style.attributes.polygonStrokeColor),
    strokeWidth =
      style.polygonStrokeWidth ||
      (style.attributes && style.attributes.polygonStrokeWidth),
    polygonFillColor = fillColor
      ? returnColor(fillColor || "black", "hex")
      : "black",
    polygonStrokeColor = strokeColor
      ? returnColor(strokeColor, "hex")
      : "black",
    polygonStrokeWidth = parseInt(strokeWidth, 10),
    polygonFillOpacity =
      Array.isArray(fillColor) && fillColor.length > 3
        ? fillColor[3].toString()
        : 0,
    polygonStrokeOpacity =
      Array.isArray(strokeColor) && strokeColor.length > 3
        ...
        : 0,
    polygonFillHatch =
      style.polygonFillHatch ||
      (style.attributes && style.attributes.polygonFillHatch);

 ...
 * @returns {Object} the entry/entries without the blacklisted key/keys.
 */
export default function omit(object, blacklist, ignoreCase = false) {
  const keys = Object.keys(object ? object : {}),
    ...
    filteredKeys = keys.filter(
      (key) =>
        !blacklistWithStrings.includes(ignoreCase ? key.toUpperCase() : key),
    ),
...
function convertArrayElementsToString(array = [], ignoreCase = false) {
  ...
  array.forEach((element) =>
    arrayWithStrings.push(
      ignoreCase ? String(element).toUpperCase() : String(element),
    ),
  );

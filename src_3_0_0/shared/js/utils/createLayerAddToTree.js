...
async function createLayerAddToTree(layerId, features, thfConfig = {}) {
  if (layerId) {
    const layerNameKey = thfConfig.layerName
        ? thfConfig.layerName
        : "common:shared.js.utils.selectedFeatures",
      originalLayer = getLayer(layerId);

    if (originalLayer) {
      const originalLayerName = originalLayer
          .get("name")
          .replace(i18next.t(layerNameKey), "")
          .trim(),
        layerName = i18next.t(layerNameKey) + " " + originalLayerName,
        id =
          layerId.indexOf(originalLayerName) === -1
            ? layerId + "_" + uniqueId()
            : layerId,
      ...
      layerSource
        .getFeatures()
        .forEach((feature) => layerSource.removeFeature(feature));
      layerSource.addFeatures(features);
    } else {
      console.warn(
        "Layer with id ",
        layerId,
        " not found, layer with features was not created!",
      );
    }
  }
}
...
async function addLayerModel(attributes, id) {
  await store.dispatch(
    ...
    { layerConfig: attributes, parentKey: treeSubjectsKey },
    { root: true },
  );

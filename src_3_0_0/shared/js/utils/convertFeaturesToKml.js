...
function getIconColor(color) {
  const selectedOption = colorOptions.filter((option) =>
    allCompareEqual(color, option.value),
  );
...
function getKMLWithCustomAttributes(features, format) {
  if (
    ...
    !features.some(
      (feature) =>
        typeof feature.get === "function" && feature.get("attributes"),
    )
  ) {
    return null;
  }
    ...
    attributeKeys.forEach((attrKey) => {
      if (placemarks[idx]) {
        const extendedData = placemarks[idx].querySelector("ExtendedData"),
          data =
            typeof extendedData.querySelector === "function"
              ? extendedData.querySelector(`Data[name='${attrKey}']`)
              : undefined,
          existingDataNode =
            typeof extendedData.querySelector === "function"
              ? extendedData.querySelector(
                  `Data[name='custom-attribute____${attrKey}']`,
                )
              : undefined;
...
export default async function convertFeaturesToKml(features) {
  const featureCount = features.length,
    ...
    convertedFeatures = new DOMParser().parseFromString(
      convertFeatures(features, format),
      "text/xml",
    );

  features.forEach((feature, i) => {
    const type = feature.getGeometry().getType();
    let color, style, styles;

    if (
      type === "Point" &&
      feature.values_.drawState &&
      feature.values_.drawState.text !== undefined
    ) {
      ...
      if (type === "Point") {
        if (
          style.getImage() !== null &&
          style.getImage().iconImage_ !== undefined
        ) {
  ...
  Array.from(convertedFeatures.getElementsByTagName("Placemark")).forEach(
    (placemark, i) => {
      if (
        placemark.getElementsByTagName("Point").length > 0 &&
        skip[i] === false
      ) {
      ...
      if (
        ...
        const scale = textFontSize[i] / 16,
          ...
          maskIcon = new DOMParser().parseFromString(
            "<IconStyle><scale>0</scale><Icon><href>" +
              iconUrl +
              "</href></Icon></IconStyle>",
            "text/xml",
          ),
          maskScale = new DOMParser().parseFromString(
            "<scale>" + scale + "</scale>",
            "text/xml",
          );

        style
          .getElementsByTagName("LabelStyle")[0]
          .appendChild(maskScale.getElementsByTagName("scale")[0]);
...
export { convertFeatures, transformCoordinates, getKMLWithCustomAttributes };

...
function isRgbArray(color) {
  if (!Array.isArray(color) || color.length !== 3) {
    ...
    isNaN(parseInt(color[0], 10)) ||
    parseInt(color[0], 10) < 0 ||
    parseInt(color[0], 10) > 255
  ) {
    return false;
  } else if (
    isNaN(parseInt(color[1], 10)) ||
    parseInt(color[1], 10) < 0 ||
    parseInt(color[1], 10) > 255
  ) {
    return false;
  } else if (
    isNaN(parseInt(color[2], 10)) ||
    parseInt(color[2], 10) < 0 ||
...
function isRgbaArray(color) {
  if (!Array.isArray(color) || color.length !== 4) {
    ...
    isNaN(parseFloat(color[3])) ||
    parseFloat(color[3]) < 0 ||
    parseFloat(color[3]) > 1
  ) {
    return false;
  }
  return true;
...
function isCssColorString(color) {
  if (typeof color !== "string") {
    ...
    !Object.prototype.hasOwnProperty.call(getCssColorMap(), color.toLowerCase())
  ) {
    return false;
  }
  return true;

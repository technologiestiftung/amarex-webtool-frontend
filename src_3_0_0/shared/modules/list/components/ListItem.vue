...
import {
  ...
  extend,
  getCenter as getCenterExtent,
} from "ol/extent";
import { isWebLink } from "../../../js/utils/urlHelper";
import {
  isPhoneNumber,
  getPhoneNumberAsWebLink,
} from "../../../js/utils/isPhoneNumber";
  ...
  methods: {
    ...
    setCenter(features) {
      ...
      for (let i = 0; i < features.length; i++) {
        const pointCoordinate = getCenterExtent(geometries[i].getExtent());
      ...
      if (typeof value === "string") {
        if (value === "true") {
          return value.replace(
            "true",
            i18next.t("common:shared.modules.list.replace.true"),
          );
        }
        if (value === "No") {
          return value.replace(
            "No",
            i18next.t("common:shared.modules.list.replace.no"),
          );
        }
      }
      return "";
        ...
        >
          ...
          >
            ...
            <template v-else-if="isPhoneNumber(data.values_[key])">
              <a :href="getPhoneNumberAsWebLink(data.values_[key])">{{
                data.values_[key]
              }}</a>
            </template>
            <template v-else-if="isEmailAddress(data.values_[key])">
              <a :href="`mailto:${data.values_[key]}`">{{
                data.values_[key]
              }}</a>
            </template>
            <template
              v-else-if="
                data.values_[key] === 'true' || data.values_[key] === 'No'
              "
            >

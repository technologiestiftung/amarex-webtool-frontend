...
export default {
  ...
  watch: {
    selectedInteraction(selectedInteraction) {
      if (selectedInteraction !== "delete") {
        this.currentSelectInteractions.forEach((currentSelectInteraction) =>
          this.removeInteraction(currentSelectInteraction),
        );
  ...
  methods: {
    ...
    regulateDelete() {
      ...
      this.currentSelectInteractions = this.createSelectInteractions(
        this.layer,
      );
      selectInteraction.removeSelectedFeature(
        this.currentSelectInteractions[0],
        this.source,
      );
      this.currentSelectInteractions.forEach((currentSelectInteraction) =>
        this.addInteraction(currentSelectInteraction),
      );
    },
    ...
    regulateModify() {
      ...
      this.removeInteraction(this.currentModifyInteraction);
      this.currentModifyInteraction = modifyInteraction.createModifyInteraction(
        this.source,
      );
...
<template>
  <div
    v-if="
      ...
      undoFeatures.length > 0 ||
      redoFeatures.length > 0
    "
    class="mb-4"
  >
    <div class="d-flex">

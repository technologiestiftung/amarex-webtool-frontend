...
export default {
  ...
  methods: {
    ...
    updateRadiusInCircleOptions(value, radius) {
      const optionsKey =
        radius === "outerRadius" ? "outerRadius" : "innerRadius";
...
<template>
  <div>
    <div class="mb-3">
      <span>{{
        $t(
          "common:shared.modules.draw.drawSettingsCircle.settings." +
            selectedDrawType,
        )
      }}</span>
    </div>
    ...
    >
      <SwitchInput
        ...
        :interaction="
          (event) => setCircleOptions({ interactive: event.target.checked })
        "
    ...
    <div
      v-if="!circleOptions.interactive || selectedDrawType === 'doubleCircle'"
    >
      ...
      >
        <div class="form-floating input-radius">
          <input
            ...
            :value="
              radius === 'outerRadius'
                ? circleOptions.outerRadius
                : circleOptions.innerRadius
            "
            type="number"
            class="form-control"
            min="0"
          ...
          <label :for="'draw-circle-settings-' + radius">
            {{
              $t(
                "common:shared.modules.draw.drawSettingsCircle.radii." + radius,
              )
            }}
          </label>
        </div>

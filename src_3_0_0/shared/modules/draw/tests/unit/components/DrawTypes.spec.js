...
describe("src_3_0_0/shared/modules/draw/components/DrawTypes.vue", () => {
  ...
  beforeEach(() => {
    ...
    createDrawInteractionSpy = sinon.spy(
      drawInteraction,
  ...
  it("renders the drawTypes", () => {
    ...
    expect(wrapper.find("#draw-pen").attributes().aria).to.equals(
      "common:shared.modules.draw.drawTypes.pen",
    );

    expect(wrapper.find("#draw-geometries").exists()).to.be.true;
    expect(wrapper.find("#draw-geometries").attributes().aria).to.equals(
      "common:shared.modules.draw.drawTypes.geometries",
    );

    expect(wrapper.find("#draw-symbols").exists()).to.be.true;
    expect(wrapper.find("#draw-symbols").attributes().aria).to.equals(
      "common:shared.modules.draw.drawTypes.symbols",
    );
  });
  ...
  describe("startDrawingInitial", () => {
    it("should start drawing initial, if selectedDrawTypeMain === 'pen' and selectedDrawType === 'pen'", () => {
      ...
      const regulateDrawInteractionSpy = sinon.spy(
        wrapper.vm,
        "regulateDrawInteraction",
      );
    ...
    it("should start drawing initial, if selectedDrawTypeMain === '' and selectedDrawType === 'circle'", async () => {
      ...
      const regulateInteractionSpy = sinon.spy(
        wrapper.vm,
        "regulateInteraction",
      );
  ...
  describe("selectedInteraction", () => {
    ...
    it("should set selectedDrawType and selectedDrawTypeMain to empty string, if selectedInteraction is not 'draw'", () => {
      ...
      wrapper.vm.$options.watch.selectedInteraction.call(
        wrapper.vm,
        selectedInteraction,
      );
  ...
  describe("regulateDrawInteraction", () => {
    ...
    it("should create 'circle' draw interaction and add interaction ", () => {
      ...
      expect(drawCircleSpy.firstCall.args[0]).to.deep.equals(
        wrapper.vm.currentDrawInteraction,
      );

...
describe("src_3_0_0/shared/modules/draw/components/DrawEdit.vue", () => {
  ...
  beforeEach(() => {
    ...
    createModifyInteractionSpy = sinon.spy(
      modifyInteraction,
      "createModifyInteraction",
    );
    createSelectInteractionSpy = sinon.spy(
      selectInteraction,
      "createSelectInteraction",
    );
    layer = new VectorLayer({ source: new VectorSource() });
    removeSelectedFeatureSpy = sinon.spy(
      selectInteraction,
  ...
  it("renders the drawEdits, if undoFeatures or redoFeatures length > 0", async () => {
    ...
    expect(wrapper.find("#draw-edit-deleteAll").attributes().aria).to.equals(
      "common:shared.modules.draw.drawEdit.deleteAll",
    );

    expect(wrapper.find("#draw-edit-delete").exists()).to.be.true;
    expect(wrapper.find("#draw-edit-delete").attributes().aria).to.equals(
      "common:shared.modules.draw.drawEdit.delete",
    );

    expect(wrapper.find("#draw-edit-edit").exists()).to.be.true;
    expect(wrapper.find("#draw-edit-edit").attributes().aria).to.equals(
      "common:shared.modules.draw.drawEdit.edit",
    );

    expect(wrapper.find("#draw-edit-undo").exists()).to.be.true;
    expect(wrapper.find("#draw-edit-undo").attributes().aria).to.equals(
      "common:shared.modules.draw.drawEdit.undo",
    );

    expect(wrapper.find("#draw-edit-redo").exists()).to.be.true;
    expect(wrapper.find("#draw-edit-redo").attributes().aria).to.equals(
      "common:shared.modules.draw.drawEdit.redo",
    );
  });
  ...
  describe("regulateDelete", () => {
    it("should create select interactons for deleteting features", () => {
      ...
      expect(createSelectInteractionSpy.firstCall.args[0]).to.deep.equals(
        layer,
      );
      expect(createSelectInteractionSpy.secondCall.args[0]).to.deep.equals(
        layer,
      );
      expect(createSelectInteractionSpy.secondCall.args[1]).to.deep.equals(
        pointerMove,
      );

      expect(removeSelectedFeatureSpy.calledOnce).to.be.true;
      expect(removeSelectedFeatureSpy.firstCall.args[0]).to.deep.equals(
        wrapper.vm.currentSelectInteractions[0],
      );
      expect(removeSelectedFeatureSpy.firstCall.args[1]).to.deep.equals(
        wrapper.vm.source,
      );

      expect(addInteractionSpy.calledTwice).to.be.true;
      expect(addInteractionSpy.firstCall.args[1]).to.deep.equals(
        wrapper.vm.currentSelectInteractions[0],
      );
      expect(addInteractionSpy.secondCall.args[1]).to.deep.equals(
        wrapper.vm.currentSelectInteractions[1],
      );
    });
  });
  ...
  describe("createSelectInteractions", () => {
    it("should create two select interactons for deleteting features", () => {
      ...
      const currentSelectInteractions =
        wrapper.vm.createSelectInteractions(layer);
  ...
  describe("regulateModify", () => {
    it("should create modify interaction", async () => {
      ...
      expect(createModifyInteractionSpy.firstCall.args[0]).to.deep.equals(
        wrapper.vm.source,
      );
      ...
      expect(addInteractionSpy.firstCall.args[1]).to.deep.equals(
        wrapper.vm.currentModifyInteraction,
      );
    });
  });
  ...
  describe("regulateUndo and regulateRedo", () => {
    ...
    it("should start undoRedoFeatures with 'undo'", async () => {
      ...
      expect(undoRedoFeaturesSpy.firstCall.args[0]).to.deep.equals(
        wrapper.vm.undoFeatures,
      );
  ...
  describe("undoRedoFeatures", () => {
    let feature1, feature2, undoRedoDeleteAllSpy;
    ...
    it("should start undoRedoDeleteAllSpy, if the last feature is an array", () => {
      ...
      expect(undoRedoDeleteAllSpy.firstCall.args[2]).to.deep.equals(
        wrapper.vm.source,
      );
    });
  });

...
function dimensionValidator(val) {
  return !isNaN(val) && typeof val === "number" && val >= 0 && val <= 1;
}
...
export default {
  ...
  computed: {
     ...
     */
    cursorClass() {
      const indexWithoutRotation = Object.keys(handleSigns).indexOf(
          this.handlePosition,
        ),
        indexAfterRotation = Math.floor(
          Math.abs(this.initialRotation + Math.PI / 8) / (Math.PI / 4),
        );
    ...
    handleElement() {
      ...
      if (element === null) {
        console.warn(
          "ResizeHandle: No target element found. Please verify your markup and selector. Currently defined target element selector:",
          this.targetSelector,
        );
      }
      return element;
  ...
  watch: {
    ...
    isResizing(newValue) {
      ...
      if (newValue) {
        ...
        document
          .querySelector("body")
          .classList.add("resize-handle-is-resizing");
        return;
      }
      ...
      document
        .querySelector("body")
        .classList.remove("resize-handle-is-resizing");
    },
  },
  mounted() {
  ...
  methods: {
    ...
    onMouseMove(event) {
      ...
      this.deltaCursorPosition.x =
        deltaX * Math.cos(this.initialRotation) +
        deltaY * Math.sin(this.initialRotation);
      this.deltaCursorPosition.y =
        deltaY * Math.cos(this.initialRotation) -
        deltaX * Math.sin(this.initialRotation);

      if (this.grid > 1) {
        this.deltaCursorPosition.x =
          Math.round(this.deltaCursorPosition.x / this.grid) * this.grid;
        this.deltaCursorPosition.y =
          Math.round(this.deltaCursorPosition.y / this.grid) * this.grid;
      }
      this.setNewSize();

      if (
        ["absolute", "fixed"].indexOf(
          window.getComputedStyle(this.handleElement, null).position,
        ) !== -1
      ) {
        this.setNewPosition();
      }
    ...
    saveInitialCursorCoordinates(event) {
      this.initialCursorPosition.x = event.touches
        ? event.touches[0].clientX
        : event.clientX;
      this.initialCursorPosition.y = event.touches
    ...
    saveInitialDimensions() {
      this.initialDimensions.width =
        this.handleElement.offsetWidth < this.minWidth
          ? this.minWidth
          : this.handleElement.offsetWidth;
      this.initialDimensions.height =
        this.handleElement.offsetHeight < this.minHeight
          ? this.minHeight
          : this.handleElement.offsetHeight;
    ...
    saveInitialRotation() {
      const transformation = this.getTransformation(
        window.getComputedStyle(this.handleElement, null),
      );
      ...
      if (transformation !== "none" && transformation !== false) {
        ...
        this.initialRotation =
          (Math.atan2(values[1], values[0]) + 2 * Math.PI) % (2 * Math.PI);
      } else {
    ...
    setNewPosition() {
      ...
      if (!this.symmetricResize) {
        left -=
          0.5 *
          (signX * deltaWidth * Math.cos(this.initialRotation) -
            signY * deltaHeight * Math.sin(this.initialRotation));
        top -=
          0.5 *
          (signY * deltaHeight * Math.cos(this.initialRotation) +
            signX * deltaWidth * Math.sin(this.initialRotation));
      }
    ...
    setNewSize() {
      ...
      if (this.handlePosition !== "top" && this.handlePosition !== "bottom") {
        let newWidth =
          this.initialDimensions.width +
          handleSigns[this.handlePosition][0] * this.deltaCursorPosition.x;
      ...
      if (this.handlePosition !== "left" && this.handlePosition !== "right") {
        let newHeight =
          this.initialDimensions.height +
          handleSigns[this.handlePosition][1] * this.deltaCursorPosition.y;

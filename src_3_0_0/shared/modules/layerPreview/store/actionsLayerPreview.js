    ...
    let previewCenter = center ? center : rootGetters["Maps/initialCenter"],
      previewZoomLevel =
        typeof zoomLevel === "number"
          ? zoomLevel
          : rootGetters["Maps/initialZoom"];

    if (
      !Array.isArray(previewCenter) &&
      typeof previewCenter === "string" &&
      previewCenter.indexOf(",") > -1
    ) {
      previewCenter = [
        parseFloat(previewCenter.split(",")[0]),
        parseFloat(previewCenter.split(",")[1]),
      ];
    }
    if (resolutions[previewZoomLevel] === undefined) {
      previewZoomLevel = rootGetters["Maps/initialZoom"];
      console.warn(
        `Unusable zoomLevel ${zoomLevel} at layer with id:${id} configured, corrected to initialZoom ${previewZoomLevel}`,
      );
    }

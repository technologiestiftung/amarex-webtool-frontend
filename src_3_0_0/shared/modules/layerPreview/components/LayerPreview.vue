...
export default {
  ...
  computed: {
    ...
    ...mapGetters("Modules/LayerPreview", [
      "previewCenter",
      "previewZoomLevel",
  ...
  methods: {
    ...mapActions("Modules/LayerPreview", ["initialize"]),
    ...mapMutations("Modules/LayerPreview", ["addPreviewUrl"]),
     ...
     */
    calculateExtent() {
      const resolution = mapCollection.getMapView("2D").getResolutions()[
          this.previewZoomLevel(this.layerId)
        ],
    ...
    buildWMSUrl(layerConfig) {
      ...
      params.CRS = layerConfig.crs
        ? layerConfig.crs
        : mapCollection.getMapView("2D").getProjection().getCode();
    ...
    buildWMTSUrl(layerConfig) {
      wmts
        .getWMTSCapabilities(layerConfig.capabilitiesUrl)
        .then((capabilities) => {
          this.createWMTSPreviewUrlFromCapabilities(layerConfig, capabilities);
        })
        .catch((error) => {
          console.warn(
            "Error occured during creation of url for preview of wmts-layer",
            layerConfig,
            error,
          );
        });
    ...
    createWMTSPreviewUrlFromCapabilities(layerConfig, capabilities) {
      ...
      transformedCoords = proj4(
        proj4(mapView.getProjection().getCode()),
        proj4("EPSG:3857"),
        this.previewCenter(this.layerId),
      );
      tileZ = options?.tileGrid.getZForResolution(
        mapView.getResolutions()[this.previewZoomLevel(this.layerId)],
      );
      tileCoord = options?.tileGrid.getTileCoordForCoordAndZ(
        transformedCoords,
        tileZ,
      );
    ...
    buildVectorTileUrl(layerConfig) {
      this.addPreviewUrl({
        id: layerConfig.id,
        previewUrl: layerConfig.preview?.src,
      });
    },
...
<template>
  <div
    ...
    role="button"
    tabindex="0"
    :class="[customClass, 'layerPreview']"

...
describe("src_3_0_0/modules/layerPreview/components/LayerPreview.vue", () => {
  ...
  beforeEach(() => {
    ...
    sinon
      .stub(LayerPreviewComponent.methods, "calculateExtent")
      .returns([1, 2, 3, 4]);
    getWMTSCapabilitiesStub = sinon.stub(wmts, "getWMTSCapabilities").callsFake(
      () =>
        new Promise((resolve) =>
          resolve({
            Contents: {
  ...
  it("do render the LayerPreview for supported layer-typ WMS", async () => {
    ...
    sinon
      .stub(LayerPreviewComponent.methods, "getPreviewUrl")
      .returns(layerWMS.url);
    wrapper = shallowMount(LayerPreviewComponent, {
      global: {
        plugins: [store],
      },
    ...
    expect(
      wrapper.find("img").attributes().src.indexOf(layerWMS.url),
    ).to.be.equals(0);
  ...
  it("do render the LayerPreview for supported layer-typ VectorTile", async () => {
    ...
    expect(wrapper.find(".layerPreview").exists()).to.be.true;
    expect(wrapper.find("img").attributes().src).to.be.equals(
      layerVectorTile.preview.src,
    );
    expect(warnSpy.notCalled).to.be.true;
  });
  ...
  it("do render the LayerPreview for supported layer-typ WMTS", async () => {
    ...
    sinon
      .stub(LayerPreviewComponent.methods, "getPreviewUrl")
      .returns(layerWMTS.capabilitiesUrl);
    wrapper = shallowMount(LayerPreviewComponent, {
      global: {
        plugins: [store],
      },
    ...
    expect(getWMTSCapabilitiesStub.firstCall.args[0]).to.be.equals(
      layerWMTS.capabilitiesUrl,
    );
    expect(
      wrapper
        .find("img")
        .attributes()
        .src.indexOf(layerWMTS.capabilitiesUrl.split("?")[0]),
    ).to.be.equals(0);
    expect(loadSpy.calledOnce).to.be.true;
    expect(
      loadSpy.firstCall.args[0].indexOf(
        layerWMTS.capabilitiesUrl.split("?")[0],
      ),
    ).to.be.equals(0);
    expect(warnSpy.notCalled).to.be.true;
  });
  ...
  it("generates new previewUrl if layerId changes", () => {
    const props = {
      ...
      generatePreviewUrlByConfigTypeSpy = sinon.spy(
        LayerPreviewComponent.methods,

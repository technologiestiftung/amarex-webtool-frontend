...
export default {
  ...
  computed: {
    ...
    optionsValue: function () {
      return this.options
        ...
        : {
            Box: this.$t(
              "common:shared.modules.graphicalSelect.selectBySquare",
            ),
            Circle: this.$t(
              "common:shared.modules.graphicalSelect.selectByCircle",
            ),
            Polygon: this.$t(
              "common:shared.modules.graphicalSelect.selectByPolygon",
            ),
          };
    },
  },
  ...
  mounted() {
    ...
    this.createDomOverlay({
      id: "sdp-circle-overlay",
      overlay: this.circleOverlay,
    });
    this.createDomOverlay({
      id: "sdp-tooltip-overlay",
  ...
  methods: {
    ...
    checkOptions: function () {
      if (
        !this.geographicValues.every((key) =>
          Object.keys(this.optionsValue).includes(key),
        )
      ) {
        this.addSingleAlert({
          content:
            this.$t(
              "common:shared.modules.graphicalSelect.alert.notSupportedOption",
            ) + this.geographicValues,
        });
      }
      ...
      if (!this.geographicValues.includes(this.selectedOption)) {
        this.addSingleAlert({
          content:
            this.$t(
              "common:shared.modules.graphicalSelect.alert.notSupportedSelection",
            ) + this.geographicValues,
        });
      }
      ...
      if (!this.selectionElements.includes(this.selectElement)) {
        this.addSingleAlert({
          content:
            this.$t(
              "common:shared.modules.graphicalSelect.alert.notSupportedElement",
    ...
    createDrawInteraction: function () {
      ...
      this.toggleOverlay({
        type: drawtype,
        overlayCircle: this.circleOverlay,
        overlayTool: this.tooltipOverlay,
      });
      this.updateDrawInteractionListener({
      ...
      this.registerListener({
        type: "pointermove",
        listener: this.showTooltipOverlay,
      });
      if (
        !mapCollection
          .getMap("2D")
          .getLayers()
          .getArray()
          .find((l) => l.get("id") === this.layer.get("id"))
      ) {
...
<template>
  ...
  <div class="form-floating mb-3">
    <select
      ...
      :aria-label="label"
      @change="
        selectedOptionData = $event.target.value;

...
describe("src_3_0_0/share-components/graphicalSelect/store/actionsGraphicalSelect", () => {
  ...
  it("showTooltipOverlay sets tooltipOverlay text for polygon", async () => {
    ...
    expect(definitionsGraphicalSelect.tooltipOverlay.element.innerHTML).to.eql(
      state.tooltipMessagePolygon,
    );
  });
  it("showTooltipOverlay sets tooltipOverlay position coordinates", async () => {
    ...
    expect(definitionsGraphicalSelect.tooltipOverlay.getPosition()).to.eql(
      rootState.Maps.mouseCoordinate,
    );
  });

...
export default {
  ...
  allFolders: (state) => {
    return searchInTree(
      state.layerConfig[treeSubjectsKey],
      "elements",
      "type",
      "folder",
    );
  ...
  allLayerConfigsByParentKey: (state) => (parentKey) => {
    return getNestedValues(state.layerConfig[parentKey], "elements", true).flat(
      Infinity,
    );
  },
  ...
  configuredModules: (_, getters) => {
    ...
    if (getters.controlsConfig.startModule?.secondaryMenu) {
      content = content.concat(
        getters.controlsConfig.startModule.secondaryMenu,
      );
    }
    return content;
  },
  ...
  determineZIndex: (_, getters) => (id) => {
    ...
    if (layerConf) {
      if (
        Object.prototype.hasOwnProperty.call(layerConf, "zIndex") &&
        typeof layerConf.zIndex === "number"
      ) {
        return layerConf.zIndex;
      }
      let maxZIndex = -1;
      const isBaselayer =
          Object.prototype.hasOwnProperty.call(layerConf, "baselayer") &&
          layerConf.baselayer,
      ...
      if (maxZIndex === -1) {
        if (isBaselayer) {
          maxZIndex =
            getters.maxZIndexOfLayerConfigsByParentKey(treeSubjectsKey);
        } else {
          maxZIndex =
            getters.maxZIndexOfLayerConfigsByParentKey(treeBaselayersKey);
        }
      }
  ...
  isBaselayer: (_, getters) => (layerId) => {
    const layerConfig = getters.allLayerConfigs.find(
      (layerConf) => layerConf.id === layerId,
    );
  ...
  isModuleAvailable: (state) => (moduleType) => {
    return JSON.stringify(state.portalConfig).includes(
      '"type":"' + moduleType + '"',
    );
  },
  ...
  showFolderPath: (state) => {
    return typeof state.portalConfig?.tree?.showFolderPath === "boolean"
      ? state.portalConfig?.tree?.showFolderPath
      : false;
  },
  ...
  showLayerAddButton: (state) => {
    return typeof state?.portalConfig?.tree?.addLayerButton === "object"
      ? state.portalConfig.tree.addLayerButton.active

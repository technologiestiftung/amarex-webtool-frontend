...
export default {
  ...
  loadConfigJson({ commit, state, dispatch, getters }) {
    ...
    axios
      ...
      .then((response) => {
        ...
        commit(
          "setPortalConfig",
          response.data ? response.data[portalConfigKey] : null,
        );
        if (getters.isMobile) {
        ...
        commit(
          "setLayerConfig",
          response.data ? response.data[treeTopicConfigKey] : null,
        );
        commit("setLoadedConfigs", "configJson");
      })
      .catch((error) => {
        console.error(
          `Error occured during loading config.json specified by config.js (${targetPath}).`,
          error,
        );
      });
  },
  ...
  loadRestServicesJson({ commit, state }) {
    axios
      ...
      .catch((error) => {
        console.error(
          `Error occured during loading rest-services.json specified by config.js (${state.configJs?.restConf}).`,
          error,
        );
      });
  },
    ...
    rawLayerList.initializeLayerList(state.configJs?.layerConf, (_, error) => {
      if (error) {
        dispatch(
          ...
          {
            category: "error",
            content: i18next.t("common:app-store.loadServicesJsonFailed", {
              layerConf: state.configJs?.layerConf,
            }),
          },
          { root: true },
        );
  ...
  initializeVectorStyle({ state, commit, dispatch, getters }) {
    const styleGetters = {
      ...
      highlightFeaturesPointStyleId:
        getters["Modules/HighlightFeatures/pointStyleId"],
      highlightFeaturesPolygonStyleId:
        getters["Modules/HighlightFeatures/polygonStyleId"],
      highlightFeaturesLineStyleId:
        getters["Modules/HighlightFeatures/lineStyleId"],
      zoomToFeatureId: getters.zoomTo?.find(
        (entry) => entry.id === "zoomToFeatureId",
      )?.styleId,
    };
    ...
    styleList
      .initializeStyleList(
        styleGetters,
        state.configJs,
        getters.allLayerConfigs,
        getters.configuredModules,
        (initializedStyleList, error) => {
          if (error) {
            dispatch(
              ...
              {
                category: "warning",
                content: i18next.t("common:app-store.loadStylev3JsonFailed", {
                  style_v3: state.configJs?.styleConf
                    ? state.configJs?.styleConf
                    : "style_v3.json",
                }),
              },
              { root: true },

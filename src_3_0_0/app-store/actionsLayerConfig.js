...
export default {
  ...
  addLayerToLayerConfig(
    ...
    { layerConfig, parentKey },
  ) {
    let maxZIndex = -1,
      configsByParentKey = [];
    const layerContainer = getters.allLayerConfigs.filter(
        (config) =>
          Object.prototype.hasOwnProperty.call(config, "zIndex") &&
          typeof config.zIndex === "number",
      ),
      matchingLayer = layerContainer.find(
        (layer) => layer.id === layerConfig.id,
      );

    if (state.layerConfig[parentKey]) {
      configsByParentKey = getters
        .allLayerConfigsByParentKey(parentKey)
        .filter(
          (config) =>
            Object.prototype.hasOwnProperty.call(config, "zIndex") &&
            typeof config.zIndex === "number",
      ...
      configsByParentKey = getters.visibleSubjectDataLayerConfigs.filter(
        (config) =>
          Object.prototype.hasOwnProperty.call(config, "zIndex") &&
          typeof config.zIndex === "number",
      );
    ...
    maxZIndex =
      configsByParentKey.length > 0
        ? Math.max(...configsByParentKey.map((layerConf) => layerConf.zIndex))
        : -Infinity;
    ...
    if (matchingLayer === undefined) {
      ...
      if (state.layerConfig[parentKey]) {
        ...
        if (
          folder &&
          folder.elements.find((config) => config.id === layerConfig.id) ===
            undefined
        ) {
  ...
  replaceByIdInLayerConfig(
    ...
    layerConfigs.forEach((config) => {
      ...
      if (assigned.length > 1) {
        console.warn(
          `Replaced ${assigned.length} layers in state.layerConfig with id: ${id}. Layer was found ${assigned.length} times. You have to correct your config!`,
        );
      }
      ...
      if (
        typeof replacement.visibility === "boolean" &&
        typeof lastVisibility === "boolean" &&
        lastVisibility !== replacement.visibility
      ) {
        layerCollection
          .getLayerById(id)
          ?.visibilityChanged(replacement.visibility);
      }
  ...
  addOrReplaceLayer: function (
    ...
    if (!layer) {
      ...
      if (config) {
        ...
        console.warn(
          "addOrReplaceLayer- layer with id: " +
            layerId +
            " not added, because it was not found in services.json.",
        );
        return false;
      }
    } else {
  ...
  showLayerAttributions({ dispatch }, layerAttributes) {
    ...
    if (
      layerAttributes?.visibility &&
      typeof layerAttribution !== "undefined" &&
  ...
  updateAllZIndexes({ getters }) {
    ...
    treeOrder.forEach((parentKey) => {
      const configsByParentKey = getters
        .allLayerConfigsByParentKey(parentKey)
        .filter(
          (config) =>
            Object.prototype.hasOwnProperty.call(config, "zIndex") &&
  ...
  extendLayers({ dispatch, getters, state }) {
    ...
    dispatch("addBaselayerAttribute");
    layerContainer = getNestedValues(state.layerConfig, "elements", true).flat(
      Infinity,
    );
    if (state.portalConfig?.tree?.type === "auto") {
      ...
      const allLayerConfigsStructured = getters.allLayerConfigsStructured(),
        folders = allLayerConfigsStructured.filter(
  ...
  processTreeTypeAuto({ commit, getters, state }, layerContainer) {
    const rawlayers = getAndMergeAllRawLayers(
        ...
        getters.showLayerAddButton,
      ),
      layersStructured = buildTreeStructure.build(
        rawlayers,
        state.layerConfig,
        getters.activeOrFirstCategory,
        layerContainer,
      );

    commit("setLayerConfigByParentKey", {
      layerConfigs: layersStructured,
  ...
  changeCategory({ commit, dispatch, getters, rootGetters, state }, category) {
    const layerContainer = getNestedValues(
        state.layerConfig,
        "elements",
        true,
      ).flat(Infinity),
  ...
  updateLayerConfigs({ dispatch, getters }, layerContainer) {
    layerContainer.forEach((layerConf) => {
      const rawLayer = getAndMergeRawLayer(
        layerConf,
        !getters.showLayerAddButton,
      );
      ...
      if (rawLayer) {
        dispatch("replaceByIdInLayerConfig", {
          layerConfigs: [{ layer: rawLayer, id: layerConf.id }],
        });
      }
    });
  },

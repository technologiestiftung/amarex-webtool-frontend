...
describe("src_3_0_0/app-store/js/getProxyUrl.js", () => {
  ...
  describe("getProxyUrl", () => {
    it("should generate key without hostname from url", function () {
      expect(
        getProxyUrl("https://dies.ist.ein.test/PFAD_ZU_TEST-QUELLE"),
      ).to.be.equal("/dies_ist_ein_test/PFAD_ZU_TEST-QUELLE");
    });
    ...
    it("should generate key with hostname from url", function () {
      ...
      expect(getProxyUrl(url, proxyHost)).to.be.equal(
        "https://test-proxy.example.com/dies_ist_ein_test/PFAD_ZU_TEST-QUELLE",
      );
    });
  ...
  describe("updateProxyUrl", () => {
    ...
    it("replaces the url if useProxy is set in nested array", async function () {
      const testproxyObject = {
          testproxyArray: [
            { url: "https://test.proxyurl.de/folder/", useProxy: true },
            { url: "https://test.proxyurl.de/folder2/", useProxy: false },
            { url: "https://test.proxyurl.de/folder3/", useProxy: false },
          ],
        },
        testobj = await updateProxyUrl(testproxyObject);

...
describe("src_3_0_0/app-store/actions.js", () => {
  ...
  beforeEach(() => {
    ...
    axiosMock = sinon
      .stub(axios, "get")
      .returns(Promise.resolve({ request: { status: 200, data: [] } }));
  ...
  describe("app-store actions", () => {
    ...
    it("initializeVectorStyle", () => {
      ...
      expect(initializeStyleListSpy.firstCall.args[0]).to.be.deep.equals(
        firstCallArg,
      );
      expect(initializeStyleListSpy.firstCall.args[1]).to.equals(
        state.configJs,
      );
      expect(initializeStyleListSpy.firstCall.args[2]).to.be.deep.equals(
        getters.allLayerConfigs,
      );
      expect(initializeStyleListSpy.firstCall.args[3]).to.be.deep.equals([
        { a: "b" },
        { c: "d" },
      ]);
      expect(typeof initializeStyleListSpy.firstCall.args[4]).to.be.equals(
        "function",
      );
    });
    ...
    it("moveStartModuleControls", () => {
      ...
      expect(state.portalConfig.secondaryMenu.sections[0].length).to.be.equals(
        3,
      );
      expect(state.portalConfig.secondaryMenu.sections[0][2].type).to.be.equals(
        "module_1",
      );
      expect(

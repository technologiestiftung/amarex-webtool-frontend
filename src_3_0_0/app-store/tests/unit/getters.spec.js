...
describe("src_3_0_0/app-store/getters.js", () => {
  ...
  describe("allLayerConfigs", () => {
    it("should returns the configs of all layers", () => {
      ...
      expect(getters.allLayerConfigs(stateAppStore)).to.be.an("array").that.is
        .empty;
      expect(getters.allLayerConfigs(state))
        .to.be.an("array")
        .to.deep.equals([
          {
            id: 1,
  ...
  describe("visibleLayerConfigs", () => {
    it("should return all visible layers", () => {
      const greenLayer = {
        ...
        myGetters = {
          allLayerConfigs: getNestedValues(layerConfig, "elements", true).flat(
            Infinity,
          ), // layerConfig[treeSubjectsKey].elements
        };

      expect(getters.visibleLayerConfigs(null, myGetters)).to.be.an("array");
      expect(getters.visibleLayerConfigs(null, myGetters).length).to.be.equals(
        2,
      );
      expect(getters.visibleLayerConfigs(null, myGetters)[0].id).to.be.equals(
        "453",
      );
      expect(getters.visibleLayerConfigs(null, myGetters)[0].visibility).to.be
        .true;
      expect(getters.visibleLayerConfigs(null, myGetters)[1]).to.be.equals(
        greenLayer,
      );
    });
    ...
    it("should return all visible layers - ids as array", () => {
      const layer = {
          id: ["717", "718", "719", "720", "13712", "13709", "13714", "13716"],
          visibility: true,
      ...
      expect(getters.visibleLayerConfigs(null, myGetters)[0]).to.be.equals(
        layer,
      );
    });
    ...
    it("should return all visible layers - grouped layer", () => {
      ...
      expect(getters.visibleLayerConfigs(null, myGetters)).to.be.an("array");
      expect(
        getters.visibleLayerConfigs(null, myGetters)[0].children.length,
      ).to.be.equals(2);
      expect(
        getters.visibleLayerConfigs(null, myGetters)[0].children[0].id,
      ).to.be.equals("682");
      expect(
        getters.visibleLayerConfigs(null, myGetters)[0].children[1].id,
      ).to.be.equals("1731");
      expect(getters.visibleLayerConfigs(null, myGetters)[0].id).to.be.equals(
        "xyz",
      );
      expect(getters.visibleLayerConfigs(null, myGetters)[0].visibility).to.be
        .true;
      expect(getters.visibleLayerConfigs(null, myGetters)[0].name).to.be.equals(
        "Kita und Krankenhäuser",
      );
    });
  });
  ...
  describe("visibleBaselayerConfigs", () => {
    it("should return all visible baselayers", () => {
      ...
      expect(getters.visibleBaselayerConfigs(null, myGetters)[0]).to.deep.equal(
        layerConfig[treeBaselayersKey].elements[0],
      );
      expect(
        getters.visibleBaselayerConfigs(null, myGetters)[0].id,
      ).to.deep.equal("453");
    });
  });

  describe("allLayerConfigsStructured, allLayerConfigsByParentKey, allFolders and folderById", () => {
    let state, layerConfig, layersWithFolder;
    ...
    it("should return all layerConfigs of first level", () => {
      ...
      expect(configs[2].elements).to.be.deep.equals(
        layersWithFolder[0].elements,
      );
    });
    ...
    it("should return all layerConfigs of first level by key 'subjectlayer'", () => {
      ...
      expect(configs[0].elements).to.be.deep.equals(
        layersWithFolder[0].elements,
      );
    });
    it("should return all layerConfigs of first level by key 'baselayer'", () => {
      const configs =
        getters.allLayerConfigsStructured(state)(treeBaselayersKey);
    ...
    it("allLayerConfigsByParentKey should return all layerConfigs key 'baselayer'", () => {
      const configs =
        getters.allLayerConfigsByParentKey(state)(treeBaselayersKey);
    ...
    it("allLayerConfigsByParentKey should return all layerConfigs key 'subjectlayer'", () => {
      const configs =
        getters.allLayerConfigsByParentKey(state)(treeSubjectsKey);
  ...
  describe("layerConfigsByAttributes", () => {
    it("should return the layers for requested attributes", () => {
      const greenLayer = {
        ...
        myGetters = {
          allLayerConfigs: layerConfig[treeBaselayersKey].elements.concat(
            layerConfig[treeSubjectsKey].elements,
          ),
        };

      expect(
        getters.layerConfigsByAttributes(null, myGetters)(undefined),
      ).to.be.an("array");
      expect(
        getters.layerConfigsByAttributes(null, myGetters)({ id: "1132" }),
      ).to.be.an("array");
      expect(
        getters.layerConfigsByAttributes(null, myGetters)({ id: "1132" })
          .length,
      ...
      expect(
        getters.layerConfigsByAttributes(null, myGetters)({ id: "1132" })[0],
      ).to.be.deep.equals(greenLayer);
      expect(
        getters.layerConfigsByAttributes(null, myGetters)({ visibility: true })
          .length,
      ).to.be.equals(2);
      expect(
        getters.layerConfigsByAttributes(null, myGetters)({ visibility: true }),
      ).to.be.deep.equals([bgLayer, greenLayer]);
      expect(
        getters.layerConfigsByAttributes(
          null,
          myGetters,
        )({ visibility: true, id: "1132" }).length,
      ).to.be.equals(1);
      expect(
        getters.layerConfigsByAttributes(
          null,
          myGetters,
        )({ visibility: true, id: "453" }).length,
      ).to.be.equals(1);
      // @todo testen
  ...
  describe("maxZIndexOfLayerConfigsByParentKey", () => {
    ...
    it("maxZIndexOfLayerConfigsByParentKey for basedata", () => {
      expect(
        getters.maxZIndexOfLayerConfigsByParentKey(
          null,
          myGetters,
        )(treeBaselayersKey),
      ).to.be.equals(1);
  ...
  describe("invisibleBaselayerConfigs", () => {
    it("should return all invisible baselayers", () => {
      ...
      expect(getters.invisibleBaselayerConfigs(null, myGetters)).to.be.an(
        "array",
      );
      expect(
        getters.invisibleBaselayerConfigs(null, myGetters).length,
      ).to.be.equals(2);
      expect(
        getters.invisibleBaselayerConfigs(null, myGetters)[0].id,
      ).to.be.equals("452");
      expect(
        getters.invisibleBaselayerConfigs(null, myGetters)[1].id,
      ).to.be.equals("1132");
    });
  });
  ...
  describe("configuredModules", () => {
    ...
    it("configuredModules only mainMenu", () => {
      ...
      expect(getters.configuredModules(null, myGetters)[0].type).to.be.equals(
        "fileImport",
      );
      expect(getters.configuredModules(null, myGetters)[1].type).to.be.equals(
        "openConfig",
      );
      expect(getters.configuredModules(null, myGetters)[2].type).to.be.equals(
        "contact",
      );
      expect(getters.configuredModules(null, myGetters)[3].type).to.be.equals(
        "language",
      );
    });
    it("configuredModules only mainMenu with 2 sections", () => {
      ...
      expect(getters.configuredModules(null, myGetters)[4].type).to.be.equals(
        "print",
      );
      expect(getters.configuredModules(null, myGetters)[5].type).to.be.equals(
        "draw",
      );
    });
    it("configuredModules mainMenu with 2 sections and secondary menu with one section", () => {
      ...
      expect(getters.configuredModules(null, myGetters).length).to.be.equals(
        10,
      );
      expect(getters.configuredModules(null, myGetters)[0].type).to.be.equals(
        "fileImport",
      ...
      expect(getters.configuredModules(null, myGetters)[6].type).to.be.equals(
        "fileImport",
      );
      expect(getters.configuredModules(null, myGetters)[7].type).to.be.equals(
        "openConfig",
      );
      expect(getters.configuredModules(null, myGetters)[8].type).to.be.equals(
        "contact",
      );
      expect(getters.configuredModules(null, myGetters)[9].type).to.be.equals(
        "language",
      );
    });
    it("configuredModules mainMenu with 2 sections and secondary menu with 2 section", () => {
      ...
      expect(getters.configuredModules(null, myGetters)[10].type).to.be.equals(
        "print",
      );
      expect(getters.configuredModules(null, myGetters)[11].type).to.be.equals(
        "draw",
      );
    });

...
describe("src_3_0_0/app-store/actionsLayerConfig.js", () => {
  ...
  beforeEach(() => {
    ...
    sinon
      .stub(rawLayerList, "getLayerWhere")
      .callsFake(function (searchAttributes) {
        return (
          layerList.find((entry) =>
            Object.keys(searchAttributes).every(
              (key) => entry[key] === searchAttributes[key],
            ),
          ) || null
        );
      });
  ...
  describe("addLayerToLayerConfig", () => {
    it("addLayerToLayerConfig no folders - add config to 'treeSubjectsKey'", () => {
      ...
      actions.addLayerToLayerConfig(
        { dispatch, getters, state },
        { layerConfig: layerToAdd, parentKey: treeSubjectsKey },
      );
      ...
      expect(state.layerConfig[treeSubjectsKey]?.elements[0]).to.deep.equal(
        layerToAdd,
      );
    });
    ...
    it("addLayerToLayerConfig no folders - add config to 'treeBaselayersKey'", () => {
      ...
      actions.addLayerToLayerConfig(
        { dispatch, getters, state },
        { layerConfig: layerToAdd, parentKey: treeBaselayersKey },
      );
      expect(dispatch.callCount).to.equals(2);
      expect(dispatch.firstCall.args[0]).to.equals("updateLayerConfigZIndex");
      expect(dispatch.firstCall.args[1]).to.deep.equals({
      ...
      expect(state.layerConfig[treeBaselayersKey]?.elements[0]).to.deep.equal(
        layerToAdd,
      );
    });
    ...
    it("addLayerToLayerConfig with folders - layer not contained in folder - add config to folder elements", () => {
      ...
      const layerToAdd = {
        ...
        folder_3 =
          layerConfigCustom[treeSubjectsKey].elements[0].elements[0]
            .elements[0];
      ...
      actions.addLayerToLayerConfig(
        { dispatch, getters, state },
        { layerConfig: layerToAdd, parentKey: "folder_3" },
      );
      expect(dispatch.callCount).to.equals(2);
      expect(dispatch.firstCall.args[0]).to.equals("updateLayerConfigZIndex");
      expect(dispatch.firstCall.args[1]).to.deep.equals({
      ...
      expect(dispatch.secondCall.args[1]).to.be.undefined;
      expect(
        state.layerConfig[treeSubjectsKey].elements[0].elements[0].elements[0]
          .elements.length,
      ).to.equal(2);
      expect(
        state.layerConfig[treeSubjectsKey].elements[0].elements[0].elements[0]
          .elements[1],
      ).to.deep.equal(layerToAdd);
    });
  ...
  describe("extendLayers", () => {
    ...
    describe("replaceByIdInLayerConfig", () => {
      it("replaceByIdInLayerConfig layer is contained in layerConfig", () => {
        ...
        actions.replaceByIdInLayerConfig(
          { dispatch, getters, state },
          { layerConfigs: [{ layer: toReplace, id: "453" }] },
        );

        expect(state.layerConfig[treeBaselayersKey].elements).to.be.an("array");
        expect(
          state.layerConfig[treeBaselayersKey].elements.length,
        ).to.be.equals(2);
        expect(
          Object.keys(state.layerConfig[treeBaselayersKey]?.elements[0]).length,
        ).to.be.equals(5);
        expect(
          state.layerConfig[treeBaselayersKey]?.elements[0].id,
        ).to.be.equals("453");
        expect(state.layerConfig[treeBaselayersKey]?.elements[0].visibility).to
          .be.true;
        expect(
          state.layerConfig[treeBaselayersKey]?.elements[0].att1,
        ).to.be.equals("bla");
        expect(
          state.layerConfig[treeBaselayersKey]?.elements[0].att2,
        ).to.be.deep.equals(toReplace.att2);
        expect(
          state.layerConfig[treeBaselayersKey]?.elements[1].id,
        ).to.be.equals("452");
        expect(
          Object.keys(state.layerConfig[treeBaselayersKey]?.elements[1]).length,
        ).to.be.equals(1);

        expect(state.layerConfig[treeSubjectsKey]?.elements).to.be.an("array");
        expect(
          state.layerConfig[treeSubjectsKey]?.elements.length,
        ).to.be.equals(2);
        expect(state.layerConfig[treeSubjectsKey]?.elements[0].id).to.be.equals(
          "1132",
        );
        expect(
          Object.keys(state.layerConfig[treeSubjectsKey]?.elements[0]).length,
        ).to.be.equals(3);
        expect(state.layerConfig[treeSubjectsKey]?.elements[1].id).to.be.equals(
          "10220",
        );
        expect(
          Object.keys(state.layerConfig[treeSubjectsKey]?.elements[1]).length,
        ).to.be.equals(1);
      ...
      it("replaceByIdInLayerConfig layer is not contained in layerConfig", () => {
        ...
        actions.replaceByIdInLayerConfig(
          { dispatch, getters, state },
          { layerConfigs: [{ layer: toReplace, id: "unknown" }] },
        );
    ...
    describe("addOrReplaceLayer", () => {
      ...
      it("layer is contained in layerConfig - change visibility", () => {
        ...
        actions.addOrReplaceLayer({ dispatch, getters }, { layerId: "1132" });
        expect(dispatch.calledOnce).to.be.true;
        expect(dispatch.firstCall.args[0]).to.equals(
          "replaceByIdInLayerConfig",
        );
        expect(dispatch.firstCall.args[1]).to.deep.equals({
          layerConfigs: [{ id: "1132", layer: expectedConfig }],
        });
      ...
      it("layer is contained in layerConfig - change visibility, new zIndex", () => {
        ...
        actions.addOrReplaceLayer(
          { dispatch, getters },
          { layerId: "1132", showInLayerTree: true },
        );
        expect(dispatch.calledOnce).to.be.true;
        expect(dispatch.firstCall.args[0]).to.equals(
          "replaceByIdInLayerConfig",
    ...
    describe("updateAllZIndexes", () => {
      it("updateAllZIndexes does not set zIndexes, if no zIndexes are set before", () => {
        ...
        expect(layerConfig[treeBaselayersKey].elements[0].zIndex).to.be
          .undefined;
        expect(layerConfig[treeBaselayersKey].elements[1].zIndex).to.be
          .undefined;
      ...
      it("updateAllZIndexes with all zIndexes are set before", () => {
        ...
        expect(layerConfig[treeBaselayersKey].elements[0].zIndex).to.be.equals(
          1,
        );
        expect(layerConfig[treeBaselayersKey].elements[1].zIndex).to.be.equals(
          2,
        );
  ...
  describe("changeCategory", () => {
    ...
    it("changes the category to inspire", () => {
      ...
      expect(commit.firstCall.args[0]).to.equals("setLayerConfigByParentKey");
      expect(
        commit.firstCall.args[1].layerConfigs.elements[0].name,
      ).to.be.equals("Gebäude");
      expect(
        commit.firstCall.args[1].layerConfigs.elements[1].name,
      ).to.be.equals("kein INSPIRE-Thema");
      expect(commit.firstCall.args[1].parentKey).to.be.equals(treeSubjectsKey);
      expect(commit.secondCall.args[0]).to.equals(
        "Modules/LayerSelection/clearLayerSelection",
      );
      expect(dispatch.calledOnce).to.be.true;
      expect(dispatch.firstCall.args[0]).to.equals(
        "Modules/LayerSelection/navigateForward",
      );
      expect(dispatch.firstCall.args[1].lastFolderName).to.equals("root");
      expect(
        dispatch.firstCall.args[1].subjectDataLayerConfs[0].name,
      ).to.equals("Gebäude");
      expect(
        dispatch.firstCall.args[1].subjectDataLayerConfs[1].name,
      ).to.equals("kein INSPIRE-Thema");
      expect(dispatch.firstCall.args[1].baselayerConfs).to.deep.equals(
        invisibleBaselayer,
      );
      expect(buildSpy.calledOnce).to.be.true;
    });
    ...
    it("changes the category to organisation", () => {
      ...
      expect(
        commit.firstCall.args[1].layerConfigs.elements[0].name,
      ).to.be.equals("Landesbetrieb Geoinformation und Vermessung");
      expect(
        commit.firstCall.args[1].layerConfigs.elements[1].name,
      ).to.be.equals("Landesbetrieb Straßen, Brücken und Gewässer");
      expect(commit.firstCall.args[1].parentKey).to.be.equals(treeSubjectsKey);
      expect(commit.secondCall.args[0]).to.equals(
        "Modules/LayerSelection/clearLayerSelection",
      );
      ...
      expect(
        dispatch.firstCall.args[1].subjectDataLayerConfs[0].name,
      ).to.equals("Landesbetrieb Geoinformation und Vermessung");
      expect(
        dispatch.firstCall.args[1].subjectDataLayerConfs[1].name,
      ).to.equals("Landesbetrieb Straßen, Brücken und Gewässer");
      expect(dispatch.firstCall.args[1].baselayerConfs).to.deep.equals(
        invisibleBaselayer,
      );
      expect(buildSpy.calledOnce).to.be.true;

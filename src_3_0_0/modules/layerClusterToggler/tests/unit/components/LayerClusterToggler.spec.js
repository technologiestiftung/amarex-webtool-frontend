...
describe("src_3_0_0/modules/layerClusterToggler/components/LayerClusterToggler.vue", () => {
  let layerNames, store, wrapper;

  beforeEach(() => {
    layerNames = ["Abc", "Example", "Xyz"];
          ...
          modules: {
            ...
            LayerClusterToggler: {
              ...
              getters: {
                icon: () => "",
                isToggled: () => true,
                layerIdList: () => ["1234", "8712.1", "8712.2"],
  ...
  it("should find the component LayerClusterToggler", () => {
    ...
    expect(wrapper.findComponent({ name: "LayerClusterToggler" }).exists()).to
      .be.true;
  ...
  it("should find the list for configured layerIdList", () => {
    ...
    expect(wrapper.find("p").text()).to.equals(
      "common:modules.layerClusterToggler.info",
    );
  ...
  it("should toogle visible of the layers, if the flat button is clicked", async () => {
    ...
    const toggleLayerVisibilitySpy = sinon.spy(
      wrapper.vm,

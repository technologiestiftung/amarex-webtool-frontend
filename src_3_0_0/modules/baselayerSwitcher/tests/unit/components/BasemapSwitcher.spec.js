...
describe("src_3_0_0/modules/BaselayerSwitcher.vue", () => {
  ...
  beforeEach(() => {
    ...
    visibleBaselayerConfigs = [vectorTileLayer, layer_453];

    originalUpdateLayerVisibilityAndZIndex =
      BaselayerSwitcher.actions.updateLayerVisibilityAndZIndex;
  ...
  describe("baselayerSwitcher DOM elements", () => {
    ...
    it("baselayerSwitcher is expanded", () => {
      ...
      expect(wrapper.findAll("#bs-expanded").length).to.equal(
        store.state.Modules.BaselayerSwitcher.baselayerIds.length,
      );
    });
    ...
    it("baselayerId of layer with highest zIndex is shown as preview", () => {
      ...
      expect(
        wrapper
          .find(
            "#layer-tree-layer-preview-" +
              store.state.Modules.BaselayerSwitcher.topBaselayerId,
          )
          .exists(),
      ).to.be.true;
    });
  ...
  describe("watcher", () => {
    it("visibleBaselayerConfigs with new baselayer", () => {
      ...
      wrapper.vm.$options.watch.visibleBaselayerConfigs.handler.call(
        wrapper.vm,
        newValue,
        oldValue,
      );
      expect(store.state.Modules.BaselayerSwitcher.topBaselayerId).to.equal(
        "WMTS",
      );
    });
  ...
  describe("created", () => {
    it("baselayerIds and topBaselayer", () => {
      ...
      expect(store.state.Modules.BaselayerSwitcher.baselayerIds).to.deep.equal([
        "Karte1",
        "Karte2",
        "Karte4",
      ]);
  ...
  describe("BaselayerSwitcher methods", () => {
    it("switchActiveBackgroundLaye to new layerId", () => {
      ...
      expect(
        BaselayerSwitcher.actions.updateLayerVisibilityAndZIndex.calledOnce,
      ).to.equal(true);
      expect(
        store.state.Modules.BaselayerSwitcher.topBaselayerId,
      ).to.deep.equal(layerId);
      expect(
        store.state.Modules.BaselayerSwitcher.activatedExpandable,
      ).to.equal(false);
    });
  });
});

...
const { addLayerConfig, importGeoJSON, importFile, setFeatureExtents } =
    actions,
  namedProjections = [
    [
      "EPSG:31467",
...
let dispatch, test1KML, test2KML;
...
before(() => {
  ...
  test1KML = fs.readFileSync(
    "./src_3_0_0/modules/fileImport/tests/resources/test1.kml",
    "utf8",
  );
  test2KML = fs.readFileSync(
    "./src_3_0_0/modules/fileImport/tests/resources/test2.kml",
    "utf8",
  );
});
...
describe("src_3_0_0/modules/fileImport/store/actionsFileImport.js", () => {
  beforeEach(() => {
    ...
    layer
      .getSource()
      .getFeatures()
      .forEach((feature) => layer.getSource().removeFeature(feature));
  });
  ...
  describe("file import - file should add some features to the current draw layer", () => {
        ...
        [
          {
            ...
            payload: {
              category: "success",
              content: i18next.t(
                "common:modules.fileImport.alertingMessages.success",
        ...
        [
          {
            ...
            payload: {
              category: "error",
              content: i18next.t(
                "common:modules.fileImport.alertingMessages.missingFormat",
              ),
            },
            dispatch: true,
          },
        ...
        [
          {
            ...
            payload: {
              category: "error",
              content: i18next.t(
                "common:modules.fileImport.alertingMessages.missingFileContent",
              ),
            },
            dispatch: true,
          },
      ...
      const payload = {
        ...
        state = {
          ...
          supportedFiletypes: {
            auto: {
              caption:
                "common:modules.fileImport.captions.supportedFiletypes.auto",
            },
            kml: {
              caption:
                "common:modules.fileImport.captions.supportedFiletypes.kml",
              rgx: /\.kml$/i,
            },
          },
        };
    ...
    it("adds a polygon from the kml file with name and style", () => {
      const payload = {
        ...
        recomendedFillColor = [255, 0, 63, 0.6901960784313725];
      ...
      expect(
        layer
          .getSource()
          .getFeatures()[0]
          .getStyle()(layer.getSource().getFeatures()[0])[0]
          .getFill()
          .getColor(),
      ).to.deep.equal(recomendedFillColor);
    });
    ...
    it("should set label style with color and font style", () => {
      ...
      expect(layer.getSource().getFeatures().length).to.equal(1);
      expect(
        layer.getSource().getFeatures()[0].getStyle().getText().getFill(),
      ).to.deep.equals({
        color_: [228, 26, 28, 1],
      });
      expect(
        layer.getSource().getFeatures()[0].getStyle().getText().getFont(),
      ).to.equals("16px Arial");
      expect(
        layer.getSource().getFeatures()[0].getStyle().getText().getScale(),
      ).to.equals(2);
      expect(
        layer.getSource().getFeatures()[0].getStyle().getText().getText(),
      ).to.equals("Jungfernstieg");
      expect(
        layer.getSource().getFeatures()[0].getStyle().getText().getTextAlign(),
      ).to.equals("left");
      expect(
        layer
          .getSource()
      ...
      expect(
        layer.getSource().getFeatures()[0].get("drawState"),
      ).to.deep.equals({
        fontSize: 32,
        text: "Jungfernstieg",
        color: [228, 26, 28, 1],
    ...
    it("adds a text style from the geojson file", () => {
      const payload = {
          layer: layer,
          raw: '{"type":"FeatureCollection","features":[{"type":"Feature","geometry":{"type":"Point","coordinates":[9.999147727017332,53.56029963338006]},"properties":{"isOuterCircle":false,"isVisible":true,"drawState":{"opacity":1,"font":"Arial","fontSize":16,"text":"Mein Schatzzzz","drawType":{"id":"writeText","geometry":"Point"},"symbol":{"id":"iconPoint","type":"simple_point","value":"simple_point"},"zIndex":0,"imgPath":"https://geodienste.hamburg.de/lgv-config/img/","pointSize":16,"color":[77,175,74,1]},"fromDrawTool":true,"invisibleStyle":{"geometry_":null,"fill_":null,"image_":null,"renderer_":null,"hitDetectionRenderer_":null,"stroke_":null,"text_":{"font_":"16px Arial","scaleArray_":[1,1],"text_":"Mein Schatzzzz","textAlign_":"left","textBaseline_":"bottom","fill_":{"color_":[77,175,74,1]},"maxAngle_":0.7853981633974483,"placement_":"point","overflow_":false,"stroke_":null,"offsetX_":0,"offsetY_":0,"backgroundFill_":null,"backgroundStroke_":null,"padding_":null},"zIndex_":9999},"styleId":"1"}}]}',
          filename: "beispielText.geojson",
        },
        state = {
          selectedFiletype: "auto",
          supportedFiletypes: {
            ...
            geojson: {
              caption:
                "common:modules.fileImport.captions.supportedFiletypes.geojson",
              rgx: /\.(geo)?json$/i,
            },
          },
        };
    ...
    it("Sets feature extent", (done) => {
      const payload = {
        ...
        tmpState = {
          ...importedState,
          ...{ featureExtents: { file1: [100, 100, 100, 100] } },
        };

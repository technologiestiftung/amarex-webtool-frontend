...
const actions = {
  ...
  checkIntersection({
    dispatch,
    getters: { selectedTargetLayer, bufferLayer },
  }) {
    dispatch("Maps/areLayerFeaturesLoaded", selectedTargetLayer.id, {
      root: true,
    }).then(() => {
  ...
  showBuffer({
    dispatch,
    commit,
    getters: { selectedSourceLayer, jstsParser, bufferRadius, bufferStyle },
  }) {
  ...
  resetModule({
    commit,
    dispatch,
    getters: { selectedSourceLayer, selectedTargetLayer },
  }) {
   ...
   * @return {void}
   */
  checkIntersectionWithBuffers(
    {
      commit,
      dispatch,
      getters: { selectedTargetLayer, jstsParser, resultType },
    },
    bufferFeatures,
  ) {
    ...
    targetFeatures.forEach((targetFeature) => {
      const targetGeometry = targetFeature.getGeometry(),
        foundIntersection = bufferFeatures.some((bufferFeature) => {
          const sourceGeometry = bufferFeature.getGeometry(),
            ...
            sameFeature =
              bufferFeature.get("originFeature").getId() ===
              targetFeature.getId(),
            sourcePoly = jstsParser.read(sourceGeometry),
            targetPoly = jstsParser.read(targetGeometry);

          if (
            targetGeometry.getType() === "Point" &&
            sourceGeometry.intersectsCoordinate(
      ...
      if (
        (!foundIntersection && resultType === ResultType.OUTSIDE) ||
   ...
   * @returns {void}
   */
  generateIntersectionPolygon(
    { commit, getters: { resultType } },
    { sourcePoly, targetPoly, properties = {} },
  ) {
    const subsetPoly =
      resultType === ResultType.WITHIN
        ? sourcePoly.intersection(targetPoly)
        : targetPoly.difference(sourcePoly);
   ...
   * @return {void}
   */
  checkIntersectionsWithIntersections(
    { dispatch, getters: { intersections, jstsParser } },
    bufferFeatures,
  ) {
   ...
   * @return {void}
   */
  convertIntersectionsToPolygons({
    commit,
    getters: { intersections, geoJSONWriter },
  }) {
  ...
  applyValuesFromSavedUrlBuffer({ rootState, state, dispatch, commit }) {
    ...
    if (
      rootState.urlParams &&
      rootState.urlParams["Tools/bufferAnalysis/active"]
    ) {
      const extractedParams = rootState.urlParams.initvalues.map((element) => {
        ...
        selectedSourceLayerFromUrl = state.selectOptions.find(
          (layer) => layer.id === extractedParams[0],
        ),
        bufferRadiusFromUrl = extractedParams[1],
        resultTypeFromUrl = extractedParams[2],
        selectedTargetLayerFromUrl = state.selectOptions.find(
          (layer) => layer.id === extractedParams[3],
        );

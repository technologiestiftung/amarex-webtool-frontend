 ...
 * @return {void}
 */
function applySelectedSourceLayer(
  ...
  const selectedLayer =
    typeof selectedSourceLayer === "string"
  ...
  // Value selected layer will be set to null on reset, so this tests for an undefined layer
    ...
    throw new Error(
      i18next.t("common:modules.bufferAnalysis.sourceLayerNotFound", {
        layerId: selectedSourceLayer,
      }),
    );
  }
  ...
  if (getters.bufferRadius && selectedLayer) {
    dispatch("Maps/areLayerFeaturesLoaded", selectedLayer.id, {
      root: true,
    }).then(() => {
 ...
 * @return {void}
 */
function applySelectedTargetLayer(
  { commit, getters, dispatch },
  selectedTargetLayer,
) {
  // find the layer in select options if selected layer is provided by id
  const selectedLayer =
    typeof selectedTargetLayer === "string"
  ...
  // Value selected layer will be set to null on reset, so this tests for an undefined layer
    ...
    throw new Error(
      i18next.t("common:modules.bufferAnalysis.targetLayerNotFound", {
        layerId: selectedTargetLayer,
      }),
    );
  }
}

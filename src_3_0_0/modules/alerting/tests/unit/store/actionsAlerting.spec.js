...
describe("src_3_0_0/modules/alerting/store/actionsAlerting.js", () => {
  ...
  it("cleanup", () => {
    ...
    expect(commit.getCall(0).args).to.eql([
      "addToDisplayedAlerts",
      { hash: "123", initial: true, mustBeConfirmed: false },
    ]);
  ...
  it("setAlertAsRead", () => {
    ...
    expect(commit.firstCall.args).to.eql([
      "setAlertAsRead",
      { hash: "found", mustBeConfirmed: true },
    ]);
  });
  ...
  it("setAlertAsUnread", () => {
    ...
    expect(commit.firstCall.args).to.eql([
      "setAlertAsUnread",
      { hash: "found", mustBeConfirmed: false },
    ]);
  });
  ...
  it("addSingleAlert adds a valid alert", () => {
    const state = {
      ...
      checkValue = actions.addSingleAlert(
        { state, commit },
        { hash: "123", content: "123", displayFrom: false, isNews: true },
      );
  ...
  it("addAlertsFromConfig", () => {
    actions.addAlertsFromConfig(
      { dispatch },
      {
        testAlert: { title: "testAlert" },
        testAlert2: { title: "testAlert2" },
      },
    );

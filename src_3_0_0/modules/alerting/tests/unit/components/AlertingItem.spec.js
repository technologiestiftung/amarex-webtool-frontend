...
describe("src_3_0_0/modules/alerting/components/AlertingItem.vue", () => {
  let alerts, sortedAlerts, store, wrapper;
  ...
  const alertFromConfig = [
      ...
      },
      {
        ...
        content:
          "2Dieses Geoportal dient der Qualitätskontrolle durch den Kunden.<br>Es ist aufgrund von möglichen Fehlern <b>nicht</b> zur Nutzung für alltägliche oder berufliche Aufgaben geeignet!<br><br>",
    ...
    sortedAlerts = [
      ...
      },
      {
        ...
        content: alerts.filter(
          (alert) => alert.category === "Portal zur Abnahme!",
        ),
      },
      {
        category: "Portal zur Abnahme!2",
        content: alerts.filter(
          (alert) => alert.category === "Portal zur Abnahme!2",
        ),
      },
      {
        category: "info",
  ...
  it("Checking the initially displayed alerts", async () => {
    ...
    describe("Expecting initially shown 4 category groups", () => {
      ...
      it("and contains 1 alerts", () => {
        expect(
          categoryContainers[0].findAll(".singleAlertContainer").length,
        ).to.equal(2);
      });
      ...
      it("and has no confirmation link", () => {
        expect(alertWrappers[0].html().indexOf("form-check-label")).to.equal(
          -1,
        );
      });
      ...
      it("and has confirmation link", () => {
        expect(
          alertWrappers[2]
            .html()
            .indexOf("form-check form-check-reverse form-switch mt-1"),
        ).not.to.equal(-1);
      });
      ...
      it("and contains 2 alert", () => {
        expect(
          categoryContainers[3].findAll(".singleAlertContainer").length,
        ).to.equal(2);
      });
      it('1st category group is named "Portal zur Abnahme!"', () => {
        expect(categoryContainers[2].find("h3").exists()).to.be.true;
        expect(categoryContainers[2].find("h3").text()).to.equal(
          "Portal zur Abnahme!2",
        );
      });
    });
  ...
  describe("Add some alerts", () => {
    ...
    it("displayed alert is the new one", () => {
      ...
      expect(
        wrapper.findAll(".singleAlertContainer")[0].html().indexOf("copycat"),
      ).not.to.equal(-1);
    });
    ...
    it("show an alert with creationDate", () => {
      ...
      expect(
        wrapper
          .find(".singleAlertContainer")
          .html()
          .indexOf("with creation date"),
      ).not.to.equal(-1);
      ...
      const addSingleAlertSpy = sinon.spy(wrapper.vm, "addSingleAlert"),
        parseObject = {
          data: {
            globalAlerts: [],
            restrictedAlerts: {
              "https://localhost:9001/portal/master/": ["testParsing"],
            },
            alerts: {
              testParsing: {
                category: "info",

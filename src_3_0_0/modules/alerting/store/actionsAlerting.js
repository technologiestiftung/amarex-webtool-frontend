...
function findSingleAlertByHash(haystackAlerts, needleHash) {
  const foundAlerts = haystackAlerts.filter(
    (singleAlert) => singleAlert.hash === needleHash,
  );
...
function checkAlertLifespan(alertToCheck) {
  if (
    alertToCheck.displayFrom === false &&
    alertToCheck.displayUntil === false
  ) {
    return true;
  }

  return (
    (!alertToCheck.displayFrom || dayjs().isAfter(alertToCheck.displayFrom)) &&
    (!alertToCheck.displayUntil || dayjs().isBefore(alertToCheck.displayUntil))
  );
}
/**
...
function checkAlertViewRestriction(displayedAlerts, alertToCheck) {
  ...
  if (
    localStorage[
      store.getters["Alerting/localStorageDisplayedAlertsKey"]
    ]?.includes(alertToCheck.hash)
  ) {
    return false;
  }
...
export default {
  ...
  cleanup: function ({ state, commit }) {
    ...
    state.alerts.forEach((singleAlert) => {
      if (
        !singleAlert.mustBeConfirmed &&
        singleAlert.initialConfirmed !== false &&
        singleAlert.initial !== undefined
      ) {
  ...
  addSingleAlert: function ({ state, commit }, newAlert) {
    const objectHash = require("object-hash"),
      newAlertObj =
        typeof newAlert === "string" ? { content: newAlert } : newAlert,
    ...
    if (state.availableCategories.includes(category)) {
      ...
      newAlertObj.category === undefined ||
      newAlertObj.category === ""
    ) {
    ...
    if (
      typeof newAlertObj.text === "string" &&
      typeof newAlertObj.content !== "string"
    ) {
      console.warn(
        'Deprecated: Alerting module - property "text" is deprecated. Use "content" instead.',
      );
    ...
    if (typeof alertProtoClone.displayUntil === "string") {
      alertProtoClone.hash =
        alertProtoClone.hash + alertProtoClone.displayUntil;
    }
    alertProtoClone.hash = objectHash(alertProtoClone.hash);
    isUnique =
      findSingleAlertByHash(state.alerts, alertProtoClone.hash) === false;
    onceInSession = !isUnique ? alertProtoClone.onceInSession : false;
    isInTime = checkAlertLifespan(alertProtoClone);
    isNotRestricted = checkAlertViewRestriction(
      state.displayedAlerts,
      alertProtoClone,
    );
    ...
    if (displayAlert) {
      ...
      if (
        !localStorage[state.localStorageDisplayedAlertsKey]?.includes(
          alertProtoClone.hash,
        )
      ) {

...
export default {
  ...
  created() {
    try {
      ...
      console.error(
        "Spelling localestorage is not available in this application. Please allow third party cookies in your browser!",
      );
    }
  },
  /**
  ...
  mounted() {
    ...
    if (
      this.fetchBroadcastUrl !== undefined &&
      this.fetchBroadcastUrl !== false
    ) {
  ...
  methods: {
    ...
    ...mapMutations("Alerting", ["removeFromAlerts", "setFetchBroadcastUrl"]),
    ...
    axiosCallback: function (response) {
      ...
      if (
        data.restrictedAlerts !== undefined &&
        typeof data.restrictedAlerts === "object"
      ) {
        Object.keys(data.restrictedAlerts).forEach((restrictedAlertUrl) => {
          if (
            this.currentUrl
              .toLowerCase()
              .startsWith(restrictedAlertUrl.toLowerCase()) &&
            Array.isArray(data.restrictedAlerts[restrictedAlertUrl])
          ) {
            collectedAlertIds = [
              ...collectedAlertIds,
              ...data.restrictedAlerts[restrictedAlertUrl],
            ];
          }
        });
      }
    ...
    fetchBroadcast: function (fetchBroadcastUrl) {
      axios
        .get(fetchBroadcastUrl)
        .then(this.axiosCallback)
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          >
            ...
            >
              <div class="singleAlertContainer row">
                <hr v-if="singleAlertIndex > 0 || categoryIndex > 0" />
                <h3 class="mt-1 ms-2 col-11">
                  <b>
                    {{ singleAlert.title }}
                  </b>
                </h3>
                ...
                <div v-html="singleAlert.content" />
                <div
                ...
                >
                  <div
                    ...
                    v-html="
                      $t(`common:modules.alerting.created`) +
                      singleAlert.creationDate
                    "
                  />
                  <div
                    ...
                    class="mt-1"
                  >
                    <div class="form-check form-check-reverse form-switch mt-1">
                      <SwitchInput
                        :id="'flexSwitchCheckDefault'"
                        :aria="
                          singleAlert.mustBeConfirmed
                            ? $t(singleAlert.confirmText)
                            : $t(singleAlert.reConfirmText)
                        "

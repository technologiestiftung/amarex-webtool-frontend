...
describe("src_3_0_0/modules/newsView/components/NewsView.vue", () => {
  ...
  it("checks content of news and sorting by date", () => {
    const date_0 =
        dayjs(news[0].displayFrom).format("DD.MM.YYYY") +
        " - " +
        dayjs(news[0].displayUntil).format("DD.MM.YYYY"),
      date_1 =
        dayjs(news[1].displayFrom).format("DD.MM.YYYY") +
        " - " +
        dayjs(news[1].displayUntil).format("DD.MM.YYYY"),
      date_2 =
        dayjs(news[2].displayFrom).format("DD.MM.YYYY") +
        " - " +
        dayjs(news[2].displayUntil).format("DD.MM.YYYY");
    ...
    expect(wrapper.find("#news_category_0").text()).to.be.equals(
      news[0].category,
    );
    expect(wrapper.find("#news_category_1").text()).to.be.equals(
      news[2].category,
    );
    expect(wrapper.find("#news_category_2").text()).to.be.equals(
      news[1].category,
    );
    expect(wrapper.find("#news_content_0").text()).to.be.equals(
      news[0].content,
    );
    expect(wrapper.find("#news_content_1").text()).to.be.equals(
      news[2].content,
    );
    expect(wrapper.find("#news_content_2").text()).to.be.equals(
      news[1].content,
    );
  });
  ...
  it("test method getDate if date one or two date attributes are missing ", () => {
    ...
    expect(wrapper.vm.getDate(noDate)).to.be.equals("");
    expect(wrapper.vm.getDate(onlyDisplayFrom)).to.be.equals(
      dayjs(onlyDisplayFrom.displayFrom).format("DD.MM.YYYY"),
    );

...
describe("src_3_0_0/modules/wfsSearch/js/buildFilter.js", () => {
  ...
  describe("buildStoredFilter", () => {
    it("should build a filter based upon the literal structure defined in the config and given user inputs", () => {
      expect(buildStoredFilter(values)).to.equal(
        "&gemname=Waldesch&flur=1&zaehler=741&nenner=0",
      );
    });
  });
  ...
  describe("buildXmlFilter", () => {
    it("should build the XML filter for the given fieldName and value", () => {
      ...
      expect(buildXmlFilter(field)).to.equal(
        '<ogc:PropertyIsEqualTo matchCase="false"><ogc:PropertyName>gemname</ogc:PropertyName><ogc:Literal>Waldesch</ogc:Literal></ogc:PropertyIsEqualTo>',
      );
    });
  });
});

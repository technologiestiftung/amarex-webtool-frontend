...
describe("src_3_0_0/modules/wfsSearch/store/mutationsWfsSearch.js", () => {
  ...
  describe("addOptions", () => {
    const addedOptions = ["", "flur", "zaehler", "nenner"],
    ...
    it("should add a value if it has not already been added as an option", () => {
      ...
      expect(instances[currentInstanceIndex].addedOptions).to.eql([
        ...addedOptions,
        option,
      ]);
    });
    it("should log a warning on the console if the value is already present in the addedOptions", () => {
      ...
      expect(warn.firstCall.args).to.eql([
        `WfsSearch: The option ${zaehler} is added multiple times to the formular. This entry will be skipped.`,
      ]);
    });
  });

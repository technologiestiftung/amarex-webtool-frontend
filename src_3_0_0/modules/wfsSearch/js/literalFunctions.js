 ...
 */
export function prepareLiterals(
  stateLiterals,
  literals = null,
  clauseId = "",
  requiredValues = {},
    ...
    if (literal.field) {
      ...
      if (literal.field.required) {
        ...
        literal.field.inputLabel =
          typeof literal.field.inputLabel === "object"
...
export function fieldValueChanged(
  ...
  arr.forEach((literal) => {
    if (literal.clause) {
      ...
      const fieldRequired = Array.isArray(literal.field.required)
        ? literal.field.required[parameterIndex]
        : literal.field.required;

...
import {
  buildPath,
  getOptions,
  prepareOptionsWithId,
} from "../js/pathFunctions";
...
export default {
  ...
  computed: {
    ...
    selectableParameters() {
      ...
      if (Array.isArray(this.fieldName)) {
        ...
        return {
          ...
          options:
            Array.isArray(this.options) && !isObject(this.options[0])
              ? this.options[this.parameterIndex]
              : this.options,
    ...
    disabled() {
      ...
      if (
        typeof this.selectableParameters.options === "string" &&
        notRoot &&
        ...
        return !(
          this.currentInstance.addedOptions?.includes("") &&
          optionsArr.every((option) =>
            this.currentInstance.addedOptions.includes(option),
          ) &&
          selectedValues.includes("") &&
          optionsArr
            .slice(0, optionsArr.length - 1)
            .every((option) => selectedValues.includes(option))
        );
      }
      // Disable all options depending on the root source in the beginning or if the external source hasn't been loaded yet
      return typeof this.selectableParameters.options === "string"
        ? this.parsedSource === null || notRoot
        : false;
    },
    selectableOptions() {
      ...
      if (
        ...
        if (Object.keys(this.selectedOptions).includes("")) {
          const optionKeysWithoutRoot = Object.keys(
              this.selectedOptions,
            ).filter((option) => option !== ""),
            previousElementsSelected = optionsArr
              .slice(0, lastIndex)
    ...
    showSuggestions() {
      const length =
        typeof this?.suggestionsConfig?.length !== "undefined"
          ? this.suggestionsConfig.length
          : 3;

      return (
        !this.options && this.suggestionsConfig && this.value.length >= length
      );
    },
  },
  watch: {
  ...
  methods: {
    ...
    async valueChanged(val) {
      ...
      this.setRequiredValues({
        ...fieldValueChanged(
          this.selectableParameters.fieldId,
          value,
          this.currentInstance.literals,
          this.requiredValues,
          this.parameterIndex,
        ),
      });
      ...
      if (typeof this.selectableParameters.options === "string") {
        ...
        this.setSelectedOptions({
          options: this.selectableParameters.options,
          value,
          index,
        });
      } else if (this.showSuggestions) {
        this.showLoader = true;
        const fieldName = Array.isArray(this.fieldName)
            ? this.fieldName[this.parameterIndex]
            : this.fieldName,
          xmlFilter = buildXmlFilter({ fieldName, queryType: "like", value }),
          suggestions = await requestProvider.searchFeatures(
            this.$store,
            this.currentInstance,
            this.service,
            xmlFilter,
            this?.suggestionsConfig?.featureType,
          );

        this.showLoader = false;
        // Retrieve the values for the fieldName and make sure they are unique.
        this.suggestions = [
          ...new Set(suggestions.map((v) => v.values_[fieldName])),
        ];
      }
    },
    isObject,
...
<template>
  <div class="mb-3">
    ...
    >
      ...
      <label
        for="`module-wfsSearch-${selectableParameters.fieldName}-${selectableParameters.fieldId}-fieldSelection`"
      >
    ...
    <div class="form-floating">
      <component
        ...
        :default-value="
          htmlElement === 'input' ? selectableParameters.defaultValue : ''
        "
        :required="selectableParameters.required"
        :disabled="disabled"
        :list="
          htmlElement === 'input' && showSuggestions
            ? `module-wfsSearch-${fieldName}-${fieldId}-input-suggestions`
            : ''
        "
        :aria-label="
          Array.isArray(inputLabel) ? selectableParameters.inputLabel : ''
        "

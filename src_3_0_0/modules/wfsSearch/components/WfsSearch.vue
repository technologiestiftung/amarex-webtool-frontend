...
export default {
  ...
  computed: {
    ...
    headers() {
      ...
      if (resultList === "showAll") {
        const lengths = this.results.map(
            (feature) => Object.keys(feature.values_).length,
          ),
          indexOfFeatureWithMostAttr = lengths.indexOf(Math.max(...lengths));

        return Object.keys(
          this.results[indexOfFeatureWithMostAttr].values_,
        ).reduce((acc, curr) => {
          acc[curr] = curr;
          return acc;
        }, {});
  ...
  methods: {
    ...
    async search() {
      ...
      const features = await requestProvider.searchFeatures(
        this.$store,
  ...
  <div>
    <div>
      ...
      >
        ...
        >
          <WfsSearchLiteral :literal="literal" />
        </div>
        <div>
          ...
          >
            <FlatButton
              ...
              :text="
                $t('common:modules.wfsSearch.showResults') +
                ' ' +
                `(${results.length})`
              "
              :icon="'bi-x'"
    ...
    >
      <template v-if="showResults && results.length">
        <header>
          <h4>
            {{
              currentInstance.resultDialogTitle
                ? $t(currentInstance.resultDialogTitle)
                : $t(name)
            }}
          </h4>
        </header>

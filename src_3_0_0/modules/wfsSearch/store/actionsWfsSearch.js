...
const actions = {
  ...
  prepareModule({ commit, dispatch, getters, rootGetters }) {
    ...
    const { currentInstance } = getters,
      {
        requestConfig: { layerId, likeFilter, restLayerId, storedQueryId },
        title,
      } = currentInstance,
      wfs = restLayerId
    ...
    if (wfs) {
      ...
      commit("setRequiredValues", {
        ...prepareLiterals(currentInstance.literals),
      });
      commit(
        "setUserHelp",
        currentInstance.userHelp || createUserHelp(currentInstance.literals),
      );
      ...
      if (!storedQueryId && layerId) {
        service.typeName =
          wfs.featureType || (wfs.get ? wfs.get("featureType") : undefined);
      }
      ...
      dispatch(
        ...
        i18next.t("common:modules.wfsSearch.wrongConfig", {
          id: restLayerId ? restLayerId : layerId,
          title,
        }),
        { root: true },
      );
  ...
  retrieveData({ commit, getters }) {
    const {
      currentInstance: { selectSource },
    } = getters;

    axios
      .get(encodeURI(selectSource))
      .then((response) =>
        handleAxiosResponse(response, "WfsSearch, retrieveData"),
      )

...
describe("src_3_0_0/modules/addWMS/components/AddWMS.vue", () => {
  ...
  it("getParsedTitle", () => {
    ...
    it("should return parsed title without slash and be replaced with minus", function () {
      expect(
        wrapper.vm.getParsedTitle(
          "WMS Digitales Höhenmodell Hamburg (DGM1) - 1:10.000",
        ),
      ).to.equal("WMS-Digitales-Höhenmodell-Hamburg--DGM1----1-10.000");
    });
    it("should return parsed title without slash and be replaced with minus", function () {
      expect(wrapper.vm.getParsedTitle("test/title")).to.equal("test-title");
    });
  ...
  describe("getIfInExtent", () => {
            ...
            BoundingBox: [
              {
                crs: "EPSG:25832",
                extent: [
                  302907.887193, 5435104.982326, 389523.673913, 5508222.768538,
                ],
              },
            ],
          },
    ...
    it("schould return true if the currentExtent intersects the capability extent", function () {
      currentExtent = [205000, 5009000, 730000, 6075800];
      expect(wrapper.vm.getIfInExtent(capability, currentExtent)).to.be.true;
    });

    it("schould return true if the currentExtent intersects the capability extent", function () {
      currentExtent = [205000, 5009000, 730000];
      expect(wrapper.vm.getIfInExtent(capability, currentExtent)).to.be.true;
    });
    ...
    it("should return true if the layer in Capability does not have the right crs", function () {
            ...
            BoundingBox: [
              {
                crs: "EPSG:3067",
                extent: [
                  336385.4535501953, 6628495.2621008465, 447592.181149918,
                  7646073.290737241,
                ],
              },
            ],
      ...
      currentExtent = [455000, 5809000, 730000, 6075800];
      expect(wrapper.vm.getIfInExtent(capability, currentExtent)).to.be.true;
    });
        ...
        Capability: {
          Layer: {
            BoundingBox: [
              {
                crs: "EPSG:25832",
                extent: [302907.887193, 5435104.982326, 389523.673913],
              },
            ],
          },
        },
  ...
  it("getReversedData", () => {
    const data =
        '<Layer><SRS>EPSG:4326</SRS><Layer queryable="1"><SRS>EPSG:102100</SRS><BoundingBox SRS="EPSG:4326" minx="6.355978" miny="49.11015" maxx="7.413363" maxy="49.644331"/></Layer></Layer>',
    ...
    it("should replace all SRS with CRS in the xml node and attribute", function () {
      expect(
        wrapper.vm.getReversedData(dataXml).getElementsByTagName("SRS").length,
      ).to.equal(0);
      expect(
        wrapper.vm.getReversedData(dataXml).getElementsByTagName("CRS").length,
      ).not.to.equal(0);
    });
  });
});

...
function changeActiveMemory(state, map, diff) {
  ...
  if (targetIndex !== null) {
    if (memory[targetIndex]) {
      ...
      console.error(
        `Tried to change active backForward memory by ${diff} positions, but such a memory does not exist in memory ${JSON.stringify(memory)}`,
      );
    }
  }
}
...
export default {
  ...
  memorize(state, mapView) {
    const current =
        state.position === null ? null : state.memory[state.position],
      upToNext =
        state.position === null
          ? []

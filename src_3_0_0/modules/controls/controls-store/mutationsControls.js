...
export default {
  ...
  registerControl(
    state,
    { name, control, hiddenMobile = false, expandableControls = false },
  ) {
    // todo : das Registrieren an der ControlBar von addons-controls fehlt noch  (wenn das von außen geht?)
    if (hiddenMobile) {
      state.mobileHiddenControls = [...state.mobileHiddenControls, name];
    }
    if (expandableControls) {
      state.expandableControls = [...state.expandableControls, name];
    }
  },
  ...
  unregisterControl(state, name) {
    ...
    state.expandableControls = state.expandableControls.filter(
      (s) => s !== name,
    );
  },
};

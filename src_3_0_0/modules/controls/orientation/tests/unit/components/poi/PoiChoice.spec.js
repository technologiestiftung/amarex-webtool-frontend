...
describe("src_3_0_0/modules/controls/orientation/components/PoiChoice.vue", () => {
  ...
  describe("Render Component", function () {
    it("renders the Poi choice component", () => {
      ...
      expect(
        wrapper.find(".choice-content >label.currentPosition").text(),
      ).to.equal("common:modules.controls.orientation.poiChoiceCurrentPostion");
      expect(
        wrapper.find(".choice-content >label.currentPosition input").element
          .value,
      ).to.equal("currentPosition");
      expect(
        wrapper.find(".choice-content >label.customPosition").text(),
      ).to.equal("common:modules.controls.orientation.poiChoiceCustomPostion");
      expect(
        wrapper.find(".choice-content >label.customPosition input").element
          .value,
      ).to.equal("customPosition");
    });

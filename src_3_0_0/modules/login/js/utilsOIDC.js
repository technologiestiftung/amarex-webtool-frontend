...
function getVerifier(reset = false) {
  const storage = useSessionStorage
    ...
    codeVerifier =
      existingCodeVerifier && !reset
        ? existingCodeVerifier
        : b64Uri(generateRandomString()).substring(0, verifierLength);
...
async function getAuthCodeUrl(
  ...
  return (
    ...
    "?response_type=code" +
    "&client_id=" +
    encodeURIComponent(oidcClientId) +
    "&state=" +
    encodeURIComponent(state) +
    "&scope=" +
    encodeURIComponent(oidcScope) +
    "&redirect_uri=" +
    encodeURIComponent(oidcRedirectUri) +
    "&code_challenge=" +

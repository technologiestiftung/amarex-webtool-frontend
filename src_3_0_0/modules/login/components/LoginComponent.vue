...
export default {
  ...
  methods: {
    ...mapMutations("Menu", [
      ...
      "setCurrentComponentPropsDescription",
    ]),
    ...
    async openLoginWindow() {
      ...
      const params =
          "width=500,height=500,status=no,location=no,menubar=no," +
          `top=${window.screenY + (window.outerHeight - 500) / 2.5},` +
          `left=${window.screenX + (window.outerWidth - 500) / 2}`,
        loginPopup = window.open(
          await this.getAuthCodeUrl(),
          this.translate("common:modules.login.login"),
          params,
        );
    ...
    setLoginProps() {
      const iconType = this.loggedIn ? this.iconLogged : this.iconLogin,
        componentName = this.loggedIn
          ? "common:modules.login.logout"
          : "common:modules.login.login",
        componentDescription = this.loggedIn
          ? "common:modules.login.descriptionLoggedIn"
          : "common:modules.login.description";

      this.setIcon(iconType);
      this.setCurrentComponentPropsName({
        side: "secondaryMenu",
        name: componentName,
      });
      this.setCurrentComponentPropsDescription({
        side: "secondaryMenu",

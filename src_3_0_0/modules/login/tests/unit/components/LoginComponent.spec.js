...
const fakeToken =
  "eyJ0eXAiOiJKV1QiLCJhbGciOiJFUzI1NiIsImtpZCI6IjViY2IzZThmNDM2MGI0YjI4MWRjNDM4ZTExODE4YzZlIn0.e30.LEo9imrM5zuR1yo-KMzdY62XbnL7UBO2AImB2Pf-bD35NVZlMsU7xsXMUX6petNWU61tJSFzyMk4nWZQHm3LBQ";
...
describe("src_3_0_0/modules/Modules/Login/components/LoginComponent.vue", () => {
  ...
  describe("LoginComponent template", () => {
    it("should render Login", () => {
      ...
      expect(wrapper.find("#login-component button#logout-button").exists()).to
        .be.true;
    });
    ...
    it("should call logout fn if button is clicked", async () => {
      ...
      await wrapper
        .find("#login-component button#logout-button")
        .trigger("click");
    ...
    it("should have values from cookies after Login renders", async () => {
      ...
      expect(wrapper.vm.$store.state.Modules.Login.screenName).to.be.equal(
        fakeToken,
      );
      expect(wrapper.vm.$store.state.Modules.Login.username).to.be.equal(
        fakeToken,
      );
      expect(wrapper.vm.$store.state.Modules.Login.email).to.be.equal(
        fakeToken,
      );

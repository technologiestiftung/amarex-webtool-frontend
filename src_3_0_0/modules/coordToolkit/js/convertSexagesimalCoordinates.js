...
function convertSexagesimalStringToDecimal(coordinates) {
  ...
  eastingDegree = Number(
    eastingSplit[0].substring(0, eastingSplit[0].length - 1),
  );
  northingDegree = Number(
    northingSplit[0].substring(0, northingSplit[0].length - 1),
  );
  eastingMin =
    Number(eastingSplit[1].substring(0, eastingSplit[1].length - 1)) / 60;
  northingMin =
    Number(northingSplit[1].substring(0, northingSplit[1].length - 1)) / 60;
  eastingSecs =
    Number(eastingSplit[2].substring(0, eastingSplit[2].length - 1)) / 3600;
  northingSecs =
    Number(northingSplit[2].substring(0, northingSplit[2].length - 1)) / 3600;
...
export function convertSexagesimalFromString(coord) {
  ...
  let coordinates = coord.replace(/E/gi, ""),
    index = -1,
    secondIndex = -1,
    easting = "No value",
    northing = "No value";
  const pairs =
    coord.indexOf("N") > -1 ? coordinates.split("N") : coordinates.split("S");
  ...
  if (pairs.length === 2) {
    ...
    coordinates = coordinates.replace(/N/gi, "");
    coordinates = coordinates.replace(/S/gi, "");

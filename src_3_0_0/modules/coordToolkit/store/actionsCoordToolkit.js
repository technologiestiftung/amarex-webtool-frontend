...
} from "../js/convertSexagesimalCoordinates";
...
export default {
  ...
  initHeightLayer({ commit, state, rootGetters }) {
    ...
    if (!layerConfig) {
      console.warn(
        "CoordToolkit: the layer with id " +
          state.heightLayerId +
          " to retrieve height from is not available. Check the Id in config.json with path 'portalConfig.mainMenu.sections.coordToolkit.heightLayerId'!",
      );
    }
  },
  /**
  ...
  getHeight({ dispatch, rootGetters, state }, position) {
    const projection = rootGetters["Maps/projection"],
      ...
      url = state.heightLayer
        .getLayerSource()
        .getFeatureInfoUrl(position, resolution, projection, gfiParams);
  ...
  newProjectionSelected({ dispatch, commit, state, getters }, value) {
    ...
    dispatch("formatInput", [
      state.coordinatesEasting,
      state.coordinatesNorthing,
    ]);
  ...
  changedPosition({ dispatch, state, getters }) {
    if (state.mode === "supply") {
      const targetProjectionName = state.currentProjection?.epsg,
        position = getters.getTransformedPosition(
          mapCollection.getMap("2D"),
          targetProjectionName,
        );

      if (position) {
        dispatch("adjustPosition", {
          position: position,
          targetProjection: state.currentProjection,
        });
      }
    }
  },
  ...
  setFirstSearchPosition({ dispatch, commit, state, rootState, getters }) {
    if (state.mode === "search") {
      ...
      if (
        ...
        position[0] === 0 &&
        position[1] === 0 &&
        rootState.Maps.center
      ) {
        commit("setCoordinatesEasting", {
          id: "easting",
          value: String(rootState.Maps.center[0]).substring(0, 9),
        });
        commit("setCoordinatesNorthing", {
          id: "northing",
          value: String(rootState.Maps.center[1]).substring(0, 9),
        });
  ...
  adjustPosition({ commit, rootGetters }, { position, targetProjection }) {
    ...
    if (targetProjection && Array.isArray(position) && position.length >= 2) {
        ...
        rootGetters.namedProjections.find((el) => {
          if (el[1].includes("ETRS89_3GK3")) {
            const searchString = "x_0=",
              x0 = parseInt(
                el[1].substring(
                  el[1].lastIndexOf(searchString) + searchString.length,
                  el[1].indexOf(" +y_0="),
                ),
                10,
              );
      ...
      commit("setCoordinatesEasting", {
        id: "easting",
        value: String(easting),
      });
      commit("setCoordinatesNorthing", {
        id: "northing",
        value: String(northing),
      });
    }
  },
  /**
  ...
  validateInput({ state, commit }, coord) {
    const validETRS89UTM = /^[0-9]{6,7}[.,]{0,1}[0-9]{0,3}\s*$/,
      ...
      validWGS84 =
        /^\d[0-9]{0,2}[°]{1}\s*[0-9]{0,2}['`´′]{0,1}\s*[0-9]{0,2}['`´′]{0,2}["″]{0,2}[\sNS]*\s*$/,
  ...
  transformCoordinatesFromTo({ state, commit }, targetProjection) {
    ...
    if (state.selectedCoordinates.length === 2) {
      if (state.currentProjection.projName === "longlat") {
        ...
        coordinates = [
          Math.round(state.selectedCoordinates[0]),
          Math.round(state.selectedCoordinates[1]),
        ];
      }
      if (
      ...
      if (
        ...
        transformedCoordinates = [
          convertSexagesimalFromDecimal(transformedCoordinates[1]),
          convertSexagesimalFromDecimal(transformedCoordinates[0]),
        ];
      } else if (
        targetProjection.id ===
        "http://www.opengis.net/gml/srs/epsg.xml#4326-DG"
        ...
        transformedCoordinates = [
          transformedCoordinates[0].toFixed(2),
          transformedCoordinates[1].toFixed(2),
        ];
      } else {
        transformedCoordinates = [
          transformedCoordinates[0].toFixed(2),
      ...
      commit("setCoordinatesEasting", {
        id: "easting",
        value: transformedCoordinates[0],
      });
      commit("setCoordinatesNorthing", {
        id: "northing",
        value: transformedCoordinates[1],
      });
    }
  },
  ...
  transformCoordinates({ state, dispatch }) {
    const mapProjection = mapCollection
      .getMapView("2D")
      .getProjection()
      .getCode();
    ...
    if (state.selectedCoordinates.length === 2) {
      ...
      if (state.currentProjection.id.indexOf("4326") > -1) {
        const coordinates = convertSexagesimalToDecimal(
          state.selectedCoordinates,
        );

        state.transformedCoordinates = proj4(
          proj4("EPSG:4326"),
          proj4(mapProjection),
          coordinates,
        );
        ...
        const coordinates = [
          Math.round(state.selectedCoordinates[0]) - 3000000,
          Math.round(state.selectedCoordinates[1]),
        ];
        ...
        state.transformedCoordinates = proj4(
          proj4(state.currentProjection.id),
          proj4(mapProjection),
          coordinates,
        );
        dispatch("moveToCoordinates", state.transformedCoordinates);

...
describe("src_3_0_0/modules/coordToolkit/components/CoordToolkit.vue", () => {
  ...
  beforeEach(() => {
    ...
    origtransformCoordinatesFromTo =
      CoordToolkit.actions.transformCoordinatesFromTo;
  ...
  it('has initially selected projection "EPSG:25832"', async () => {
    ...
    expect(selected.at(0).attributes().value).to.equal(
      "http://www.opengis.net/gml/srs/epsg.xml#25832",
    );
  });
  ...
  describe("CoordToolkit.vue methods", () => {
    it("method selectionChanged sets currentProjection", () => {
      ...
      expect(
        store.state.Modules.CoordToolkit.currentProjection.name,
      ).to.be.equals(value);
      expect(
        store.state.Modules.CoordToolkit.currentProjection.projName,
      ).to.be.equals("tmerc");
      expect(
        store.state.Modules.CoordToolkit.coordinatesEasting.value,
      ).to.be.equals("0.00");
      expect(
        store.state.Modules.CoordToolkit.coordinatesNorthing.value,
      ).to.be.equals("0.00");
    });
    ...
    it("createInteraction sets projections and adds interaction", () => {
      ...
      expect(
        typeof store.state.Modules.CoordToolkit.selectPointerMove,
      ).to.be.equals("object");
      expect(
        typeof store.state.Modules.CoordToolkit.selectPointerMove
          .handleMoveEvent,
    ...
    it("initProjections adds WGS84 decimal projection", () => {
      ...
      expect(
        projections.filter(
          (proj) =>
            proj.id === "http://www.opengis.net/gml/srs/epsg.xml#4326-DG",
        ).length,
      ).to.be.equals(1);
    });
    ...
    it("initProjections adds ETRS89_3GK3", () => {
      ...
      expect(
        projections.filter(
          (proj) =>
            proj.id === "http://www.opengis.net/gml/srs/epsg.xml#ETRS893GK3",
        ).length,
      ).to.be.equals(1);
    });
    ...
    it("getClassForEasting no error", async () => {
      ...
      await wrapper.vm.$nextTick();
      expect(wrapper.vm.getClassForEasting()).to.be.equals(
    ...
    it("getClassForNorthing no error", async () => {
      ...
      store.commit("Modules/CoordToolkit/setEastingNoCoord", false);
      expect(wrapper.vm.getClassForNorthing()).to.be.equals(
  ...
  describe("CoordToolkit.vue watcher", () => {
    ...
    it("watch to clickCoordinate in mode supply shall call positionClicked", async () => {
      ...
      await wrapper.vm.$nextTick();
      expect(store.state.Modules.CoordToolkit.mode).to.be.equals("supply");
      wrapper.vm.$options.watch.clickCoordinate.handler.call(
        wrapper.vm,
        [10, 20],
      );

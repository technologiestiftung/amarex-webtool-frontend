...
describe("src_3_0_0/modules/coordToolkit/store/gettersCoordToolkit.js", () => {
  ...
  describe("getters supplyCoord", () => {
    ...
    it("returns the projections from state", () => {
      const proj = [
          { name: "projection 1", projName: "utm" },
    ...
    it("returns the positionMapProjection from state", () => {
      ...
      expect(getters.positionMapProjection(stateCoord)).to.be.an("array").that
        .is.empty;
    ...
    it("returns the currentProjection from state", () => {
      ...
      expect(getters.currentProjection(stateCoord).name).to.be.equals(
        proj1.name,
      );
      expect(getters.currentProjection(stateCoord).projName).to.be.equals(
        proj1.projName,
      );
    ...
    it("returns the coordinatesEastingField from state", () => {
      ...
      expect(getters.coordinatesEasting(state).value).to.be.equals(
        easting.value,
      );
    });
    it("returns the coordinatesNorthingField from state", () => {
      ...
      expect(getters.coordinatesNorthing(state).value).to.be.equals(
        northing.value,
      );
    });
  });
  ...
  describe("testing default values", () => {
    it("returns the name default value from state", () => {
      expect(getters.name(stateCoord)).to.be.equals(
        "common:modules.coordToolkit.name",
      );
    });

...
export default {
  ...
  computed: {
    ...
    ...mapGetters("Modules/SearchBar", ["selectedSearchResults"]),
    ...
    isChecked() {
      const layerId =
        this.searchResult.events?.onClick?.activateLayerInTopicTree?.layerId ||
        this.searchResult.events?.onClick?.addLayerToTopicTree?.layerId;

      if (
        this.visibleLayerConfigs?.find((layer) => layer.id === layerId) !==
        undefined
      ) {
        return true;
      }
      ...
      return (
        typeof this.selectedSearchResults.find(
          (item) => this.searchResult.id === item.id,
        ) !== "undefined"
      );
    },
     ...
     */
    actions() {
      return this.searchResult.events.buttons
        ? this.searchResult.events.buttons
        : [];
    },
  },
  methods: {
    ...
    addOrRemoveLayer() {
      const actionArgs =
        this.searchResult.events?.onClick?.activateLayerInTopicTree ||
        this.searchResult.events?.onClick?.addLayerToTopicTree;
...
<template lang="html">
  ...
  >
    ...
    >
      <span
        :id="
          'search-bar-result-list-topic-tree-item-checkbox-' + searchResult.id
        "
        :class="[
      ...
      <span
        :class="[
          ...
          'mt-0 d-flex flex-column align-self-start',
          isChecked ? 'bold' : '',
        ]"
        :for="
          'search-bar-result-list-topic-tree-item-checkbox-' + searchResult.id
...
.search-bar-result-list-topic-tree-item-title,
.search-bar-result-list-topic-tree-item-checkbox {
  &:hover {

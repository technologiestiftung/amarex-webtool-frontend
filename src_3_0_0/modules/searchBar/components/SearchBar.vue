...
export default {
  ...
  computed: {
    ...
    searchResultsWithUniqueCategories() {
      if (
        this.searchInterfaceInstances.every(
          (searchInterfaceInstance) =>
            searchInterfaceInstance.searchState !== "running",
        )
      ) {
        const categories = [
          ...new Set(
            this.searchResults.map((searchResult) => searchResult.category),
          ),
        ];

        categories.forEach((category) => {
          const searchResultsByCategory = this.searchResults.filter(
              (searchResult) => searchResult.category === category,
            ),
            searchInterfaceIds = [
              ...new Set(
                searchResultsByCategory.map(
                  (searchResult) => searchResult.searchInterfaceId,
                ),
              ),
            ];
    ...
    limitedSortedSearchResults() {
      ...
      this.searchInterfaceInstances.forEach((searchInterfaceInstance) => {
        for (const [index, value] of Object.entries(
          this.searchResultsWithUniqueCategories,
        )) {
          if (
            value.searchInterfaceId ===
            searchInterfaceInstance.searchInterfaceId
          ) {
            results[value.category + "Count"] =
              results[value.category + "Count"] === undefined
                ? 1
                : ++results[value.category + "Count"];

            if (
              results.availableCategories.includes(value.category) === false
            ) {
              results.availableCategories.push(value.category);
              results.categoryProvider[value.category] =
                value.searchInterfaceId;
            }
            ...
            if (
              results[value.category + "Count"] <= this.suggestionListLength
            ) {
              results[index] = value;
  ...
  watch: {
    ...
    currentComponentSide: {
      handler(newVal) {
        if (
          newVal === "root" ||
          (newVal === "layerSelection" &&
            this.addLayerButtonSearchActive === true)
        ) {
  ...
  mounted() {
    ...
    this.setCurrentSide(
      this.portalConfig?.mainMenu?.searchBar !== undefined
        ? "mainMenu"
        : "secondaryMenu",
    );
    this.currentComponentSide = this.menuCurrentComponent(
      this.currentSide,
    ).type;
    ...
    zoomToAndMarkSearchResult(searchInputValue) {
      if (searchInputValue !== undefined) {
        ...
        this.searchResults.forEach((searchResult) => {
          if (
            searchResult.category.startsWith("Adresse") ||
            searchResult.category.startsWith("Straße")
          ) {
            if (
              searchInputValue.toLowerCase() === searchResult.name.toLowerCase()
            ) {

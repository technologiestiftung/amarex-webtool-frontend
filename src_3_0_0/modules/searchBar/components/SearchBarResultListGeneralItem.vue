...
export default {
  ...
  methods: {
    ...mapActions("Modules/SearchBar", ["activateActions"]),
  },
};
</script>
  ...
  <div :id="'search-bar-result-list-general-item' + searchResult.id">
    <div class="d-flex flex-row bd-highlight bold">
      <button
        ...
        :aria-label="
          searchResult.toolTip ? searchResult.toolTip : searchResult.name
        "
        @click="
          activateActions({ searchResult: searchResult, actionType: 'onClick' })
        "
        @keydown.enter="
          activateActions({ searchResult: searchResult, actionType: 'onClick' })
        "
        @mouseover="
          activateActions({ searchResult: searchResult, actionType: 'onHover' })
        "
        @focus="
          activateActions({ searchResult: searchResult, actionType: 'onHover' })
        "

...
export default {
  ...
  methods: {
    ...
    prepareShowAllResults(categoryItem) {
      ...
      this.setShowAllResultsSearchInterfaceInstance(
        this.limitedSortedSearchResults.results.categoryProvider[categoryItem],
      );
      if (this.menuBySide(side)) {
        this.setNavigationCurrentComponentBySide({
          side: side,
          newComponent: {
            props: { name: "common:modules.searchBar.searchResults" },
            type: "searchbar",
          },
        });
        ...
        this.setNavigationHistoryBySide({
          ...
          newHistory: [
            ...
            {
              type: "searchBar",
              props: { name: "modules.searchBar.searchBar" },
            },
            {
              type: "searchBar",
              props: { name: "modules.searchBar.searchResultList" },
            },
          ],
        });
      }
      ...
      this.currentShowAllList =
        this.limitedSortedSearchResults.currentShowAllList.filter((value) => {
...
<template lang="html">
  ...
  >
    <div
      v-for="categoryItem in limitedSortedSearchResults.results
        .availableCategories"
    ...
    >
      <h5
        ...
        :title="
          $t('common:modules.searchBar.searchResultsFrom') +
          limitedSortedSearchResults.results.categoryProvider[categoryItem] +
          '-' +
          $t('common:modules.searchBar.search')
        "
      >
        <img
        ...
        {{
          categoryItem +
          ": " +
          limitedSortedSearchResults.results[categoryItem + "Count"] +
          "    " +
          $t("common:modules.searchBar.searchResults")
        }}
      </h5>
      <div
        v-for="(item, index) in showAllResults === false
          ? limitedSortedSearchResults.results
          : limitedSortedSearchResults.currentShowAllList"
        :key="item.id + '-' + index"
      >
        <p
        ...
        >
          <SearchBarSuggestionListItem :search-suggestion="item" />
        </p>
      </div>

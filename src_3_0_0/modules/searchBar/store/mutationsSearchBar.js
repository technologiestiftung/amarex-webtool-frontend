...
export default {
  ...
  addMultipleSearchInterfaceIds(state) {
    const types = state.searchInterfaces.map(
        (searchInterface) => searchInterface.type,
      ),
      duplicates = [
        ...new Set(
          types.filter(
            (searchInterfaces, index) =>
              types.indexOf(searchInterfaces) !== index,
          ),
        ),
      ];
    let count = 0;
    ...
    duplicates.forEach((duplicate) => {
      state.searchInterfaces.forEach((searchInterface) => {
        if (searchInterface.type === duplicate) {
          searchInterface.searchInterfaceId =
            searchInterface.type + "_" + count++;
        }
      });
    });
  },

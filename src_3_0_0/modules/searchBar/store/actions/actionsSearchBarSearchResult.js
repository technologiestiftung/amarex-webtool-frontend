...
export default {
  ...
  addLayerToTopicTree: (
    { dispatch, rootGetters },
    { layerId, source, showInLayerTree = true, visibility = true },
  ) => {
        ...
        if (!added) {
          dispatch(
            "Alerting/addSingleAlert",
            {
              category: "error",
              content: i18next.t(
                "common:modules.searchBar.layerResultNotShown",
              ),
            },
            { root: true },
          );
  ...
  openGetFeatureInfo: ({ commit }, { feature, layer }) => {
    const gfiFeature = wmsGFIUtil.createGfiFeature(layer, "", feature);

    commit("Modules/GetFeatureInfo/setGfiFeatures", [gfiFeature], {
      root: true,
    });
  },
  ...
  setMarker: ({ dispatch, rootGetters }, { coordinates, feature, layer }) => {
    const numberCoordinates = coordinates?.map((coordinate) =>
      parseFloat(coordinate, 10),
    );

    if (layer && feature?.getGeometry().getType() === "MultiPolygon") {
      const highlightObject = {},
        highlightVectorRules =
          rootGetters["Modules/GetFeatureInfo/highlightVectorRules"],
        styleObject = highlightVectorRules
          ? highlightVectorRules
          : styleList.returnStyleObject("defaultMapMarkerPolygon"),
  ...
  showInTree: ({ commit, dispatch, rootGetters }, { layerId }) => {
    ...
    if (layerConfig) {
      commit(
        "Menu/setNavigationHistoryBySide",
        { side: "mainMenu", newHistory: [] },
        { root: true },
      );
      dispatch(
        "Menu/changeCurrentComponent",
        { type: "layerSelection", side: "mainMenu", props: {} },
        { root: true },
      );
      ...
      if (layerConfig) {
        ...
        dispatch(
          "Modules/LayerSelection/showLayer",
          { layerId },
          { root: true },
        );
      } else {
        console.warn(
          "Cannot show layer with id ",
          layerId,
          ": is not contained in services.json",
        );
      }
    }
  ...
  showLayerInfo: ({ commit, dispatch, rootGetters }, { layerId }) => {
    ...
    if (layerConfig) {
      dispatch("Modules/LayerInformation/startLayerInformation", layerConfig, {
        root: true,
      });
      commit("Modules/LayerSelection/setLayerInfoVisible", true, {
        root: true,
      });
    } else {
      console.warn(
        "Cannot show info for layer with id ",
        layerId,
        ": is not contained in services.json",
      );
    }
  ...
  startRouting: ({ dispatch, rootGetters }, { coordinates, name }) => {
    ...
    dispatch(
      "Menu/changeCurrentComponent",
      {
        type: "routing",
        side: menuSide,
        props: { name: i18next.t("common:modules.routing.name") },
      },
      { root: true },
    );
    ...
    dispatch(
      "Modules/Routing/setFirstWayPoint",
      { displayName: name, coordinates },
      { root: true },
    );
  },
  ...
  zoomToResult: ({ dispatch, getters }, { coordinates }) => {
    ...
    dispatch(
      "Maps/zoomToCoordinates",
      { center: numberCoordinates, zoom: getters.zoomLevel },
      { root: true },
    );
  },
};

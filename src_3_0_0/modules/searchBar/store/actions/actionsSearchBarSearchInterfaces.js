...
export default {
    ...
    state.searchInterfaces.forEach((searchInterface) => {
      ...
      if (searchInterfacesMapper[type]) {
        commit(
          ...
          new searchInterfacesMapper[type](searchInterface),
        );
      } else {
        console.warn(
          `The searchInterface: "${type}" hasn't been implemented yet`,
        );
      }
    });
  },
  ...
  search: ({ getters, commit, dispatch, state }, { searchInput }) => {
    ...
    if (getters.showAllResults) {
      currentSearchInterfaceInstances = state.searchInterfaceInstances.filter(
        (instance) =>
          instance.searchInterfaceId ===
          getters.showAllResultsSearchInterfaceInstance,
      );
    }
    ...
    currentSearchInterfaceInstances.forEach((instance) => {
      ...
      instance
        ...
        .catch((error) => {
          if (
            String(error) !== "AbortError: The user aborted a request." &&

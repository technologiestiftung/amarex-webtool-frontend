...
export default {
  ...
  updateSearchNavigation: ({ getters, commit, rootGetters }, side) => {
    ...
    if (
      getters.showAllResults === true &&
      side === getters.currentSide &&
      getters.currentActionEvent === ""
    ) {
      if (type !== "searchbar" && type !== "layerselection") {
        ...
        commit("setShowAllResults", false);
        commit(
          "Menu/setCurrentComponentPropsName",
          { side: side, name: "common:modules.searchBar.searchResultList" },
          { root: true },
        );
        commit(
          "Menu/setNavigationHistoryBySide",
          { side: side, newHistory: [{ type: "root", props: [] }] },
          { root: true },
        );
      }
    }
    ...
    if (
      getters.currentSearchInputValue !== "" &&
      getters.currentActionEvent !== ""
    ) {
  ...
  startLayerSelectionSearch: ({ dispatch, commit }, side) => {
    ...
    commit(
      "Menu/setCurrentComponent",
      { type: "layerSelection", side: side, props: [] },
      { root: true },
    );
    commit(
      "Menu/setCurrentComponentPropsName",
      { side: side, name: "common:modules.searchBar.searchResults" },
      { root: true },
    );
    commit(
      "Menu/setNavigationHistoryBySide",
      {
        ...
        newHistory: [
          ...
          {
            type: "layerSelection",
            props: { name: "common:modules.layerSelection.name" },
          },
          {
            type: "layerSelection",
            props: { name: "common:modules.layerSelection.name" },
  ...
  checkLayerSelectionSearchConfig: ({ commit, rootGetters, rootState }) => {
    ...
    if (
      rootGetters.showLayerAddButton === true &&
      searchBar &&
      searchBar?.active !== undefined
    ) {
      if (searchBar.searchInterfaceInstanceId || searchBar.searchCategory) {
        if (searchBar.searchCategory && searchBar?.searchInterfaceInstanceId) {
          commit(
            ...
            searchBar.searchInterfaceInstanceId,
          );
          commit("setShowAllResultsSearchCategory", searchBar.searchCategory);
        } else {
          console.warn(
            "Please check the searchBar configuration at tree.addLayerButton",
          );
        }
      }

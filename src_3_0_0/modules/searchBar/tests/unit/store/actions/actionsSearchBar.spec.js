...
describe("src_3_0_0/modules/searchBar/store/actions/actionsSearchBar.js", () => {
  let commit, dispatch, getters, rootGetters, rootState;
  ...
  describe("updateSearchNavigation", () => {
    it("it updates the search navigation", () => {
      ...
      expect(commit.secondCall.args[0]).to.eql(
        "Menu/setCurrentComponentPropsName",
      );
      expect(commit.secondCall.args[1]).to.deep.eql({
        side: "mainMenu",
        name: "common:modules.searchBar.searchResultList",
      });
      expect(commit.thirdCall.args[0]).to.eql(
        "Menu/setNavigationHistoryBySide",
      );
      expect(commit.thirdCall.args[1]).to.deep.eql({
        side: "mainMenu",
        newHistory: [{ type: "root", props: [] }],
      });
    });
    ...
    it("currentComponent is not searchbar, side is mainMenu: it switches to previous component", () => {
      ...
      expect(commit.secondCall.args[0]).to.eql(
        "Menu/switchToPreviousComponent",
      );
  ...
  describe("startLayerSelectionSearch", () => {
    it("it updates the search navigation", () => {
      ...
      expect(dispatch.firstCall.args[1]).to.be.deep.eql({
        name: "common:modules.searchBar.searchResultList",
        side: "mainMenu",
        type: "searchbar",
      });
      ...
      expect(commit.args[3][1]).to.deep.equal({
        side: "mainMenu",
        newHistory: [
          { type: "root", props: [] },
          {

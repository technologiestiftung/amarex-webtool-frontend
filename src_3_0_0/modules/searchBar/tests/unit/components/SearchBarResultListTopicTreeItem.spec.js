...
describe("src_3_0_0/modules/searchBar/components/SearchBarResultListTopicTreeItem.vue", () => {
  ...
  describe("render elements", () => {
    it("should render result list topic tree item and check actionButtons", () => {
      ...
      expect(
        wrapper
          .find(
            "#search-bar-result-list-topic-tree-item-BeidemNeuenKrahnStraße",
          )
          .exists(),
      ).to.be.true;
      expect(
        wrapper
          .find(
            "span#search-bar-result-list-topic-tree-item-checkbox-BeidemNeuenKrahnStraße",
          )
          .exists(),
      ).to.be.true;
      expect(
        wrapper
          .find("span.search-bar-result-list-topic-tree-item-label")
          .exists(),
      ).to.be.true;
      expect(
        wrapper
          .find("span.search-bar-result-list-topic-tree-item-label > span")
          .exists(),
      ).to.be.true;
      expect(
        wrapper
          .find("span.search-bar-result-list-topic-tree-item-label > span")
          .text(),
      ).to.equals("Bei dem Neuen Krahn");
      expect(wrapper.findAll("action-button-stub").length).to.be.equals(1);
      expect(
        wrapper.find("action-button-stub").attributes("actionname"),
      ).to.be.equals("activateLayerInTopicTree");
    });
  ...
  describe("addOrRemoveLayer", () => {
    ...
    it("should remove a layer from selectedSearchResults, action activateLayerInTopicTree", () => {
      ...
      expect(removeLayerFromTopicTreeSpy.firstCall.args[1]).to.deep.equals({
        layerId: "123",
      });

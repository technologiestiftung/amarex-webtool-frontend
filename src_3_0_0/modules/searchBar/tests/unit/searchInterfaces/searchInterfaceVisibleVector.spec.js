...
describe("src_3_0_0/modules/searchBar/searchInterfaces/searchInterfaceVisibleVector.js", () => {
  ...
  beforeEach(() => {
    ...
    featureMultiPolygon.getGeometry().getExtent = sinon
      .stub()
      .returns([0, 0, 500, 500]);
    feature1.setId("1");
    feature2.setId("2");
  ...
  describe("findMatchingFeatures", () => {
    describe("vectorLayer", () => {
      ...
      it("should return one matched feature", () => {
        const visibleVectorLayerConfigs = [
          ...
          searchInput = "hos",
          matchingFeatures = SearchInterface1.findMatchingFeatures(
    ...
    describe("clusterLayer", () => {
      ...
      it("should return one matched feature from cluster layer", () => {
        ...
        expect(
          SearchInterface1.findMatchingFeatures(
            visibleVectorLayerConfigs,
            searchInput,
          ),
        ).to.deep.equals([
  ...
  describe("normalizeLayerResult", () => {
    ...
    it("should normalize result", () => {
      ...
      expect(
        SearchInterface1.normalizeResult(feature1, layer1, searchField),
      ).to.deep.equals({
        events: {
          onClick: {
            openGetFeatureInfo: {
  ...
  describe("getAdditionalInfo", () => {
    it("should return the additional info field of the feature", () => {
      expect(SearchInterface1.getAdditionalInfo(feature1, layer1)).to.equals(
        "Example-Street 1",
      );
    });
  ...
  describe("createPossibleActions", () => {
    it("should create possible events from search result", () => {
      expect(
        SearchInterface1.createPossibleActions(feature1, layer1),
      ).to.deep.equals({
        openGetFeatureInfo: {
          feature: feature1,
          layer: layer1,
  ...
  describe("getCoordinates", () => {
    it("should return a random coordinate from MultiPolygon feature - center coordinates do not intersect features geometry", () => {
      const getRandomCoordinateSpy = sinon.spy(
        SearchInterfaceVisibleVector.prototype,
        "getRandomCoordinate",
      );
      let result = null;

      featureMultiPolygon.getGeometry().intersectsCoordinate = sinon
        .stub()
        .returns(false);
      featureMultiPolygon.getGeometry().getCoordinates = sinon
        .stub()
        .returns(coordinates);

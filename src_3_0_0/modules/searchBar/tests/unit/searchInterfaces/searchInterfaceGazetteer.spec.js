...
describe("src_3_0_0/modules/searchBar/searchInterfaces/searchInterfaceGazetteer.js", () => {
  ...
  describe("getTranslationByType", () => {
    it("returns the translation for type = addressAffixed", () => {
      expect(SearchInterface1.getTranslationByType("addressAffixed")).to.equal(
        "modules.searchBar.type.address",
      );
    });
    it("returns the translation for type = addressUnaffixed", () => {
      expect(
        SearchInterface1.getTranslationByType("addressUnaffixed"),
      ).to.equal("modules.searchBar.type.address");
    });
    it("returns the translation for type = district", () => {
      expect(SearchInterface1.getTranslationByType("district")).to.equal(
        "modules.searchBar.type.district",
      );
    });
    it("returns the translation for type = houseNumbersForStreet", () => {
      expect(
        SearchInterface1.getTranslationByType("houseNumbersForStreet"),
      ).to.equal("modules.searchBar.type.address");
    });
    it("returns the translation for type = parcel", () => {
      expect(SearchInterface1.getTranslationByType("parcel")).to.equal(
        "modules.searchBar.type.parcel",
      );
    });
    it("returns the translation for type = street", () => {
      expect(SearchInterface1.getTranslationByType("street")).to.equal(
        "modules.searchBar.type.street",
      );
    });
  });
  ...
  describe("normalizeResultEvents", () => {
    it("should normalize result events", () => {
      ...
      expect(
        SearchInterface1.normalizeResultEvents(resultEvents, searchResult),
      ).to.deep.equals({
        buttons: {
          startRouting: {
            coordinates: [10, 20],
  ...
  describe("createPossibleActions", () => {
    it("should create possible events from search result", () => {
      ...
      expect(
        SearchInterface1.createPossibleActions(searchResult),
      ).to.deep.equals({
        setMarker: {
          coordinates: [10, 20],
        },

...
describe("src_3_0_0/modules/searchBar/searchInterfaces/searchInterfaceOsmNominatim.js", () => {
    ...
    searchResults = [
      {
        ...
        class: "shop",
        display_name:
          "ABC, 1a, Pferdebade, Eckernfeld, Lehe, Stadtbezirk Bremerhaven-Nord, Bremerhaven, Bremen, 27580, Deutschland",
        extratags: {
          opening_hours:
            "Mo-We 09:30-19:30; Th,Fr 09:30-20:00; Sa 09:30-19:00; PH off",
  ...
  describe("createSearchUrl", () => {
    it("should create the search url with GET parameters", () => {
      ...
      expect(SearchInterface1.createSearchUrl(searchInput)).to.equals(
        "test.url?countrycodes=de&format=json&polygon=0&addressdetails=1&extratags=1&limit=50&q=abc",
      );
    });
  });
  ...
  describe("normalizeResults", () => {
    ...
    it("should not normalize the search result, if the class is not configured", () => {
      ...
      expect(SearchInterface1.normalizeResults(searchResults)).to.be.an("array")
        .that.is.empty;
    });
  ...
  describe("createDisplayName", () => {
    it("should create the display name for search result", () => {
      expect(SearchInterface1.createDisplayName(searchResults[0])).to.equals(
        "Bremerhaven, Pferdebade 1a",
      );
    });
  });
  ...
  describe("createToolTipName", () => {
    it("should create the tool tip name for search result", () => {
      ...
      expect(
        SearchInterface1.createToolTipName(searchResults[0], displayName),
      ).to.equals("Bremerhaven, Pferdebade 1a, 27580 Bremen (Lehe)");
    });
  });
  ...
  describe("createPossibleActions", () => {
    it("should create possible events from search result", () => {
      expect(
        SearchInterface1.createPossibleActions(searchResults[0]),
      ).to.deep.equals({
        setMarker: {
          coordinates: [1, 2],
        },
  ...
  describe("processCoordinatesForActions", () => {
    it("should create possible events from search result", () => {
      expect(
        SearchInterface1.processCoordinatesForActions(searchResults[0]),
      ).to.deep.equals([1, 2]);
    });
  });
});

...
describe("src_3_0_0/modules/searchBar/searchInterfaces/searchInterfaceElasticSearch.js", () => {
  ...
  describe("appendSearchStringToPayload", () => {
    it("should append string to searchStringAttribute", () => {
      ...
      expect(
        SearchInterface1.appendSearchStringToPayload(
          payload,
  ...
  describe("addIgnoreIdsToPayload", () => {
    ...
    it("add layerIDsToIgnore and metaIDsToIgnore to the payload, if configTree is defined and not empty", () => {
      ...
      expect(
        SearchInterface1.addIgnoreIdsToPayload(payload, configTree),
      ).deep.equals({
        id: "The_ID",
        params: {
          query_string: "sturm",
  ...
  describe("sendRequest", () => {
    it("should return promise after sending a request", async () => {
      ...
      expect(axiosStub.calledOnce).to.be.true;
      expect(axiosStub.firstCall.args[0]).to.equals(
        'https://geodienste.hamburg.de/?source_content_type=application/json&source={"id":"query","params":{"query_string":"fer"}}',
      );
  ...
  describe("getResultByPath", () => {
    it("should return the value from path for String mappingAttribute", () => {
      ...
      expect(
        SearchInterface1.getResultByPath(searchResult, mappingAttribute),
      ).to.deep.equals([10, 20]);
    });
    ...
    it("should return the value from path for String[] mappingAttribute", () => {
      const searchResult = {
        ...
        mappingAttribute = ["_source.name", "_source.datasets.md_name"];

      expect(
        SearchInterface1.getResultByPath(searchResult, mappingAttribute),
      ).to.deep.equals("name - md_name");
    });
  });
  ...
  describe("getResultByPathArray", () => {
    it("should return the value from path", () => {
      const searchResult = {
        ...
        mappingAttributes = ["_source.name", "_source.datasets.md_name"];

      expect(
        SearchInterface1.getResultByPath(searchResult, mappingAttributes),
      ).to.deep.equals("name - md_name");
    });
  });
});

...
describe("src_3_0_0/modules/searchBar/searchInterfaces/searchInterfaceLocationFinder.js", () => {
  ...
  describe("handleServerError", () => {
    it("should print console.error with info", () => {
      ...
      expect(error.firstCall.args[0]).to.equals(
        "modules.searchBar.locationFinder.errorMsg: modules.searchBar.locationFinder.serverError: The info",
      );
    });
    ...
    it("should print console.error without info", () => {
      ...
      expect(error.calledOnce).to.be.true;
      expect(error.firstCall.args[0]).to.equals(
        "modules.searchBar.locationFinder.errorMsg: modules.searchBar.locationFinder.serverError",
      );
    });
  });
  ...
  describe("showError", () => {
    ...
    it("should print console.error without message", () => {
      ...
      expect(error.calledOnce).to.be.true;
      expect(error.firstCall.args[0]).to.equals(
        "modules.searchBar.locationFinder.errorMsg: The status text",
      );
    });
  });
  ...
  describe("normalizeResults", () => {
    it("should normalize search results", () => {
      ...
      expect(
        SearchInterface1.normalizeResults(searchResults, epsgCode),
      ).to.deep.equals([
        {
          events: {
            onClick: {
  ...
  describe("normalizeResult", () => {
    it("should normalize search result", () => {
      ...
      expect(
        SearchInterface1.normalizeResult(searchResult, extendedData),
      ).to.deep.equals({
        events: {
          onClick: {
            setMarker: {
  ...
  describe("createPossibleActions", () => {
    it("should create possible events from search result", () => {
      ...
      expect(
        SearchInterface1.createPossibleActions(searchResult, extendedData),
      ).to.deep.equals({
        setMarker: {
          coordinates: [1, 2],
        },
  ...
  describe("processCoordinatesForActions", () => {
    it("should create possible events from search result", () => {
      ...
      expect(
        SearchInterface1.processCoordinatesForActions(
          searchResult,
          extendedData,
        ),
      ).to.deep.equals([1, 2]);
    ...
    it("should create possible events from search result with classDefinition", () => {
      ...
      ).to.deep.equals([1, 2, 1, 2, 1, 2, 1, 2, 1, 2]);
    });
  });
});

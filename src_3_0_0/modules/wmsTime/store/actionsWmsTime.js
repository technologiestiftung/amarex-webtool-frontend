...
export default {
  windowWidthChanged({ commit, dispatch, state, getters }) {
    ...
    if (!getters.minWidth && state.layerSwiper.active) {
      dispatch(
        "toggleSwiper",
        state.timeSlider.currentLayerId + state.layerAppendix,
      );
    }
  },
  /**
  ...
  watchVisibleLayerConfig({ commit, state, rootGetters }) {
    ...
    store.watch(
      (_, getters) => getters.visibleLayerConfigs,
      (layerConfig) => {
        if (!state.timeSlider.active) {
          ...
          let currentLayerConf = rootGetters.layerConfigById(
              state.timeSlider.currentLayerId,
            ),
            visLayerConf = layerConfig.find(
              (layerConf) => layerConf.id === currentLayerConf.id,
            );

          if (state.layerSwiper.targetLayer) {
            currentLayerConf = rootGetters.layerConfigById(
              state.layerSwiper.targetLayer.get("id"),
            );
            visLayerConf = layerConfig.find(
              (layerConf) => layerConf.id === currentLayerConf.id,
            );
  ...
  async toggleSwiper({ commit, state, dispatch }, id) {
    ...
    const secondId = id.endsWith(state.layerAppendix)
        ? id
    ...
    if (state.layerSwiper.active) {
      ...
      commit(
        "setLayerSwiperTargetLayer",
        layerCollection.getLayerById(secondId),
      );
    } else {
  ...
  moveSwiper({ state, commit, dispatch, getters }, event) {
    const position = getPosition(
      event,
      state.layerSwiper.valueX,
      getters.currentTimeSliderObject.keyboardMovement,
    );
  ...
  async updateMap({ state, rootGetters, dispatch }) {
    ...
    state.layerSwiper.targetLayer
      ?.getLayer()
      .once("prerender", (renderEvent) => dispatch("drawLayer", renderEvent));
    state.layerSwiper.targetLayer
      ?.getLayer()
      .once("postrender", ({ context }) => {
        context.restore();
      });

    state.layerSwiper.sourceLayer
      ?.getLayer()
      .once("prerender", (renderEvent) => dispatch("drawLayer", renderEvent));
    state.layerSwiper.sourceLayer
      ?.getLayer()
  ...
  drawLayer({ state, rootGetters }, renderEvent) {
    ...
    context.moveTo(
      ...getRenderPixel(
        renderEvent,
        isRightSided ? [state.layerSwiper.valueX, 0] : [0, 0],
      ),
    );
    context.lineTo(
      ...getRenderPixel(
        renderEvent,
        isRightSided ? [state.layerSwiper.valueX, mapSize[1]] : [0, mapSize[1]],
      ),
    );
    context.lineTo(
      ...getRenderPixel(
        renderEvent,
        isRightSided ? mapSize : [state.layerSwiper.valueX, mapSize[1]],
      ),
    );
    context.lineTo(
      ...getRenderPixel(
        renderEvent,
        isRightSided ? [mapSize[0], 0] : [state.layerSwiper.valueX, 0],
      ),
    );
    context.closePath();
    context.clip();

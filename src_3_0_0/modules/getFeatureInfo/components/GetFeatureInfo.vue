...
export default {
  ...
  computed: {
    ...mapGetters(["ignoredKeys", "visibleSubjectDataLayerConfigs"]),
    ...
    feature() {
      if (
        this.gfiFeatures !== null &&
        Array.isArray(this.gfiFeatures) &&
        this.gfiFeatures.length > 0
      ) {
    ...
    rightPager() {
      return (
        this.gfiFeatures.length > 1 &&
        this.pagerIndex < this.gfiFeatures.length - 1
      );
    },
  },
  watch: {
    ...
    visible(value) {
      const menuSides = ["mainMenu", "secondaryMenu"],
        otherSide = menuSides.find(
          (element) => element !== this.initialMenuSide,
        );
        ...
        if (oldVal.length !== newVal.length && this.feature) {
          if (newVal.filter((x) => !oldVal.includes(x)).length === 0) {
            ...
            oldValues.forEach((config) => {
              if (
                this.feature?.getLayerId() === config.id &&
        ...
        if (newFeatures?.length > 0) {
          if (oldFeatures !== null) {
            newFeatures.forEach((newFeature) => {
              if (
                !oldFeatures.find(
                  (oldFeat) => oldFeat.getId() === newFeature.getId(),
                )
              ) {
            ...
            oldFeatures.forEach((oldFeature) => {
              if (
                !newFeatures.find(
                  (newFeat) => newFeat.getId() === oldFeature.getId(),
                )
              ) {
                featuresChanged = true;
              }

...
export default {
  ...
  gfiFeaturesAtPixel: () => (clickPixel, clickCoordinate, mode) => {
    ...
    if (clickPixel && mode === "2D") {
      mapCollection.getMap("2D").forEachFeatureAtPixel(
        clickPixel,
        (feature, layer) => {
          if (
            layer?.getVisible() &&
    ...
    if (mode === "3D") {
      ...
      clickFeatures.forEach((clickFeature) => {
        if (clickFeature instanceof Cesium.Cesium3DTileFeature) {
          const gfiFeatures =
            gfiFeatureProvider.getGfiFeaturesByTileFeature(clickFeature);
          ...
          clickFeature?.primitive instanceof Cesium.Billboard &&

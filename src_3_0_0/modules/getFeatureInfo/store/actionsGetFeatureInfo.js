...
let globeEventHandler, colored3DTile, old3DTileColor;
...
export default {
  ...
  highlight3DTile({ getters, dispatch }) {
    ...
    if (getters.coloredHighlighting3D?.color !== undefined) {
      ...
      if (configuredColor instanceof Array) {
        highlightColor = Cesium.Color.fromBytes(
          configuredColor[0],
          configuredColor[1],
          configuredColor[2],
          configuredColor[3],
        ...
        console.warn(
          "The color for the 3D highlighting is not valid. Please check the config or documentation.",
        );
      }
    }
    globeEventHandler.setInputAction(function onLeftClick(click) {
  ...
  removeHighlight3DTile({ dispatch }) {
    dispatch("removeHighlightColor");
    if (
      globeEventHandler !== undefined &&
      globeEventHandler instanceof Cesium.ScreenSpaceEventHandler
    ) {
  ...
  collectGfiFeatures({ getters, commit, dispatch, rootGetters }) {
    const clickCoordinate = rootGetters["Maps/clickCoordinate"],
      ...
      gfiWmsLayerList = getVisibleWmsLayersAtResolution(
        resolution,
        rootGetters.visibleSubjectDataLayerConfigs,
      ).filter((layer) => {
        return layer.get("gfiAttributes") !== "ignore";
      });

    Promise.all(
      gfiWmsLayerList.map((layer) => {
        const gfiParams = {
      ...
      .then((gfiFeatures) => {
        ...
        let allGfiFeatures = gfiFeaturesAtPixel(
          clickPixel,
          clickCoordinate,
          mode,
        ).concat(...gfiFeatures);
        ...
        allGfiFeatures.sort((a, b) => {
          const zIndexA =
              rootGetters.layerConfigById(a.getLayerId())?.zIndex || 0,
      ...
      .catch((error) => {
        ...
        dispatch(
          ...
          i18next.t("common:modules.getFeatureInfo.errorMessage"),
          { root: true },
        );
      });
  },

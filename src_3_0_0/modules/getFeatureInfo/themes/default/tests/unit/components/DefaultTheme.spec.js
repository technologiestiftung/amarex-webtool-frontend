...
describe("src_3_0_0/modules/getFeatureInfo/themes/default/components/DefaultTheme.vue", () => {
  ...
  describe("template", () => {
    ...
    it("should always upper case the first letter in the table headers", () => {
      wrapper.findAll("th").forEach(function (th) {
        expect(th.text().charAt(0) === th.text().charAt(0).toUpperCase()).to.be
          .true;
      });
    });
    ...
    it("should render an a and img if imageAttribute is existst in feature.properties", () => {
      expect(wrapper.find(".gfi-theme-images > div:nth-child(2) > a").exists())
        .to.be.true;
      expect(
        wrapper.find(".gfi-theme-images > div:nth-child(2) > a > img").exists(),
      ).to.be.true;
    });
    ...
    it("should the img have the source of feature properties", () => {
      expect(
        wrapper
          .find(".gfi-theme-images > div:nth-child(2) > a > img")
          .classes(),
      ).includes("gfi-theme-images-image");
      expect(
        wrapper
          .find(".gfi-theme-images > div:nth-child(2) > a > img")
          .attributes().src,
      ).equals("https://test.png");
    });
    ...
    it("should show no attribute is available message if getMappedProperties is empty", () => {
      ...
      expect(wrapper1.find("td").text()).equals(
    ...
    it("should have beautifyKeys enabled by default", async () => {
      ...
      expect(wrapper.find(".table-hover .firstCol > span").text()).equals(
        "Test",
      );
    });
  ...
  describe("mappedPropertiesExists", () => {
    ...
    it("should return true if the given feature is an object an has a function mappedProperties", () => {
      expect(
        wrapper.vm.mappedPropertiesExists({
          getMappedProperties: () => {
            return true;
          },
        }),
  ...
  describe("hasMappedProperties", () => {
    it("should return false if feature has no mapped properties", () => {
      expect(
        wrapper.vm.hasMappedProperties({
          getMappedProperties: () => {
            return {};
          },
        }),
  ...
  describe("getMappedPropertiesOfFeature", () => {
    it("should return the properties of the given feature", () => {
      expect(
        wrapper.vm.getMappedPropertiesOfFeature({
          getMappedProperties: () => {
            return "test";
          },
        }),
    ...
    it("should return properties if feature has properties and when showObjectKeysParam is true", () => {
      ...
      ).to.be.an("object").and.not.to.be.empty;
    });
  });
});

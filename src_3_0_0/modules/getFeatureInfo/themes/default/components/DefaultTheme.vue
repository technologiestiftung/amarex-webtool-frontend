...
export default {
  ...
  created() {
    this.showFavoriteIcons = this.feature.getTheme()?.params?.showFavoriteIcons
      ? this.feature.getTheme().params.showFavoriteIcons
      : this.showFavoriteIcons;
  ...
  methods: {
    ...
    mappedPropertiesExists(feature) {
      return (
        ...
        feature !== null &&
        typeof feature.getMappedProperties === "function"
      );
    },
    ...
    getMappedPropertiesOfFeature(feature, showObjectKeysParam = false) {
      if (showObjectKeysParam === true) {
        const properties = getPropertiesWithFullKeys(
          feature.getMappedProperties(),
        );
    ...
    isSensorChart(value) {
      return (
        ...
        typeof value.staObject === "object" &&
        value.staObject !== null &&
    ...
    setMaxWidth: function (params) {
      if (this.mimeType !== "text/html") {
        const gfiThemeContainer =
          document.getElementsByClassName("gfi-theme-images")[0];

        if (
          typeof gfiThemeContainer?.style !== "object" ||
          gfiThemeContainer?.style === null
        ) {
          return;
        }
...
<template>
  <div
    :class="mimeType === 'text/html' ? 'gfi-theme-iframe' : 'gfi-theme-images'"
  >
    <div
      ...
      <tbody v-if="mappedPropertiesExists(feature)">
        <tr v-if="!hasMappedProperties(feature)">
          <td>
            {{
              $t(
                "common:modules.getFeatureInfo.themes.default.noAttributeAvailable",
              )
            }}
          </td>
        </tr>
        <tr
          v-for="(value, key) in getMappedPropertiesOfFeature(
            feature,
            showObjectKeysParam,
          )"
          v-else
        ...
        >
          ...
          >
            <span v-if="beautifyKeysParam">
              {{
                beautifyKey(
                  translateKeyWithPlausibilityCheck(key, (v) => $t(v)),
                )
              }}
            </span>
            <span v-else>

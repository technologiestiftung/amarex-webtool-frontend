...
export default {
  ...
  computed: {
    ...
    weekday: function () {
      return this.momentLocale
        .localeData()
        .weekdays(dayjs().add(this.weekdayIndex, "day"));
    },
  },
  watch: {
    show(show) {
  ...
  created() {
    ...
    this.hoverBackgroundColor =
      this.chartsParams?.hoverBackgroundColor || this.hoverBackgroundColor;
  ...
  methods: {
    ...
    calculateDataForOneWeekday() {
      const processedData = calculateWorkloadForOneWeekday(
        this.targetValue,
        this.processedHistoricalDataByWeekday[this.weekdayIndex],
      );
      ...
      this.titleText = [
        ...
        `${this.$t("common:modules.getFeatureInfo.themes.sensor.sensorBarChart.chartTitleAverage")} ` +
          this.$t(
            `common:modules.getFeatureInfo.themes.sensor.sensorBarChart.${this.periodUnit}`,
            { count: this.periodLength },
          ),
    ...
    createChart() {
      ...
      Chart.defaults.font.family =
        "'MasterPortalFont', 'Arial Narrow', 'Arial', 'sans-serif'";
    ...
    createChartTooltip(maxValue) {
      return {
        callbacks: {
          label: (tooltipItem) =>
            ((tooltipItem.raw / maxValue) * 100).toFixed(0) + "%",
          title: () => false,
        },
      };
    },
          ...
          ticks: {
            callback: (value) => {
              return value % 2
                ? ""
                : this.$t(
                    "common:modules.getFeatureInfo.themes.sensor.sensorBarChart.clock",
                    { value },
                  );
            },
          },
  ...
  <div v-if="show">
    <div class="sensor-button-container">
      <button
        ...
        :title="
          $t(
            'common:modules.getFeatureInfo.themes.sensor.sensorBarChart.previousWeekday',
          )
        "
      ...
      <button
        ...
        :title="
          $t(
            'common:modules.getFeatureInfo.themes.sensor.sensorBarChart.nextWeekday',
          )
        "

...
describe("src_3_0_0/modules/getFeatureInfo/themes/senor/components/SensorTheme.vue", () => {
  ...
  it("should return a date before 3 month + 1  week as buffer", () => {
    ...
    expect(wrapper.vm.createFilterDate(periodLength, periodUnit)).equals(
      dayjs()
        .subtract(periodLength, periodUnit)
        .subtract(1, "week")
  ...
  it("should return a filter for the dataStreamId", () => {
    expect(wrapper.vm.createFilterDataStream("1234")).equals(
      "@iot.id%20eq%201234",
    );
    expect(wrapper.vm.createFilterDataStream("1111 | 9999")).equals(
      "@iot.id%20eq%201111%20or%20@iot.id%20eq%209999",
    );
  });
  ...
  it("should render the header", () => {
    expect(wrapper.findAll("div > div > strong")[0].text()).equals(
      "common:modules.getFeatureInfo.themes.sensor.sensor.header.name: Name",
    );
    expect(wrapper.findAll("div > div > strong")[1].text()).equals(
      "common:modules.getFeatureInfo.themes.sensor.sensor.header.description: Beschreibung",
    );
    expect(wrapper.findAll("div > div > strong")[2].text()).equals(
      "common:modules.getFeatureInfo.themes.sensor.sensor.header.ownerThing: Eigentümer",
    );
  });

  it("should render a menulist (div > div > ul) with class 'nav nav-pills'", () => {
    expect(wrapper.find("div > div > ul").exists()).to.be.true;
    expect(wrapper.find("div > div > ul").classes()).to.includes(
      "nav",
      "nav-pills",
    );
  });
  ...
  it("should render the data tab in menulist with class active by start", () => {
    ...
    expect(wrapper.find("div > div > ul > li > a").classes("active")).to.be
      .true;
    expect(wrapper.find("div > div > ul > li > a").text()).equals(
      "common:modules.getFeatureInfo.themes.sensor.sensor.dataName",
    );
  });
  ...
  it("should render the four tabs in menulist for chart values configured as array", () => {
    ...
    expect(wrapper.findAll("div > div > ul > li > a")[0].text()).equals(
      "common:modules.getFeatureInfo.themes.sensor.sensor.dataName",
    );
    expect(wrapper.findAll("div > div > ul > li > a")[1].text()).equals(
      "available",
    );
    expect(wrapper.findAll("div > div > ul > li > a")[2].text()).equals(
      "charging",
    );
    expect(wrapper.findAll("div > div > ul > li > a")[3].text()).equals(
      "outoforder",
    );
  });
  ...
  it("should render the 4 tabs in menulist for chart values and data configured as object", () => {
    ...
    expect(wrapper1.findAll("div > div > ul > li > a")[0].text()).equals(
      "Daten",
    );
    expect(wrapper1.findAll("div > div > ul > li > a")[1].text()).equals(
      "Verfügbar",
    );
    expect(wrapper1.findAll("div > div > ul > li > a")[2].text()).equals(
      "Auslastung",
    );
    expect(wrapper1.findAll("div > div > ul > li > a")[3].text()).equals(
      "Außer Betrieb",
    );
  });
  ...
  it("should only data tab is active and the other tabs disabled by default", () => {
    expect(
      wrapper.findAll("div > div > ul > li")[0].find("a").classes("active"),
    ).to.be.true;
    expect(wrapper.findAll("div > div > ul > li")[0].classes("disabled")).to.be
      .false;
    expect(
      wrapper.findAll("div > div > ul > li")[1].find("a").classes("active"),
    ).to.be.false;
    expect(wrapper.findAll("div > div > ul > li")[1].classes("disabled")).to.be
      .true;
    expect(
      wrapper.findAll("div > div > ul > li")[2].find("a").classes("active"),
    ).to.be.false;
    expect(wrapper.findAll("div > div > ul > li")[2].classes("disabled")).to.be
      .true;
    expect(
      wrapper.findAll("div > div > ul > li")[3].find("a").classes("active"),
    ).to.be.false;
    expect(wrapper.findAll("div > div > ul > li")[3].classes("disabled")).to.be
      .true;
  });

...
describe("src_3_0_0/modules/getFeatureInfo/themes/senor/components/SensorThemeBarChart.vue", () => {
  ...
  it("should render a two buttons with two span for left and right side if show is true", () => {
    ...
    expect(wrapper.findAll("button")[0].classes()).includes(
      "leftButton",
      "kat",
      "btn",
    );
    expect(
      wrapper
        .findAll("button > span > i")[0]
        .classes()
        .includes("bi-chevron-left"),
    );
    expect(wrapper.findAll("button")[1].classes()).includes(
      "rightButton",
      "kat",
      "btn",
    );
    expect(
      wrapper
        .findAll("button > span > i")[1]
        .classes()
        .includes("bi-chevron-right"),
    );
  });
  ...
  it("should returns an object with scales for the charts ", () => {
    ...
    expect(result.x.ticks.callback()).equals(
  ...
  it("should show today by initial loading ", () => {
    expect(wrapper.find("div > div > span").text()).equals(
      dayjs().format("dddd"),
    );
  });

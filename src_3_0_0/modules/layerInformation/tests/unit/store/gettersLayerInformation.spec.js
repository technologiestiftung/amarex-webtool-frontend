...
describe("src_3_0_0/modules/layerInformation/store/gettersLayerInformation.js", () => {
  it("urlParams", () => {
    ...
    expect(urlParamsState.layerInfo.layername).to.be.equals(
      "Luftbilder DOP 20 (DOP 40 mit Umland)",
    );
    expect(urlParamsState.layerInfo.typ).to.be.equals("WMS");
    expect(urlParamsState.layerInfo.urlIsVisible).to.be.true;
    expect(decodeURIComponent(urlParamsState.layerInfo.cswUrl)).to.be.equals(
      "https://metaver.de/csw",
    );
    expect(decodeURIComponent(urlParamsState.layerInfo.legendURL)).to.be.equals(
      legendURL,
    );

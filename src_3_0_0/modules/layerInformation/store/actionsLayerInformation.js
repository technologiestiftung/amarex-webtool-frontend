...
export default {
  ...
  startLayerInformation({ commit, dispatch, rootGetters }, layerConf) {
    const name =
        layerConf?.datasets?.length > 0
          ? layerConf.datasets[0].md_name
          : layerConf.name,
      mdid =
        layerConf?.datasets?.length > 0 ? layerConf.datasets[0].md_id : null;

    if (rootGetters["Modules/Legend/layerInfoLegend"].id !== layerConf.id) {
      commit("Modules/Legend/setLayerInfoLegend", {}, { root: true });
      dispatch("Modules/Legend/createLegendForLayerInfo", layerConf.id, {
        root: true,
      });
    }
    ...
    dispatch(
      ...
      { type: "layerInformation", side: "mainMenu", props: { name } },
      { root: true },
    );
   ...
   * @returns {void}
   */
  getAbstractInfo: async function (
    ...
    // use default csw_url from rest-services.json if csw_url not stated in the specific service
      ...
      if (service === undefined) {
        console.warn(
          "Rest Service with the ID " +
            Config.cswId +
            " is not configured in rest-services.json!",
        );
      } else {
        metaURL = service.url;
      }
    ...
    if (typeof metadata === "undefined") {
      ...
      commit(
        "setAbstractText",
        i18next.t("common:modules.layerInformation.noMetadataLoaded"),
      );
      commit(
        "setNoMetadataLoaded",
        i18next.t("common:modules.layerInformation.noMetadataLoaded"),
      ...
      commit(
        "setDatePublication",
        metadata?.getPublicationDate() || metadata?.getCreationDate(),
      );
    }
  ...
  getCustomMetaData: function ({ commit }, payload) {
    const customMetadata = Object.entries(payload.attributes).map(
        ([key, value]) => {
          return {
            [key]: value
              .split(".")
              .reduce((o, i) => o[i], payload.metadataAsJson)
              .getValue(),
          };
        },
      ),
  ...
  setMetadataURL({ state, commit, rootGetters }, metaId) {
    ...
    if (
      typeof state.layerInfo.showDocUrl !== "undefined" &&
      state.layerInfo.showDocUrl !== null
    ) {
  ...
  setConfigParams({ commit }, config) {
    if (
      config.layerInformation !== undefined &&
      config.layerInformation.showUrlGlobal !== null
    ) {

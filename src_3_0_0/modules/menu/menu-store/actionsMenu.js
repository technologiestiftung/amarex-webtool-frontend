...
export default {
  ...
  changeCurrentComponent({ commit, dispatch, state }, { type, side, props }) {
    ...
    if (
      currentType !== type ||
      (currentType === "folder" && type === "folder") ||
      (currentType === "layerSelection" && type === "layerSelection")
    ) {
   ...
   * @returns {void}
   */
  changeCurrentMouseMapInteractionsComponent(
    { commit, rootGetters, state },
    { type, side },
  ) {
    if (
      type !== state.currentMouseMapInteractionsComponent &&
      rootGetters[
        `Modules/${changeCase.upperFirst(type)}/hasMouseMapInteractions`
      ]
    ) {
      const otherSide = side === "mainMenu" ? "secondaryMenu" : "mainMenu";

      if (
        state[otherSide].navigation.currentComponent.type ===
        state.currentMouseMapInteractionsComponent
      ) {
  ...
  clickedMenuElement(
    ...
    { name, path, side, type, properties },
  ) {
    if (type) {
      if (type === "customMenuElement") {
        ...
        if (properties.execute !== undefined) {
          dispatch(properties.execute.action, properties.execute.payload, {
            root: true,
          });
        }
        if (
          properties.htmlContent === undefined &&
          properties.pathToContent === undefined
        ) {
  ...
  navigateBack({ commit, dispatch, getters, state, rootGetters }, side) {
    ...
    nextTick(() => {
      if (
        getters.currentComponent(side).type ===
          state.currentMouseMapInteractionsComponent &&
        getters.currentComponent(side).type !== state.defaultComponent
      ) {
        dispatch("changeCurrentMouseMapInteractionsComponent", {
          type: state.defaultComponent,
          side,
        });
      }
      if (
      ...
      if (getters.currentComponent(side).type === "searchbar") {
        dispatch("Modules/SearchBar/updateSearchNavigation", side, {
          root: true,
        });
      }
  ...
  handleActionButtons(
    ...
    if (getters.currentComponent(side).type === "layerSelection") {
      ...
      if (rootGetters["Modules/SearchBar/searchInput"] !== "") {
        ...
        commit("Modules/SearchBar/setCurrentSearchInputValue", "", {
          root: true,
        });
    ...
    if (
      getters.navigationHistory(side)[1]?.type === "searchBar" &&
      getters.navigationHistory(side)[2]?.type === "searchBar"
    ) {
      dispatch("Modules/SearchBar/updateSearchNavigation", side, {
        root: true,
      });
      commit("Modules/SearchBar/setSearchInput", currentSearchInput, {
        root: true,
      });
    }
  },
  ...
  toggleMenu({ commit, rootGetters, state }, side) {
    if (side === "mainMenu") {
      ...
      commit("setExpandedBySide", {
        expanded: !state.secondaryMenu.expanded,
        side,
      });
    }
  },
  ...
  updateComponentState({ dispatch }, { type, attributes }) {
    if (this._actions[`Modules/${type}/urlParams`]) {
      dispatch(`Modules/${type}/urlParams`, JSON.parse(attributes), {
        root: true,
      });
    } else {

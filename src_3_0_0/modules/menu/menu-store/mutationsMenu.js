...
export default {
  ...
  setCurrentComponent(state, { type, side, props }) {
    state[side].navigation.history.push(
      state[side].navigation.currentComponent,
    );
  ...
  switchToPreviousComponent(state, side) {
    if (state[side].navigation.history.slice(-1)[0]?.type) {
      state[side].navigation.currentComponent = {
        type: state[side].navigation.history.slice(-1)[0].type,
        props: state[side].navigation.history.slice(-1)[0].props,
      };
      state[side].currentComponent =
        state[side].navigation.history.slice(-1)[0].type;
    }

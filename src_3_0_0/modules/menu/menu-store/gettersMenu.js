...
const menuGetters = {
  ...
  currentComponentName: (state) => (side) => {
    ...
    switch (currentComponent.type) {
      ...
      default:
        name =
          currentComponent.props !== undefined && currentComponent.props.name
            ? i18next.t(currentComponent.props.name)
  ...
  previousNavigationEntryText: (state) => (side) => {
    const previousEntry =
      state[side].navigation.history.length !== 0
        ? state[side].navigation.history.slice(-1)[0]
        : "";
    ...
    if (previousEntry !== "") {
      switch (previousEntry.type) {
        ...
        default: {
          previousEntryText =
            previousEntry.props !== undefined && previousEntry.props.name
              ? i18next.t(previousEntry.props.name)
              : previousEntry.type;
          break;
        }
      }
  ...
  section: (_, getters) => (path) => {
    if (path && getters[path[0]]) {
      ...
      if (section === badPathSymbol) {
        console.error(
          `Menu.getters.section: ${badPathSymbol.description} ${path}.`,
        );
        return null;
      }
  ...
  urlParams: (state, getters) => {
    const params = {
      ...
      mainAttributes = getters.getComponentAttributes(
        state.mainMenu.currentComponent,
      ),
      secondaryAttributes = getters.getComponentAttributes(
        state.secondaryMenu.currentComponent,
      );
  ...
  getComponentAttributes:
    (_, __, rootState, rootGetters) => (currentComponent) => {

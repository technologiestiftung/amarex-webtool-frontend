...
describe("src_3_0_0/modules/menu/MenuContainerBodyRootLogo.vue", () => {
  ...
  it("renders the logo and other props", () => {
    ...
    expect(
      wrapper.find("#mp-menu-logo-idAppendix").attributes("title"),
    ).to.be.equals(props.toolTip);
    expect(
      wrapper.find("#mp-menu-logo-idAppendix").attributes("href"),
    ).to.be.equals(props.link);
    expect(
      wrapper.find("#mp-menu-logo-idAppendix > img").attributes("src"),
    ).to.be.equals(props.logo);
    expect(
      wrapper.find("#mp-menu-logo-idAppendix > img").attributes("alt"),
    ).to.be.equals(props.text);
    expect(wrapper.find("#mp-menu-logo-idAppendix > h1").text()).to.be.equals(
      props.text,
    );
  });

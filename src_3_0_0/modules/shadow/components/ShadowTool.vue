...
export default {
  ...
  computed: {
    ...mapGetters("Modules/Shadow", ["isShadowEnabled", "shadowTime"]),
  ...
  methods: {
    ...mapMutations("Modules/Shadow", ["setShadowTime"]),
    updateShadow,
    updateCesiumTime,
     ...
     */
    createDate() {
      const year = this.shadowTime.year
          ? this.shadowTime.year
          : this.currentYear,
        checkedMonth =
          this.shadowTime?.month?.length === 1
            ? "0" + this.shadowTime.month
            : this.shadowTime.month,
        checkedDay =
          this.shadowTime?.day?.length === 1
            ? "0" + this.shadowTime.day
            : this.shadowTime.day,
        checkedHour =
          this.shadowTime?.hour?.length === 1
            ? "0" + this.shadowTime.hour
            : this.shadowTime.hour,
        checkedMinute =
          this.shadowTime?.minute?.length === 1
            ? "0" + this.shadowTime.minute
            : this.shadowTime.minute,
     ...
     */
    updateShadowTime() {
      this.displayedShadowTime = new Date(
        this.date + " " + this.showTime + ":00",
      );
    ...
    <div class="form-check form-switch mb-3 d-flex align-items-center">
      <SwitchInput
        ...
        :label="
          $t(
            shadowActivated
              ? 'common:modules.shadow.shadowDisplayOff'
              : 'common:modules.shadow.shadowDisplayOn',
          )
        "
      ...
      <div class="d-flex justify-content-between">
        ...
        <div id="dateSelector">
          <DatePicker
            ...
            :lang="
              $t('common:libraries.vue-datepicker-next.lang', {
                returnObjects: true,
              })
            "

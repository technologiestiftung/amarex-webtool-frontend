...
describe("src_3_0_0/modules/contact/js/httpClient", function () {
  describe("httpClient", function () {
    ...
    it("calls onSuccess parameter on success", function (done) {
      sinon
        .stub(axios, "post")
        .returns(Promise.resolve({ status: 200, data: { success: true } }));
    ...
    it("calls onError parameter on internal client error", function (done) {
      sinon
        .stub(axios, "post")
        .returns(Promise.reject("Internal Client Error"));
    ...
    it("calls onError parameter if response status is not 200", function (done) {
      sinon.stub(axios, "post").returns(Promise.resolve({ status: 500 }));
    ...
    it("calls axios.post in expected fashion", function () {
      sinon.stub(axios, "post").returns(Promise.resolve({ status: 200 }));

...
const { validForm, validMail, validMessage, validPhone, validUsername } =
  getters;
...
describe("src_3_0_0/modules/contact/store/gettersContact", function () {
  describe("validMessage", function () {
    it("confirms valid messages", function () {
      expect(
        validMessage({ message: String().padStart(minMessageLength, "x") }),
      ).to.be.true;
      expect(
        validMessage({ message: String().padStart(minMessageLength + 1, "x") }),
      ).to.be.true;
      expect(
        validMessage({ message: String().padStart(minMessageLength - 1, "x") }),
      ).to.be.false;
  ...
  describe("validUsername", function () {
    it("confirms valid usernames", function () {
      ...
      expect(
        validUsername({
          username: "Janice Keihanaikukauakahihulihe'ekahaunaele",
        }),
      ).to.be.true;
  ...
  describe("validMail", function () {
    it("confirms valid mails", function () {
      ...
      expect(validMail({ mail: "very.unusual.”@”.unusual.com@example.com" })).to
        .be.true;
      expect(
        validMail({
          mail: 'very.”(),:;<>[]”.VERY.”very@\\\\ "very”\\.unusual@strange.example.com',
        }),
      ).to.be.true;

...
const actions = {
  ...
  onSendSuccess: ({ state, commit, dispatch, rootGetters }, ticketId) => {
    ...
    if (closeAfterSend) {
      ...
      if (rootGetters[menuCurrentComponent]("mainMenu").type === state.type) {
        ...
        rootGetters[menuCurrentComponent]("secondaryMenu").type === state.type
      ) {
  ...
  send: ({ state, dispatch, getters, rootGetters }) => {
    ...
    if (!getters.validForm) {
      console.warn(
        "An error occurred sending an email: send with incorrect fields aborted",
      );
      dispatch(
        "showWarningAlert",
        "common:modules.contact.errorIncompleteDeclarations",
      );
      return;
    }
    ...
    if (!mailServiceUrl) {
      console.warn(
        `"An error occurred sending an e-mail: serviceId ${id} is unknown.`,
      );
    ...
    httpClientModule.httpClient(
      mailServiceUrl,
      {
        ...
        subject: createSubject(
          ticketId,
          state.subject ||
            i18next.t("common:modules.contact.mailSubject") +
              systemInfo.portalTitle,
        ),

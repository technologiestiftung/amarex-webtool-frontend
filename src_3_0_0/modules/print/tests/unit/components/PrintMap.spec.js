...
describe("src_3_0_0/modules/Print/components/PrintMap.vue", () => {
  ...
  describe("PrintMap.vue methods", () => {
    it("method layoutChanged sets other print layout", () => {
      ...
      expect(store.state.Modules.Print.currentLayout).to.be.deep.equals(
        printLayout,
      );
  ...
  describe("template", () => {
    ...
    it("should have a downloads container", () => {
      expect(wrapper.find("#modules-print-downloads-container").exists()).to.be
        .true;
    });
    ...
    it("should show finish download file", () => {
      ...
      expect(wrapper.find("#modules-print-downloads-container").exists()).to.be
        .true;
      expect(wrapper.find(".modules-print-download-title-container").exists())
        .to.be.true;
      expect(wrapper.find(".modules-print-download-title").exists()).to.be.true;
      expect(wrapper.find(".modules-print-download-icon-container").exists()).to
        .be.true;
      expect(wrapper.find(".spinner-border").exists()).to.be.false;
      expect(wrapper.find(".modules-print-download-icon").exists()).to.be.true;
      expect(wrapper.find(".modules-print-download-button-container").exists())
        .to.be.true;
    ...
    it("should show loader download file", () => {
      ...
      expect(wrapper.find(".modules-print-download-button-container").exists())
        .to.be.true;
      expect(wrapper.find(".modules-print-download-button-active").exists()).to
        .be.false;
    });
  ...
  describe("returnScale", () => {
    it("should return an empty string if anything but a number is given", () => {
      expect(wrapper.vm.returnScale(undefined)).to.be.a("string").and.to.be
        .empty;
      expect(wrapper.vm.returnScale(null)).to.be.a("string").and.to.be.empty;
      expect(wrapper.vm.returnScale("string")).to.be.a("string").and.to.be
        .empty;
  ...
  describe("hasLayoutAttribute", () => {
    it("should return true if the layout has the attribute", () => {
      ...
      const hasAttr = wrapper.vm.hasLayoutAttribute(
        wrapper.vm.currentLayout,
        "title",
      );
  ...
  describe("getLayoutAttributes", () => {
    it("should return an object the correct attribute", () => {
      ...
      const attributes = wrapper.vm.getLayoutAttributes(
        wrapper.vm.currentLayout,
        ["title"],
      );

...
describe("src_3_0_0/modules/print/js/buildSpec", function () {
  ...
  before(() => {
    ...
    multiPointFeatures = createTestFeatures(
      "resources/testFeaturesSpassAmWasserMultiPoint.xml",
    );
    polygonFeatures = createTestFeatures(
      "resources/testFeaturesNaturschutzPolygon.xml",
    );
    multiPolygonFeatures = createTestFeatures(
      "resources/testFeaturesBplanMultiPolygon.xml",
    );
    lineStringFeatures = createTestFeatures(
      "resources/testFeaturesVerkehrsnetzLineString.xml",
    );
    multiLineStringFeatures = createTestFeatures(
      "resources/testFeaturesVeloroutenMultiLineString.xml",
    );
  });
  ...
  describe("parseAddressToString", function () {
    ...
    it("should return parsed complete address", function () {
      ...
      expect(buildSpec.parseAddressToString(address)).to.equal(
        "Hufnerstraße 7\n 22305 Hamburg",
      );
    });
  ...
  describe("isOwnMetaRequest", function () {
    ...
    it("should return false if uniqueId is undefined", function () {
      expect(buildSpec.isOwnMetaRequest(["1234", "5678"], undefined)).to.be
        .false;
    });
  ...
  describe("prepareLegendAttributes", function () {
    ...
    it("should return prepared legend for a svg polygon style", function () {
      const legend = [
        {
          graphic:
            "data:image/svg+xml;charset=utf-8,<svg height='35' width='35' version='1.1' xmlns='http://www.w3.org/2000/svg'><polygon points='5,5 30,5 30,30 5,30' style='fill:rgb(10, 200, 0);fill-opacity:0.2;stroke:rgb(0, 0, 0);stroke-opacity:1;stroke-width:1;'/></svg>",
    ...
    it("should return prepared legend attributes with sldVersion for GetLegendGraphic-Requests", function () {
      ...
      expect(
        buildSpec.prepareLegendAttributes(legend, sldVersion),
      ).to.deep.equal([
        {
          legendType: "wmsGetLegendGraphic",
          geometryType: "",
  ...
  describe("prepareGfiAttributes", function () {
    it("should create gfi attributes array", function () {
      ...
      expect(
        buildSpec.prepareGfiAttributes(gfiAttributes)[0],
      ).to.deep.own.include({
        key: "attr1",
        value: "value1",
      });
      expect(
        buildSpec.prepareGfiAttributes(gfiAttributes)[1],
      ).to.deep.own.include({
        key: "attr2",
        value: "value2",
      });
      expect(
        buildSpec.prepareGfiAttributes(gfiAttributes)[2],
      ).to.deep.own.include({
        key: "attr3",
        value: "value3",
      });
    ...
    it("should create empty gfi attributes array for empty attributes", function () {
      expect(buildSpec.prepareGfiAttributes({})).to.be.an("array").that.is
        .empty;
    });
  ...
  describe("buildScale", function () {
    ...
    it('should create scale that is "1:undefined" for undefined input', function () {
      ...
      expect(buildSpec.defaults.attributes.scale).to.deep.include(
        "1:undefined",
      );
    });
  });
  ...
  describe("buildWmts", () => {
    ...
    it("should buildWmts", function () {
      ...
      expect(
        buildSpec.buildWmts(wmtsLayer, wmtsLayer.getSource()),
      ).to.deep.own.include({
        baseURL: "url",
        opacity: 1,
        type: "WMTS",
    ...
    const tileWmsLayer = new Tile({
      source: new TileWMS({
        ...
        tileGrid: new TileGrid({
          ...
          resolutions: [
            78271.51696401172, 305.7481131406708, 152.8740565703354,
            76.4370282851677, 2.3886571339114906,
          ],
          tileSize: [512, 512],
        }),
      }),
  ...
  describe("getFeatureStyle", function () {
    ...
    it("should return array with an ol-style", function () {
      expect(buildSpec.getFeatureStyle(pointFeatures[0], vectorLayer)).to.be.an(
        "array",
      );
      expect(
        buildSpec.getFeatureStyle(pointFeatures[0], vectorLayer)[0],
      ).to.be.an.instanceof(OlStyle);
    });
  });
  describe("getStyleAttributes", function () {
    ...
    it('should return "styleId" if styleList is not available', function () {
      expect(
        buildSpec.getStyleAttributes(vectorLayer, pointFeatures[0], false),
      ).to.eql(["styleId"]);
    });
  });
  ...
  describe("checkPolygon", function () {
      ...
      styles.forEach((aStyle) => {
        ...
        if (coordinates.length === 1) {
          if (coordinates[0].length === 4) {
            expect(corrected.getGeometry().getCoordinates()).to.deep.equals([
              [
                [0, 0],
                [0, 1],
                [1, 1],
    ...
    it("should convert point feature to JSON and remove all @ and . in key if it includes @Datastream", function () {
      ...
      expect(
        buildSpec.convertFeatureToGeoJson(testFeature, style),
      ).to.deep.own.include({
        type: "Feature",
        properties: {
          _label: "veryCreativeLabelText",
  ...
  describe("getStylingRules", function () {
    ...
    it('should return "*" if styleAttribute is empty string', function () {
      expect(
        buildSpec.getStylingRules(vectorLayer, pointFeatures[0], [""]),
      ).to.equal("*");
    });
    it('should return "[styleId=\'undefined\']" if styleAttribute is "styleId"', function () {
      expect(
        buildSpec.getStylingRules(vectorLayer, pointFeatures[0], ["styleId"]),
      ).to.equal("[styleId='undefined']");
    });
    it('should return "[kh_nummer=\'20\']" if styleAttribute is "kh_nummer"', function () {
      expect(
        buildSpec.getStylingRules(vectorLayer, pointFeatures[0], ["kh_nummer"]),
      ).to.equal("[kh_nummer='20']");
    });
    it('should return "[styleId=\'*\']" if styleAttribute is "styleId"', function () {
      ...
      expect(
        buildSpec.getStylingRules(vectorLayer, clusteredFeature, ["styleId"]),
      ).to.equal("[styleId='*']");
    });
  });
  describe("addFeatureToGeoJsonList", function () {
      ...
      expect(list[0]).to.deep.own.include({
        ...
        properties: {
          ...
          krankenhausverzeichnis:
            "www.krankenhausverzeichnis.de|www.google.com|www.hamburg.de",
    ...
    it("should return array with multiPoint JSON", function () {
      ...
      expect(list[0]).to.deep.own.include({
        ...
        properties: {
          ...
          kurztext:
            "Das Strandbad Ostende verfügt über einen Sandstrand und eine große Liegewiese mit Spielgeräten für Kinder",
          _label: "veryCreativeLabelText",
        },
        geometry: {
          type: "MultiPoint",
          coordinates: [[573983.957, 5938583.644, 0]],
        },
      });
    });
    ...
    it("should return array with multiLineString JSON", function () {
      ...
      buildSpec.addFeatureToGeoJsonList(
      ...
      expect(list[0]).to.deep.own.include({
        ...
        properties: {
          ...
          kompensationsmassnahme_detail:
            "Bepflanzung mit Gehölzen und/oder Sträuchern",
          _label: "veryCreativeLabelText",
        },
        geometry: {
          type: "Polygon",
    ...
    it("should return array with multiPolygon JSON", function () {
      ...
      expect(list[0]).to.deep.own.include({
        ...
        properties: {
          ...
          goplink: undefined,
          hotlink:
            "http://daten-hamburg.de/infrastruktur_bauen_wohnen/bebauungsplaene/pdfs/bplan/Bahrenfeld18.pdf                                                                                                                                                                 ",
          hotlink_begr:
            "http://daten-hamburg.de/infrastruktur_bauen_wohnen/bebauungsplaene/pdfs/bplan_begr/Bahrenfeld18.pdf                                                                                                                                                            ",
          nachricht: undefined,
  ...
  describe("convertFeatureToGeoJson", function () {
    it("should convert point feature to JSON", function () {
      expect(
        buildSpec.convertFeatureToGeoJson(pointFeatures[0], style),
      ).to.deep.own.include({
        type: "Feature",
        properties: {
          anzahl_plaetze_teilstationaer: "43",
    ...
    it("should convert multiPoint feature to JSON", function () {
      expect(
        buildSpec.convertFeatureToGeoJson(multiPointFeatures[0], style),
      ).to.deep.own.include({
        type: "Feature",
        properties: {
          nummer: "1",
    ...
    it("should convert lineString feature to JSON", function () {
      expect(
        buildSpec.convertFeatureToGeoJson(lineStringFeatures[0], style),
      ).to.deep.own.include({
        type: "Feature",
        properties: {
          abs: "252500101 252500102",
    ...
    it("should convert multiLineString feature to JSON", function () {
      expect(
        buildSpec.convertFeatureToGeoJson(multiLineStringFeatures[0], style),
      ).to.deep.own.include({
        type: "Feature",
        properties: {
          RoutenTyp: "Radfernwege",
    ...
    it("should convert polygon feature to JSON", function () {
      expect(
        buildSpec.convertFeatureToGeoJson(polygonFeatures[0], style),
      ).to.deep.own.include({
        type: "Feature",
        properties: {
          vorhaben: "W-006 - BPlan Marienthal 22 (Husarenweg)",
    ...
    it("should convert multiPolygon feature to JSON", function () {
      expect(
        buildSpec.convertFeatureToGeoJson(multiPolygonFeatures[0], style),
      ).to.deep.own.include({
        type: "Feature",
        properties: {
          aenderung1: undefined,
  ...
  describe("buildPointStyleIcon", function () {
    ...
    it("should return  an style object", function () {
      ...
      expect(buildSpec.buildPointStyleIcon(style, vectorLayer)).to.be.an(
        "object",
      );
      expect(buildSpec.buildPointStyleIcon(style, vectorLayer).type).to.be.eql(
        "point",
      );
    });
  });
  describe("buildGraphicPath", function () {
    ...
    it("should return a url", function () {
      ...
      expect(buildSpec.buildGraphicPath(src)).to.eql(
        "https://test/lgv-config/img/img",
      );
    });
    it("should return a url", function () {
  ...
  describe("getFontFamily", function () {
    it("should return the font family", function () {
      expect(buildSpec.getFontFamily("bold 16px Helvetica", "16")).to.equals(
        "Helvetica",
      );
      expect(buildSpec.getFontFamily("bold 20px Sans Serif", "20")).to.equals(
        "Sans Serif",
      );
      expect(buildSpec.getFontFamily("20px Sans Serif", "20")).to.equals(
        "Sans Serif",
      );
    });
  ...
  describe("buildPolygonStyle", function () {
    ...
    it("should return a polygon style", function () {
      ...
      expect(buildSpec.buildPolygonStyle(style, vectorLayer).type).to.eql(
        "polygon",
      );
    });
  });
  describe("buildLineStringStyle", function () {
    ...
    it("should return a line string style", function () {
      ...
      buildSpec.buildStrokeStyle = sinon.spy();
      expect(buildSpec.buildLineStringStyle(style, vectorLayer).type).to.eql(
        "line",
      );
    });
  });

...
export default {
  ...
  retrieveCapabilites: function ({ state, dispatch, rootGetters, commit }) {
    ...
    if (state.serviceId !== "") {
      ...
      if (
        state.printService !== "plotservice" &&
        !serviceUrl.includes("/print/")
      ) {
      ...
      serviceUrl =
        serviceUrl + state.printAppId + "/" + state.printAppCapabilities;
   ...
   * @returns {void}
   */
  parseMapfishCapabilities: function (
   ...
   * @returns {void}
   */
  parsePlotserviceCapabilities: function (
    { state, commit, dispatch, rootGetters },
    response,
  ) {
    commit("setLayoutList", response.layouts);
    ...
    commit(
      "setScaleList",
      response.scales
        .map((scale) => parseInt(scale.value, 10))
        .sort((a, b) => a - b),
    );
    commit(
      "setFormatList",
      response.outputFormats.map((format) => format.name),
    );
    commit("setCurrentScale", rootGetters["Maps/scale"]);
    dispatch("togglePostrenderListener");
    if (state.isGfiAvailable) {
  ...
  chooseCurrentLayout: function ({ state, commit }, layouts) {
    const configuredLayout = layouts.find(
        (layout) => layout.name === state.currentLayoutName,
      ),
  ...
  getGfiForPrint: function ({ commit, rootGetters }) {
    if (
      rootGetters["Modules/GetFeatureInfo/currentFeature"] !== null &&
      typeof rootGetters["Modules/GetFeatureInfo/currentFeature"]
        ?.getMappedProperties === "function" &&
      typeof rootGetters["Modules/GetFeatureInfo/currentFeature"]?.getTitle ===
        "function"
    ) {
      commit("setGfiForPrint", [
        rootGetters[
          "Modules/GetFeatureInfo/currentFeature"
        ].getMappedProperties(),
        rootGetters["Modules/GetFeatureInfo/currentFeature"].getTitle(),
        rootGetters["Maps/clickCoordinate"],
      ]);
    } else {
      commit("setGfiForPrint", []);
    }
  ...
  setDpiList: function ({ state, commit }) {
    state.currentLayout.attributes.forEach((attribute, idx) => {
      if (attribute.name === "map") {
        commit(
          "setDpiList",
          state.currentLayout.attributes[idx].clientInfo?.dpiSuggestions || [],
        );
      }
    });
  },
   ...
   * @returns {void}
   */
  togglePostrenderListener: function (
    ...
    if (
      active &&
      state.layoutList.length !== 0 &&
      state.visibleLayerList.length >= 1
    ) {
      if (state.eventListener !== undefined) {
        dispatch(
          "Maps/unregisterListener",
      ...
      commit(
        "setEventListener",
        canvasLayer.on("postrender", (evt) => dispatch("createPrintMask", evt)),
      );
  ...
  setPrintLayers: function ({ dispatch, commit, getters }, scale) {
    ...
    hintInfo = i18next.t("common:modules.print.invisibleLayer", {
      scale: "1: " + thousandsSeparator(scale, "."),
    });
    hintInfo = hintInfo + "<br>" + getters.invisibleLayerNames;

    if (
      invisibleLayer.length &&
      hintInfo !== getters.hintInfo &&
      getters.showInvisibleLayerInfo
    ) {
      dispatch(
        "Alerting/addSingleAlert",
        {
  ...
  getOptimalScale: function ({ commit, state }, canvasOptions) {
    const mapWidth = canvasOptions.mapSize[0] * canvasOptions.resolution,
      ...
      scaleWidth =
        (mapWidth * state.INCHES_PER_METER * state.DOTS_PER_INCH) /
        canvasOptions.printMapSize[0],
      scaleHeight =
        (mapHeight * state.INCHES_PER_METER * state.DOTS_PER_INCH) /
        canvasOptions.printMapSize[1],
  ...
  drawMask: function (_, drawMaskOpt) {
    const mapSize = drawMaskOpt.frameState.size,
      ...
      ration =
        drawMaskOpt.context.canvas.width > mapSize[0] ? DEVICE_PIXEL_RATIO : 1,
  ...
  drawPrintPage: function ({ state }, canvasPrintOptions) {
    const ration =
        canvasPrintOptions.context.canvas.width > canvasPrintOptions.mapSize[0]
          ? DEVICE_PIXEL_RATIO
          : 1,
      center = [
        (canvasPrintOptions.mapSize[0] * ration) / 2,
        (canvasPrintOptions.mapSize[1] * ration) / 2,
      ],
      boundWidth =
        (((canvasPrintOptions.printMapSize[0] /
          state.DOTS_PER_INCH /
          state.INCHES_PER_METER) *
          canvasPrintOptions.scale) /
          canvasPrintOptions.resolution) *
        ration,
      boundHeight =
        (((canvasPrintOptions.printMapSize[1] /
          state.DOTS_PER_INCH /
          state.INCHES_PER_METER) *
          canvasPrintOptions.scale) /
          canvasPrintOptions.resolution) *
  ...
  getOptimalResolution: function ({ commit, state }, resolution) {
    const dotsPerMeter = state.INCHES_PER_METER * state.DOTS_PER_INCH,
      resolutionX =
        (resolution.printMapSize[0] * resolution.scale) /
        (dotsPerMeter * resolution.mapSize[0]),
      resolutiony =
        (resolution.printMapSize[1] * resolution.scale) /
        (dotsPerMeter * resolution.mapSize[1]);
  ...
  getPrintMapScales: function ({ state, dispatch, commit }) {
    if (state.printService !== "plotservice") {
      ...
      commit(
        "setScaleList",
        layoutMapInfo.scales.sort((a, b) => a - b),
      );
    }
  },
};

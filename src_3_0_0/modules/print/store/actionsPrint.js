...
const actions = {
  ...
  getMetaDataForPrint: async function ({ dispatch, rootGetters }, cswObject) {
    ...
    if (rootGetters.metadata.useProxy.includes(cswObj.cswUrl)) {
      metadata = await getCswRecordById.getRecordById(
        cswObj.cswUrl,
        cswObj.metaId,
      );
    } else {
    ...
    if (typeof metadata === "undefined") {
      dispatch(
        ...
        {
          category: "error",
          content: i18next.t("common:modules.print.errorMessage", {
  ...
  migratePayload: function ({ state, rootState }, payload) {
    const plotservicePayload = {},
      ...
      boundWidth =
        (((mapInfo[0] / state.DOTS_PER_INCH / state.INCHES_PER_METER) * scale) /
          resolution) *
        DEVICE_PIXEL_RATIO,
      boundHeight =
        (((mapInfo[1] / state.DOTS_PER_INCH / state.INCHES_PER_METER) * scale) /
          resolution) *
        DEVICE_PIXEL_RATIO,

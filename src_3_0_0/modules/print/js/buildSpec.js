...
const BuildSpecModel = {
  ...
  updateMetaData: function (layerName, parsedData) {
    const layers =
        this.defaults.attributes.legend &&
        this.defaults.attributes.legend.layers
          ? this.defaults.attributes.legend.layers
          : undefined,
    ...
    if (layer !== undefined) {
      ...
      layer.metaAddress = parsedData?.address
        ? this.parseAddressToString(parsedData.address)
        : "n.N.";
      ...
      for (const layer of layerList) {
        ...
        if (layer instanceof Group) {
          for (const childLayer of layer.getLayers().getArray()) {
            printLayers.push(
              await this.buildLayerType(childLayer, currentResolution, dpi),
            );
            visibleLayerIds.push(childLayer.get("id"));
          }
        } else {
          printLayers.push(
            await this.buildLayerType(layer, currentResolution, dpi),
          );
        }
    ...
    const featuresInExtent = layer.getSource().getFeaturesInExtent(extent),
      features = sortBy(featuresInExtent, function (feature) {
        if (
          ...
          typeof feature.getStyle().getZIndex === "function"
        ) {
   ...
   */
  buildLayerType: async function (
    layer,
    currentResolution,
    dpi,
    scaleDoesNotMatter = false,
  ) {
    const extent = store.getters["Maps/extent"],
      layerMinRes =
        typeof layer?.get === "function" ? layer.get("minResolution") : false,
      layerMaxRes =
        typeof layer?.get === "function" ? layer.get("maxResolution") : false,
      isInScaleRange = this.isInScaleRange(
        layerMinRes,
        layerMaxRes,
        currentResolution,
      );
    ...
    if (isInScaleRange || scaleDoesNotMatter) {
      ...
      if (layer instanceof VectorTileLayer) {
        ...
        returnLayer = await this.buildVectorTile(
          layer,
          currentResolution,
          maskExtent,
        );
        ...
        layer.get("name") === "importDrawLayer"
      ) {
  ...
  buildTileWms: function (layer, dpi) {
    ...
    mapObject = {
      ...
      customParams: {
        ...
        DPI:
          typeof dpi === "number" ? dpi : store.state.Modules.Print.dpiForPdf,
      },
    };
    ...
    if (!source.getParams().SINGLETILE) {
      mapObject.tileSize = [
        source.getParams().WIDTH,
        source.getParams().HEIGHT,
      ];
    }
    if (
      Object.prototype.hasOwnProperty.call(source.getParams(), "SLD_BODY") &&
      source.getParams().SLD_BODY !== undefined
    ) {
  ...
  buildImageWms: function (layer, dpi) {
    ...
    if (source instanceof StaticImageSource) {
      ...
      mapObject.styles = source.getParams().STYLES
        ? source.getParams().STYLES.split(",")
        : undefined;
  ...
  buildStyle: function (layer, features, geojsonList, extent) {
    const mapfishStyleObject = {
      ...
      featuresInExtent = layer.getSource()
    ...
    features.forEach((feature) => {
      const foundFeature = featuresInExtent.find(
          (featureInExtent) => featureInExtent.ol_uid === feature.ol_uid,
        ),
      ...
      styles.forEach((style, index) => {
        if (style !== null) {
          const styleObjectFromStyleList = styleList.returnStyleObject(
              layer.get("styleId"),
            ),
            styleFromStyleList = styleObjectFromStyleList
              ? createStyle.getGeometryStyle(
                  feature,
                  styleObjectFromStyleList.rules,
                  false,
                  Config.wfsImgPath,
                )
              : undefined;
          ...
          styleAttributes.forEach((attribute) => {
            const singleFeature = clonedFeature.get("features")
              ? clonedFeature.get("features")[0]
              : clonedFeature;

            if (attribute.includes("Datastreams")) {
              clonedFeature.set(
                attribute,
                attribute === "default" && !singleFeature.get(attribute)
                  ? "style"
                  : `${singleFeature.getProperties().Datastreams[0].Observations[0].result}_${index}`,
              );
            } else if (style.type && style.type === "CIRCLESEGMENTS") {
              clonedFeature.setId(`${feature.ol_uid}__${index}`);
              clonedFeature.set(
                attribute,
                `${style.type}_${style.scalingAttribute.replace(" | ", "_")}_${index}`,
              );
              ...
              clonedFeature.set(
                ...
                attribute === "default" && !singleFeature.get(attribute)
                  ? "style"
                  : `${singleFeature.get(attribute)}_${index}`,
              );
            }
          ...
          if (
            ...
            style.getImage().getScale() > 0
          ) {
            const coords = clonedFeature.getGeometry().getCoordinates(),
              offsetStyle = styleObjectFromStyleList?.rules?.find(
                ({ style: { imageOffsetX, imageOffsetY } }) =>
                  imageOffsetX || imageOffsetY,
              )?.style,
              [posX, posY] = mapCollection
                .getMap("2D")
                .getPixelFromCoordinate(coords),
              [offsetX, offsetY] = [
                offsetStyle?.imageOffsetX ?? 0,
                offsetStyle?.imageOffsetY ?? 0,
              ],
              mapScaleFactor =
                store.state.Modules.Print.currentScale /
                store.state.Modules.Print.currentMapScale,
              transformedCoords = mapCollection
                .getMap("2D")
                .getCoordinateFromPixel([
                  posX - offsetX * mapScaleFactor,
                  posY - offsetY * mapScaleFactor,
                  0,
                ]);
          ...
          if (
            styleGeometryFunction !== null &&
            styleGeometryFunction !== undefined
          ) {
          ...
          stylingRules = this.getStylingRules(
            layer,
            clonedFeature,
            styleAttributes,
            style,
          ...
          if (
            styleFromStyleList !== undefined &&
            styleFromStyleList.attributes?.labelField &&
            styleFromStyleList.attributes?.labelField.length > 0
          ) {
          ...
          if (Array.isArray(stylingRulesSplit) && stylingRulesSplit.length) {
            this.unsetStringPropertiesOfFeature(
              clonedFeature,
              stylingRulesSplit.reduce(
                (accumulator, current) =>
          ...
          if (geometryType === "Point" || geometryType === "MultiPoint") {
            ...
            styleObject.symbolizers.push(
              this.buildLineStringStyle(style, layer),
            );
          }
          // label styling
  ...
  getStyleAttributes: function (layer, feature) {
    ...
    if (styleObject !== undefined) {
      ...
      if (layerModel.get("styleId")) {
        ...
        styleFields = featureRules?.[0]?.conditions
          ? Object.keys(featureRules[0].conditions.properties)
          : ["default"];
      } else {
  ...
  getStyleObject(layer, layerId) {
    ...
    if (typeof layerModel?.get === "function") {
      if (layerModel.get("typ") === "GROUP") {
        foundChild = layerModel
          .get("children")
          .find((child) => child.id === layerId);
        if (foundChild) {
  ...
  getStylingRules: function (
    ...
    const styleAttr = feature.get("styleId") ? "styleId" : styleAttributes,
      styleObjectFromStyleList = styleList.returnStyleObject(
        layer.get("styleId"),
      ),
      styleFromStyleList = styleObjectFromStyleList
        ? createStyle.getGeometryStyle(
    ...
    if (styleAttr.length === 1 && styleAttr[0] === "") {
      if (feature.get("features") && feature.get("features").length === 1) {
        ...
        if (
          style
            .getImage()
            .getSrc()
            .indexOf("data:image/svg+xml;charset=utf-8") === 0
        ) {
        ...
        singleFeature.set(
          singleFeature.getId(),
          String(feature.get("features").length),
        );
        return (
          "[" +
          singleFeature.getId() +
      ...
      if (feature.get("features") !== undefined) {
        if (style !== undefined && style.getText().getText() !== undefined) {
          feature.set(
            "sensorClusterStyle",
            feature.get("features")[0].ol_uid +
              "_" +
              String(style.getText().getText()),
          );
          return (
            "[sensorClusterStyle='" +
            feature.get("features")[0].ol_uid +
    ...
    if (feature.get("features") !== undefined) {
      if (
        (style !== undefined && style?.getText()?.getText() !== undefined) ||
        feature.get("features").length > 1
      ) {
        const value =
          feature.get("features")[0].get(styleAttr[0]) + "_" + style !==
            undefined && style.getText()?.getText() !== undefined
      ...
      return (
        styleAttr
          .reduce((acc, curr) => {
            const value =
              feature.get("features")[0].get(curr) === undefined
                ? "*"
                : feature.get("features")[0].get(curr);
      ...
      return (
        styleAttr
          .reduce(
            (acc, curr) =>
              acc +
              `${curr}='${feature.get(curr)}' AND ${labelField}='${feature.get(labelField)}',`,
            "[",
          )
          .slice(0, -1) + "]"
      );
    ...
    if (styleAttr instanceof Array) {
      return (
        styleAttr
          .reduce((acc, curr) => acc + `${curr}='${feature.get(curr)}',`, "[")
          .slice(0, -1) + "]"
      );
    }
  ...
  addFeatureToGeoJsonList: function (feature, geojsonList, style) {
    ...
    if (feature.get("features") && feature.get("features").length === 1) {
      feature.get("features").forEach((clusteredFeature) => {
        ...
        convertedFeature = this.convertFeatureToGeoJson(
          clusteredFeature,
          style,
        );
  ...
  convertFeatureToGeoJson: function (feature, style) {
    ...
    Object.keys(clonedFeature.getProperties()).forEach((property) => {
      if (
        (isObject(clonedFeature.get(property)) &&
          !(clonedFeature.get(property) instanceof Geometry)) ||
        Array.isArray(clonedFeature.get(property))
      ) {
  ...
  buildPointStyle: function (style, layer) {
    if (style.getImage()?.constructor?.name === "CircleStyle") {
      ...
      style.getImage()?.constructor?.name === "Icon" &&
      style.getImage().getScale() > 0
    ) {
  ...
  buildStrokeStyle: function (style, obj) {
    ...
    if (
      typeof style.getWidth === "function" &&
      style.getWidth() !== undefined
    ) {
    ...
    if (typeof style.getLineDash === "function" && style.getLineDash()) {
      ...
      if (style.getLineDash().length > 1) {
        ...
        obj.strokeDashstyle =
          style.getLineDash()[0] + " " + style.getLineDash()[0];
      }
  ...
  buildPointStyleIcon: function (style, layer) {
    return {
      type: "point",
      graphicWidth: style.getSize()[0]
        ? style.getSize()[0] * style.getScale()
        : 60,
      graphicHeight: style.getSize()[1]
        ? style.getSize()[1] * style.getScale()
        : 60,
  ...
  getImageName: function (imageSrc) {
    ...
    return imageSrc.indexOf("/") !== -1
      ? imageSrc.substring(start + 1)
      : "/" + imageSrc;
  },
  /**
  ...
  buildTextStyle: function (style) {
    ...
    return {
      ...
      labelOutlineColor: stroke
        ? convertColor(stroke.getColor(), "hex")
        : undefined,
  ...
  getFontFamily: function (fontDef, fontSize) {
    const index = fontDef
      ? fontDef.indexOf(" ", fontDef.indexOf(fontSize))
      : -1;
    ...
    if (isGfiSelected) {
      if (gfiArray.length > 0) {
        ...
        gfiObject.layers.push({
          layerName: layerName.startsWith("common:")
            ? i18next.t(layerName)
            : layerName,
  ...
  buildLegend: function (
    ...
    if (isLegendSelected && legends.length > 0) {
      ...
      legends.forEach((legendObj) => {
        if (
          layerCollection.getLayerById(legendObj.id).get("children")?.length > 0
        ) {
          legendObj.id = layerCollection
            .getLayerById(legendObj.id)
            .get("children")[0].id;
        }
  ...
  prepareLegendAttributes: function (legend, sldVersion = "") {
    ...
    legend.forEach((legendPart) => {
      const legendObj = {
        ...
        graphic =
          typeof legendPart === "object" ? legendPart.graphic : legendPart;

      if (Array.isArray(graphic)) {
        graphic.forEach((graphicPart) => {
          if (graphicPart.indexOf("svg") !== -1) {
            legendObj.svg = decodeURIComponent(graphicPart).split(
        ...
        legendObj.imageUrl =
          graphic + (sldVersion ? "&sld_version=" + sldVersion : "");
      } else {
  ...
  getMetaData: function (layerName, getResponse, index) {
    const metadataLayer = layerCollection.find(
        (layer) => layer.attributes.name === layerName,
      ),
      metaId =
        metadataLayer.datasets && metadataLayer.datasets[0]
          ? metadataLayer.datasets[0].md_id
          : null,
  ...
  getFillColorFromSVG: function (svgString) {
    ...
    if (svgString.split(/fill:(.+)/)[1] || svgString.split("fill='")[1]) {
      color =
        svgString.split(/fill:(.+)/)[1]?.split(/;(.+)/)[0] ||
        svgString.split("fill='")[1]?.split("'")[0];
    }
    if (
      color.startsWith("rgb(") &&
      (svgString.split(/fill-opacity:(.+)/)[1]?.split(/;(.+)/)[0] ||
        svgString.split("fill-opacity='")[1])
    ) {
  ...
  getFillStrokeFromSVG: function (svgString, legendObj) {
    if (svgString.split(/stroke:(.+)/)[1] || svgString.split("stroke='")[1]) {
      legendObj.strokeColor =
        svgString.split(/stroke:(.+)/)[1]?.split(/;(.+)/)[0] ||
        svgString.split("stroke='")[1]?.split("'")[0];
    }
    if (
      legendObj.strokeColor?.startsWith("rgb(") &&
      (svgString.split(/stroke-opacity:(.+)/)[1]?.split(/;(.+)/)[0] ||
        svgString.split("stroke-opacity='")[1]?.split("'")[0])
    ) {
    ...
    if (
      svgString.split(/stroke-width:(.+)/)[1] ||
      svgString.split("stroke-width='")[1]
    ) {
      legendObj.strokeWidth =
        svgString.split(/stroke-width:(.+)/)[1]?.split(/;(.+)/)[0] ||
        svgString.split("stroke-width='")[1]?.split("'")[0];
    }
    if (
      svgString.split(/stroke-dasharray:(.+)/)[1] &&
      !svgString.split(/stroke-dasharray:(.+)/)[1]?.startsWith(";")
    ) {

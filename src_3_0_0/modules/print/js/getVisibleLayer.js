...
function checkLayersInResolution(visibleLayerList) {
  const resoByMaxScale = store.getters["Maps/getResolutionByScale"](
      store.getters["Modules/Print/currentScale"],
      "max",
    ),
  ...
  visibleLayerList.forEach((layer) => {
    if (
      ...
      const layerName = Array.isArray(layer.get("name"))
        ? layer.get("name")[0]
        : layer.get("name");
...
function sortVisibleLayerListByZindex(visibleLayerList) {
  const visibleLayerListWithZIndex = visibleLayerList.filter((layer) => {
    ...
    visibleLayerListWithoutZIndex = differenceJS(
      visibleLayerList,
      visibleLayerListWithZIndex,
    );

  visibleLayerListWithoutZIndex.push(
    sortBy(visibleLayerListWithZIndex, (layer) => layer.getZIndex()),
  );
  store.dispatch(
    "Modules/Print/setVisibleLayerList",
    [].concat(...visibleLayerListWithoutZIndex),
  );
}

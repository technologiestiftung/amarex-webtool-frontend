  ...
  methods: {
    ...
    async layoutChanged(value) {
      ...
      this.setCurrentLayout(
        this.layoutList.find((layout) => layout.name === value),
      );
     ...
     */
    print() {
      const currentPrintLength = this.fileDownloads.filter(
        (file) => file.finishState === false,
      ).length;

      if (currentPrintLength <= 10) {
        const index = this.fileDownloads.length,
          layoutAttributes = this.getLayoutAttributes(this.currentLayout, [
            "subtitle",
            "textField",
            "author",
            "overviewMap",
    ...
    isValid(value) {
      ...
      if (!valid) {
        document
          .getElementById("outputFileTitleWarning")
        ...
        if (this.hasLayoutAttribute(layout, name)) {
          if (name === "overviewMap") {
            layoutAttributes[name] = {
              layers: [
                BuildSpec.buildTileWms(
                  layerCollection
                    .getLayerById(this.getOverviewmapLayerId())
                    .getLayer(),
                  this.dpiForPdf,
                ),
              ],
            ...
            this.visibleLayerList.forEach((layer) => {
              const foundRawLayer = rawLayerList.getLayerWhere({
                id: layer.get("id"),
              });

              if (foundRawLayer) {
                layoutAttributes[name].push(
                  foundRawLayer?.datasets[0].show_doc_url +
                    foundRawLayer.datasets[0].md_id,
                );
              }
            });
    ...
    getOverviewmapLayerId() {
      const defaultLayerId = this.visibleLayerList[0].values_.id,
        visibleLayerId = this.visibleLayerList
          .filter((id) => id.values_.id === this.overviewmapLayerId)
          .map((val) => val.values_.id)
          .toString();
...
<template lang="html">
  <div id="modules-print">
    ...
    >
      ...
      <div v-if="hasLayoutAttribute(currentLayout, 'subtitle')">
        <InputText
          :id="subtitle"
      ...
      >
        <div class="form-floating mb-3">
          ...
          />
          <label for="textField">{{
            $t("common:modules.print.textFieldLabel")
          }}</label>
        </div>
      </div>
      <div v-if="hasLayoutAttribute(currentLayout, 'author')">
        <InputText
          :id="author"
      ...
      <div v-if="isLegendAvailable">
        <div class="form-check form-switch mb-3 d-flex align-items-center">
          <SwitchInput
            :id="'printLegend'"
            :aria="$t('common:modules.print.withLegendLabel')"
            :interaction="
              ($event) => setIsLegendSelected($event.target.checked)
            "
      ...
      <div v-if="is3d">
        <div class="form-check form-switch mb-3 d-flex align-items-center">
          <SwitchInput
            ...
            :interaction="
              ($event) =>
                setIsIncreased3DResolutionSelected($event.target.checked)
            "
    ...
    <div id="modules-print-downloads-container">
      ...
      >
        ...
        <div class="col-2 modules-print-download-icon-container">
          <SpinnerItem v-if="!file.finishState" />
          <div
            v-else
        ...
        <div class="col-7 modules-print-download-button-container">
          <FlatButton
            ...
            :interaction="
              ($event) =>
                download($event.target, file.downloadUrl, file.filename)
            "

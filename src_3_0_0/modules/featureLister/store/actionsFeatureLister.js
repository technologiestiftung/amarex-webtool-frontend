...
export default {
  ...
  clickOnFeature(
    ...
    if (
      featureIndex !== "" &&
      featureIndex >= 0 &&
      featureIndex <= state.shownFeatures
    ) {
      const feature = getters.selectedFeature(featureIndex),
        featureGeometry = feature.getGeometry(),
        styleObj =
          state.layer.geometryType.toLowerCase().indexOf("polygon") > -1
        ...
        if (featureGeometry && typeof featureGeometry.getType === "function") {
          if (featureGeometry.getType() === "Point") {
            dispatch(
              ...
              { center: featureGeometry.getCoordinates() },
              { root: true },
            );
          } else {
            dispatch(
              "Maps/zoomToCoordinates",
              { center: getCenter(featureGeometry.getExtent()) },
              { root: true },
            );
          }
      ...
      if (rootGetters.treeHighlightedFeatures?.active) {
        createLayerAddToTreeModule.createLayerAddToTree(
          state.layer.id,
          [feature],
  ...
  switchToList({ state, commit, dispatch }, layer) {
    ...
    if (layer) {
      ...
      commit(
        "setShownFeatures",
        state.gfiFeaturesOfLayer.length < state.maxFeatures
          ? state.gfiFeaturesOfLayer.length
          : state.maxFeatures,
      );
  ...
  showMore({ state, commit }) {
    const numberOfFeaturesToShow =
      state.shownFeatures < state.featureCount - 10
        ? state.shownFeatures + 10
        : state.featureCount;

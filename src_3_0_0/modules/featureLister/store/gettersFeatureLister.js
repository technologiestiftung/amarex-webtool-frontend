...
const getters = {
  ...
  selectedFeature: (state) => (featureIndex) => {
    ...
    return (
      layer.getLayerSource().getFeatureById(featureId) ||
      layer
        .getLayerSource()
        .getFeatures()
        .find((feat) =>
          feat.get("features")?.find((feat_) => feat_.getId() === featureId),
        )
    );
  },
  /**
  ...
  featureProperties: (state) => {
    return state.gfiFeaturesOfLayer
      ...
      .map((properties) =>
        state.headers.map(({ key }) => properties[key] ?? ""),
      );
  },
  /**
  ...
  featureDetails: (state, _, __, rootGetters) => {
    ...
    return attributesToShow === "showAll"
      ? Object.entries(featureProperties).filter(
          ([key, value]) =>
            value && !rootGetters.ignoredKeys.includes(key.toUpperCase()),
        )
  ...
  headers: (state, _, __, rootGetters) => {
    const headers = Object.entries(
      state.gfiFeaturesOfLayer.reduce((acc, it) => {

...
describe("src_3_0_0/modules/featureLister/store/gettersFeatureLister", () => {
  ...
  describe("headers", () => {
    it("lists all used attributes", () => {
      ...
      expect(
        listOfHeaders.mapHeaders(headers(state, {}, {}, { ignoredKeys: [] })),
      ).to.deep.equal({
        generic: "Show Generic",
        alpha: "Show Alpha",
        beta: "Show Beta",
    ...
    it("shows all properties with showAll feature", () => {
      ...
      ).to.deep.equal(["generic", "alpha", "beta", "gamma", "delta"]);
    });
    it("header value as object, use name as value", () => {
      ...
      expect(
        listOfHeaders.mapHeaders(headers(state, {}, {}, { ignoredKeys: [] })),
      ).to.deep.equal({ generic: "Show Generic", alpha: "Name Von Alpha" });
    });
  });
  ...
  describe("featureDetails", () => {
    ...
    it("returns the exactly the attribute titles and values that are to show", () => {
      ...
      expect(
        mapFeatureDetails(featureDetails(state, {}, {}, { ignoredKeys: [] })),
      ).to.deep.equal({ "Show Generic": "Test", "Show Alpha": "ohne" });
    });
    it("returns all attribute values if showAll is set", () => {
      state.selectedFeatureIndex = 0;
      expect(
        mapFeatureDetails(featureDetails(state, {}, {}, { ignoredKeys: [] })),
      ).to.deep.equal(gfiFeature1.getProperties());
    });
      ...
      expect(
        mapFeatureDetails(
          featureDetails(
            ...
            {},
            {},
            { ignoredKeys: ["ALPHA", "BETA", "GAMMA", "DELTA"] },
          ),
        ),
      ).to.deep.equal({ generic: "Hallo" });
    });
    it("ignores false-ish values", () => {
      state.selectedFeatureIndex = 2;
      expect(
        mapFeatureDetails(featureDetails(state, {}, {}, { ignoredKeys: [] })),
      ).to.deep.equal({ "Show Generic": "Test" });
    });
  });

  describe("featureProperties", () => {
    it("returns a nested array with equal length for each row", () => {
      state.headers = ["generic", "gamma"].map((it) => ({
        key: it,
        value: "The " + it,
      }));
      state.gfiFeaturesOfLayer = [gfiFeature2, gfiFeature3];
      expect(featureProperties(state)).to.deep.equal([
        ["Test", ""],
        ["Test", "Delta"],
      ]);
    });

...
describe("src_3_0_0/modules/featureLister/components/FeatureLister.vue", () => {
  ...
  let store, wrapper, rootGetters, removeHighlightFeatureSpy, features, layer;

  beforeEach(() => {
    FeatureLister.actions.switchTabTo = sinon.spy(
      FeatureLister.actions.switchTabTo,
    );
    FeatureLister.actions.addMouseEvents = sinon.spy(
      FeatureLister.actions.addMouseEvents,
    );
    FeatureLister.mutations.resetToThemeChooser = sinon.spy(
      FeatureLister.mutations.resetToThemeChooser,
    );
  ...
  it("renders list of 2 features", () => {
    store.dispatch(
      "Modules/FeatureLister/switchToList",
      { rootGetters },
      layer,
    );
    wrapper = shallowMount(FeatureListerComponent, {
    ...
    expect(wrapper.find("#module-feature-lister-feature-0").exists()).to.be
      .true;
    expect(wrapper.find("#module-feature-lister-feature-1").exists()).to.be
      .true;
    expect(store.state.Modules.FeatureLister.featureDetailView).to.be.false;

...
} from "../../../shared/js/utils/isPhoneNumber.js";
...
export default {
  ...
  mounted() {
    this.$nextTick(() => {
      layerCollection.getOlLayers().forEach(async (layer) => {
        if (
          (layer instanceof VectorLayer && layer.get("typ") === "WFS") ||
          layer.get("typ") === "GeoJSON"
        ) {
          ...
          this.visibleVectorLayers.push({
            name: layer.get("name"),
            id: layer.get("id"),
            geometryType: layerSource.getFeatures()[0]
              ? layerSource.getFeatures()[0].getGeometry().getType()
              : null,
          });
        }
      });
  ...
  methods: {
    ...
    ...mapMutations("Modules/FeatureLister", ["resetToThemeChooser"]),
    beautifyKey,
    isWebLink,
    isPhoneNumber,
    getPhoneNumberAsWebLink,
     ...
     */
    async sortItems() {
      const tableHeaders = await document.getElementsByClassName(
      ...
      try {
        if (tableHeaders?.length) {
          for (const th_elem of tableHeaders) {
            let asc = true;
            const index = Array.from(th_elem.parentNode.children).indexOf(
              th_elem,
            );
            ...
            th_elem.addEventListener("click", () => {
              ...
              arr.sort((a, b) => {
                ...
                if (
                  a.children[index] !== undefined &&
                  b.children[index] !== undefined
                ) {
...
<template lang="html">
  <div id="feature-lister">
    ...
    <template v-if="featureListView">
      ...
      >
        <div class="table-responsive feature-lister-list-table-container">
          <table
        ...
        <div class="panel-footer feature-lister-list-footer">
          <FlatButton
            ...
            :icon="'bi-plus'"
            :disabled="
              featureCount <= maxFeatures || shownFeatures === featureCount
            "
            :interaction="() => showMore()"
          />
          <p class="navbar-text feature-lister-list-message">
            {{
              $t("common:modules.featureLister.key", {
                shownFeatures,
                featureCount,
              })
            }}
    ...
    <template v-if="featureDetailView">
      ...
      >
        <span>
          {{ $t("common:modules.featureLister.detailsOfSelected") }}
        </span>
      </div>
      <div
        ...
        >
          ...
          <li class="list-group-item feature-lister-details-li">
            ...
            <p v-else-if="isPhoneNumber(feature[1])">
              <a :href="getPhoneNumberAsWebLink(feature[1])">{{
                feature[1]
              }}</a>
            </p>
            ...
            <p
              v-else-if="
                typeof feature[1] === 'string' && feature[1].includes(';')
              "
            >
            ...
            <p
              v-else-if="
                typeof feature[1] === 'string' && feature[1].includes('<br>')
              "
            >

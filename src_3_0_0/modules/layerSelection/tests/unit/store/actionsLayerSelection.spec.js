...
describe("src_3_0_0/modules/layerSelection/store/actionsLayerSelection", () => {
  ...
  describe("changeVisibility", () => {
    ...
    it("changeVisibility to false removes layer from layerTree", () => {
      ...
      changeVisibility(
        { dispatch, rootGetters },
        { layerId: "1", value: false },
      );
      expect(dispatch.calledTwice).to.be.true;
      expect(dispatch.secondCall.args[0]).to.be.equals(
        "replaceByIdInLayerConfig",
      );
  ...
  describe("actionsLayerSelection", () => {
    ...
    it("navigateBack inside layerSelection", () => {
      ...
      expect(commit.callCount).to.be.equals(3);
      expect(commit.firstCall.args[0]).to.be.equals(
        "reduceToPreviousLayerSelection",
      );
      expect(commit.secondCall.args[0]).to.be.equals(
    ...
    it("setNavigationByFolder", () => {
      const data = {
        ...
        collectDataByFolderStub = sinon
          .stub(collectDataByFolderModule, "collectDataByFolder")
          .returns(data);
      ...
      expect(commit.thirdCall.args[0]).to.be.equals(
        "setLastSubjectDataLayerConfs",
      );
      expect(commit.thirdCall.args[1]).to.be.equals(
        data.lastSubjectDataLayerConfs,
      );
    });
    describe("showLayer", () => {
      it("showLayer, layerConfig has parentId", () => {
        ...
        expect(dispatch.secondCall.args[1]).to.be.deep.equals({
          ...
          subjectDataLayerConfs: folder.elements,
          baselayerConfs: [],
        });
      ...
      it("showLayer, layerConfig has no parentId", () => {
        ...
        expect(dispatch.firstCall.args[1]).to.be.deep.equals({
          lastFolderName: "",
          subjectDataLayerConfs: [folder],
          baselayerConfs: [{ name: "baselayer" }],
        });
        expect(commit.calledTwice).to.be.true;
        expect(commit.firstCall.args[0]).to.be.equals("setHighlightLayerId");
        expect(commit.firstCall.args[1]).to.be.equals(layerId);

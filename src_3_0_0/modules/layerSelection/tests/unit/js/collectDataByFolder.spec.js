...
describe("src_3_0_0/modules/layerSelection/js/collectDataByFolder.js", () => {
  let rootGetters, layersWithFolder;
  ...
  describe("collectDataByFolder", () => {
    it("should collect data", () => {
      const folder2 = layersWithFolder[0].elements[0].elements[2],
        data = collectDataByFolderModule.collectDataByFolder(
          folder2,
          rootGetters,
        );
      ...
      expect(data.lastBaselayerConfs[0]).to.be.deep.equal([
        { name: "baselayer" },
      ]);
      ...
      expect(data.lastSubjectDataLayerConfs[0]).to.be.deep.equal([
        { name: "subjectlayer" },
      ]);
      expect(data.lastSubjectDataLayerConfs[1]).to.be.deep.equal(
        layersWithFolder[0].elements,
      );
      expect(data.lastSubjectDataLayerConfs[2]).to.have.deep.members(
        layersWithFolder[0].elements[0].elements,
      );

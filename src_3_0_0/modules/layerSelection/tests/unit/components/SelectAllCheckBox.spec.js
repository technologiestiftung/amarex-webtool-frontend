...
describe("src_3_0_0/modules/layerTree/components/SelectAllCheckBox.vue", () => {
  let store, wrapper, layer, propsData, changeVisibilitySpy;
  ...
  it("renders select all checkbox - unchecked", () => {
    ...
    expect(wrapper.find("#select-all-checkbox-" + layer.id).exists()).to.be
      .true;
    ...
    expect(wrapper.find(".layer-tree-layer-label").text()).to.equal(
      "modules.layerSelection.selectAll",
    );
  });
  ...
  it("renders select all checkbox - checked", () => {
    ...
    expect(wrapper.find(".layer-tree-layer-label").text()).to.equal(
      "modules.layerSelection.deselectAll",
    );
  });
  ...
  it("click on checkbox shall call changeVisibility with true", async () => {
    ...
    expect(wrapper.find("#select-all-layers-" + layer.id).exists()).to.be.true;
    expect(
      wrapper.findAll("#select-all-checkbox-" + layer.id).length,
    ).to.be.equals(1);
    expect(wrapper.findAll(".bi-square").length).to.be.equals(1);
    ...
    expect(changeVisibilitySpy.firstCall.args[1]).to.be.deep.equals({
      layerId: layer.id,
      value: true,
    });
  });

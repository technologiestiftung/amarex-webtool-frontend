...
describe("src_3_0_0/modules/layerSelection/components/LayerSelection.vue", () => {
  ...
  beforeEach(() => {
    ...
    store.commit(
      "Modules/LayerSelection/setSubjectDataLayerConfs",
      subjectDataLayers,
    );
  ...
  it("renders the LayerSelection without layers", () => {
    ...
    expect(
      wrapper.findAll("layer-selection-tree-node-stub").length,
    ).to.be.equals(0);
  ...
  it("renders the LayerSelection - check categories select", () => {
    ...
    expect(wrapper.findAll("option").at(0).text()).to.be.equals(
      "common:modules.layerTree.categoryOpendata",
    );
    expect(wrapper.findAll("option").at(1).text()).to.be.equals(
      "common:modules.layerTree.categoryInspire",
    );
    expect(wrapper.findAll("option").at(2).text()).to.be.equals(
      "common:modules.layerTree.categoryOrganisation",
    );
  });
  ...
  it("navigateStepsBack shall call action navigateBack", async () => {
    const provideSelectAllPropsSpy = sinon.spy(
      LayerSelectionComponent.methods,
  ...
  it("renders the LayerSelection with breadcrumbs ", async () => {
    ...
    const navigateStepsBackSpy = sinon.spy(
      LayerSelectionComponent.methods,
      "navigateStepsBack",
    );
    ...
    expect(breadCrumbsA.at(0).text()).to.be.equals(
      "common:modules.layerSelection.datalayer",
    );

...
const actions = {
  ...
  createLegend({ commit, dispatch, getters }) {
    layerCollection.getLayers().forEach((layer) => {
      if (
        typeof layer.layerSource?.getFeatures === "function" &&
        layer.getLayerSource().getFeatures().length === 0
      ) {
        const layerSource =
    ...
    getters.waitingLegendsInfos?.forEach((layer) =>
  ...
  generateLegend({ dispatch, getters }, layer) {
    const id = layer.get("id"),
      zIndex =
        typeof layer.getLayer().getZIndex === "function"
          ? layer.getLayer().getZIndex()
          : 0,
      legendObj = {
        id: id,
        name: layer.get("name"),
      ...
      isLegendChanged =
        isValidLegend && !isNotInLegend && getters.isLegendChanged(legendObj);
  ...
  prepareLegend({ commit }, legendInfos) {
    ...
    if (
      Array.isArray(legendInfos) &&
      legendInfos.every((value) => typeof value === "string") &&

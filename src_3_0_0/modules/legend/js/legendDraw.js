...
export default {
  ...
  drawNominalCircleSegments(styleObject) {
    ...
    Object.keys(scalingValues).forEach((key) => {
      ...
      let svg, svgSize, image, imageSize, imageSizeWithScale, svgPath;

      if (Array.isArray(styleObject.style)) {
        const olFeature = new Feature();

        olFeature.set(scalingAttribute, key);
        svgPath = createNominalCircleSegments(
          olFeature,
          styleObject.attributes,
        );
        ...
        imageSize = [
          Math.round(svgSize[0] * 1.04),
          Math.round(svgSize[1] * 1.04),
        ];
        imageSizeWithScale = [
          imageSize[0] * imageScale,
          imageSize[1] * imageScale,
        ];
        ...
        nominalCircleSegments.push({
          ...
          iconSizeDifferenz: Math.abs(
            (imageSize[0] * imageScale - svgSize[0]) / 2,
          ),
        });
      } else {
        nominalCircleSegments.push({
  ...
  drawCircleStyle(style) {
    const circleStrokeColor = style.circleStrokeColor
        ? convertColor(style.circleStrokeColor, "rgbString")
        : "black",
      ...
      circleFillColor = style.circleFillColor
        ? convertColor(style.circleFillColor, "rgbString")
        : "black",
  ...
  prepareLegendForLineString(legendObj, style) {
    const strokeColor = style.lineStrokeColor
        ? convertColor(style.lineStrokeColor, "rgbString")
        : "black",
      strokeWidth = style.lineStrokeWidth,
      strokeOpacity = style.lineStrokeColor[3] || 0,
      strokeDash = style.lineStrokeDash
        ? style.lineStrokeDash.join(" ")
        : undefined,
      type = style.type
        ? style.type.toLowerCase()
        : style.attributes?.type.toLowerCase();
  ...
  prepareLegendForPolygon(legendObj, style) {
    const fillColor = style.polygonFillColor
        ? convertColor(style.polygonFillColor, "rgbString")
        : "black",
      strokeColor = style.polygonStrokeColor
        ? convertColor(style.polygonStrokeColor, "rgbString")
        : "black",
    ...
    if (fillHatch) {
      legendObj.graphic =
        StylePolygon.prototype.getPolygonFillHatchLegendDataUrl(style);
    } else if (type === "icon") {
      legendObj.graphic = style.imagePath + style.imageName;
    } else {
      let svg = "data:image/svg+xml;charset=utf-8,";
  ...
  prepareLegendForPoint(legendObj, style) {
    ...
    if (type === "icon") {
      if (imageName.startsWith("https:")) {
        ...
        newLegendObj.graphic =
          "data:image/svg+xml;charset=utf-8," + encodeURIComponent(imageName);
      } else {

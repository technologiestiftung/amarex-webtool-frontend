...
describe("src_3_0_0/modules/legend/components/LegendContainer.vue", () => {
  ...
  beforeEach(() => {
    ...
    sinon.stub(layerCollection, "getLayers").returns(layersInCollection);
  });
  ...
  describe("render LegendContainer", () => {
    ...
    it("renders the legend container with legends", () => {
      ...
      expect(wrapper.findAll("legend-single-layer-stub").length).to.be.equals(
        2,
      );
    });
  });
  describe("LegendContainer.vue methods", () => {
    ...
    it("method generateId", () => {
      ...
      expect(wrapper.vm.generateId("Layername 1")).to.be.equals(
        "legend_Layername_1",
      );
      expect(wrapper.vm.generateId("Layername 1:")).to.be.equals(
        "legend_Layername_1_",
      ...
      expect(wrapper.vm.generateId("Layername (/)")).to.be.equals(
        "legend_Layername_",
      );
      expect(wrapper.vm.generateId("Layername (//)")).to.be.equals(
        "legend_Layername_",
      );
      expect(wrapper.vm.generateId("Layername (/abc/)")).to.be.equals(
        "legend_Layername_abc_",
      );
    });
    ...
    describe("watcher", () => {
      ...
      beforeEach(() => {
        ...
        sinon.stub(layerCollection, "getLayerById").callsFake(function (id) {
          if (id === "1") {
            return config_1;
          }
          if (id === "2") {
      ...
      it("visibleLayerConfigs shall call toggleLayerInLegend with true", async () => {
        ...
        wrapper.vm.$options.watch.visibleLayerConfigs.handler.call(
          wrapper.vm,
          newLayerConfigs,
          oldLayerConfigs,
        );
        ...
        expect(
          Legend.actions.toggleLayerInLegend.firstCall.args[1],
        ).to.be.deep.equals({ layer: config_2, visibility: true });
      });
      ...
      it("visibleLayerConfigs shall call toggleLayerInLegend with false", async () => {
        ...
        expect(
          Legend.actions.toggleLayerInLegend.firstCall.args[1],
        ).to.be.deep.equals({ layer: config_2, visibility: false });
      });

...
describe("src_3_0_0/modules/legend/components/LegendSingleLayer.vue", () => {
  ...
  describe("LegendSingleLayerComponent.vue rendering", () => {
    describe("renders legend with pdf", () => {
      it("renders the legend with pdf from array with single string", () => {
        ...
        expect(wrapper.find(".layer-legend > div:nth-child(1) a").exists()).to
          .be.true;
        expect(
          wrapper.find(".layer-legend > div:nth-child(1) a").attributes().href,
        ).to.equal("https://link_to_pdf.pdf");
      });
      it("renders the legends with pdf from array of strings", () => {
        const propsData = {
          legendObj: {
            name: "myLayer",
            legend: [
              "https://link_to_pdf.pdf",
              "https://another_link_to_pdf.pdf",
            ],
            position: 1,
          },
        };
        ...
        expect(wrapper.find(".layer-legend > div:nth-child(2) a").exists()).to
          .be.true;
        expect(
          wrapper.find(".layer-legend > div:nth-child(2) a").attributes().href,
        ).to.equal("https://another_link_to_pdf.pdf");
      });
      it("renders the legend with pdf from array with single object", () => {
        ...
        expect(
          wrapper.find(".layer-legend > div:nth-child(1) span").text(),
        ).to.equal("foobar");
      });
      it("renders the legends with pdf from array of objects", () => {
        ...
        expect(
          wrapper.find(".layer-legend > div:nth-child(2) span").text(),
        ).to.equal("barfoo");
      });
    });
  });
  describe("renders legend with img", () => {
    it("renders the legend with img from array with single string", () => {
      ...
      expect(wrapper.find(".layer-legend > div:nth-child(1) img").exists()).to
        .be.true;
      expect(
        wrapper.find(".layer-legend > div:nth-child(1) img").attributes().src,
      ).to.equal("some_string_interpreted_as_image");
    });
    it("renders the legends with img from array of strings", () => {
      const propsData = {
        legendObj: {
          name: "myLayer",
          legend: [
            "some_string_interpreted_as_image",
            "another_string_interpreted_as_image",
          ],
          position: 1,
        },
      };
      ...
      expect(wrapper.find(".layer-legend > div:nth-child(2) img").exists()).to
        .be.true;
      expect(
        wrapper.find(".layer-legend > div:nth-child(2) img").attributes().src,
      ).to.equal("another_string_interpreted_as_image");
    });
    ...
    it("renders the legend with img with sldVersion request", () => {
      ...
      expect(
        wrapper.find("#legend_myLayer > div:nth-child(1) img").attributes().src,
      ).includes("&sld_version=1.1.0");
    });
  });
  describe("renders legend with svg", () => {
    ...
    it("renders the legends with graphic is an array", () => {
          ...
          legend: [
            {
              name: "foobar",
              graphic: [
                "some_string_interpreted_as_image1",
                "some_string_interpreted_as_image2",
              ],
              iconSize: ["60", "60"],
      ...
      expect(
        wrapper.findAll(".layer-legend > div:nth-child(1) img")[0].exists(),
      ).to.be.true;
      expect(
        wrapper.findAll(".layer-legend > div:nth-child(1) img")[0].attributes()
          .src,
      ).to.equal("some_string_interpreted_as_image2");
      expect(
        wrapper.findAll(".layer-legend > div:nth-child(1) img")[0].attributes()
          .style,
      ).to.equal("width: 60px; height: 60px; margin: 10px;");
      expect(
        wrapper.find(".layer-legend > div:nth-child(1) span").text(),
      ).to.equal("foobar");
      expect(
        wrapper.findAll(".layer-legend > div:nth-child(1) img")[1].exists(),
      ).to.be.true;
      expect(
        wrapper.findAll(".layer-legend > div:nth-child(1) img")[1].attributes()
          .src,
      ).to.equal("some_string_interpreted_as_image1");
    });
  });
});

      ...
      handler(newLayerConfigs, oldLayerConfigs) {
        this.$nextTick(() => {
          newLayerConfigs.forEach((newConfig) => {
            const oldConfig = oldLayerConfigs.find(
                (config) => config.id === newConfig.id,
              ),
              existingLegend = this.legends.find(
                (legendConf) => legendConf.id === newConfig.id,
              );

            if (
              !oldConfig ||
              (existingLegend && existingLegend.position !== newConfig.zIndex)
            ) {
              const layer = layerCollection.getLayerById(newConfig.id);

              this.toggleLayerInLegend({
                layer: layer,
          ...
          oldLayerConfigs.forEach((oldConfig) => {
            const newConfig = newLayerConfigs.find(
...
<template>
  <div id="legend">
    ...
    >
      ...
      />
      <hr v-if="index < legends.length - 1" />
    </div>
  </div>
</template>

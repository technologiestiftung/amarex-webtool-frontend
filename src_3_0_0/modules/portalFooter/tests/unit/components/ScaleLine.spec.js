...
describe("src_3_0_0/modules/portalFooter/components/ScaleLine.vue", () => {
  ...
  describe("scaleToOne", () => {
    ...
    it('should return "1 : scale must be a positive number" if anything but a number is given', () => {
      expect(wrapper.vm.scaleToOne(undefined)).to.equal(
        "1 : scale must be a positive number",
      );
      expect(wrapper.vm.scaleToOne(null)).to.equal(
        "1 : scale must be a positive number",
      );
      expect(wrapper.vm.scaleToOne("string")).to.equal(
        "1 : scale must be a positive number",
      );
      expect(wrapper.vm.scaleToOne(true)).to.equal(
        "1 : scale must be a positive number",
      );
      expect(wrapper.vm.scaleToOne(false)).to.equal(
        "1 : scale must be a positive number",
      );
      expect(wrapper.vm.scaleToOne({})).to.equal(
        "1 : scale must be a positive number",
      );
      expect(wrapper.vm.scaleToOne([])).to.equal(
        "1 : scale must be a positive number",

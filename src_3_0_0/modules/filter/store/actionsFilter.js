...
export default {
  ...
  updateRules: (context, { filterId, snippetId, rule }) => {
    ...
    if (!Array.isArray(context.state.rulesOfFilters[filterId])) {
      ...
      try {
        rules = JSON.parse(
          JSON.stringify(context.state.rulesOfFilters[filterId]),
        );
      } catch (error) {
  ...
  serializeState: (context) => {
    const rulesOfFilters = context.state.rulesOfFilters,
      ...
      geometrySelectorOptions = JSON.parse(
        JSON.stringify(context.state.geometrySelectorOptions),
      ),
      geometryFeature = getGeometryFeature(
        context.state.geometryFeature,
        geometrySelectorOptions.invertGeometry,
      ),
      result = {
        rulesOfFilters,
        selectedAccordions,
    ...
    if (Array.isArray(result.geometrySelectorOptions?.additionalGeometries)) {
      result.geometrySelectorOptions.additionalGeometries.forEach(
        (additionalGeometry) => delete additionalGeometry.features,
      );
    }
    try {
  ...
  deserializeState: async (context, payload) => {
    const rulesOfFilters = payload?.rulesOfFilters,
      ...
      selectedGroups = Array.isArray(payload?.selectedGroups)
    ...
    if (Array.isArray(rulesOfFilters) && Array.isArray(selectedAccordions)) {
      ...
      await context.dispatch("setRulesArray", {
        rulesOfFilters: rulesOfFiltersCopy,
      });
      context.commit("setSelectedAccordions", selectedAccordions);
      context.commit("setSelectedGroups", selectedGroups);
      await context.dispatch("setGeometryFilterByFeature", {
        jsonFeature: payload?.geometryFeature,
        invert: payload?.geometrySelectorOptions?.invertGeometry,
      });
      if (typeof payload?.geometrySelectorOptions !== "undefined") {
        context.commit(
          "setGeometrySelectorOptions",
          payload?.geometrySelectorOptions,
        );
      }
      if (payload.geometrySelectorOptions) {
        additionalGeometries = await getFeaturesOfAdditionalGeometries(
          payload.geometrySelectorOptions.additionalGeometries,
        );
  ...
  setGeometryFilterByFeature(context, { jsonFeature, invert }) {
    ...
    if (
      invert &&
      isObject(feature) &&
      typeof feature.setGeometry === "function"
    ) {
...
function getGeometryFeature(feature, invert) {
  ...
  if (invert) {
    geometryFeature.setGeometry(new Polygon([coordinates]));
  }

...
export default {
    ...
    return {
      mapHandler: new MapHandler({
        ...
        setParserAttributeByLayerId:
          openlayerFunctions.setParserAttributeByLayerId,
  ...
  created() {
    getFeaturesOfAdditionalGeometries(
      this.geometrySelectorOptions.additionalGeometries,
    ).then((additionalGeometries) => {
      if (
        !Array.isArray(additionalGeometries) ||
        !additionalGeometries.length
      ) {
        return;
      }
  ...
  mounted() {
    ...
    if (
      Array.isArray(this.layerConfigs.groups) &&
      this.layerConfigs.groups.length > 0
    ) {
      ...
      if (
        Array.isArray(this.preparedLayerGroups) &&
        this.preparedLayerGroups.length > 0
      ) {
    ...
    if (
      Array.isArray(this.layerConfigs?.layers) &&
      this.layerConfigs.layers.length > 0
    ) {
      ...
      this.layerConfigs.layers.forEach((config) => {
        if (
          ...
          config.active &&
          typeof config?.filterId !== "undefined"
        ) {
    ...
    this.urlHandler.readFromUrlParams(
      this.urlParams,
      this.layerConfigs,
      this.mapHandler,
      (params) => {
  ...
  methods: {
    ...
    handleStateForAlreadyActiveLayers(params) {
      if (
        !isObject(params) ||
        !Object.prototype.hasOwnProperty.call(params, "selectedAccordions") ||
        !Object.prototype.hasOwnProperty.call(params, "rulesOfFilters")
      ) {
        return;
      }
      let selecetedAccordionsLen = Array.isArray(params?.selectedAccordions)
        ? params.selectedAccordions.length
        : 0;
      ...
      while (selecetedAccordionsLen--) {
        ...
        layerSource =
          layerModel.layer.getSource() instanceof Cluster
            ? layerModel.layer.getSource().getSource()
            : layerModel.layer.getSource();
        if (!layerSource) {
          continue;
        }
        if (
          ...
          (layerConfig?.typ === "SensorThings" ? layerModel : layerSource).once(
            "featuresloadend",
            async () => {
    ...
    addWatcherToWriteUrl() {
      if (this.saveTo === "url") {
        ...
        this.alreadyWatching = this.$watch(
          "$store.state.Modules.Filter",
          this.writeUrlParams,
          {
            deep: true,
          },
    ...
    updateSelectedAccordions(filterId) {
      ...
      if (!this.multiLayerSelector) {
        selectedFilterIds = this.selectedAccordions.some(
          (accordion) => accordion.filterId === filterId,
        )
          ? []
          : [filterId];
        this.setSelectedAccordions(
      ...
      this.preparedLayerGroups.forEach((layerGroup, groupIdx) => {
        if (
          layerGroup.layers.some((layer) => layer.filterId === filterId) &&
          !this.selectedGroups.includes(groupIdx)
        ) {
      ...
      selectedAccordionIndex = this.selectedAccordions.findIndex(
        (accordion) => accordion.filterId === filterId,
      );

      this.selectedAccordions.forEach((accordion) =>
        filterIdsOfAccordions.push(accordion.filterId),
      );
    ...
    transformLayerConfig(configs, filterIds) {
      ...
      configs.forEach((layerConfig) => {
        if (
          ...
          filterIds.includes(layerConfig.filterId)
        ) {
          layers.push({
    ...
    updateGeometrySelectorOptions(options) {
      this.setGeometrySelectorOptions(
        Object.assign({}, this.geometrySelectorOptions, options),
      );
    },
    /**
    ...
    isGeometrySelectorVisible() {
      return (
        isObject(this.geometrySelectorOptions) &&
        this.geometrySelectorOptions.visible !== false
      );
    },
    /**
    ...
    writeUrlParams(newState) {
      const params = this.urlHandler.getParamsFromState(
          newState,
          this.neededUrlParams,
        ),
    ...
    registerMapMoveListeners() {
      this.registerListener({
        type: "loadstart",
        listener: this.executeListeners.bind(this),
      });
      this.registerListener({
        type: "loadend",
        listener: this.executeListeners.bind(this),
    ...
    addToMapMoveListeners(options) {
      if (
        !isObject(options) ||
        !Object.prototype.hasOwnProperty.call(options, "filterId") ||
        !Object.prototype.hasOwnProperty.call(options, "listener")
      ) {
        return;
      }
...
<template lang="html">
  <div id="filter">
    ...
    <div
      v-if="
        Array.isArray(layerGroups) && layerGroups.length && layerSelectorVisible
      "
    >
      <div
          ...
          <div class="panel-body">
            <h2 class="panel-title">
              <a
                ...
                @keydown.enter="
                  updateSelectedGroups(layerGroups.indexOf(layerGroup))
                "
              >
                {{ layerGroup.title ? layerGroup.title : key }}
                <span
                  v-if="
                    !selectedGroups.includes(layerGroups.indexOf(layerGroup))
                  "
            ...
            >
              <FilterList
                v-if="
                  Array.isArray(preparedLayerGroups) &&
                  preparedLayerGroups.length &&
                  layerSelectorVisible
                "
                class="layerSelector"
                :filters="
                  preparedLayerGroups[layerGroups.indexOf(layerGroup)].layers
                "
                ...
                <template #default="slotProps">
                  <div
                    :class="[
                      'accordion-collapse',
                      'collapse',
                      isLayerFilterSelected(slotProps.layer.filterId)
                        ? 'show'
                        : '',
                    ]"
                    role="tabpanel"
    ...
    <div
      v-else-if="
        (Array.isArray(layerConfigs.layers) && layerConfigs.layers.length) ||
        (Array.isArray(layerConfigs.groups) && layerConfigs.groups.length)
      "
    >
      <div
        v-for="(layerGroup, key) in layerGroups"
      ...
      >
        <div>
          <h2
            v-for="(layerConfig, indexLayer) in preparedLayerGroups[
              layerGroups.indexOf(layerGroup)
            ].layers"
    ...
    <FilterList
      v-if="
        (Array.isArray(layerConfigs.layers) &&
          layerConfigs.layers.length &&
          layerSelectorVisible) ||
        (Array.isArray(layerConfigs.groups) &&
          layerConfigs.groups.length &&
          layerSelectorVisible)
      "
      class="layerSelector"
      :filters="filters"
      ...
      <template #default="slotProps">
        <div
          :class="[
            'accordion-collapse',
            'collapse',
            isLayerFilterSelected(slotProps.layer.filterId) ? 'show' : '',
          ]"
          role="tabpanel"
        >
          <LayerFilterSnippet
            v-if="
              isLayerFilterSelected(slotProps.layer.filterId) ||
              layerLoaded[slotProps.layer.filterId]
            "
            :api="slotProps.layer.api"

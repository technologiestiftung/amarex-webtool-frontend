...
export default {
  ...
  computed: {
    titleText() {
      if (typeof this.title === "string") {
        return translateKeyWithPlausibilityCheck(this.title, (key) =>
          this.$t(key),
        );
      }
      return "";
  ...
  watch: {
    filteredItems: {
      handler(items) {
        const attributesObject = this.getUniqueObjectFromAttributes(
            this.attrName,
            items,
          ),
    ...
    featureInfo: {
      handler() {
        if (
          isObject(this.featureInfo) &&
          Object.keys(this.featureInfo).length > 0
        ) {
          this.visible = true;
          return;
        }
  ...
  methods: {
    ...
    setGfiAttributes(layerId) {
      ...
      this.gfiAttributes = isObject(layer?.gfiAttributes)
        ? layer.gfiAttributes
        : undefined;
    },

...
export default {
  ...
  data() {
    return {
      ...
      operatorWhitelist: ["EQ", "GT", "GE", "LT", "LE"],
    };
  },
  computed: {
    ariaLabelDate() {
      return this.$t("common:modules.filter.ariaLabel.date", {
        param: this.attrName,
      });
    },
    titleText() {
  ...
  watch: {
    value() {
      if (
        !this.isAdjusting &&
        (!this.isInitializing || this.precheckedIsValid)
      ) {
        this.emitCurrentRule(
          dayjs(this.inRangeValue, this.internalFormat).format(this.format),
          this.isInitializing,
        );
      }
    },
    ...
    this.$nextTick(() => {
      ...
      if (momentMin.isValid() && momentMax.isValid()) {
        ...
        this.api.getMinMax(
          ...
          (minMaxObj) => {
            ...
            if (Object.prototype.hasOwnProperty.call(minMaxObj, "min")) {
              this.minimumValue = dayjs(minMaxObj.min, this.format).format(
                this.internalFormat,
              );
            } else {
              this.minimumValue = momentMin.format(this.internalFormat);
            ...
            if (Object.prototype.hasOwnProperty.call(minMaxObj, "max")) {
              this.maximumValue = dayjs(minMaxObj.max, this.format).format(
                this.internalFormat,
              );
            } else {
              this.maximumValue = momentMax.format(this.internalFormat);
        ...
        this.value = this.precheckedIsValid
          ? momentPrechecked.format(this.internalFormat)
          : "";
        this.$nextTick(() => {
          this.isInitializing = false;
          this.disable = false;
  ...
  methods: {
    ...
    emitSnippetPrechecked(precheckedIsValid, snippetId, visible) {
      this.$emit(
        ...
        visible && precheckedIsValid ? snippetId : false,
      );
    },
    /**

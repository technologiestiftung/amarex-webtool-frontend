    ...
    infoText() {
      if (typeof this.info === "boolean") {
        const translationKey =
          "common:modules.filter.info." + this.translationKey;

        return this.translateKeyWithPlausibilityCheck(translationKey, (key) =>
          this.$t(key),
        );
      } else if (typeof this.info === "string") {
        return this.translateKeyWithPlausibilityCheck(this.info, (key) =>
          this.$t(key),
        );
      }
      return "";

...
export default {
  ...
  data() {
    return {
      ...
      operatorWhitelist: ["EQ", "IN", "STARTSWITH", "ENDSWITH"],
      source: "",
      allValues: false,
    };
  },
  computed: {
    ariaLabelDropdown() {
      return this.$t("common:modules.filter.ariaLabel.dropdown", {
        param: this.attrName,
      });
    },
    ariaLabelRadio() {
      return this.$t("common:modules.filter.ariaLabel.radio", {
        param: this.attrName,
      });
    },
    ariaLabelCheckbox() {
        ...
        dropdownValue.sort((a, b) => {
          if (typeof this.localeCompareParams === "string") {
            ...
            return localeCompare(
              a,
              b,
              this.localeCompareParams.locale,
              this.localeCompareParams.options,
            );
          }
    ...
    selectAllTitle() {
      return !this.allSelected
        ? this.$t("common:modules.filter.dropdown.selectAll")
        : this.$t("common:modules.filter.dropdown.deselectAll");
    },
    securedOperator() {
      if (!this.operatorWhitelist.includes(this.operator)) {
        return getDefaultOperatorBySnippetType(
          "dropdown",
          typeof this.delimiter === "string" && this.delimiter,
        );
      }
  ...
  watch: {
    dropdownSelected: {
      handler(value) {
        const prechecked = this.getPrecheckedExistingInValue(
          this.prechecked,
          this.dropdownValue,
        );
        ...
        if (
          ...
          (!this.isInitializing ||
            (this.isInitializing &&
              ((Array.isArray(prechecked) && prechecked.length) ||
                this.prechecked === "all")))
        ) {
          if (
        ...
        this.allSelected =
          ...
          Array.isArray(this.dropdownSelected) &&
          this.dropdownValue.length === this.dropdownSelected.length;
      },
      deep: true,
    },
    adjustment(adjusting) {
      ...
      this.$nextTick(() => {
        if (adjusting?.start) {
          if (
            this.snippetId !== adjusting.snippetId &&
            (!Array.isArray(adjusting.snippetId) ||
              !adjusting.snippetId.includes(this.snippetId))
          ) {
            this.dropdownValue = [];
          }
        ...
        this.addDropdownValueForAdjustment(
          this.dropdownValue,
          this.value,
          adjusting?.adjust?.value,
          this.delimiter,
        );
        ...
        if (adjusting?.finish) {
          ...
          this.setDropdownSelectedAfterAdjustment(
            this.dropdownValue,
          ...
          this.$nextTick(() => {
            ...
            if (this.delayedPrechecked === "all") {
              ...
              Array.isArray(this.delayedPrechecked) &&
              this.delayedPrechecked.length
            ) {
              this.dropdownSelected = this.getPrecheckedExistingInValue(
                this.delayedPrechecked,
                this.dropdownValue,
              );
              this.delayedPrechecked = false;
            }
    ...
    legendsInfo: {
      handler(value) {
        if (this.renderIcons === "fromLegend") {
          this.iconList = getIconListFromLegendModule.getIconListFromLegend(
            value,
            this.styleModel,
          );
        }
  ...
  mounted() {
    this.$nextTick(() => {
      ...
      if (!this.visible) {
        this.dropdownValue = Array.isArray(this.prechecked)
          ? this.prechecked
          : [];
        this.dropdownSelected = this.getInitialDropdownSelected(
          this.prechecked,
          this.dropdownValue,
          this.multiselect,
        );
        ...
        this.$nextTick(() => {
          this.api.getUniqueValues(
            ...
            (list) => {
              this.$nextTick(() => {
                this.dropdownValue = this.splitListWithDelimiter(
                  list,
                  this.delimiter,
                );
                this.dropdownSelected = this.getInitialDropdownSelected(
        ...
        if (
          this.isChild &&
          ((Array.isArray(this.prechecked) && this.prechecked.length) ||
            this.prechecked === "all")
        ) {
  ...
  methods: {
    ...
    initializeIcons() {
      if (this.renderIcons === "fromLegend") {
        ...
        this.styleModel = getIconListFromLegendModule.getStyleModel(
          this.layerId,
        );
        if (
          !layerCollection.getLayerById(this.layerId) &&
          ["WFS", "OAF", "GeoJSON"].includes(layerConfig.typ)
        ) {
          ...
          if (
            mapCollection
              ...
              .getLayers()
              .getArray()
              .find((aLayer) => aLayer.get("id") === this.layerId) === undefined
          ) {
    ...
    getLegendByStyleId(styleId, olLayer, callback) {
      createStyle.returnLegendByStyleId(styleId).then((legendInfos) => {
        if (
          !this.styleModel ||
          !legendInfos.legendInformation ||
          !Array.isArray(legendInfos.legendInformation)
        ) {
          this.legendsInfo = [];
        } else {
    ...
    getDropdownValueForList() {
      ...
      if (!Array.isArray(dropdownValue) || !dropdownValue.length) {
        ...
        return Array.isArray(dropdownValue[0].list)
          ? dropdownValue[0].list
          : [];
      }
      return dropdownValue;
    },
    ...
    addDropdownValueForAdjustment(
      ...
      adjustmentValue.forEach((value) => {
        if (
          ...
          !dropdownValueAssoc[value] &&
          (!Array.isArray(configValue) ||
            (Array.isArray(configValue) && configValueAssoc[value]))
        ) {
          dropdownValueAssoc[value] = true;
          this.dropdownValue.push(value);
        }
     ...
     * @returns {void}
     */
    setDropdownSelectedAfterAdjustment(
      dropdownValue,
      dropdownSelected,
      setDropdownSelected,
    ) {
      const selected =
          typeof dropdownSelected === "string"
            ? [dropdownSelected]
            : dropdownSelected,
        result = [],
      ...
      <div class="grid-container">
        ...
        >
          <span
            ...
            class="subItem"
          >
            <label :for="'snippetRadioCheckbox-' + snippetId + '-' + val">
              <img
...
.filter-select-box-container .multiselect__input,
.filter-select-box-container .multiselect__single {
  font-family: inherit;
  font-size: $font-size-base;
}
.filter-select-box-container .multiselect .multiselect__spinner:after,
...
.filter-select-box-container .multiselect .multiselect__tag-icon:focus,
...
.snippetDropdownContainer .radio,
.snippetDropdownContainer .checkbox {
  display: inline-block;
}
...
.panel .snippetDropdownContainer .right,

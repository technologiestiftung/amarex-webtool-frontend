...
export default {
  ...
  data() {
    return {
      ...
      slider: 0,
      input: 0,
      operatorWhitelist: ["EQ", "GT", "LT", "GE", "LE"],
    };
  },
  computed: {
    ariaLabelSlider() {
      return this.$t("common:modules.filter.ariaLabel.slider", {
        param: this.attrName,
      });
    },
    titleText() {
  ...
  methods: {
    ...
    initSlider(min, max) {
      ...
      if (this.isPrecheckedValid()) {
        this.slider = this.isPrecheckedHigherThanMin()
          ? this.prechecked
          : this.currentSliderMin;
      } else {
        this.slider = this.currentSliderMin;
    ...
    emitSnippetPrechecked(prechecked, snippetId, visible) {
      this.$emit(
        "setSnippetPrechecked",
        visible && typeof prechecked !== "undefined" ? snippetId : false,
      );
    },
    /**
    ...
    getAlertRangeText(value) {
      if (value === undefined) {
        this.addSingleAlert(
          i18next.t("common:snippets.slider.valueEmptyErrorMessage"),
        );
      } else {
        this.addSingleAlert(
          i18next.t("common:snippets.slider.valueOutOfRangeErrorMessage", {
            inputValue: value,
...
<template>
  ...
  >
    ...
    <span class="min">{{
      checkNaN(currentSliderMin) ? 0 : currentSliderMin
    }}</span>
    <span class="max">{{
      checkNaN(currentSliderMax) ? 0 : currentSliderMax
    }}</span>
  </div>
</template>

...
export default {
  ...
  computed: {
    ...
    labelFilterButton() {
      if (typeof this.layerConfig.labelFilterButton === "string") {
        return translateKeyWithPlausibilityCheck(
          this.layerConfig.labelFilterButton,
          (key) => this.$t(key),
        );
      }
  ...
  watch: {
    ...
    paging(val) {
      if (val.page >= val.total) {
        ...
        if (
          !this.isRefreshing &&
          !this.getSearchInMapExtent() &&
          this.liveZoomToFeatures
        ) {
          if (this.filterGeometry) {
            this.mapHandler.zoomToGeometry(
              this.filterGeometry,
              this.minScale,
              (error) => {
                console.warn(error);
            ...
            this.mapHandler.zoomToFilteredFeature(
              this.layerConfig?.filterId,
              this.minScale,
              (error) => {
                console.warn(error);
              },
  ...
  created() {
    const filterId = this.layerConfig.filterId,
      layerId = this.getLayerId(
        ...
        this.layerConfig?.service?.layerId,
      );

    if (
      this.api instanceof FilterApi &&
      this.mapHandler instanceof MapHandler
    ) {
      this.mapHandler.initializeLayer(
        filterId,
        layerId,
        this.isExtern(),
        (error) => {
      ...
      this.$nextTick(() => {
        this.api.setServiceByLayerModel(
          layerId,
          this.mapHandler.getLayerModelByFilterId(filterId),
          this.isExtern(),
          (error) => {
            console.warn(error);
          },
      ...
      this.$nextTick(() => {
        if (!this.mapHandler.getLayerModelByFilterId(filterId)) {
          console.warn(
            new Error(
              "Please check your filter configuration: The given layerId does not exist in your config.json.",
            ),
          );
        }
      });
    }
    this.filterButtonDisabled =
      typeof this.layerConfig?.filterButtonDisabled === "boolean"
        ? this.layerConfig.filterButtonDisabled
        : false;
  },
  mounted() {
    this.$nextTick(() => {
      ...
      if (
        layerConfig.typ === "VectorTile" &&
        this.mapHandler.isLayerActivated(filterId) === false
      ) {
  ...
  beforeUnmount() {
    if (
      this.layerConfig.filterOnMove === true &&
      !this.openMultipleAccordeons &&
      this.layerConfig?.strategy === "active"
    ) {
      ...
      if (
        this.mapHandler.getLayerModelByFilterId(
          this.layerConfig.filterId,
        ) instanceof VectorTileLayer
      ) {
  ...
  methods: {
    ...
    onMounted() {
      ...
      compileSnippets(
        ...
        (snippets) => {
          ...
          if (
            this.layerSelectorVisible &&
            this.layerConfig.filterOnOpen &&
            this.layerConfig.strategy === "active"
          ) {
            this.$nextTick(() => {
              this.handleActiveStrategy();
      ...
      if (
        !this.mapHandler.isLayerActivated(this.layerConfig.filterId) &&
      ...
      if (
        typeof this.layerConfig.minZoom === "number" ||
        typeof this.layerConfig.maxZoom === "number"
      ) {
    ...
    setSearchInMapExtent(value) {
      ...
      if (
        value === true &&
        this.layerConfig?.searchInMapExtentProactive !== false &&
        !this.layerConfig?.searchInMapExtentPreselected &&
        this.isStrategyActive()
      ) {
        this.handleActiveStrategy();
      }
    },
    ...
    handleActiveStrategy(snippetId, reset = undefined) {
      ...
      this.filter(
        snippetId,
        (filterAnswer) => {
          const adjustments = getSnippetAdjustments(
              this.snippets,
              filterAnswer?.items,
              filterAnswer?.paging?.page,
              filterAnswer?.paging?.total,
            ),
            start =
              typeof adjustments?.start === "boolean"
                ? adjustments.start
                : false,
            finish =
              typeof adjustments?.finish === "boolean"
                ? adjustments.finish
                : false;
          ...
          this.snippets.forEach((snippet) => {
            snippet.adjustment = {
              ...
              start,
              finish,
              adjust: isObject(adjustments[snippet.snippetId])
                ? adjustments[snippet.snippetId]
                : false,
            };
          });
        },
    ...
    changeRule(rule) {
      if (this.isRule(rule)) {
        ...
        if (
          !rule.startup &&
          (this.isStrategyActive() || this.isParentSnippet(rule.snippetId))
        ) {
          this.$nextTick(() => {
    ...
    deleteRule(snippetId) {
      ...
      if (this.isStrategyActive() || this.isParentSnippet(snippetId)) {
        this.$nextTick(() => {
          this.handleActiveStrategy(
            snippetId,
            !this.hasUnfixedRules(this.filterRules) &&
              this.layerConfig.resetLayer &&
              !this.layerConfig.clearAll
              ? true
              : undefined,
          );
        });
    ...
    deleteAllRules() {
      ...
      if (this.isStrategyActive()) {
        this.$nextTick(() => {
          ...
          this.handleActiveStrategy(
            undefined,
            this.layerConfig.resetLayer && !this.layerConfig.clearAll
              ? true
              : undefined,
          );
        });
    ...
    filter(
      ...
      const filterId = this.layerConfig.filterId,
        filterQuestion = {
          filterId,
          snippetId:
            typeof snippetId === "number" || Array.isArray(snippetId)
              ? snippetId
              : false,
          commands: {
      ...
      if (
        ...
        this.mapHandler.activateLayer(filterId, () => {
          if (
            Object.prototype.hasOwnProperty.call(this.layerConfig, "wmsRefId")
          ) {
            this.mapHandler.toggleWMSLayer(
              this.layerConfig.wmsRefId,
              !this.hasUnfixedRules(filterQuestion.rules) &&
                !filterQuestion.commands.filterGeometry,
            );
            this.mapHandler.toggleWFSLayerInTree(
              filterId,
              this.hasUnfixedRules(filterQuestion.rules) ||
                filterQuestion.commands.filterGeometry,
            );
          ...
          this.api.filter(
            ...
            (filterAnswer) => {
              ...
              if (
                !this.isParentSnippet(snippetId) &&
                !this.hasOnlyParentRules()
              ) {
                if (
                  !this.hasUnfixedRules(filterQuestion.rules) &&
                  (this.layerConfig.clearAll ||
                ...
                this.mapHandler.addItemsToLayer(
                  filterId,
                  filterAnswer.items,
                  this.isExtern(),
                );
                if (
                  ...
                  this.layerConfig.showHits
                ) {
                  this.amountOfFilteredItems =
                    this.mapHandler.getAmountOfFilteredItemsByFilterId(
                      filterId,
                    );
                }
                ...
                if (
                  !this.autoRefreshSet &&
                  this.mapHandler.hasAutoRefreshInterval(filterId)
                ) {
                ...
                if (
                  Array.isArray(filterAnswer?.items) &&
                  filterAnswer?.items.length > 0
                ) {
    ...
    checkZoomLevel(minZoom, maxZoom) {
      ...
      this.$store.watch(
        ...
        (scale) => {
          ...
          this.outOfZoom = this.checkOutOfZoomLevel(
            minZoom,
            maxZoom,
            zoomLevel,
          );
    ...
    getTitle(snippet, layerId) {
      ...
      const model = openlayerFunctions.getLayerByLayerId(layerId),
        title =
          isObject(model) && isObject(model.gfiAttributes)
            ? model.gfiAttributes[
...
<template>
  <div :class="['panel-body', outOfZoom ? 'disabled' : '']">
    <div
      v-if="outOfZoom"
    ...
    <template v-else>
      ...
      >
        {{
          translateKeyWithPlausibilityCheck(layerConfig.description, (key) =>
            $t(key),
          )
        }}
      </div>
      ...
      <div
        v-if="
          layerConfig.showHits !== false &&
          typeof amountOfFilteredItems === 'number' &&
          !outOfZoom
        "
        class="filter-result"
      >
        ...
        <span>
          {{
            $t("common:modules.filter.filterResult.unit", {
      ...
      <div class="snippet">
        ...
        <ProgressBar :paging="paging" />
        <div
          v-if="
            layerConfig.download &&

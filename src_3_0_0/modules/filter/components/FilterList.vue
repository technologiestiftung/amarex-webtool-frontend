...
export default {
  ...
  computed: {
    ...
    disabled(filterId) {
      return (
        !this.multiLayerSelector &&
        this.selectedLayers.length > 0 &&
        this.selectedLayers.some((accordion) => accordion.filterId === filterId)
      );
    },
  },
  watch: {
  ...
  methods: {
    ...
    scrollToView(filterId) {
      if (
        ...
        !this.filters.some((filter) => filter.filterId === filterId)
      ) {
        return;
      }
      const filter = Array.isArray(this.$refs[filterId])
        ? this.$refs[filterId][0]
        : this.$refs[filterId];
      ...
      if (filter && typeof filter.scrollIntoView === "function") {
        ...
        this.addSingleAlert(
          i18next.t(
            "common:modules.filter.alertingMessages.noMatchingFilterId",
          ),
          { root: true },
        );
      }
      ...
      if (
        !this.selectedLayers.some(
          (selectedLayer) => selectedLayer.filterId === filterId,
        )
      ) {
  ...
  >
    ...
    >
      <button
        ...
        @click="
          setLayerLoaded(filter.filterId), updateSelectedLayers(filter.filterId)
        "
        @keydown.enter="
          setLayerLoaded(filter.filterId), updateSelectedLayers(filter.filterId)
        "
      >
        <h2 :class="['panel-title', disabled ? 'disabled' : '']">
          {{ filter.title ? filter.title : filter.layerId }}
        </h2>
        <span
          v-if="
            !selectedLayers.some(
              (layers) => layers.filterId === filter.filterId,
            )
          "
      ...
      <div
        v-if="
          filter.shortDescription && !selectedLayers.includes(filter.filterId)
        "
        class="layerInfoText"
      >
        {{
          translateKeyWithPlausibilityCheck(filter.shortDescription, (key) =>
            $t(key),
          )
        }}
      </div>

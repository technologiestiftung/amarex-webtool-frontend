...
export default class InterfaceGeojsonExtern {
      ...
      (geojson) => {
        if (
          ...
          if (typeof onerror === "function") {
            onerror(
              new Error(
                "InterfaceGeojsonExtern.getUniqueValues: The received geojson has not the expected format.",
              ),
            );
          }
          return;
        ...
        Object.entries(geojson.features[0].properties).forEach(
          ([key, value]) => {
      ...
      (geojson) => {
        ...
        geojson.features.forEach((feature) => {
          if (
            ...
            !Object.prototype.hasOwnProperty.call(feature.properties, attrName)
          ) {
            return;
          }
  ...
  stop(filterId, onsuccess, onerror) {
    onerror(
      new Error(
        "InterfaceGeojsonExtern: stop not implemented, no external filtering with geojson",
      ),
    );
  }
  ...
  filter(filterQuestion, onsuccess, onerror, axiosMock = false) {
    if (axiosMock) {
      onerror(
        new Error(
          "InterfaceGeojsonExtern: filter not implemented for testing, no external filtering with geojson",
        ),
      );
    } else {
      onerror(
        new Error(
          "InterfaceGeojsonExtern: filter not implemented, no external filtering with geojson",
        ),
      );
    }
  }
  ...
  handleGeojsonRequest(service, onsuccess, onerror, axiosMock = false) {
    if (!isObject(service)) {
      if (typeof onerror === "function") {
        onerror(
          new Error(
            "InterfaceGeojsonExtern.handleGeojsonRequest: missing service object",
          ),
        );
      }
      return;
        ...
        .then((response) => {
          if (!isObject(response) || !isObject(response.data)) {
            if (typeof onerror === "function") {
              onerror(
                new Error(
                  "InterfaceGeojsonExtern.handleGeojsonRequest: the response data does not exist",
                ),
              );
            }
            return;

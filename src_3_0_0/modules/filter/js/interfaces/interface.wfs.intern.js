...
export default class InterfaceWfsIntern {
  ...
  constructor(
    intervalRegister,
    { getFeaturesByLayerId, isFeatureInMapExtent, isFeatureInGeometry },
  ) {
  ...
  getMinMax(
    ...
    return this.interfaceWfsExtern.getMinMax(
      service,
      attrName,
      onsuccess,
      onerror,
  ...
  getUniqueValues(service, attrName, onsuccess, onerror, filterQuestion) {
    return this.interfaceWfsExtern.getUniqueValues(
      service,
      attrName,
      onsuccess,
      onerror,
  ...
  filter(filterQuestion, onsuccess, onerror) {
    if (
      ...
      if (typeof onerror === "function") {
        onerror(
          new Error("InterfaceWfsIntern.filter: invalid intervalRegister"),
        );
      }
      return;
      ...
      if (typeof onerror === "function") {
        onerror(
          new Error(
            "InterfaceWfsIntern.filter: getFeaturesByLayerId must be a function",
          ),
        );
      }
      return;
      ...
      if (typeof onerror === "function") {
        onerror(
          new Error(
            "InterfaceWfsIntern.filter: isFeatureInMapExtent must be a function",
          ),
        );
      }
      return;
      ...
      if (typeof onerror === "function") {
        onerror(
          new Error(
            "InterfaceWfsIntern.filter: isFeatureInGeometry must be a function",
          ),
        );
      }
      return;
      ...
      if (typeof onerror === "function") {
        onerror(
          new Error(
            "InterfaceWfsIntern.filter: filterQuestion must be an object",
          ),
        );
      }
      return;
    ...
    const clonedQuestion = JSON.parse(JSON.stringify(filterQuestion)),
      ...
      filterGeometry =
        typeof filterQuestion?.commands?.filterGeometry === "boolean"
          ? filterQuestion?.commands?.filterGeometry
          : filterQuestion?.commands?.filterGeometry?.clone(),
    ...
    this.intervalRegister.startPagingInterval(
      filterId,
      () => {
        const items = [];
        ...
        for (let n = 0; n < paging; n++) {
          if (
            ...
            (!isObject(filterGeometry) ||
              this.isFeatureInGeometry(features[idx], filterGeometry)) &&
  ...
  checkRules(feature, rules) {
    ...
    for (let i = 0; i < len; i++) {
      if (!isObject(rules[i])) {
        ...
        !Array.isArray(rules[i].attrName) &&
        Object.prototype.hasOwnProperty.call(rules[i], "attrName")
      ) {
        if (
          !this.checkRule(
            rules[i],
            this.getPropertyFromFeature(feature, rules[i].attrName),
          )
        ) {
          return false;
        }
        ...
        rules[i].attrName.forEach((attributeName) => {
          propertiesOfFeaturesByAttrNames.push(
            this.getPropertyFromFeature(feature, attributeName),
          );
        });
  ...
  checkRule(rule, featureValue, featureValue2) {
    ...
    if (
      ...
      Object.prototype.hasOwnProperty.call(rule, "operatorForAttrName") &&
      rule.operatorForAttrName === "OR"
    ) {
    ...
    let ruleValueA = Array.isArray(rule.value) ? rule.value[0] : rule.value,
      ...
      featValueB =
        typeof featureValue2 !== "undefined" ? featureValue2 : featureValue;
    ...
    return (
      (Array.isArray(rule.value) &&
        ((rule.operator === "INTERSECTS" &&
          ...
          (rule.operator === "EQ" &&
            equalsForArray(featValueA, rule.value, rule.format)) ||
          (rule.operator === "IN" && inForArray(featValueA, rule.value)) ||
          (rule.operator === "STARTSWITH" &&
            startswithForArray(featValueA, rule.value)) ||
          (rule.operator === "ENDSWITH" &&
            endswithForArray(featValueA, rule.value)))) ||
      (!Array.isArray(rule.value) &&
        typeof ruleValueA !== "undefined" &&
        ((rule.operator === "BETWEEN" &&
          between(featValueA, featValueB, ruleValueA, rule.format)) ||
          (rule.operator === "EQ" &&
            equals(featValueA, ruleValueA, rule.format)) ||
          ...
          (rule.operator === "STARTSWITH" &&
            startswith(featValueA, ruleValueA)) ||
  ...
  checkRuleForAttributesORHandler(rule, featureValue) {
    if (Array.isArray(featureValue)) {
      for (const featureVal of featureValue) {
        if (
          typeof featureVal !== "undefined" &&
          this.checkRuleForAttributesORHandler(rule, featureVal)
        ) {
          return true;
        }
      }
    ...
    return (
      (Array.isArray(rule.value) &&
        ((rule.operator === "INTERSECTS" &&
          ...
          (rule.operator === "BETWEEN" &&
            between(ruleValueA, ruleValueB, featValueA, rule.format)) ||
          (rule.operator === "EQ" &&
            equalsForArray(
              featValueA,
              rule.value,
      ...
      (!Array.isArray(rule.value) &&
        typeof ruleValueA !== "undefined" &&
        ((rule.operator === "BETWEEN" &&
          between(featValueA, featValueA, ruleValueA, rule.format)) ||
          (rule.operator === "EQ" &&
            equals(featValueA, ruleValueA, rule.format, rule.delimiter)) ||
          (rule.operator === "NE" && ne(featValueA, ruleValueA, rule.format)) ||
          (rule.operator === "GT" && gt(featValueA, ruleValueA, rule.format)) ||
          (rule.operator === "GE" && ge(featValueA, ruleValueA, rule.format)) ||
          (rule.operator === "LT" && lt(featValueA, ruleValueA, rule.format)) ||

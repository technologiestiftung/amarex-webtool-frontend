...
export default class InterfaceWfsExtern {
  ...
  getAttrTypes(service, onsuccess, onerror) {
    if (!isObject(service)) {
      if (typeof onerror === "function") {
        onerror(
          new Error("InterfaceWfsExtern.getAttrTypes: missing service object"),
        );
      }
      return;
    }
  ...
  getMinMax(
    ...
    if (
      (Array.isArray(filterQuestion?.rules) && filterQuestion.rules.length) ||
  ...
  getMinMaxPOST(
    ...
    maxOnly,
    isDate,
    { rules, filterId, format, commands },
  ) {
    ...
    if (Array.isArray(this.allFetchedItems[filterId])) {
      onsuccess(
        this.getMinMaxFromFeaturesByAttrName(
          this.allFetchedItems[filterId],
          attrName,
          isDate,
          format,
          minOnly,
   ...
   */
  getMinMaxFromFeaturesByAttrName(
    features,
    attrName,
    isDate,
    format,
  ...
  getMinMaxGET(
    ...
    const url = service?.url,
      ...
      axiosObject =
        typeof axiosMock === "object" && axiosMock !== null ? axiosMock : axios,
      result = {};

    if (!maxOnly) {
      const minParams = !isDate
        ? Object.assign({}, params, { maxfeatures: 1, sortby: attrName + " A" })
        : params;
      ...
      axiosObject
        ...
        .then((response) => {
          this.parseResponseMinMax(
            service?.typename,
            attrName,
            response?.request?.responseXML,
            isDate ? "min" : undefined,
            (min) => {
              result.min = min;
              if (
                (minOnly || result.max !== undefined) &&
    ...
    if (!minOnly) {
      const maxParams = !isDate
        ? Object.assign({}, params, { sortby: attrName + " D" })
        : params;
  ...
  getUniqueValues(
    ...
    if (
      (Array.isArray(filterQuestion?.rules) &&
        filterQuestion.rules.length > 0) ||
      filterQuestion.commands?.filterGeometry
    ) {
      this.getUniqueValueByPOST(
        service,
        attrName,
        onsuccess,
        onerror,
  ...
  getUniqueValueByPOST(
    ...
    if (Array.isArray(this.allFetchedItems[filterId])) {
      onsuccess(
        this.getUniqueValuesFromFeatures(
          this.allFetchedItems[filterId],
          attrName,
        ),
      );
  ...
  getUniqueValueByGET(service, attrName, onsuccess, onerror, axiosMock) {
    ...
    axiosObject
      .get(url, { params })
      .then((response) => {
        this.parseResponseUniqueValues(
          service?.typename,
          attrName,
          response?.request?.responseXML,
          (list) => {
  ...
  stop(filterId, onsuccess, onerror) {
    if (
      this.axiosCancelTokenSources[filterId] &&
      typeof this.axiosCancelTokenSources[filterId].cancel === "function"
    ) {
      ...
      onerror(
        new Error(
          "InterfaceWfsExtern.stop: The request can't be stopped because it does not exist or has already finished.",
        ),
      );
    }
  }
  ...
  filter(filterQuestion, onsuccess, onerror, axiosMock = false) {
    const filter =
        (Array.isArray(filterQuestion?.rules) &&
          filterQuestion?.rules.length) ||
        filterQuestion.commands?.filterGeometry
          ? this.getFilter(
              filterQuestion.rules,
              filterQuestion.commands?.searchInMapExtent,
              filterQuestion.commands?.geometryName,
              filterQuestion.commands?.filterGeometry,
            )
          : undefined,
    ...
    this.axiosCancelTokenSources[filterQuestion.filterId] =
      axiosObject.CancelToken.source();
    axiosObject
      .post(filterQuestion?.service?.url, payload, {
        headers: {
          "Content-Type": "text/xml",
        },
        cancelToken:
          this.axiosCancelTokenSources[filterQuestion.filterId].token,
        onDownloadProgress: (progressEvent) => {
          if (
            ...
            progress = Math.max(
              ...
              Math.min(
                99,
                Math.round((100 / progressEvent.total) * progressEvent.loaded),
              ),
            );
          }
  ...
  getNodeByTagname(responseXML, tagname, attrName) {
    ...
    if (
      typeof node?.getElementsByTagNameNS !== "function" ||
      elements?.length === 0
    ) {
      return node;
    }
    ...
    if (typeof attrName !== "undefined") {
      node = Array.from(elements).find((element) => {
        return (
          typeof element?.getElementsByTagNameNS("*", attrName)[0] !==
          "undefined"
        );
      });
    } else {
  ...
  parseMinMaxDate(responseXML, attrName, dateParam) {
    ...
    Array.prototype.slice
      .call(responseXML.getElementsByTagName(nodeName))
      .forEach((value) => {
  ...
  parseResponseMinMax(
    ...
    if (!node) {
      if (typeof onerror === "function") {
        onerror(
          new Error(
            "InterfaceWfsExtern.parseResponseMinMax: The requested typename '" +
              typename +
              "' wasn't found.",
          ),
        );
    ...
    if (dateParam) {
      ...
      onerror(
        new Error(
          "InterfaceWfsExtern.parseResponseMinMax: The requested attrName '" +
            attrName +
            "' wasn't found.",
        ),
      );
  ...
  parseResponseUniqueValues(typename, attrName, responseXML, onsuccess) {
    ...
    if (elementCollections.length === 1) {
      ...
      elementCollections =
        [...elementCollections].find(
          (el) => el.tagName === "gml:featureMembers",
        )?.children ?? elementCollections;
    }
  ...
  getFilter(rules, searchInMapExtent, geometryName, filterGeometry) {
    ...
    rules.forEach((rule) => {
      if (
        rule?.format &&
        !this.isIso8601(rule?.format) &&
        this.isRangeOperator(rule?.operator)
      ) {
        console.error(
          "Time-related snippets (`date` and `dateRange`) can only be operated in `external` mode or as a fixed rule (`visible`: `false`) if their counterpart at the WFS service is in a correct time format (ISO8601: `YYYY-MM-DD`). Current format is `" +
            rule?.format +
            "`.",
        );
        return;
      ...
      args.push(
        this.getRuleFilter(
          ...
          rule?.operator,
          rule?.value,
          this.getLogicalHandlerByOperator(
            rule?.operator,
            this.isIso8601(rule?.format),
          ),
        ),
    ...
    if (typeof geometryName === "string" && isObject(filterGeometry)) {
      ...
      searchInMapExtent &&
      typeof geometryName === "string" &&
      typeof this.getCurrentExtent === "function"
    ) {
  ...
  isRangeOperator(operator) {
    return (
      this.isBetweenOperator(operator) || this.isIntersectsOperator(operator)
    );
  }
  /**

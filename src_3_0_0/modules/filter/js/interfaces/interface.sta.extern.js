...
} from "../../utils/fetchAllStaProperties.js";
...
export default class InterfaceStaExtern {
  ...
  getAttrTypes(service, onsuccess, onerror, filterId) {
    this.getUniqueValues(
      ...
      () => {
        onsuccess(
          this.getAttrTypesByAllFetchedProperties(
            this.allFetchedProperties[filterId],
            this.observationType[filterId],
            this.listOfResourceTypes,
          ),
        );
      },
  ...
  getUniqueValues(service, attrName, onsuccess, onerror, filterId) {
    if (isObject(this.allFetchedProperties[filterId])) {
      if (typeof onsuccess === "function") {
        const uniqueValue = getUniqueValuesFromFetchedFeatures(
          this.allFetchedProperties[filterId],
          attrName,
        );
    ...
    if (
      !Object.prototype.hasOwnProperty.call(this.allFetchedProperties, filterId)
    ) {
  ...
  filter(filterQuestion, onsuccess, onerror, axiosMock = false) {
    if (axiosMock) {
      onerror(
        new Error(
          "tbd: InterfaceStaExtern filter not implemented yet for testing",
        ),
      );
    } else {
   ...
   */
  getAttrTypesByAllFetchedProperties(
    properties,
    observationType,
    listOfResourceTypes,
  ) {
    ...
    Object.entries(properties).forEach(([attrName, values]) => {
      if (Array.isArray(values) && values.length) {
        if (
          Object.prototype.hasOwnProperty.call(listOfResourceTypes, attrName) &&
          Object.prototype.hasOwnProperty.call(
            listOfResourceTypes[attrName],
            observationType,
          )
        ) {

...
describe("src_3_0_0/modules/filter/interfaces/utils/interface.sta.extern.js", () => {
    ...
    it("should set expected variables in the instance", () => {
      const expected = {
        "@Datastreams.0.Observations.0.result": {
          "http://defs.opengis.net/elda-common/ogc-def/resource?uri=http://www.opengis.net/def/ogc-om/OM_CountObservation":
            "number",
          "http://defs.opengis.net/elda-common/ogc-def/resource?uri=http://www.opengis.net/def/observationType/OGC-OM/2.0/OM_CategoryObservation":
            "string",
        },
      };
  ...
  describe("getAttrTypesByAllFetchedProperties", () => {
    it("should return an empty object if anything but an object is given", () => {
      expect(
        interfaceStaExtern.getAttrTypesByAllFetchedProperties(null),
      ).to.deep.equal({});
      expect(
        interfaceStaExtern.getAttrTypesByAllFetchedProperties(undefined),
      ).to.deep.equal({});
      expect(
        interfaceStaExtern.getAttrTypesByAllFetchedProperties("string"),
      ).to.deep.equal({});
      expect(
        interfaceStaExtern.getAttrTypesByAllFetchedProperties(1234),
      ).to.deep.equal({});
      expect(
        interfaceStaExtern.getAttrTypesByAllFetchedProperties(true),
      ).to.deep.equal({});
      expect(
        interfaceStaExtern.getAttrTypesByAllFetchedProperties(false),
      ).to.deep.equal({});
      expect(
        interfaceStaExtern.getAttrTypesByAllFetchedProperties([]),
      ).to.deep.equal({});
    });
    it("should return empty object if the values of the keys from given object are not an array", () => {
      expect(
        interfaceStaExtern.getAttrTypesByAllFetchedProperties({ foo: "bar" }),
      ).to.deep.equal({});
      expect(
        interfaceStaExtern.getAttrTypesByAllFetchedProperties({ foo: 123 }),
      ).to.deep.equal({});
      expect(
        interfaceStaExtern.getAttrTypesByAllFetchedProperties({
          foo: undefined,
      ...
      expect(
        interfaceStaExtern.getAttrTypesByAllFetchedProperties({ foo: false }),
      ).to.deep.equal({});
      expect(
        interfaceStaExtern.getAttrTypesByAllFetchedProperties({ foo: true }),
      ).to.deep.equal({});
      expect(
        interfaceStaExtern.getAttrTypesByAllFetchedProperties({ foo: null }),
      ).to.deep.equal({});
      expect(
        interfaceStaExtern.getAttrTypesByAllFetchedProperties({ foo: {} }),
      ).to.deep.equal({});
    });
    ...
    it("should return an object with key value pair where value is the type of the key", () => {
      ...
      expect(
        interfaceStaExtern.getAttrTypesByAllFetchedProperties(obj, "", {}),
      ).to.deep.equal(expected);
    });
  });
  describe("getMinMaxFromUniqueValue", () => {
    it("should return an empty object if anything but an array is given", () => {
      expect(
        interfaceStaExtern.getMinMaxFromUniqueValue(undefined),
      ).to.deep.equal({});
      ...
      expect(
        interfaceStaExtern.getMinMaxFromUniqueValue("string"),
      ).to.deep.equal({});
    });
    it("should return the min if min is set", () => {
      const list = [20, 9, 30, 1],
        expected = {
          min: 1,
        };

      expect(
        interfaceStaExtern.getMinMaxFromUniqueValue(list, true, false),
      ).to.deep.equal(expected);
    });
    it("should return the max if max is set", () => {
      ...
      expect(
        interfaceStaExtern.getMinMaxFromUniqueValue(list, false, true),
      ).to.deep.equal(expected);
    });
    it("should return the min and max if min and max are set", () => {
      ...
      expect(
        interfaceStaExtern.getMinMaxFromUniqueValue(list, true, true),
      ).to.deep.equal(expected);
    });
  });
});

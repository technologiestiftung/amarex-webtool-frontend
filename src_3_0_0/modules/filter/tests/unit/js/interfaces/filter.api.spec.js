...
describe("src_3_0_0/modules/filter/interfaces/filter.api.js", () => {
  describe("constructor", () => {
    ...
    it("should set the correct interfaces", () => {
      new FilterApi(0);
      expect(FilterApi.interfaces.wfsIntern).to.be.an.instanceOf(
        InterfaceWfsIntern,
      );
      expect(FilterApi.interfaces.wfsExtern).to.be.an.instanceOf(
        InterfaceWfsExtern,
      );
      expect(FilterApi.interfaces.oafIntern).to.be.an.instanceOf(
        InterfaceOafIntern,
      );
      expect(FilterApi.interfaces.oafExtern).to.be.an.instanceOf(
        InterfaceOafExtern,
      );
      expect(FilterApi.interfaces.geojsonIntern).to.be.an.instanceOf(
        InterfaceGeojsonIntern,
      );
      expect(FilterApi.interfaces.geojsonExtern).to.be.an.instanceOf(
        InterfaceGeojsonExtern,
      );
      expect(FilterApi.interfaces.staIntern).to.be.an.instanceOf(
        InterfaceStaIntern,
      );
      expect(FilterApi.interfaces.staExtern).to.be.an.instanceOf(
        InterfaceStaExtern,
      );
    });
  });
  describe("methods", () => {
    describe("setServiceByLayerModel", () => {
      ...
      it("should call error function if typ is unknown", () => {
        const filterApi = new FilterApi(0),
          ...
          expected = new Error(
            "FilterApi.setServiceByLayerModel: Unknown layer type foo",
          );
    ...
    describe("getAttrTypes", () => {
      ...
      it("should push object with onsuccess and onerror if waitinglist with key is already an array", () => {
        ...
        expect(FilterApi.waitingList["fow.bar"]).to.deep.equal([
          { onsuccess, onerror },
        ]);
        sinon.restore();
      });

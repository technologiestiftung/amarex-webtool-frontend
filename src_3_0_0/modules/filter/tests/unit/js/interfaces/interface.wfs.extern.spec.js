...
describe("src_3_0_0/modules/filter/interfaces/utils/interface.wfs.extern.js", () => {
  ...
  describe("constructor", () => {
    it("should set a variable axiosCancelTokenSources as object at the instance", () => {
      expect(interfaceWfsExtern.axiosCancelTokenSources).to.be.an("object").and
        .to.be.empty;
    });
  });
  ...
  describe("getLogicalHandlerByOperator", () => {
    ...
    it("should return the betweenFilter if the operator is BETWEEN and this is not a temporal operator", () => {
      const handler = interfaceWfsExtern.getLogicalHandlerByOperator(
        "BETWEEN",
        false,
      );
  ...
  describe("getOrFilter", () => {
    it("should return the result of the logical handler if the given array has a length of 1", () => {
      ...
      expect(
        interfaceWfsExtern.getOrFilter(
          "attrName",
          ["firstEntry"],
          (attrName, entry) => {
    ...
    it("should return the orFilter instance with the given conditions", () => {
      const result = interfaceWfsExtern.getOrFilter(
        "attrName",
  ...
  describe("getRuleFilter", () => {
    it("should return the result of the logical handler if value is not an array", () => {
      expect(
        interfaceWfsExtern.getRuleFilter(
          "attrName",
          "operator",
          "value",
          (attrName, value) => {
    ...
    it("should return an or condition if value is an array and operator is not a range operator", () => {
      const result = interfaceWfsExtern.getRuleFilter(
        "attrName",
        "operator",
  ...
  describe("getAttrTypes", () => {
    it("should call an error if the given service is not an object", () => {
      ...
      interfaceWfsExtern.getAttrTypes(
        undefined,
        "onsuccess",
        () => errorCount++,
      );
  ...
  describe("parseMinMaxDate", () => {
    it("should return undefined if there are no response xml", () => {
      expect(
        interfaceWfsExtern.parseMinMaxDate(
    ...
    it("should return minimum date if the dateParam is min", () => {
      expect(
        interfaceWfsExtern.parseMinMaxDate(
          new DOMParser().parseFromString(responseXML[0], "text/xml"),
          "baubeginn",
          "min",
        ),
      ).to.equal("01.01.2020");
  ...
  describe("getSortedDate", () => {
    ...
    it("should return sorted ascending value if the parameter is not an empty array with date", () => {
      expect(
        interfaceWfsExtern.getSortedDate([
          "07.03.2022",
          "08.03.2022",
          "04.01.2021",
          "14.03.2022",
        ]),
      ).to.deep.equal(["04.01.2021", "07.03.2022", "08.03.2022", "14.03.2022"]);
      expect(
        interfaceWfsExtern.getSortedDate([
          "07.03.2022",
          "08.03.2022",
  ...
  describe("getUniqueValuesFromFeatures", () => {
    it("should return an empty array if anything but an array is given", () => {
      expect(interfaceWfsExtern.getUniqueValuesFromFeatures()).to.be.an("array")
        .and.to.be.empty;
      expect(interfaceWfsExtern.getUniqueValuesFromFeatures(null)).to.be.an(
        "array",
      ).and.to.be.empty;
      expect(interfaceWfsExtern.getUniqueValuesFromFeatures({})).to.be.an(
        "array",
      ).and.to.be.empty;
      expect(interfaceWfsExtern.getUniqueValuesFromFeatures(1234)).to.be.an(
        "array",
      ).and.to.be.empty;
      expect(interfaceWfsExtern.getUniqueValuesFromFeatures("string")).to.be.an(
        "array",
      ).and.to.be.empty;
      expect(interfaceWfsExtern.getUniqueValuesFromFeatures(false)).to.be.an(
        "array",
      ).and.to.be.empty;
      expect(interfaceWfsExtern.getUniqueValuesFromFeatures(true)).to.be.an(
        "array",
      ).and.to.be.empty;
    });
    it("should return an empty array if anything but an string is given as second argument", () => {
      ...
      expect(
        interfaceWfsExtern.getUniqueValuesFromFeatures([{}], 1234),
      ).to.be.an("array").and.to.be.empty;
      expect(
        interfaceWfsExtern.getUniqueValuesFromFeatures([{}]),
        true,
      ).to.be.an("array").and.to.be.empty;
      expect(
        interfaceWfsExtern.getUniqueValuesFromFeatures([{}]),
    ...
    it("should return an empty array if first param is an empty array", () => {
      expect(
        interfaceWfsExtern.getUniqueValuesFromFeatures([], "string"),
      ).to.be.an("array").and.to.be.empty;
    });
    it("should return an empty array if the objects of the array from the first argument dont have a get function", () => {
      ...
      expect(
        interfaceWfsExtern.getUniqueValuesFromFeatures(features, "attrName"),
      ).to.be.an("array").and.to.be.empty;
    });
    it("should return a list of values based on the values from the features", () => {
      ...
      expect(
        interfaceWfsExtern.getUniqueValuesFromFeatures(features, ""),
  ...
  describe("getNodeByTagname", () => {
    it("should return a node by the given document and tagname", () => {
      const node = interfaceWfsExtern.getNodeByTagname(
        new DOMParser().parseFromString(responseXML[1], "text/xml"),
        "WKA",
      );
  ...
  describe("intersectsGeometryFilter", () => {
    it("should return an 'Or filter' if the given geometry collection has more than one geometry", () => {
      const polygonFirst = new Polygon([
        ...
        node = interfaceWfsExtern.intersectsGeometryFilter(
          "foreverYoung",
          geometryCollection,
        );

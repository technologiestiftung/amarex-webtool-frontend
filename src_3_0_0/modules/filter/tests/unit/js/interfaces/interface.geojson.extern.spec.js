...
describe("src_3_0_0/modules/filter/interfaces/utils/interface.geojson.extern.js", () => {
  ...
  describe("handleGeojsonRequest", () => {
    it("should call an error if service is anything but an object", async () => {
      await interfaceGeojsonExtern.handleGeojsonRequest(
        undefined,
        dummyCalls.onsuccess,
        dummyCalls.onerror,
        axiosMock,
      );
    ...
    it("should call axios and start the waitinglist if there is no data known for the given service", async () => {
      ...
      expect(interfaceGeojsonExtern.waitinglist[hashKey]).to.be.an("array").and
        .to.be.empty;
      expect(interfaceGeojsonExtern.cache[hashKey]).to.deep.equal({
        payload: "payload",
      });
    ...
    it("should add onsuccess to the waitinglist if waitinglist is an array already", async () => {
      ...
      expect(interfaceGeojsonExtern.waitinglist[hashKey]).to.be.an("array").and
        .not.to.be.empty;
      expect(interfaceGeojsonExtern.waitinglist[hashKey][0]).to.be.a(
        "function",
      );
    });
  });
  describe("getAttrTypes", () => {
    it("should call an error if the received geojson is not an object", async () => {
      ...
      await interfaceGeojsonExtern.getAttrTypes(
        service,
        dummyCalls.onsuccess,
        dummyCalls.onerror,
        axiosMock,
  ...
  describe("getMinMax", () => {
    it("should call an error if the received geojson is not an object", async () => {
      ...
      geojsonPayload = undefined;
      await interfaceGeojsonExtern.getMinMax(
        service,
        "attrName",
        dummyCalls.onsuccess,
        dummyCalls.onerror,
  ...
  describe("getUniqueValues", () => {
    it("should call an error if the received geojson is not an object", async () => {
      ...
      geojsonPayload = undefined;
      await interfaceGeojsonExtern.getUniqueValues(
        service,
        "attrName",
        dummyCalls.onsuccess,
        dummyCalls.onerror,

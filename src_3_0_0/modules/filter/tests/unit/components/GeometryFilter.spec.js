...
describe("src_3_0_0/modules/filter/components/GeometryFilter.vue", () => {
  ...
  describe("User Interactions", () => {
    ...
    it("should render the correct value in the dropdown if user changes it", async () => {
      ...
      expect(wrapper.find("option:checked").text()).to.be.equal(
        "common:modules.filter.geometryFilter.geometries.rectangle",
      );
    });
  ...
  describe("methods", () => {
    ...
    describe("prepareAdditionalGeometries", () => {
      it("should return an empty array", () => {
        expect(wrapper.vm.prepareAdditionalGeometries(false)).to.be.an("array")
          .that.is.empty;
      });
      ...
      it("should return the correct name and type of the additonal geometry", () => {
        const feature = new Feature({
          ...
          results =
            wrapper.vm.prepareAdditionalGeometries(additionalGeometries);
    ...
    it("should return the given geometry", () => {
      wrapper.vm.reset();
      expect(wrapper.emitted()).to.have.all.keys(
    ...
    it("should return the given geometry", () => {
      wrapper.vm.reset();
      expect(wrapper.emitted().updateGeometryFeature[0]).to.deep.equal([
        undefined,
      ]);
    });
    it("should return the given geometry", () => {
      ...
      wrapper.vm.reset();
      expect(wrapper.emitted().updateGeometrySelectorOptions[0]).to.deep.equal([
        expectedSelectorOptions,
      ]);
    });
  });
  ...
  describe("update", () => {
    ...
    it("should emit the correct geometry", () => {
      wrapper.vm.update(feature, "LineString", feature.getGeometry());
      expect(wrapper.emitted().updateFilterGeometry[0]).to.deep.equal([
        feature.getGeometry(),
      ]);
    });

...
describe("src_3_0_0/modules/filter/components/SnippetDropdown.vue", () => {
  ...
  describe("constructor", () => {
    ...
    it("should render hidden if visible is false", () => {
      ...
      expect(
        wrapper.find(".snippetDropdownContainer").element.style._values.display,
      ).to.be.equal("none");
    });
    ...
    it("should only set the dropdown values based on the given values", async () => {
      ...
      await wrapper.vm.$nextTick();
      await wrapper.vm.$nextTick();
      expect(wrapper.vm.dropdownValue).to.deep.equal([
        "Altona",
        "Eimsbüttel",
        "Bergedorf",
      ]);
  ...
  describe("display list", () => {
    ...
    it("should render a list with checkbox", async () => {
      ...
      await wrapper.vm.$nextTick();
      expect(wrapper.find(".snippetListContainer").exists()).to.be.true;
      expect(wrapper.find(".snippetListContainer .checkbox").exists()).to.be
        .true;
    });
  ...
  describe("methods", () => {
    describe("initializeIcons", () => {
      ...
      beforeEach(() => {
        ...
        sinon
          .stub(openlayerFunctions, "getLayerByLayerId")
          .returns(layerConfig);
        sinon.stub(getIconListFromLegend, "getStyleModel").returns({});
        returnLegendByStyleIdSpy = sinon.spy(
          createStyle,
      ...
      it("initializeIcons with not visible WMS-layer", async () => {
        ...
        expect(returnLegendByStyleIdSpy.firstCall.args[0]).to.be.equals(
          "layerId",
        );
      });
    });
    describe("getPrecheckedExistingInValue", () => {
      ...
      it("should return false if anything but an array is given as second parameter", () => {
        expect(wrapper.vm.getPrecheckedExistingInValue([], undefined)).to.be
          .false;
        expect(wrapper.vm.getPrecheckedExistingInValue([], null)).to.be.false;
        expect(wrapper.vm.getPrecheckedExistingInValue([], 1234)).to.be.false;
        expect(wrapper.vm.getPrecheckedExistingInValue([], "string")).to.be
          .false;
        expect(wrapper.vm.getPrecheckedExistingInValue([], true)).to.be.false;
        expect(wrapper.vm.getPrecheckedExistingInValue([], false)).to.be.false;
        expect(wrapper.vm.getPrecheckedExistingInValue([], {})).to.be.false;
      ...
      it("should return entries of prechecked only if exists in dropdownValue", () => {
        ...
        expect(
          wrapper.vm.getPrecheckedExistingInValue(prechecked, dropdownValue),
    ...
    describe("getInitialDropdownSelected", () => {
      it("should return an empty array if dropdownValue is not an array", () => {
        expect(
          wrapper.vm.getInitialDropdownSelected("prechecked", undefined),
        ).to.be.an("array").that.is.empty;
        expect(
          wrapper.vm.getInitialDropdownSelected("prechecked", null),
        ).to.be.an("array").that.is.empty;
        expect(
          wrapper.vm.getInitialDropdownSelected("prechecked", 1234),
        ).to.be.an("array").that.is.empty;
        expect(
          wrapper.vm.getInitialDropdownSelected("prechecked", "string"),
        ).to.be.an("array").that.is.empty;
        expect(
          wrapper.vm.getInitialDropdownSelected("prechecked", true),
        ).to.be.an("array").that.is.empty;
        expect(
          wrapper.vm.getInitialDropdownSelected("prechecked", false),
        ).to.be.an("array").that.is.empty;
        expect(
          wrapper.vm.getInitialDropdownSelected("prechecked", {}),
        ).to.be.an("array").that.is.empty;
      });
      it("should return prechecked if prechecked is an array", () => {
        expect(
          wrapper.vm.getInitialDropdownSelected(["prechecked"], ["prechecked"]),
        ).to.deep.equal(["prechecked"]);
      });
      it("should return dropdownValue if prechecked is 'all' and multiselect is set", () => {
        expect(
          wrapper.vm.getInitialDropdownSelected("all", ["prechecked"], true),
        ).to.deep.equal(["prechecked"]);
      });
      it("should return an empty array if prechecked is 'all' and multiselect is not set", () => {
        expect(
          wrapper.vm.getInitialDropdownSelected("all", ["prechecked"], false),
        ).to.be.an("array").that.is.empty;
      });
      it("should return an empty array if prechecked is neither 'all' nor an array", () => {
        expect(wrapper.vm.getInitialDropdownSelected(undefined, [])).to.be.an(
          "array",
        ).that.is.empty;
        expect(wrapper.vm.getInitialDropdownSelected(null, [])).to.be.an(
          "array",
        ).that.is.empty;
        expect(wrapper.vm.getInitialDropdownSelected(1234, [])).to.be.an(
          "array",
        ).that.is.empty;
        expect(wrapper.vm.getInitialDropdownSelected("string", [])).to.be.an(
          "array",
        ).that.is.empty;
        expect(wrapper.vm.getInitialDropdownSelected(true, [])).to.be.an(
          "array",
        ).that.is.empty;
        expect(wrapper.vm.getInitialDropdownSelected(false, [])).to.be.an(
          "array",
        ).that.is.empty;
        expect(wrapper.vm.getInitialDropdownSelected({}, [])).to.be.an("array")
          .that.is.empty;
      });
    });
  });

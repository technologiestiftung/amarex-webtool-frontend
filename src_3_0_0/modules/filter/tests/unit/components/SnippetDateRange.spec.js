  ...
  describe("mounted", () => {
    describe("api", () => {
      ...
      it("should call the api once if attrName is a string", async () => {
        ...
        wrapper = shallowMount(SnippetDateRange, {
          propsData: { api, attrName: "attrName" },
        });
        await wrapper.vm.$nextTick();
        await wrapper.vm.$nextTick();
      ...
      it("should call the api once if attrName is an array", async () => {
        ...
        wrapper = shallowMount(SnippetDateRange, {
          propsData: { api, attrName: ["attrNameA", "attrNameB"] },
        });
        await wrapper.vm.$nextTick();
        await wrapper.vm.$nextTick();
        expect(spy.calledOnce).to.be.true;
      ...
      it("should call the api if operatorForAttrName is 'OR' and attrName is an array", async () => {
        ...
        wrapper = shallowMount(SnippetDateRange, {
          propsData: {
            api,
            attrName: ["attrNameA", "attrNameB", "attrNameC"],
    ...
    describe("template", () => {
      it("should render itself", () => {
        ...
        expect(wrapper.find("div").classes("snippetDateRangeContainer")).to.be
          .true;
      });
      describe("titleWrapper", () => {
        ...
        it("should render the attrName as title if title is true", () => {
          ...
          expect(wrapper.find(".titleWrapper").find(".title").text()).to.equal(
            "attrName",
          );
        });
        ...
        it("should render info if info is set", () => {
          ...
          expect(wrapper.find(".titleWrapper").find(".info").exists()).to.be
            .true;
        });
      });
      describe("datepickerWrapper", () => {
        ...
        describe("min and max", () => {
          it("should set min and max date for datepicker based on api response", async () => {
            const api = {
              getUniqueValues: (attrName, onsuccess) =>
                onsuccess([
                  "20.08.2022",
                  "30.08.2022",
                  "26.08.2022",
        ...
        describe("prechecked", () => {
          it("should set value to both borders for datepicker if no prechecked is given", async () => {
            ...
            await wrapper.vm.$nextTick();
            expect(
              wrapper.find(".datepickerWrapper").find(".from").find("input")
                .element.value,
            ).to.equal("2022-08-10");
            expect(
              wrapper.find(".datepickerWrapper").find(".until").find("input")
                .element.value,
            ).to.equal("2022-08-30");
          });
      ...
      describe("sliderWrapper", () => {
        ...
        describe("min and max", () => {
          it("should set min and max value for slider based on api response", async () => {
            ...
            await wrapper.vm.$nextTick();
            expect(
              wrapper.find(".sliderWrapper").find(".from").attributes("min"),
            ).to.equal("0");
            expect(
              wrapper.find(".sliderWrapper").find(".from").attributes("max"),
            ).to.equal("4");
            expect(
              wrapper.find(".sliderWrapper").find(".until").attributes("min"),
            ).to.equal("0");
            expect(
              wrapper.find(".sliderWrapper").find(".until").attributes("max"),
            ).to.equal("4");
          });
        ...
        describe("prechecked", () => {
          it("should set value to both borders for slider if no prechecked is given", async () => {
            ...
            wrapper = await shallowMount(SnippetDateRange, {
              propsData: {
                api,
                attrName: "attrName",
                format: "DD.MM.YYYY",
            ...
            await wrapper.vm.$nextTick();
            expect(
              wrapper.find(".sliderWrapper").find(".from").element.value,
            ).to.equal("0");
            expect(
              wrapper.find(".sliderWrapper").find(".until").element.value,
            ).to.equal("4");
          });
  ...
  describe("methods", () => {
    ...
    describe("deleteCurrentRule", () => {
      it("should emit deleteRule with its snippetId", () => {
        ...
        expect(wrapper.emitted("deleteRule"))
          .to.be.an("array")
          .and.to.have.lengthOf(1);
        expect(wrapper.emitted("deleteRule")[0])
          .to.be.an("array")
          .and.to.have.lengthOf(1);
    ...
    describe("emitCurrentRule", () => {
      it("should emit changeRule function with the expected values", async () => {
        ...
        expect(wrapper.emitted("changeRule"))
          .to.be.an("array")
          .and.to.have.lengthOf(1);
        expect(wrapper.emitted("changeRule")[0])
          .to.be.an("array")
          .and.to.have.lengthOf(1);
    ...
    describe("isPrecheckedValid", () => {
      ...
      it("should return true if prechecked is valid", () => {
        ...
        expect(wrapper.vm.isPrecheckedValid(["11.08.2022", "2022/29/08"])).to.be
          .true;
      });
    });
    ...
    describe("addListToUnixAssoc", () => {
      ...
      it("should return false if maxValid is not boolean", () => {
        wrapper = shallowMount(SnippetDateRange, {});
        expect(wrapper.vm.addListToUnixAssoc([], "format", true, undefined)).to
          .be.false;
        expect(wrapper.vm.addListToUnixAssoc([], "format", true, null)).to.be
          .false;
        expect(wrapper.vm.addListToUnixAssoc([], "format", true, 1234)).to.be
          .false;
        expect(wrapper.vm.addListToUnixAssoc([], "format", true, "string")).to
          .be.false;
        expect(wrapper.vm.addListToUnixAssoc([], "format", true, {})).to.be
          .false;
        expect(wrapper.vm.addListToUnixAssoc([], "format", true, [])).to.be
          .false;
      });
      it("should return false if mindayjs  is not recognized as dayjs  object", () => {
        wrapper = shallowMount(SnippetDateRange, {});
        expect(
          wrapper.vm.addListToUnixAssoc([], "format", true, true, undefined),
        ).to.be.false;
        expect(wrapper.vm.addListToUnixAssoc([], "format", true, true, null)).to
          .be.false;
        expect(wrapper.vm.addListToUnixAssoc([], "format", true, true, 1234)).to
          .be.false;
        expect(
          wrapper.vm.addListToUnixAssoc([], "format", true, true, "string"),
        ).to.be.false;
        expect(wrapper.vm.addListToUnixAssoc([], "format", true, true, true)).to
          .be.false;
        expect(wrapper.vm.addListToUnixAssoc([], "format", true, true, false))
          .to.be.false;
        expect(wrapper.vm.addListToUnixAssoc([], "format", true, true, {})).to
          .be.false;
        expect(wrapper.vm.addListToUnixAssoc([], "format", true, true, [])).to
          .be.false;
      });
      it("should return false if maxdayjs  is not recognized as dayjs object", () => {
        ...
        expect(
          wrapper.vm.addListToUnixAssoc([], "format", true, true, mindayjs, {}),
        ).to.be.false;
        expect(
          wrapper.vm.addListToUnixAssoc([], "format", true, true, mindayjs, []),
        ).to.be.false;
    ...
    describe("getInitialDateReference", () => {
      it("should merge both given lists into one, sorted by time, should remove doublings", () => {
        ...
        expect(
          wrapper.vm.getInitialDateReference(
            [
              "20.08.2022",
              "30.08.2022",
              "26.08.2022",
            ...
            ["16.08.2022", "22.08.2022", "26.08.2022"],
          ),
        ).to.deep.equal([
          "1950-08-10",
          "2022-08-16",
    ...
    describe("getSliderIdxCloseToUntilDate", () => {
      ...
      it("should return the next downward index", async () => {
        ...
        await wrapper.vm.$nextTick();
        await wrapper.vm.$nextTick();
        expect(wrapper.vm.getSliderIdxCloseToUntilDate("2022-08-29")).to.equal(
          3,
        );
      });
      it("should return the index equal to given untilDate", async () => {
        ...
        await wrapper.vm.$nextTick();
        await wrapper.vm.$nextTick();
        expect(wrapper.vm.getSliderIdxCloseToUntilDate("2022-08-26")).to.equal(
          3,
        );
      });
    });
    describe("getSliderIdxCloseToFromDate", () => {
      ...
      it("should return the next upward index", async () => {
        ...
        await wrapper.vm.$nextTick();
        await wrapper.vm.$nextTick();
        expect(wrapper.vm.getSliderIdxCloseToFromDate("2022-08-11")).to.equal(
          1,
        );
      });
      it("should return the index equal to given fromDate", async () => {
        ...
        await wrapper.vm.$nextTick();
        await wrapper.vm.$nextTick();
        expect(wrapper.vm.getSliderIdxCloseToFromDate("2022-08-16")).to.equal(
          1,
        );
      });
    });

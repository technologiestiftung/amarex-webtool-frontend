...
describe("src_3_0_0/modules/generalFilter/components/SnippetDate.vue", () => {
  describe("constructor", () => {
    ...
    it("should render correctly with prechecked as set value", async () => {
      ...
      await wrapper.vm.$nextTick();
      expect(wrapper.find(".snippetDate").element.value).to.be.equal(
        "2021-12-24",
      );
    });
    it("should render hidden if visible is false", () => {
      ...
      expect(
        wrapper.find(".snippetDateContainer").element.style._values.display,
      ).to.be.equal("none");
    });

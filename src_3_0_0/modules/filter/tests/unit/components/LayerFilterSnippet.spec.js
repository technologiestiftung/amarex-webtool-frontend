...
describe("src_3_0_0/modules/filter/components/LayerFilterSnippet.vue", () => {
  ...
  describe("hasThisSnippetTheExpectedType", () => {
    it("should return false if the given snippet has not the expected type", () => {
      ...
      expect(
        wrapper.vm.hasThisSnippetTheExpectedType(
          { type: "anything" },
          "something",
        ),
      ).to.be.false;
  ...
  describe("renderCheckboxSearchInMapExtent", () => {
    it("Should render the checkbox component correctly", async () => {
      ...
      expect(wrapper.findComponent(SnippetCheckboxFilterInMapExtent).exists())
        .to.be.true;
    });
  });
  it("should call handleActiveStrategy if strategy is active", async () => {
    const spyHandleActiveStrategy = sinon.spy(
      wrapper.vm,
  ...
  describe("getTagTitle", () => {
    it("should return value if there is no tagTitle defined", () => {
      ...
      expect(wrapper.vm.getTagTitle({ value: undefined })).to.equal(
        "undefined",
      );
    ...
    it("should return tagTitle if there is tagTitle defined", () => {
      expect(
        wrapper.vm.getTagTitle({ value: "title", tagTitle: "tagTitle" }),
      ).to.equal("tagTitle");
      expect(
        wrapper.vm.getTagTitle({ value: "title", tagTitle: false }),
      ).to.equal("false");
      expect(wrapper.vm.getTagTitle({ value: "title", tagTitle: 0 })).to.equal(
        "0",
      );
      expect(
        wrapper.vm.getTagTitle({ value: "title", tagTitle: null }),
      ).to.equal("null");
    });
  });
  ...
  describe("labelFilterButton", () => {
    it("should render correct name for label filter button", async () => {
      ...
      expect(wrapper.find("button").text()).to.equal(
        "common:modules.filter.filterButton",
      );
    });
  });
  ...
  it("should render amount of filtered items", async () => {
    ...
    expect(wrapper.find(".filter-result").text()).contain(
      "common:modules.filter.filterResult.unit",
    );
    ...
    describe("setSnippetValueByState", async () => {
      ...
      it("should not set prechecked value if given structure is not a rule", () => {
        const isRuleStub = sinon
            .stub(LayerFilterSnippet, "isRule")
            .returns(false),
          noRule = [
            {
              something: "something",
  ...
  describe("checkOutOfZoomLevel", () => {
    it("should set the variable outOfZoom false", () => {
      expect(wrapper.vm.checkOutOfZoomLevel(undefined, undefined, undefined)).to
        .be.false;
      expect(wrapper.vm.checkOutOfZoomLevel(undefined, undefined, 17)).to.be
        .false;
  ...
  describe("deleteAllRules", () => {
    it("should emit the deleteAllRules function", async () => {
      ...
      await wrapper.vm.$nextTick();
      expect(wrapper.emitted().deleteAllRules)
        .to.be.an("array")
        .with.lengthOf(1);
    });
    it("should call handleActiveStrategy if strategy is active", async () => {

...
describe("src_3_0_0/modules/filter/utils/describeFeatureTypeWFS.js", () => {
  ...
  describe("describeFeatureTypeWFS", () => {
    ...
    it("should handle a specific error if the first child of the responseXML is not valid", async () => {
      ...
      expect(lastError).to.be.an.instanceof(Error);
      expect(lastError.message).to.equal(
        "The response from the server has an invalid schema.",
      );
    });
    it("should handle a specific error if the first child of the responseXML has invalid children", async () => {
      ...
      expect(lastError).to.be.an.instanceof(Error);
      expect(lastError.message).to.equal(
        "The response from the server has an empty schema.",
      );
    });
                        ...
                        children: [
                          {
                            tagName: "element",
                            getAttribute: (key) =>
                              key === "exceptionCode" ? false : "anyTypename",
                          },
                        ],
                      },
                    ],
                        ...
                        children: [
                          {
                            ...
                            hasAttribute: () => false,
                            getAttribute: (key) =>
                              key === "exceptionCode" ? false : "typename",
                            getElementsByTagName: () => [
                              {
                                getAttribute: (key) =>
                                  key === "name" ? "nameA" : "typeA",
                              },
                              {
                                getAttribute: (key) =>
                                  key === "name" ? "nameB" : "double",
                              },
                              {
                                getAttribute: (key) =>
                                  key === "name" ? "nameC" : "boolean",
                              },
                              {
                                getAttribute: (key) =>
                                  key === "name" ? "nameD" : "string",
                              },
                            ],
                          },
                        ],

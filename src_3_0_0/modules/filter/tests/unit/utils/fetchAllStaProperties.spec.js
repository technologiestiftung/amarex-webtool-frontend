...
} from "../../../utils/fetchAllStaProperties";
...
describe("src_3_0_0/modules/filter/utils/fetchAllStaProperties.js", () => {
  describe("getUniqueValuesFromFetchedFeatures", () => {
    it("should return empty array if anything but object is given", () => {
      expect(getUniqueValuesFromFetchedFeatures([])).to.be.an("array").that.is
        .empty;
      expect(getUniqueValuesFromFetchedFeatures(null)).to.be.an("array").that.is
        .empty;
      expect(getUniqueValuesFromFetchedFeatures(true)).to.be.an("array").that.is
        .empty;
      expect(getUniqueValuesFromFetchedFeatures(false)).to.be.an("array").that
        .is.empty;
      expect(getUniqueValuesFromFetchedFeatures("string")).to.be.an("array")
        .that.is.empty;
      expect(getUniqueValuesFromFetchedFeatures(1234)).to.be.an("array").that.is
        .empty;
      expect(getUniqueValuesFromFetchedFeatures(undefined)).to.be.an("array")
        .that.is.empty;
    });
    it("should return an empty array if the string is not included in the given object", () => {
      ...
      expect(getUniqueValuesFromFetchedFeatures(obj, "bar")).to.be.an("array")
        .that.is.empty;
    });
  ...
  describe("getFilterableProperties", () => {
    ...
    it("should return thing properties and datastream properties for 2 datastreams and identical values", () => {
      expect(
        getFilterableProperties(dataTwoThingTwoDatastreamsIdenticalValues),
      ).to.deep.equal({
        foo: {
          bar: true,
        },
    ...
    it("should return thing properties and datastream properties for 2 datastreams and Datastreams", () => {
      expect(
        getFilterableProperties(dataTwoThingTwoDatastreamsObservations),
      ).to.deep.equal({
        foo: {
          bar: true,
          bar2: true,
    ...
    it("should return  properties and 1st observation if entity has observations", () => {
      expect(
        getFilterableProperties(dataOneDatastreamWithPropsAndObservation),
      ).to.deep.equal({
        foo: {
          bar: true,
        },

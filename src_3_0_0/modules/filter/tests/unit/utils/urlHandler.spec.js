...
describe("src_3_0_0/modules/filter/utils/mapHandler.js", () => {
  ...
  describe("getParamsFromState", () => {
    it("should return an empty object if first param is not an object", () => {
      expect(urlHandler.getParamsFromState(undefined)).to.be.an("object").that
        .is.empty;
      expect(urlHandler.getParamsFromState(null)).to.be.an("object").that.is
        .empty;
      expect(urlHandler.getParamsFromState([])).to.be.an("object").that.is
        .empty;
      expect(urlHandler.getParamsFromState(true)).to.be.an("object").that.is
        .empty;
      expect(urlHandler.getParamsFromState(false)).to.be.an("object").that.is
        .empty;
      expect(urlHandler.getParamsFromState(1234)).to.be.an("object").that.is
        .empty;
      expect(urlHandler.getParamsFromState("string")).to.be.an("object").that.is
        .empty;
    });
    it("should return an empty object if second param is not an array", () => {
      expect(urlHandler.getParamsFromState({}, undefined)).to.be.an("object")
        .that.is.empty;
      expect(urlHandler.getParamsFromState({}, null)).to.be.an("object").that.is
        .empty;
      expect(urlHandler.getParamsFromState({}, {})).to.be.an("object").that.is
      ...
      expect(urlHandler.getParamsFromState({}, true)).to.be.an("object").that.is
        .empty;
      expect(urlHandler.getParamsFromState({}, false)).to.be.an("object").that
        .is.empty;
      expect(urlHandler.getParamsFromState({}, "string")).to.be.an("object")
        .that.is.empty;
      expect(urlHandler.getParamsFromState({}, 1234)).to.be.an("object").that.is
        .empty;
    });
    it("should return an object with matching attributes", () => {
      ...
      expect(
        urlHandler.getParamsFromState(stateObject, neededParams),
    ...
    it("should return an empty object if no matches found", () => {
      ...
      expect(urlHandler.getParamsFromState(stateObject, neededParams)).to.be.an(
        "object",
      ).that.is.empty;
    });
  });
  ...
  describe("transformOldUrl", () => {
    ...
    it("should return the basic template without any data if no matching filter is found", () => {
      const matchingFilterStub = sinon.stub(
          urlHandler,
          "getMatchingFilterFromGroupsOrLayers",
        ),
        expected = {
          rulesOfFilters: [],
          selectedAccordions: [],
  ...
  describe("getMatchingFilterFromGroupsOrLayers", () => {
    it("should return template object without changed informations", () => {
      ...
      expect(urlHandler.getMatchingFilterFromGroupsOrLayers()).to.deep.equal(
        expected,
      );
    });
    it("should return an object with informations if in groups found", () => {
      ...
      expect(
        urlHandler.getMatchingFilterFromGroupsOrLayers({ name: "foo" }, [
          { layers: [{ title: "foo" }] },
        ]),
      ).to.deep.equal(expected);
    });
    it("should return an object with informations if in layers found", () => {
      ...
      expect(
        urlHandler.getMatchingFilterFromGroupsOrLayers(
          { name: "foo" },
          undefined,
          [{ title: "foo" }],
        ),

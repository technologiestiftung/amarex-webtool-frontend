...
describe("src_3_0_0/modules/filter/utils/getOafAttributeTypes.js", () => {
  ...
  describe("getAttrTypeFromSchema", () => {
    it("should return an empty object if anything but an oaf schema is given", () => {
      expect(getAttrTypeFromSchema(undefined)).to.be.an("object").and.to.be
        .empty;
      ...
      expect(getAttrTypeFromSchema("string")).to.be.an("object").and.to.be
        .empty;
      ...
      expect(getAttrTypeFromSchema({})).to.be.an("object").and.to.be.empty;
      expect(getAttrTypeFromSchema({ name: true })).to.be.an("object").and.to.be
        .empty;
      expect(getAttrTypeFromSchema({ name: true, schema: true })).to.be.an(
        "object",
      ).and.to.be.empty;
      expect(
        getAttrTypeFromSchema({ name: "limit", schema: { type: true } }),
      ).to.be.an("object").and.to.be.empty;
    });
    it("should return a key value pair if an oaf schema is given", () => {
      expect(
        getAttrTypeFromSchema({ name: "name", schema: { type: "type" } }),
      ).to.deep.equal({ name: "type" });
    });
  });
  describe("getAttrTypeFromRef", () => {
    it("should return the oaf schema as key value pair", () => {
      const data = {
          a: { b: { c: { name: "name", schema: { type: "type" } } } },
        },
        path = "#/a/b/c";
  ...
  describe("getAttrTypesFromProperties", () => {
    it("should return an empty object if anything but an array is given", () => {
      expect(getAttrTypesFromProperties(undefined)).to.be.an("object").and.to.be
        .empty;
      expect(getAttrTypesFromProperties(null)).to.be.an("object").and.to.be
        .empty;
      expect(getAttrTypesFromProperties(1234)).to.be.an("object").and.to.be
        .empty;
      expect(getAttrTypesFromProperties("string")).to.be.an("object").and.to.be
        .empty;
      expect(getAttrTypesFromProperties(true)).to.be.an("object").and.to.be
        .empty;
      expect(getAttrTypesFromProperties(false)).to.be.an("object").and.to.be
        .empty;
  ...
  describe("getAttrTypesFromOafResponse", () => {
    it("should return a key value list of all given schemas found in the response object of the collection", () => {
      ...
      expect(
        getAttrTypesFromOafResponse(collection, responseData, () => {
          lastError = true;
        }),
      ).to.deep.equal({
        nameA: "typeA",

 ...
 * @returns {void}
 */
function fetchAllStaProperties(
  url,
  rootNode,
  onsuccess,
  onerror,
  ...
  http.get(
    ...
    (data) => {
      ...
      const observationType =
          rootNode === "Datastreams"
            ? data[0].observationType
            : data[0].Datastreams[0].observationType,
...
function getFilterableProperties(data) {
  ...
  data.forEach((entity) => {
    ...
    datastreams.forEach((Datastream, index) => {
      ...
      if (ds_properties && isObject(ds_properties)) {
        Object.entries(ds_properties).forEach(([key, value]) => {
          if (
            !Object.prototype.hasOwnProperty.call(
              resultAssoc,
              "@Datastreams." + index + ".properties." + key,
            )
          ) {
            resultAssoc["@Datastreams." + index + ".properties." + key] = {};
          }
          resultAssoc["@Datastreams." + index + ".properties." + key][value] =
            true;
        });
      }
      ...
      if (
        ds_observations &&
        Array.isArray(ds_observations) &&
        ds_observations.length > 0
      ) {
        if (
          !Object.prototype.hasOwnProperty.call(
            resultAssoc,
            "@Datastreams." + index + ".Observations.0.result",
          )
        ) {
          resultAssoc["@Datastreams." + index + ".Observations.0.result"] = {};
        }
        resultAssoc["@Datastreams." + index + ".Observations.0.result"][
          ds_observations[0].result
        ] = true;
      }
    });
  });

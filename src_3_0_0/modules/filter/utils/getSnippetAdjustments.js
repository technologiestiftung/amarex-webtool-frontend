  ...
  items.forEach((item) => {
    ...
    relevantAttrNames.forEach((attrName) => {
      ...
      if (attrName.startsWith("@") && item.get("Datastreams")) {
        const pathArray = attrName
            .substring(1, attrName.length)
            .split(".")
            .filter((path) => path !== "."),
          dIdx = pathArray[1],
        ...
        if (
          ...
          assoc[attrName][
            item.get("Datastreams")[dIdx][observation][oIdx][res]
          ] = true;
        }
        return;
...
function getSnippetAdjustments(snippets, items, page, total) {
  ...
  const relevantAttrNames = getListOfRelevantAttrNames(snippets),
    valueByAttrName = getAttrValuesOfItemsGroupedByAttrNames(
      items,
      relevantAttrNames,
    ),
    result = {
  ...
  snippets.forEach((snippet) => {
    if (
      ...
      (typeof snippet.attrName === "string" &&
        !Array.isArray(valueByAttrName[snippet.attrName])) ||
      (Array.isArray(snippet.attrName) &&
        !snippet.attrName.every((value) =>
          Array.isArray(valueByAttrName[value]),
        ))
    ) {
      return;
    }
    if (snippet.type === "dropdown") {
      ...
      const mergedValue = Array.isArray(snippet.attrName)
          ? getMergedValuesByAttrNames(snippet.attrName, valueByAttrName)
          : valueByAttrName[snippet.attrName],
      ...
      const format =
          typeof snippet.format === "string" ? snippet.format : "YYYY-MM-DD",
        values = Array.isArray(snippet.attrName)
          ? getMergedValuesByAttrNames(snippet.attrName, valueByAttrName)
          : valueByAttrName[snippet.attrName],
        sortedValue = Array.isArray(values)
          ? values.sort((a, b) => {

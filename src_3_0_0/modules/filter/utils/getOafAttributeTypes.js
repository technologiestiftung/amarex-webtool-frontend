...
function getOafAttributeTypes(
  ...
  if (typeof url !== "string") {
    onerror(
      new Error("getOafAttributeTypes: url is needed as string, no url given"),
    );
    return;
  } else if (typeof collection !== "string") {
    onerror(
      new Error(
        "getOafAttributeTypes: collection is needed as string, no collection given",
      ),
    );
    return;
  }
  ...
  axiosObject({
    ...
    .then((response) => {
      if (typeof onsuccess === "function") {
        onsuccess(
          getAttrTypesFromOafResponse(collection, response.data, onerror),
        );
      }
    })
    .catch((error) => {
...
function getAttrTypesFromOafResponse(collection, responseData, onerror) {
  const propertiesKey =
      "paths./collections/" + collection + "/items.get.parameters",
    properties = getValueFromObjectByPath(responseData, propertiesKey, false);

  if (!Array.isArray(properties)) {
    onerror(
      new Error(
        "getOafAttributeTypes: properties not found under " + propertiesKey,
      ),
    );
    return {};
  }

 ...
 * @returns {void}
 */
function fetchAllOafProperties(
  url,
  collection,
  limit,
  onsuccess,
  ...
  if (typeof url !== "string") {
    if (typeof onerror === "function") {
      onerror(
        new Error(
          "fetchAllOafProperties: the url parameter has to be a string",
        ),
      );
    }
    return;
  ...
  if (typeof collection !== "string") {
    if (typeof onerror === "function") {
      onerror(
        new Error(
          "fetchAllOafProperties: the collection parameter has to be a string",
        ),
      );
    }
    return;
  ...
  const limitParam = typeof limit === "number" ? limit : 10,
    ...
    axiosUrl =
      url + "/collections/" + collection + "/items?limit=" + limitParam,
    result = [];
 ...
 * @returns {void}
 */
function fetchAllOafPropertiesRecursionHelper(
  result,
  url,
  onsuccess,
  onerror,
    ...
    .then((response) => {
      if (!isObject(response) || !isObject(response.data)) {
        if (typeof onerror === "function") {
          onerror(
            new Error(
              "fetchAllOafProperties: the response data does not exist",
            ),
          );
        }
        return;
      }
      const nextLink = getFeature.getNextLinkFromFeatureCollection(
        response.data,
      );
...
function getUniqueValuesFromFetchedFeatures(allFetchedProperties, attrName) {
  ...
  if (Array.isArray(attrName)) {
    attrName.forEach((attributeName) => {
      Object.assign(
        result,
        getUniqueValuesFromFetchedFeatures(allFetchedProperties, attributeName),
      );
    });
    return result;
  }
 ...
 */
function getMinMaxFromFetchedFeatures(
  ...
  if (Array.isArray(attrName)) {
    ...
    attrName.forEach((attributeName) => {
      const minMaxOfAttrName = getMinMaxFromFetchedFeatures(
        allFetchedProperties,
        attributeName,
        minOnly,
        maxOnly,
      ...
      if (
        (!Object.prototype.hasOwnProperty.call(minMax, "min") &&
          Object.prototype.hasOwnProperty.call(minMaxOfAttrName, "min")) ||
        minMax?.min > minMaxOfAttrName?.min
      ) {
        minMax.min = minMaxOfAttrName.min;
      }
      if (
        (!Object.prototype.hasOwnProperty.call(minMax, "max") &&
          Object.prototype.hasOwnProperty.call(minMaxOfAttrName, "max")) ||

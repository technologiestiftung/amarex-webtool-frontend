...
export default class UrlHandler {
  ...
  transformOldUrl(oldObjectArray, config, mapHandler, onsuccess) {
    ...
    oldObjectArray.forEach(
      (filterObject) => {
        const matchingFilter = this.getMatchingFilterFromGroupsOrLayers(
            filterObject,
            config?.groups,
            config?.layers,
          ),
          extern =
            typeof matchingFilter?.filter?.extern === "boolean"
              ? matchingFilter.filter.extern
              : false;
        ...
        mapHandler.initializeLayer(
          matchingFilter.filter.api.filterId,
          matchingFilter.filter.layerId,
          extern,
          (error) => {
            console.warn(error);
          },
        );
        matchingFilter.filter.api.setServiceByLayerModel(
          matchingFilter.filter.layerId,
          mapHandler.getLayerModelByFilterId(
            matchingFilter.filter.api.filterId,
          ),
          extern,
          (error) => {
        ...
        matchingFilter.filter.api.getAttrTypes(
          (attrTypes) => {
            apiCallsDone++;
            ...
            if (
              apiCallsDone === oldObjectArray.length - 1 &&
  ...
  setResultValues(oldFilter, matchingFilter, attrTypes, result) {
    ...
    if (
      Object.prototype.hasOwnProperty.call(oldFilter, "isSelected") &&
      oldFilter.isSelected
    ) {
    ...
    oldFilter.rules.forEach((rule) => {
      result.rulesOfFilters[matchingFilter.index][rule.snippetId] =
        rule.value === null ? null : rule;
    });
  }
  /**
  ...
  getPreparedRule(rule, snippet, idx) {
    ...
    if (
      (typeof snippet?.attrName === "string" &&
        snippet?.attrName === preparedRule?.attrName) ||
      (Array.isArray(snippet?.attrName) &&
        Array.isArray(preparedRule?.attrName) &&
        snippet.attrName.length === preparedRule.attrName.length &&
        snippet.attrName.every(
          (value, index) => value === preparedRule.attrName[index],
        ))
    ) {
      ...
      if (
        Array.isArray(preparedRule.values) &&

...
export default {
  ...
  highlightFeature({ state, rootGetters, dispatch }, { feature, layerId }) {
    ...
    const layer = layerCollection
        .getOlLayers()
        .find((l) => l.values_.id === layerId),
      featureGeometryType = feature.getGeometry().getType(),
      featureId = feature.getId(),
      styleObj =
        featureGeometryType.toLowerCase().indexOf("polygon") > -1
    ...
    if (styleObj && styleObj.zoomLevel) {
      if (featureGeometryType === "Point") {
        dispatch("Maps/setCenter", feature.getGeometry().getCoordinates(), {
          root: true,
        });
      } else {
        dispatch(

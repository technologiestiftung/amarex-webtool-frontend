...
describe("src_3_0_0/modules/selectFeatures/store/actionsSelectFeatures.js", () => {
  ...
  beforeEach(() => {
    dispatch = sinon.spy();
    createLayerAddToTreeStub = sinon.spy(
      createLayerAddToTreeModule,
    ...
    sinon.stub(layerCollectionModule, "getOlLayers").returns(visibleLayerList);
  });
  ...
  describe("highlightFeature", () => {
    it("highlightFeature treeHighlightedFeatures is active", () => {
      ...
      expect(dispatch.secondCall.args[1].highlightStyle.fill).to.deep.equal(
        state.highlightVectorRulesPointLine.fill,
      );
      ...
      expect(dispatch.getCall(3).args[1]).to.deep.equal(
        state.highlightVectorRulesPointLine.zoomLevel,
      );
      expect(createLayerAddToTreeStub.calledOnce).to.be.true;
      expect(createLayerAddToTreeStub.firstCall.args[0]).to.be.deep.equals(
        layerId,
      );
      ...
      expect(createLayerAddToTreeStub.firstCall.args[3]).to.be.deep.equals(
        rootGetters.treeHighlightedFeatures,
      );
    });
    ...
    it("highlightFeature treeHighlightedFeatures is not active", () => {
      ...
      expect(dispatch.secondCall.args[1].type).to.equal("increase");
      expect(dispatch.secondCall.args[1].feature).to.deep.equal(feature);
      expect(dispatch.secondCall.args[1].layer.id).to.deep.equal(
        layerVisible.id,
      );
      expect(dispatch.secondCall.args[1].highlightStyle.fill).to.deep.equal(
        state.highlightVectorRulesPointLine.fill,

...
describe("src_3_0_0/modules/selectFeatures/components/SelectFeatures.vue", () => {
  ...
  it("renders the SelectFeatures-Table if it has data", async () => {
    await store.commit(
      "Modules/SelectFeatures/setSelectedFeaturesWithRenderInformation",
      mockSelectedFeaturesWithRenderInformation,
    );
    const wrapper = shallowMount(SelectFeaturesComponent, {
      global: {
  ...
  it("selectFeatures functions return correct results", async () => {
    ...
    expect(wrapper.vm.beautifyKey("very_important_field")).to.equal(
      "Very Important Field",
    );
    expect(wrapper.vm.beautifyValue("very|important|field")).to.equal(
      "very<br/>important<br/>field",
    );

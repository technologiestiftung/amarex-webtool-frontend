  ...
  methods: {
    ...
    setFeaturesFromDrag: function () {
      ...
      mapCollection
        ...
        .filter(
          (layer) =>
            layer.get("visible") && layer.get("source") instanceof VectorSource,
        )
        .forEach((layer) => {
          if (
    ...
    processLinksAndBreaks: function (properties) {
      ...
      Object.entries(properties).forEach(([key, propValue]) => {
        ...
        if (
          this.isValidKey(key) &&
          this.isValidValue(propertyValue) &&
    ...
    translateGFI: function (properties, gfiAttributes) {
      ...
      if (gfiAttributes === "showAll") {
        return Object.entries(resultProperties).map(([key, value]) => {
      ...
      if (typeof gfiAttributes === "object") {
        return Object.keys(gfiAttributes).map((key) => [
          gfiAttributes[key],
      ...
      if (gfiAttributes !== "ignore") {
        console.warn(
          `Layer has invalid gfiAttributes "${gfiAttributes}". Acting as if "ignore" was given.`,
        );
      }
    ...
    translate(key, options = null) {
      if (key === "common:" + this.$t(key)) {
        console.warn(
          "the key " +
            JSON.stringify(key) +
            " is unknown to the common translation",
        );
      }
  ...
  <div id="selectFeatures">
    ...
    >
      <template
        ...
        ) in selectedFeaturesWithRenderInformation"
        :key="index"
      >
        <table
          ...
          <tbody>
            ...
            >
              ...
              <td
                ...
                class="featureValue"
              >
                <a :href="getPhoneNumberAsWebLink(property[1])">{{
                  property[1]
                }}</a>
              </td>
              <td
                v-else-if="
                  property[1] &&
                  (property[1].includes('<br') || property[1].includes('<a'))
                "
                class="featureValue"
                v-html="property[1]"
              />

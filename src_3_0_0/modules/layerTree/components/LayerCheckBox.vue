...
export default {
  ...
  computed: {
     ...
     */
    isLayerVisible() {
      return typeof this.conf.visibility === "boolean"
        ? this.conf.visibility
        : false;
    },
    /**
  ...
  mounted() {
    if (this.highlightLayerId === this.conf.id) {
      const el = document.querySelector(
        "#layer-selection-treenode-" + escapeId(this.highlightLayerId),
      );
  ...
  methods: {
    ...
    ...mapMutations("Modules/LayerSelection", [
      "addSelectedLayer",
      "removeSelectedLayer",
    ]),
    escapeId,
...
<template lang="html">
  ...
  >
    <LayerPreview
      ...
      :zoom-level="
        typeof conf.preview?.zoomLevel === 'number'
          ? conf.preview?.zoomLevel
          : null
      "
      :radius="conf.preview?.radius ? conf.preview?.radius : null"
      :center="conf.preview?.center ? conf.preview?.center : null"
      :custom-class="
        conf.preview?.customClass ? conf.preview?.customClass : null
      "
  ...
  >
    <span
      ...
      :class="[
        ...
        {
          'bi-check-square':
            !(conf.baselayer && singleBaselayer) && isLayerVisible,

  ...
  mounted() {
    if (this.conf.maxScale && this.conf.minScale) {
      ...
      this.tooltipText = this.$t("common:modules.layerTree.invisibleLayer", {
        ...
        maxScale:
          "1: " + thousandsSeparator(parseInt(this.conf.maxScale, 10), "."),
      });
    } else if (this.conf.maxScale || this.conf.minScale) {
      this.tooltipText = this.$t(
        "common:modules.layerTree.invisibleLayerNoScale",
      );
    }
  },
  methods: {
    ...
    show() {
      const showLayerTyp =
        this.mode === "2D"
          ? !layerFactory
              .getLayerTypes3d()
              .includes(this.conf.typ?.toUpperCase())
          : !layerFactory
    ...
    scaleIsOutOfRange() {
      ...
      if (
        this.mode === "3D" &&
        this.conf.visibility === true &&
        (this.scale > parseInt(this.conf.maxScale, 10) ||
          this.scale < parseInt(this.conf.minScale, 10))
      ) {
...
<template lang="html">
  <div
    ...
    :class="[
      ...
      !isLayerTree() ? 'layer-selection' : '',
    ]"
  >
    <div
      class="d-flex justify-content-between align-items-center handle-layer-component-drag"
    >
      <span

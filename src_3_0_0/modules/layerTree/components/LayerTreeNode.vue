    ...
    sortedLayerConfig: {
      ...
      get() {
        ...
        if (this.showLayerAddButton) {
          sortedLayerConfig = this.layerConfigsByAttributes({
  ...
  methods: {
    ...
    getLayerArray(conf) {
      return conf?.elements
        ? conf.elements.filter(
            (el) => el.type === "layer" && el.showInLayerTree === true,
          )
        : [];
    },

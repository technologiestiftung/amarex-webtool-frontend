...
describe("src_3_0_0/modules/layerTree/components/LayerComponent.vue", () => {
  ...
  it("renders the layer given as property to the component,  isLayerTree = true", () => {
    ...
    expect(wrapper.find("#layer-tree-layer-" + propsData.conf.id).exists()).to
      .be.true;
  });
  it("renders the layer given as property to the component,  isLayerTree = false, no tooltip", () => {
    ...
    expect(
      wrapper.find("layer-check-box-stub").attributes().disabled,
    ).to.be.equals("false");
  ...
  it("renders layer with visibility false and checkbox, icon and submenu for layerTree", () => {
    ...
    expect(wrapper.findAll("layer-check-box-stub").length).to.be.equals(1);
    expect(
      wrapper.findAll("layer-component-icon-sub-menu-stub").length,
    ).to.be.equals(1);
    expect(
      wrapper.findAll("layer-component-icon-info-stub").length,
    ).to.be.equals(1);
    expect(
      wrapper.findAll("layer-component-sub-menu-stub").length,
    ).to.be.equals(1);
    expect(
      wrapper.find("layer-check-box-stub").attributes().disabled,
    ).to.be.equals("false");
  ...
  it("renders layer, that is out of range with tooltip and disabled", () => {
    ...
    expect(wrapper.find("span").attributes()["data-bs-toggle"]).to.be.equals(
      "tooltip",
    );
    expect(
      wrapper.find("layer-check-box-stub").attributes().disabled,

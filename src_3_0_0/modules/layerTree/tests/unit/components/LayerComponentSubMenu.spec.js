...
describe("src_3_0_0/modules/layerTree/components/LayerComponentSubMenu.vue", () => {
  ...
  it("renders the sub menu given as property to the component without path", () => {
    ...
    expect(
      wrapper
        .find("#layer-component-sub-menu-" + propsData.layerConf.id)
        .exists(),
    ).to.be.true;
  ...
  it("renders the transparency", () => {
    ...
    expect(
      wrapper
        .find(
          "#layer-component-icon-sub-menu-transparency-container-" +
            propsData.layerConf.id,
        )
        .exists(),
    ).to.be.true;
    expect(wrapper.find(".transparency-container > i").classes()).to.includes(
      "bi-droplet-half",
    );
  ...
  it("render not the transparency-container, if layer type Terrain3D is not supported ", () => {
    ...
    expect(
      wrapper
        .find(
          "#layer-component-icon-sub-menu-transparency-container-" + layer1.id,
        )
        .exists(),
    ).to.be.false;
  });
  ...
  describe("methods", () => {
    ...
    it("getNamesOfParentFolder returns array with folder names", () => {
      ...
      expect(
        wrapper.vm.getNamesOfParentFolder("folder-1", []),
      ).to.be.deep.equals(["folder-1", "folder-2"]);
    });
    it("getNamesOfParentFolder returns empty array if parentId is undefined of folder is unknown", () => {
      ...
      expect(
        wrapper.vm.getNamesOfParentFolder("folder-unknown", []),
      ).to.be.deep.equals([]);
    });
  });
});

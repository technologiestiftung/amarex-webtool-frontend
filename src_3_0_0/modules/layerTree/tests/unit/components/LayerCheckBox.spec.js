...
describe("src_3_0_0/modules/layerTree/components/LayerCheckBox.vue", () => {
  ...
  it("renders the layer given as property to the component", () => {
    ...
    expect(wrapper.find("#layer-checkbox-" + propsData.conf.id).exists()).to.be
      .true;
  });
  ...
  it("renders the layer with special id given as property to the component", () => {
    ...
    expect(
      wrapper.find("#layer-checkbox-" + escapeId(propsData.conf.id)).exists(),
    ).to.be.true;
  });
  ...
  it("renders layer with visibility false and checkbox not disabled", () => {
    ...
    expect(wrapper.findAll(".layer-tree-layer-checkbox").length).to.be.equals(
      1,
    );
    expect(
      wrapper.find(".layer-tree-layer-checkbox pe-2 bi-check2-square").exists(),
    ).to.be.false;
    expect(wrapper.find(".layer-tree-layer-label").text()).to.equal(
      propsData.conf.name,
    );
    expect(wrapper.find("span").attributes("class")).not.to.include(
      "font-bold",
    );
  });
  ...
  it("renders background-layer as simple preview", () => {
    ...
    expect(wrapper.find("#layer-checkbox-" + propsData.conf.id).exists()).to.be
      .false;
    expect(
      wrapper.find("#layer-tree-layer-preview-" + propsData.conf.id).exists(),
    ).to.be.true;
  ...
  it("renders background-layer with preview in config", () => {
    ...
    expect(wrapper.find("layer-preview-stub").exists()).to.be.true;
    expect(
      wrapper.find("layer-preview-stub").attributes("center"),
    ).to.be.equals(layer.preview.center);
    expect(
      wrapper.find("layer-preview-stub").attributes("zoomlevel"),
    ).to.be.equals(String(layer.preview.zoomLevel));
    expect(
      wrapper.find("layer-preview-stub").attributes("customclass"),
    ).to.be.equals(layer.preview.customClass);
    expect(
      wrapper.find("layer-preview-stub").attributes("checkable"),
    ).to.be.equals("true");
  ...
  it("renders layer with visibility true and checkbox, name is bold", () => {
    ...
    expect(wrapper.find("#layer-checkbox-" + propsData.conf.id).exists()).to.be
      .true;
    expect(
      wrapper.find("#layer-checkbox-" + propsData.conf.id).attributes("title"),
    ).to.be.undefined;
    expect(wrapper.findAll(".layer-tree-layer-checkbox").length).to.be.equals(
      1,
    );
    ...
    expect(
      wrapper.find(".layer-tree-layer-label").attributes("class"),
    ).to.include("font-bold");
  });
  ...
  it("layerTree: click on checkbox of layer with visibility false, call replaceByIdInLayerConfig", async () => {
    ...
    expect(replaceByIdInLayerConfigSpy.firstCall.args[1]).to.be.deep.equals(
      spyArg,
    );
  });

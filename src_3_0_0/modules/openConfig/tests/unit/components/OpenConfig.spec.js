...
describe("src_3_0_0/modules/openConfig/components/OpenConfig.vue", () => {
  ...
  it("renders the openConfig", () => {
    ...
    expect(wrapper.find(".btn-transparent> label").text()).to.equals(
      "common:modules.openConfig.openFile",
    );
    ...
    expect(wrapper.find(".btn-transparent> label > span > i").exists()).to.be
      .true;
  });

  it("should trigger function triggerClickOnFileInput on keydown", async () => {
    nextTick(async () => {
      const openConfigComponentSpy = sinon.spy(
        OpenConfigComponent.methods,
      ...
      await wrapper
        .find("#open-config-input-button > label")
        .trigger("keydown");
  ...
  it("should trigger function loadFile on change input", async () => {
    nextTick(async () => {
      ...
      await wrapper
        .find("#open-config-input-button > label > input")
        .trigger("change");

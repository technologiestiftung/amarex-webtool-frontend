...
function routingOrsOptimization(optimization) {
  switch (optimization) {
    ...
    case "DISTANCE":
      return "distance";
    default:
      throw new Error("Missing rangeType translation");
  }
}
...
function routingOrsOptimizationMultiplicator(optimization) {
  switch (optimization) {
    ...
    case "DISTANCE":
      return 1000;
    default:
      throw new Error("Missing optimization to multiplicator translation");
  }
}
...
async function fetchRoutingOrsIsochrones({
  ...
  const url = getRoutingIsochronesSettingsUrl(speedProfile),
    rangeValue =
      optimization === "TIME"
        ? state.Isochrones.settings.timeValue
        : state.Isochrones.settings.distanceValue,
    optimizationMultiplicator =
      routingOrsOptimizationMultiplicator(optimization),
    range = rangeValue * optimizationMultiplicator,
    interval =
      state.Isochrones.settings.intervalValue * optimizationMultiplicator;
  let result = null,
    first = null,
    second = null,
    isochrones = null,
  ...
  try {
    ...
    throw new Error(
      i18next.t("common:modules.routing.errors.errorIsochronesFetch"),
    );
  }

  result = response.data;
  if (transformCoordinates) {
    first = await transformCoordinatesToLocal([result.bbox[0], result.bbox[1]]);
    second = await transformCoordinatesToLocal([
      result.bbox[2],
      result.bbox[3],
    ]);
  } else {
    first = [result.bbox[0], result.bbox[1]];
    second = [result.bbox[2], result.bbox[3]];
  }
  ...
  for (let i = result.features.length - 1; i >= 0; i--) {
    const feature = result.features[i],
      localCoordinates = transformCoordinates
        ? []
...
function getRoutingIsochronesSettingsUrl(speedProfile) {
  ...
  let serviceUrl = store.getters.restServiceById(
      state.isochronesSettings.serviceId,
    ).url,
    url;

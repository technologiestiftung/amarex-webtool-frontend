...
export default {
  ...
  async findDirections({ state, getters, commit, dispatch, rootState }) {
    ...
    try {
      const result = await dispatch("fetchDirections", {
        wgs84Coords: wgs84Coords,
        instructions: true,
      });

      if (
        JSON.stringify(wgs84Coords) !==
        JSON.stringify(await dispatch("getDirectionsCoordinatesWgs84"))
      ) {
        return;
      }

      lineStringFeature.getGeometry().setCoordinates(result.getLineString());

      lineStringFeature.set(
        "avoidFeaturesOptions",
        JSON.parse(JSON.stringify(routingAvoidFeaturesOptions)),
      );
      lineStringFeature.set(
        "speedProfile",
        JSON.parse(JSON.stringify(settings.speedProfile)),
      );
      lineStringFeature.set(
        "preference",
        JSON.parse(JSON.stringify(settings.preference)),
      );
      lineStringFeature.set(
        "hasAvoidPolygons",
        directionsAvoidSource.getFeatures().length > 0,
      );
  ...
  async resetRoutingDirectionsResults({ dispatch, commit }) {
    ...
    lineStringFeature.getGeometry().setCoordinates([]);
  },
  ...
  async fetchDirections(
    ...
    { wgs84Coords, instructions },
  ) {
    const directionSettings =
        await rootState.Modules.Routing.directionsSettings,
    ...
    if (directionSettings.type === "ORS") {
      return fetchRoutingOrsDirections({
        ...
        transformCoordinatesToLocal: (coordinates) =>
          dispatch(
  ...
  async initDirections({ state, dispatch, commit }) {
    ...
    if (!mapListenerAdded) {
      directionsWaypointsDrawInteraction.on("drawend", (event) =>
        dispatch("onDirectionsWaypointsDrawEnd", event),
      );
      directionsAvoidDrawInteraction.on("drawend", (event) =>
        dispatch("onDirectionsAvoidDrawEnd", event),
      );
      directionsAvoidSelectInteraction.on("select", (event) =>
        dispatch("onDirectionsAvoidSelect", event),
      );
  ...
  reset({ getters, commit, dispatch }) {
    ...
    if (waypoints.length > 0) {
      ...
      directionsRouteSource
        .getFeatures()
        .forEach((feature) => feature.getGeometry().setCoordinates([]));
      commit("setRoutingDirections", null);
   ...
   * @returns {void}
   */
  createDirectionsRouteModifyInteractionListener({ state, dispatch }) {
    ...
    directionsRouteModifyInteraction.on("modifyend", async (event) => {
      ...
      for (let i = 0; i < oldLineString.length; i++) {
        ...
        const newCoordinate = newCoordinates[i],
          nextIndex = await dispatch("findWaypointBetweenLineStringIndex", {
            lineStringIndex: i,
          }),
  ...
  addWaypoint(
    { state },
    { index, feature, displayName, coordinates, fromExtern },
  ) {
  ...
  removeWaypoint({ state, dispatch, commit }, { index, reload = false }) {
    const { waypoints, directionsWaypointsSource, directionsRouteSource } =
      state;
  ...
  initWaypoints({ dispatch, state }) {
    const externWayPoints = state.waypoints.filter(
      ...
      length =
        externWayPoints.length > 0
          ? externWayPoints.length
  ...
  createDirectionsWaypointsDrawInteraction({ state, dispatch }) {
    ...
    dispatch("Maps/addInteraction", directionsRouteModifyInteraction, {
      root: true,
    });
    dispatch("Maps/addInteraction", directionsRouteSnapInteraction, {
      root: true,
    });

    dispatch("Maps/addInteraction", directionsWaypointsDrawInteraction, {
      root: true,
    });
    dispatch("Maps/addInteraction", directionsWaypointsModifyInteraction, {
      root: true,
    });
    dispatch("Maps/addInteraction", directionsWaypointsSnapInteraction, {
      root: true,
    });
  },
  /**
  ...
  removeDirectionsWaypointsDrawInteraction({ state, dispatch }) {
    ...
    dispatch("Maps/removeInteraction", directionsRouteModifyInteraction, {
      root: true,
    });
    dispatch("Maps/removeInteraction", directionsRouteSnapInteraction, {
      root: true,
    });

    dispatch("Maps/removeInteraction", directionsWaypointsDrawInteraction, {
      root: true,
    });
    dispatch("Maps/removeInteraction", directionsWaypointsModifyInteraction, {
      root: true,
    });
    dispatch("Maps/removeInteraction", directionsWaypointsSnapInteraction, {
      root: true,
    });
  },
  ...
  createDirectionsAvoidDrawInteraction({ state, dispatch }) {
    ...
    dispatch("Maps/addInteraction", directionsAvoidDrawInteraction, {
      root: true,
    });
    dispatch("Maps/addInteraction", directionsAvoidModifyInteraction, {
      root: true,
    });
    dispatch("Maps/addInteraction", directionsAvoidSnapInteraction, {
      root: true,
    });
  },
  /**
  ...
  removeDirectionsAvoidDrawInteraction({ state, dispatch }) {
    ...
    dispatch("Maps/removeInteraction", directionsAvoidModifyInteraction, {
      root: true,
    });
    dispatch("Maps/removeInteraction", directionsAvoidSnapInteraction, {
      root: true,
    });
    dispatch("Maps/removeInteraction", directionsAvoidDrawInteraction, {
      root: true,
    });
  },
  ...
  createDirectionsAvoidSelectInteraction({ state, dispatch }) {
    ...
    dispatch("Maps/addInteraction", directionsAvoidSelectInteraction, {
      root: true,
    });
  },
  /**
  ...
  removeDirectionsAvoidSelectInteraction({ state, dispatch }) {
    ...
    dispatch("Maps/removeInteraction", directionsAvoidSelectInteraction, {
      root: true,
    });
  },
  /**

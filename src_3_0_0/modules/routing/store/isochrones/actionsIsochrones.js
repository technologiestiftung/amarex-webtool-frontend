...
export default {
  ...
  async findIsochrones({ rootState, state, dispatch, commit }) {
    ...
    try {
      const result = await dispatch("fetchIsochrones", {
  ...
  zoomOnWaypoint({ state, rootState }) {
    ...
    map
      .getView()
      .fit(state.waypoint.getFeature().getGeometry(), { maxZoom: 7 });
  },
  ...
  async fetchIsochrones(
    { state, dispatch, getters, rootState },
    { wgs84Coords, transformCoordinates },
  ) {
  ...
  initIsochrones({ state, commit, dispatch }) {
    ...
    if (!mapListenerAdded) {
      isochronesPointDrawInteraction.on("drawend", (event) =>
        dispatch("onIsochronesPointDrawEnd", event),
      );
  ...
  async onIsochronesPointDrawEnd({ state, dispatch }, event) {
    const { waypoint, isochronesPointSource, isochronesPointDrawInteraction } =
        state,
    ...
    dispatch("Maps/removeInteraction", isochronesPointDrawInteraction, {
      root: true,
    });
  },
  ...
  createIsochronesPointDrawInteraction({ state, dispatch }) {
    ...
    if (state.waypoint.getCoordinates().length < 2) {
      dispatch("Maps/addInteraction", isochronesPointDrawInteraction, {
        root: true,
      });
    }
    dispatch("Maps/addInteraction", isochronesPointModifyInteraction, {
      root: true,
    });
    dispatch("Maps/addInteraction", isochronesPointSnapInteraction, {
      root: true,
    });
  },
  ...
  removeIsochronesPointDrawInteraction({ state, dispatch }) {
    ...
    dispatch("Maps/removeInteraction", isochronesPointModifyInteraction, {
      root: true,
    });
    dispatch("Maps/removeInteraction", isochronesPointSnapInteraction, {
      root: true,
    });
  },
};

...
export default {
  ...
  setFirstWayPoint({ dispatch }, { displayName, coordinates }) {
    ...
    dispatch("Directions/addWaypoint", {
      index: 0,
      displayName,
      coordinates,
      fromExtern: true,
  ...
  checkNonOptionalConfigFields({ state }) {
    const checkPaths = constantsRouting.nonOptionalConfigFields.map((field) =>
        field.split("."),
      ),
    ...
    if (missing.length > 0) {
      throw new Error(
        "Routing module is not configured correctly. The following required fields are missing: " +
          missing.map((m) => m.join(".")).join(", "),
      );
    }
    if (state.geosearch.type === "ELASTIC" && !state.geosearch.searchField) {
      throw new Error(
        "Routing module is not configured correctly. The following required fields for elastic search are missing: searchField",
      );
    }
    if (state.geosearch.type === "SPECIALWFS") {
      if (
        !state.geosearch.typeName ||
        !state.geosearch.propertyNames ||
        !state.geosearch.geometryName
      ) {
        throw new Error(
          "Routing module is not configured correctly. One of the following required fields for specialWfs search are missing: typeName, propertyNames or geometryName",
        );
      }
    }
  },
      ...
      try {
        ...
        dispatch(
          "Alerting/addSingleAlert",
          {
            ...
            content: i18next.t(
              "common:modules.routing.errors.fetchingCoordinates",
            ),
          },
          { root: true },
        );
  ...
  async fetchTextByCoordinates({ state, dispatch }, { coordinates }) {
    ...
    try {
      ...
      if (state.geosearch.type === "NOMINATIM") {
        geosearchResults =
          await fetchRoutingNominatimGeosearchReverse(coordinates);
      } else if (state.geosearch.type === "BKG") {
        ...
        geosearchResults =
          await fetchRoutingKomootGeosearchReverse(coordinates);
      } else if (state.geosearch.type === "ELASTIC") {
        geosearchResults = null;
      } else if (state.geosearch.type === "SPECIALWFS") {
        geosearchResults = null;

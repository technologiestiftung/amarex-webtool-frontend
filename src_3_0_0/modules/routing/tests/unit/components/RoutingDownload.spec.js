...
describe("src_3_0_0/modules/routing/components/RoutingDownload.vue", () => {
  let activeRoutingToolOption, downloadFileName, store, wrapper, props;
  ...
  it("returns features for 'DIRECTIONS'", () => {
    ...
    expect("isHighlight" in wrapper.vm.getDownloadFeatures()[0].getProperties())
      .to.be.true;
  });
  ...
  it("converts feature to 'GEOJSON'", async () => {
    ...
    wrapper.vm.transformCoordinatesLocalToWgs84Projection = (coordinates) =>
      coordinates;
    ...
    expect(downloadString).equal(
      '{"type":"FeatureCollection","features":[{"type":"Feature","geometry":{"type":"LineString","coordinates":[[8,52],[9,53]]},"properties":null}]}',
    );
  });
  ...
  it("converts feature to 'GPX'", async () => {
    ...
    expect(downloadString).equal(
      '<gpx xmlns="http://www.topografix.com/GPX/1/1" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.topografix.com/GPX/1/1 http://www.topografix.com/GPX/1/1/gpx.xsd" version="1.1" creator="OpenLayers"><rte><rtept lat="52" lon="8"/><rtept lat="53" lon="9"/></rte></gpx>',
    );
  });

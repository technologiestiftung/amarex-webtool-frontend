...
describe("src_3_0_0/modules/routing/components/Directions/DirectionsItem.vue", () => {
  ...
  it("renders Directions", () => {
    wrapper = shallowMount(DirectionsComponent, {
      global: {
        plugins: [store],
      },
    });
  ...
  it("renders DirectionsBatchProcessing", async () => {
    ...
    await wrapper.vm.$nextTick();
    expect(
      wrapper.findComponent(DirectionsItemBatchProcessingComponent).exists(),
    ).to.be.true;
  });
  ...
  it("renders RoutingCoordinateInput", async () => {
    ...
    await wrapper.vm.$nextTick();
    expect(wrapper.find("#routing-directions-coordinate-input-form").exists())
      .to.be.true;
  });
  ...
  it("renders routing result", async () => {
    ...
    await wrapper.vm.$nextTick();
    expect(wrapper.find("#routing-directions-result-directions").exists()).to.be
      .true;
  ...
  it("should toggle mapInteractionMode WAYPOINTS => DELETE_AVOID_AREAS", () => {
    ...
    expect(setMapInteractionModeSpy.calledOnce).to.be.true;
    expect(setMapInteractionModeSpy.firstCall.args[0]).to.equals(
      "DELETE_AVOID_AREAS",
    );
  });

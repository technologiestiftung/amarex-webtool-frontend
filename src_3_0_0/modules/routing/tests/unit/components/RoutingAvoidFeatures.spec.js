...
describe("src_3_0_0/modules/routing/components/RoutingAvoidFeatures.vue", () => {
  ...
  it("does not render RoutingAvoidFeaturesOptions", () => {
    ...
    expect(wrapper.find("#routing-avoid-features-options").exists()).to.be
      .false;
  });
  ...
  it("checks input checked", () => {
    ...
    expect(
      wrapper.find("#routing-avoid-features-option-input-HIGHWAYS").element
        .checked,
    ).to.be.true;
  });
  ...
  it("checks updateAvoidSpeedProfileOptions to add speedProfile", async () => {
    wrapper = shallowMount(RoutingAvoidFeaturesComponent, {
      ...
      data: () => ({
        avoidSpeedProfileOptionsConstants: [
          { id: "HIGHWAYS", availableProfiles: ["CYCLING"] },
        ],
        showAvoidFeatures: true,
      }),
      props: {
    ...
    await wrapper.vm.$nextTick();
    expect(
      wrapper.vm.avoidSpeedProfileOptionsConstants[0].availableProfiles,
    ).deep.to.equal(["CYCLING", "CAR"]);
  });
  it("checks updateAvoidSpeedProfileOptions to add speedProfile with new avoidFeature", async () => {
    ...
    await wrapper.vm.$nextTick();
    expect(wrapper.vm.avoidSpeedProfileOptionsConstants[1].id).deep.to.equal(
      "New",
    );
  });
  it("removes existing speedProfile from avoidSpeedProfileOptionsConstants", async () => {
    wrapper = shallowMount(RoutingAvoidFeaturesComponent, {
      ...
      data: () => ({
        avoidSpeedProfileOptionsConstants: [
          { id: "HIGHWAYS", availableProfiles: ["CYCLING", "CAR"] },
        ],
        showAvoidFeatures: true,
      }),
      props: {

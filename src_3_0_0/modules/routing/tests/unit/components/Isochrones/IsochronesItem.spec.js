...
describe("src_3_0_0/modules/routing/components/Isochrones/IsochronesItem.vue", () => {
  let batchProcessingActive, batchProcessingEnabled, store, wrapper;
  ...
  it("renders Isochrones", () => {
    wrapper = shallowMount(IsochronesComponent, {
      global: {
        plugins: [store],
      },
    });
  ...
  it("renders IsochronesBatchProcessingCheckbox", async () => {
    ...
    expect(
      wrapper.findComponent(RoutingBatchProcessingCheckboxComponent).exists(),
    ).to.be.true;
  });
  ...
  it("renders RoutingCoordinateInput", async () => {
    ...
    await wrapper.vm.$nextTick();
    expect(wrapper.find("#routing-isochrones-coordinate-input-form").exists())
      .to.be.true;
  });
  ...
  it("renders RoutingSliderInput - DISTANCE", async () => {
    ...
    expect(
      wrapper
        .findAllComponents(RoutingSliderInputComponent)
        .filter(
          (el) =>
            el.vm.$props.label ===
            "common:modules.routing.isochrones.maxDistance",
        ).length,
    ).equal(1);
    expect(
      wrapper
        ...
        .filter(
          (el) =>
            el.vm.$props.label ===
            "common:modules.routing.isochrones.maxTraveltime",
        ).length,
    ).equal(0);
  });
  ...
  it("renders isochrones result", async () => {
    ...
    await wrapper.vm.$nextTick();
    expect(wrapper.find("#routing-isochrones-result-isochrones").exists()).to.be
      .true;
  ...
  it("doesn't render isochrones result", async () => {
    ...
    expect(wrapper.find("#routing-isochrones-result-isochrones").exists()).to.be
      .false;
    expect(wrapper.findComponent(RoutingDownloadComponent).exists()).to.be
      .false;
  });

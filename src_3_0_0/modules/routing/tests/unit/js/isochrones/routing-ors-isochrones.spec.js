...
import {
  fetchRoutingOrsIsochrones,
  getRoutingIsochronesSettingsUrl,
} from "../../../../js/isochrones/routing-ors-isochrones";
...
describe("src_3_0_0/modules/routing/js/directions/routing-ors-directions.js", () => {
    ...
    it("should process result correct", async () => {
          ...
          resolve({
            data: {
              ...
              features: [
                {
                  ...
                  properties: {
                    group_index: 0,
                    value: 60.0,
                    center: [6.582012174621497, 51.331735245847746],
                  },
                  geometry: {
                    coordinates: [
                      [
              ...
              metadata: {
                ...
                query: {
                  locations: [[6.5821172105612185, 51.33169403960399]],
                  location_type: "start",
                  range: [60.0],
                  range_type: "time",
                  units: "m",
      ...
      const result = await fetchRoutingOrsIsochrones({
        ...
        expectedResult = new RoutingIsochrones([
          6.577318, 51.32985, 6.5873, 51.334001,
        ]);
  ...
  describe("getRoutingIsochronesSettingsUrl", () => {
    it("service url without backslash at the end", () => {
      ...
      expect(createdUrl.href).to.eql(
        service + "/v2/isochrones/driving-car/geojson",
      );
    });
    it("service url with backslash at the end", () => {
      ...
      expect(createdUrl.origin).to.eql(
    ...
    it("service url with backslash at start", () => {
      ...
      expect(createdUrl.href).to.eql(
        global.window.location.origin +
          service +
          "v2/isochrones/driving-car/geojson",
      );
    });
  });
});

              ...
              features: [
                {
                  type: "Feature",
                  bbox: [
                    6.47933210954439, 51.2849325585585, 6.70186498354972,
                    51.4071125869666,
                  ],
                  geometry: {
                    type: "Point",
                ...
                },
                {
                  type: "Feature",
                  bbox: [
                    6.5074903650681, 51.3455092448856, 6.51893373216666,
                    51.3473656574494,
                  ],
                  geometry: {
                    type: "Point",
              ...
              features: [
                {
                  type: "Feature",
                  bbox: [
                    6.56412165145409, 51.3267394906946, 6.56944658394794,
                    51.3400332636572,
                  ],
                  geometry: {
                    type: "Point",
  ...
  describe("getRoutingBkgGeosearchReverseUrl", () => {
    ...
    it("createUrl should respect questionmark in serviceUrl", () => {
      ...
      expect(decodeURI(createdUrl)).to.eql(
        service +
          "&lon=1&lat=2&count=1&properties=text&distance=1000&filter=typ%3Aort",
      );
      expect(createdUrl.searchParams.get("lon")).to.eql(coordinates[0]);
      expect(createdUrl.searchParams.get("lat")).to.eql(coordinates[1]);
      expect(createdUrl.searchParams.get("count")).to.be.equals("1");
    ...
    it("service url with backslash at start", async () => {
      ...
      expect(createdUrl.href).to.eql(
        global.window.location.origin +
          service +
          "?count=1000&properties=text&query=search&bbox=10%2C20%2C30%2C40",
      );
    });

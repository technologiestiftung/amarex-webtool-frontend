...
import { getRoutingElasticUrl } from "../../../../js/geosearch/routing-elastic-geosearch";
describe("src_3_0_0/modules/routing/js/geosearch/routing-elastic-geosearch.js", () => {
  ...
  describe("getRoutingElasticUrl", () => {
    it("test params", () => {
      ...
      expect(createdUrl.searchParams.get("source_content_type")).to.eql(
        "application/json",
      );
      expect(createdUrl.searchParams.get("source")).to.eql(
    ...
    it("createUrl should respect questionmark in serviceUrl", () => {
      ...
      expect(decodeURI(createdUrl)).to.eql(
        service +
          '&source_content_type=application%2Fjson&source={"a"%3A"a"%2C"b"%3A[{"c"%3A"c"}]}',
      );
      expect(createdUrl.searchParams.get("source_content_type")).to.eql(
        "application/json",
      );

...
describe("src_3_0_0/modules/routing/store/actionsRouting.js", () => {
  ...
  it("should throw error because of missing required config parameter", () => {
    try {
      ...
      expect(e.message).equal(
        "Routing module is not configured correctly. The following required fields are missing: geosearch.type, geosearch.serviceId, geosearchReverse.type, geosearchReverse.serviceId, directionsSettings.type, directionsSettings.serviceId, isochronesSettings.type, isochronesSettings.serviceId",
      );
    }
  });
  ...
  it("setFirstWayPoint shall call reset and addWaypoint", () => {
    ...
    expect(dispatch.secondCall.args).to.eql([
      ...
      { index: 0, displayName, coordinates, fromExtern: true },
    ]);
  });

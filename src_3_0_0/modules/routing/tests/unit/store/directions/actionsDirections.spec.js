...
describe("src_3_0_0/modules/routing/store/directions/actionsDirections.js", () => {
  let state,
    ...
    waypoints,
    wgs84Coordinates,
    routingDirectionsWaypointSource,
    routingDirectionsAvoidSource,
    routingDirectionsResult,
    routeFeature,
    highlightFeature,
    startWaypoint,
    endWaypoint,
    avoidPolygonCoordinates;
  ...
  beforeEach(() => {
    ...
    dispatch = (...args) => {
      dispatchSpy(...args);
      if (
        args[0] === "Modules/Routing/transformCoordinatesLocalToWgs84Projection"
      ) {
        return args[1];
      } else if (args[0] === "Modules/Routing/fetchTextByCoordinates") {
        return new RoutingGeosearchResult(
          [args[1].coordinates[0], args[1].coordinates[1]],
          "test",
        );
      }
    ...
    state = {
      ...
      directionsRouteSource: new VectorSource({
        features: [routeFeature, highlightFeature],
      }),
  ...
  it("should findDirections", async () => {
    await actionsDirections.findDirections({
      state,
      getters,
      commit,
      dispatch,
    ...
    expect(dispatchSpy.args).to.deep.equal([
      ...
      [
        "fetchDirections",
        { wgs84Coords: wgs84Coordinates, instructions: true },
      ],
      ["getDirectionsCoordinatesWgs84"],
    ]);
  });
  ...
  it("should resetRoutingDirectionsResults", async () => {
    await actionsDirections.resetRoutingDirectionsResults({
      state,
      getters,
      commit,
      dispatch,
    ...
    expect(commitSpy.args).to.deep.equal([["setRoutingDirections", null]]);

    expect(dispatchSpy.args).to.deep.equal([["getRouteFeature"]]);
  });

  it("should getRouteFeature", async () => {
    const result = await actionsDirections.getRouteFeature({
      state,
      getters,
      commit,
      dispatch,
  ...
  it("should getHighlightFeature", async () => {
    const result = await actionsDirections.getHighlightFeature({
      state,
      getters,
      commit,
      dispatch,
  ...
  it("should unHighlightRoute", async () => {
    await actionsDirections.unHighlightRoute({
      state,
      getters,
      commit,
      dispatch,
    ...
    expect(dispatchSpy.args).to.deep.equal([["getHighlightFeature"]]);
  ...
  it("should highlightRoute with 'fromWaypointIndex' and 'toWaypointIndex'", async () => {
    await actionsDirections.highlightRoute(
  ...
  it("should getDirectionsCoordinatesWgs84", async () => {
    const directionsCoordinatesWgs84 =
      await actionsDirections.getDirectionsCoordinatesWgs84({
        state,
        getters,
        commit,
        dispatch,
    ...
    expect(dispatchSpy.args).to.deep.equal(
      getters.directionsCoordinates.map((coords) => [
    ...
    expect(directionsCoordinatesWgs84).to.deep.equal(
      getters.directionsCoordinates,
    );
  });

  it("should getAvoidPolygonsWgs84", async () => {
    const avoidPolygonsWgs84 = await actionsDirections.getAvoidPolygonsWgs84({
      state,
      getters,
      commit,
      dispatch,
    ...
    expect(dispatchSpy.args).to.deep.equal(
      avoidPolygonCoordinates[0].map((coords) => [
        "Modules/Routing/transformCoordinatesLocalToWgs84Projection",
        coords,
        { root: true },
      ]),
    ...
    expect(avoidPolygonsWgs84).to.deep.equal({
      type: "MultiPolygon",
      coordinates: [avoidPolygonCoordinates],
    });
  });
  ...
  it("should initDirections without mapListenerAdded", async () => {
    ...
    await actionsDirections.initDirections({
      state,
      getters,
      commit,
      dispatch,
    ...
    expect(commitSpy.args).to.deep.equal([["setMapListenerAdded", true]]);
  });
  ...
  it("should createInteractionFromMapInteractionMode with mapInteractionMode 'WAYPOINTS'", async () => {
    ...
    await actionsDirections.createInteractionFromMapInteractionMode({
      state,
      getters,
      commit,
      dispatch,
  ...
  it("should closeDirections", async () => {
    await actionsDirections.closeDirections({
      state,
      getters,
      commit,
      dispatch,
    ...
    expect(dispatchSpy.args).to.deep.equal([["removeMapInteractions"]]);
  });

  it("should findWaypointBetweenLineStringIndex", async () => {
    const waypointBetweenLineStringIndex =
      await actionsDirections.findWaypointBetweenLineStringIndex(
        { state, getters, commit, dispatch, rootState },
        {
          lineStringIndex: 0,
        },
  ...
  describe("should addWaypoint", () => {
    it("without index", async () => {
      const waypoint = await actionsDirections.addWaypoint(
        { state, getters, commit, dispatch, rootState },
        {},
      );
    ...
    it("with feature", async () => {
      const featurePoint = new Feature({
        ...
        waypoint = await actionsDirections.addWaypoint(
          { state, getters, commit, dispatch, rootState },
          {
  ...
  describe("should removeWaypoint", () => {
    it("with 2 waypoints", async () => {
      await actionsDirections.removeWaypoint(
        { state, getters, commit, dispatch, rootState },
        {
          index: 0,
        },
      ...
      expect(
        waypoints[0].getFeature().getGeometry().getCoordinates(),
      ).deep.to.equal([]);
    ...
    it("with more than 2 waypoints", async () => {
      const firstWaypoint = new RoutingWaypoint({
          index: 0,
          source: routingDirectionsWaypointSource,
        }),
        secondWaypoint = new RoutingWaypoint({
          index: 1,
          source: routingDirectionsWaypointSource,
        }),
        thirdWaypoint = new RoutingWaypoint({
          index: 2,
          source: routingDirectionsWaypointSource,
        });
    ...
    it("with more than 2 waypoints and reload", async () => {
      ...
      expect(dispatchSpy.args).to.deep.equal([["findDirections"]]);
    });
  });
  ...
  describe("should moveWaypointDown", () => {
    it("should move second waypoint to third place", async () => {
      ...
      state.waypoints = [firstWaypoint, secondWaypoint, thirdWaypoint];
      await actionsDirections.moveWaypointDown(
        { state, getters, commit, dispatch, rootState },
        1,
      );
  ...
  describe("should moveWaypointUp", () => {
    it("should move third waypoint to second place", async () => {
      ...
      state.waypoints = [firstWaypoint, secondWaypoint, thirdWaypoint];
      await actionsDirections.moveWaypointUp(
        { state, getters, commit, dispatch, rootState },
        2,
      );
  ...
  it("should initWaypoints", async () => {
    state.waypoints = [];
    await actionsDirections.initWaypoints({
      state,
      getters,
      commit,
      dispatch,
  ...
  it("should initWaypoints - if one extern waypoint exists make only one new waypoint", async () => {
    ...
    expect(dispatchSpy.args).to.deep.equal([["addWaypoint", { index: 1 }]]);
  });
  ...
  it("should onDirectionsWaypointsDrawEnd", async () => {
    ...
    await actionsDirections.onDirectionsWaypointsDrawEnd(
      { state, getters, commit, dispatch, rootState },
      {
        feature: featurePoint,
      },
  ...
  it("should removeMapInteractions", async () => {
    await actionsDirections.removeMapInteractions({
      state,
      getters,
      commit,
      dispatch,

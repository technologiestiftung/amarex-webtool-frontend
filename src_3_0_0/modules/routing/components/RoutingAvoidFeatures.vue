...
export default {
  ...
  data() {
    return {
      ...
      avoidSpeedProfileOptionsConstants:
        constantsRouting.avoidSpeedProfileOptions,
    };
  },
  computed: {
    ...
    avoidSpeedProfileOptions({ settings }) {
      return this.avoidSpeedProfileOptionsConstants?.filter((option) =>
        option.availableProfiles?.includes(settings.speedProfile),
      );
    },
  },
  created() {
  ...
  methods: {
    ...
    syncAvoidFeatures(avoidFeaturesFromConfig) {
      for (const key in avoidFeaturesFromConfig) {
        ...
        for (const i in avoidFeaturesFromConfig[key]) {
          this.updateAvoidSpeedProfileOptions(
            avoidFeaturesFromConfig[key][i],
            key,
          );
        }
      }
    ...
    removeDefaultSetting(speedProfile) {
      ...
      for (const i in avoidSpeedProfileOptions) {
        if (
          this.avoidSpeedProfileOptionsConstants[i].availableProfiles.includes(
            speedProfile,
          )
        ) {
          const index =
            this.avoidSpeedProfileOptionsConstants[i].availableProfiles.indexOf(
              speedProfile,
            );

          this.avoidSpeedProfileOptionsConstants[i].availableProfiles.splice(
            index,
            1,
          );
        }
    ...
    updateAvoidSpeedProfileOptions(avoidFeature, speedProfile) {
      ...
      for (const i in avoidSpeedProfileOptions) {
        if (avoidSpeedProfileOptions[i].id === avoidFeature) {
          if (
            !avoidSpeedProfileOptions[i].availableProfiles.includes(
              speedProfile,
            )
          ) {
      ...
      if (avoidFeatureExists === false) {
        avoidSpeedProfileOptions.push({
          id: avoidFeature,
          availableProfiles: [speedProfile],
        });
      }
    },
    /**
...
<template>
  ...
  >
    ...
    >
      <i :class="showAvoidFeatures ? 'bi-chevron-down' : 'bi-chevron-right'" />
    ...
    >
      ...
      >
        ...
        />
        <span class="ms-2">{{
          $t("common:modules.routing.avoidOptions." + option.id)
        }}</span>
      </label>
    </div>
  </div>

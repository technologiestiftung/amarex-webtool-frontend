...
export default {
  ...
  methods: {
    ...
    ...mapActions("Modules/Routing/Isochrones", [
      "fetchIsochrones",
          ...
          try {
            ...
            if (this.countFailed === this.serviceRequests) {
              this.addSingleAlert({
                ...
                content: this.$t(
                  "common:modules.routing.isochrones.batchProcessing.errorAllFailed",
                ),
              });
              ...
              this.addSingleAlert({
                ...
                category: "error",
                content: this.$t(
                  "common:modules.routing.isochrones.batchProcessing.errorSomeFailed",
        ...
        this.setTaskHandler(
          new RoutingTaskHandler(
            ...
            this.settings.batchProcessing.maximumConcurrentRequests
              ? this.settings.batchProcessing.maximumConcurrentRequests
              : this.$store.getters["Modules/Routing/Isochrones/settings"]
    ...
    async parseLineParts(lineParts) {
      ...
      try {
        ...
        return isochronesResult.getAreas().map((area) =>
          area.getGeojsonFeature({
            ID: id,
            [this.$t(
...
<template>
  <RoutingBatchProcessing
    ...
    :structure-text="
      $t('common:modules.routing.isochrones.batchProcessing.structure')
    "

...
export default {
  ...
  computed: {
    ...
    activeRoutingToolOptionComponent() {
      return this.filteredRoutingToolOptions.find(
        (option) => option.id === this.activeRoutingToolOption,
      )?.component;
    },
  },
  async created() {
...
<template lang="html">
  <div id="routing">
    <div class="d-flex">
      ...
      >
        <span>{{
          $t("common:modules.routing.tabs." + routingToolOption.id)
        }}</span>
        <SpinnerItem
          v-if="
            (routingToolOption.id === 'DIRECTIONS' && isLoadingDirections) ||
            (routingToolOption.id === 'ISOCHRONES' && isLoadingIsochrones)
          "

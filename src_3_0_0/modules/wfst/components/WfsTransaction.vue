...
export default {
  ...
  created() {
    const newLayerInformation = this.allLayerConfigs
        .filter((item) => this.layerIds.includes(item.id))
        .reverse(),
      firstActiveLayer = newLayerInformation.findIndex(
        (layer) => layer.visibility,
      ),
      currentLayerDeactivated =
        this.currentLayerIndex > -1 &&
        !newLayerInformation[this.currentLayerIndex].visibility;
  ...
  methods: {
    ...mapMutations("Modules/Wfst", [
      "setTransactionProcessing",
      "setCurrentLayerIndex",
      "setLayerInformation",
      "setShowConfirmModal",
    ]),
    ...mapActions("Modules/Wfst", [
      "prepareInteraction",
      "reset",
      "save",
      "setFeatureProperty",
            ...
            <template v-for="property of featureProperties">
              <template v-if="property.type !== 'geometry'">
                ...
                <input
                  ...
                  :checked="
                    ['true', true].includes(property.value) ? true : false
                  "
                  @input="
                    (event) =>
                      setFeatureProperty({

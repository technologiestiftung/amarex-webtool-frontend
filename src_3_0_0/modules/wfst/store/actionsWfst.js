...
const actions = {
    ...
    if (
      ...
      const { style } = layerInformation[currentLayerIndex],
        drawOptions = {
          ...
          type:
            (currentInteractionConfig[interaction].multi ? "Multi" : "") +
            interaction,
          stopClick: true,
          geometryName: featureProperties.find(
            ({ type }) => type === "geometry",
          ).key,
        };
      ...
      drawInteraction.on("drawend", (event) => {
        ...
        if (
          (currentLayer.minScale && mapScale < currentLayer.minScale) ||
          (currentLayer.maxScale && mapScale > currentLayer.maxScale)
        ) {
          drawLayer
            .getSource()
            .once("change", () => drawLayer.getSource().clear());
          dispatch(
            "Alerting/addSingleAlert",
            {
              category: "error",
              content: i18next.t(
                "common:modules.wfst.error.geometryOutOfRange",
              ),
  ...
  reset({ commit, dispatch, getters }) {
    const sourceLayer = layerCollection.getLayerById(
        getters.currentLayerId,
      )?.layer,
    ...
    commit(
      "setFeatureProperties",
      layerSelected
        ? getters.featureProperties.map((property) => ({
            ...property,
            value: null,
          }))
  ...
  async sendTransaction({ dispatch, getters, rootGetters }, feature) {
    const { currentLayerIndex, layerInformation, selectedInteraction } =
        getters,
      layer = layerInformation[currentLayerIndex],
      selectedFeature =
        feature && featureToDelete !== null ? feature : featureToDelete,
      url = layer.url,
      transactionMethod = ["LineString", "Point", "Polygon"].includes(
        selectedInteraction,
      )
        ? "insert"
        : selectedInteraction,
    ...
    try {
      ...
      const transaction = i18next.t(
        "common:modules.wfst.transaction." + messageKey,
      );
      ...
      if (response !== null) {
        dispatch(
          "Alerting/addSingleAlert",
          {
            category: "success",
            content: i18next.t(
              "common:modules.wfst.transaction.success.baseSuccess",
   ...
   * @returns {void}
   */
  async setFeatureProperties({
    commit,
    getters: { currentLayerIndex, layerInformation },
  }) {
    if (currentLayerIndex === -1) {
      commit(
        "setFeatureProperties",
        i18next.t("common:modules.wfst.error.allLayersNotSelected"),
      );
      return;
    }
    ...
    if (!Object.prototype.hasOwnProperty.call(layer, "featurePrefix")) {
      commit(
        "setFeatureProperties",
        i18next.t("common:modules.wfst.error.layerNotConfiguredCorrectly"),
      );
      return;
    }
    if (!layer.visibility) {
      commit(
        "setFeatureProperties",
        i18next.t("common:modules.wfst.error.layerNotSelected"),
      );
      return;
    }
    commit(
      "setFeatureProperties",
      await prepareFeaturePropertiesModule.prepareFeatureProperties(layer),
    );
  },
};

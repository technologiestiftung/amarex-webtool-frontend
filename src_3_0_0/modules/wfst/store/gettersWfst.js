...
const getters = {
  ...
  currentInteractionConfig(state, { currentLayerId }) {
    ...
    ["LineString", "Point", "Polygon", "update", "delete"].forEach((val) => {
      ...
      layerConfiguration = interactionConfiguration.find(
      ...
      if (layerConfiguration.text !== undefined) {
        configuration[val].text = layerConfiguration.text
          ? layerConfiguration.text
          : configuration[val].text;
      }
      configuration[val].icon = layerConfiguration.icon
        ? layerConfiguration.icon
        : configuration[val].icon;
      if (isGeometryConfiguration) {
        configuration[val].multi = layerConfiguration.multi
          ? layerConfiguration.multi
          : false;
      }
    });
  ...
  savingErrorMessage: (state) => (feature) => {
    const requiredPropertiesWithNoValue = state.featureProperties.filter(
      (property) =>
    ...
    if (requiredPropertiesWithNoValue.length > 0) {
      return i18next.t("common:modules.wfst.error.requiredPropertiesNotSet", {
        properties: requiredPropertiesWithNoValue
          .map(({ label }) => label)
          .join(", "),
      });
    }

export default {
  ...
  checkIfAllLayersAvailable: ({ commit, rootGetters }, layerIds) => {
    const validLayerIds = layerIds.filter((layer) => {
      ...
      if (
        rootGetters.allLayerConfigs.some((layerConfig) =>
          layerConfig.id.includes(layerId),
        )
      ) {
        return true;
      }
      ...
      console.warn(
        `The configuration of the module layerSlider is invalid. The layer with the id: "${layerId}" must be configured in the "layerConfig".`,
      );
      return false;
    });

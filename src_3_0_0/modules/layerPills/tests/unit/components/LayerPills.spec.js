...
describe("src_3_0_0/modules/LayerPills.vue", () => {
  ...
  describe("left scroll disabled", () => {
    it("left scroll is disabled", () => {
      ...
      expect(
        wrapper.find("#layerpills-left-button").attributes().style,
      ).to.include("visibility: hidden");
    });
  });
  describe("right scroll enabled", () => {
    it("right scroll is enabled", async () => {
      ...
      wrapper.vm.$nextTick(function () {
        expect(wrapper.find("#layerpills-right-button").attributes().style).to
          .be.undefined;
      });
    });
  });
  ...
  describe("close layerPill", () => {
    it("count close-buttons", () => {
      ...
      expect(wrapper.findAll(".close-button").length).to.equals(
        visibleSubjectDataLayers.length,
      );
    });
  });
  ...
  describe("tooltip of anchor", () => {
    it("anchors should have the correct tooltip", () => {
      ...
      wrapper.findAll("li.nav-item a").forEach((element, index) => {
        expect(element.attributes()).to.own.include({
        ...
        expect(element.attributes()).to.own.include({
          title: visibleLayers[index].name,
        });
      });
    });
  });
  ...
  describe("watcher", () => {
    it("visibleSubjectDataLayerConfigs with no oldValue", () => {
      const newValue = [
        ...
        setVisibleLayersSpy = sinon.spy(
          LayerPillsComponent.methods,
          "setVisibleLayers",
        );
      ...
      wrapper.vm.$options.watch.visibleSubjectDataLayerConfigs.handler.call(
        wrapper.vm,
        newValue,
        [],
      );
      ...
      expect(setVisibleLayersSpy.secondCall.args[0]).to.be.deep.equals(
        newValue,
      );
      expect(setVisibleLayersSpy.secondCall.args[1]).to.be.equals("2D");
      expect(setVisibleLayersSpy.secondCall.args[2]).to.be.deep.equals(
        newValue,
      );
    });
    ...
    it("mode change shall call setVisibleLayers", () => {
      ...
      store.commit(
        "setVisibleSubjectDataLayerConfigs",

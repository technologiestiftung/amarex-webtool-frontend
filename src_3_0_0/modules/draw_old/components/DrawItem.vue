...
export default {
  ...
  computed: {
    ...
    isFromDrawTool() {
      const visibleFeatures = this.layer
        ...
        ?.getFeatures()
        ?.filter(
          (feature) => feature.get("fromDrawTool") && feature.get("isVisible"),
        );
  ...
  created() {
    let importDrawLayer = mapCollection
      .getMap(this.mode)
      .getLayers()
      .getArray()
      .find((layer) => layer.get("id") === "importDrawLayer");
  ...
  methods: {
    ...
    ...mapActions("Maps", ["addLayer", "checkLayer"]),
    ...
    getIconLabelKey(option) {
      if (option && option.id) {
        if (i18next.exists(option.id)) {
          ...
          i18next.exists("common:modules.draw_old.iconList." + option.id)
        ) {
...
<template lang="html">
  <div>
    ...
    <template
      v-if="
        enableAttributesSelector && currentInteraction === 'modifyAttributes'
      "
    >
      <DrawItemAttributes
    ...
    >
      ...
      <div
        v-if="
          drawType.id === 'drawCircle' || drawType.id === 'drawDoubleCircle'
        "
        class="form-group form-group-sm row"
      >
        <label
        ...
        <div class="col-md-7">
          ...
          >
            <option
              ...
              :value="option.value"
              :selected="
                isEqualColorArrays(option.value, outerColorContourComputed)
              "
            >
              {{ $t("common:modules.draw_old.colors." + option.color) }}
            </option>
      ...
      <div class="form-group form-group-sm row">
        <div class="col-12 d-grid gap-2">
          <button
            ...
            class="btn btn-sm"
            :class="
              currentInteraction === 'draw' ? 'btn-primary' : 'btn-secondary'
            "
      ...
      <div class="form-group form-group-sm row">
        <div class="col-12 d-grid gap-2">
          <button
            ...
            class="btn btn-sm"
            :class="
              currentInteraction === 'modify' ? 'btn-primary' : 'btn-secondary'
            "
      ...
      >
        <div class="col-12 d-grid gap-2">
          <button
            ...
            "
            :disabled="
              !drawLayerVisible || currentInteraction === 'modifyAttributes'
            "
            @click="
              toggleInteraction('modifyAttributes');
              setCanvasCursorByInteraction('modifyAttributes');
            "
          >
            <span class="bootstrap-icon">
              <i class="bi-wrench" />
      ...
      <div class="form-group form-group-sm row">
        <div class="col-12 d-grid gap-2">
          <button
            ...
            class="btn btn-sm"
            :class="
              currentInteraction === 'delete' ? 'btn-primary' : 'btn-secondary'
            "

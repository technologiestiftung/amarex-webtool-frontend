    ...
    initAttributesKeyList() {
      if (this.layer && this.layer.getSource().getFeatures().length) {
        for (const feature of this.layer.getSource().getFeatures().reverse()) {
          if (
            typeof feature.get("attributes") !== "undefined" &&
            Object.keys(feature.get("attributes")).length
          ) {
            this.$emit(
              "updateAttributesKeyList",
              Object.keys(feature.get("attributes")),
            );
            break;
          }
        }
    ...
    addAttributesToFeature() {
      ...
      if (!this.attributeKey) {
        this.addKey = {
          valid: false,
          message: this.$t(
            "common:modules.draw_old.attributeSelect.attributeKeyError",
          ),
        };
        return;
      }
      if (this.attributes.some((attr) => attr?.key === this.attributeKey)) {
        this.addKey = {
          valid: false,
          message: this.$t(
            "common:modules.draw_old.attributeSelect.attributeDuplicatedKeyError",
          ),
        };
        return;
      }
      ...
      this.selectedFeature.get("attributes")[this.attributeKey] =
        this.attributeValue;
      ...
      attributes.forEach((attribute) => {
        if (!Object.prototype.hasOwnProperty.call(keyStatus, attribute.key)) {
          ...
          if (!attribute.key) {
            ...
            keyStatus[attribute.key].message = this.$t(
              "common:modules.draw_old.attributeSelect.attributeKeyError",
            );
            success = false;
          }
...
<template>
  <form id="draw-attributes">
        ...
        <div class="col-5 position-relative">
          <div class="input-group has-validation">
            <input
              ...
              type="text"
              :title="
                validKeys[attribute.key].valid === null
                  ? validKeys[attribute.key].message
                  : null
              "
              :class="[
                validKeys[attribute.key].valid === false ? 'is-invalid' : '',
                'form-control',
              ]"
      ...
      <div class="row align-items-center text-center justify-content-center">
        <div class="col-5">
          <div class="input-group has-validation">
            <input
              ...
              :class="[
                addKey.valid === false ? 'is-invalid' : '',
                'form-control',
              ]"
              :placeholder="
                $t('common:modules.draw_old.attributeSelect.input.key')
              "
        ...
        <div class="col-5">
          <input
            ...
            class="form-control"
            :placeholder="
              $t('common:modules.draw_old.attributeSelect.input.value')
            "
    ...
    <div v-else>
      <span>{{
        $t("common:modules.draw_old.attributeSelect.noFeatureSelected")
      }}</span>
    </div>
  </form>
</template>

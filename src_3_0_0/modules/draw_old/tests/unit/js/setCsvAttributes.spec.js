...
describe("src_3_0_0/modules/draw/js/setCsvAttributes.js", () => {
  let features, code;
  ...
  it("should add the key 'geometry' and 'epsg' to the feature attributes", function () {
    ...
    expect(wktFeatures[1].get("attributes")).to.have.all.keys([
      "epsg",
      "geometry",
      "name",
    ]);
  ...
  it("should set the point geometry as wkt", function () {
    ...
    expect(wktFeatures[1].get("attributes").geometry).to.be.equal(
      "POINT(123 456)",
    );
  });
  ...
  it("should set the linestring geometry as wkt", function () {
    ...
    expect(wktFeatures[2].get("attributes").geometry).to.be.equal(
  ...
  it("should set the polygon geometry as wkt", function () {
    ...
    expect(wktFeatures[0].get("attributes").geometry).to.be.equal(
      "POLYGON((30 10,40 40,130 130,240 40,30 10))",
    );
  });

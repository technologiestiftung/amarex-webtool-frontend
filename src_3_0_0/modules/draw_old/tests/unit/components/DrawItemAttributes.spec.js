...
describe("src_3_0_0/modules/draw/components/DrawItemAttributes.vue", () => {
  ...
  describe("User interaction", () => {
    it("should add attributes to feature and local attributes array", () => {
      ...
      expect(wrapper.vm.attributes)
        .to.be.an("array")
        .and.to.deep.include(expectedLocal);
    ...
    it("should remove attributes from the feature and row in local attributes array", () => {
      ...
      expect(wrapper.vm.attributes)
        .to.be.an("array")
        .and.to.not.include(notExpected);
      expect(wrapper.vm.attributes)
        .to.be.an("array")
        .and.to.deep.include(localExpected);
      expect(testFeature.get("attributes")).to.deep.equal(expected);
    });
  ...
  describe("checkAttributes", () => {
    ...
    it("should return true if valid array of objects are given", () => {
      ...
      expect(wrapper.vm.checkAttributes([{ key: "foo" }, { key: "bar" }])).to.be
        .true;
    });
    it("should return false if array of objects has the same keys", () => {
      const wrapper = factory.getShallowMount();

      expect(wrapper.vm.checkAttributes([{ key: "foo" }, { key: "foo" }])).to.be
        .false;
    });
  });
});

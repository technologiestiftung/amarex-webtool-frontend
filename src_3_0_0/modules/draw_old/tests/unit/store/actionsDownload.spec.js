...
describe("src_3_0_0/modules/draw/store/actions/actionsDownload.js", () => {
  ...
  describe("convertFeatures", () => {
    ...
    beforeEach(() => {
      ...
      state = {
        download: {
          features: [line, point, polygon].map(
        ...
        return new Promise((resolve, reject) => {
          if (actionName === "transformCoordinates") {
            ...
            if (
              ...
              reject(
                `The geometry type ${payload.getType()} is not supported!`,
              );
            }
          } else {
    ...
    it("should convert features to a GeoJSON String", async () => {
      expect(
        await actions.convertFeatures({ state, dispatch }, new GeoJSON()),
      ).to.eql(
    ...
    it("should convert features to a GPX String", async () => {
      expect(
        await actions.convertFeatures({ state, dispatch }, new GPX()),
      ).to.equal(
  ...
  describe("transformCoordinates", () => {
    it("should transform point coordinates from EPSG:25832 to EPSG:4326", () => {
      ...
      expect(
        actions.transformCoordinates({ dispatch, rootGetters }, geometry),
      ).to.eql([11.557298950390026, 48.19011285902384]);

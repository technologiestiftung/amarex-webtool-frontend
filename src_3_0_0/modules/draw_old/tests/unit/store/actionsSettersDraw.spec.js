...
describe("src_3_0_0/modules/draw/store/actions/settersDraw.js", () => {
  ...
  describe("setActive", () => {
    ...
    it("should commit and dispatch as intended if 'active' is true", async () => {
      ...
      expect(dispatch.getCall(3).args).to.eql([
        "createModifyAttributesInteractionAndAddToMap",
        false,
      ]);
  ...
  describe("setStyleSettings", () => {
    it("should commit on the mutation key recognized by the current drawType", () => {
      ...
      expect(commit.firstCall.args).to.eql([
        "setDrawTypeSettings",
        "styleSettings",
      ]);
    });
  ...
  describe("setColorContour", () => {
    it("should commit as intended", () => {
      ...
      expect(commit.firstCall.args).to.eql([
        "setTestSettings",
        { colorContour: [0, 1, 2, 3], opacityContour: 3 },
      ]);
      expect(dispatch.calledOnce).to.be.true;
      expect(dispatch.firstCall.args).to.eql(["updateDrawInteraction"]);
    });
  ...
  describe("setOuterColorContour", () => {
    it("should commit as intended", () => {
      ...
      expect(commit.firstCall.args).to.eql([
        "setTestSettings",
        { outerColorContour: [0, 1, 2, 3], opacityContour: 3 },
      ]);
      expect(dispatch.calledOnce).to.be.true;
      expect(dispatch.firstCall.args).to.eql(["updateDrawInteraction"]);
    });
  ...
  describe("setDrawType", () => {
    ...
    it("should commit 'true' to 'setFreeHand' if the id of the selectedElement equals 'drawCurve'", () => {
      target = {
        options: [{ id: "drawCurve", value: geometry }],
        selectedIndex: 0,
      };
      actions.setDrawType({ commit, dispatch }, { target });
  ...
  describe("setOpacityContour", () => {
    it("should commit as intended", () => {
      ...
      expect(commit.firstCall.args).to.eql([
        "setTestSettings",
        { colorContour: [0, 1, 2, 3.5], opacityContour: 3.5 },
      ]);
      expect(dispatch.calledOnce).to.be.true;
      expect(dispatch.firstCall.args).to.eql(["updateDrawInteraction"]);
    });
  ...
  describe("setUnit", () => {
    it("should commit as intended", () => {
      ...
      expect(commit.firstCall.args).to.eql([
        "setTestSettings",
        { unit: "km", circleRadius: 1, circleOuterRadius: 2 },
      ]);
    });
  });
});

...
import {
  ...
  featureStyle,
  handleDrawEvent,
} from "../../../store/actions/drawInteractionOnDrawEvent";
...
describe("src_3_0_0/modules/draw_old/store/actions/drawInteractionOnDrawEvent.js", () => {
  ...
  beforeEach(() => {
    ...
    createStyleStub = sinon
      .stub(createStyleModule, "createStyle")
      .returns(createdStyle);
  ...
  describe("handleDrawEvent", () => {
    ...
    it("handleDrawEvent no circle call createStyle", () => {
      ...
      expect(event.feature.set.secondCall.args[1]).to.be.deep.equal(
        createdStyle,
      );
      ...
      expect(createStyleStub.firstCall.args[1]).to.be.deep.equal(
        state.drawSymbolSettings,
      );
      expect(commit.calledOnce).to.be.true;
    ...
    it("handleDrawEvent circle: alert radius not defined", () => {
      ...
      expect(dispatch.secondCall.args[1].content).to.be.equal(
        "modules.draw_old.undefinedRadius",
      );
      expect(event.feature.set.calledTwice).to.be.true;
      expect(event.feature.set.firstCall.args[0]).to.equal("fromDrawTool");
      expect(event.feature.set.firstCall.args[1]).to.be.true;
    ...
    it("handleDrawEvent doubleCircle: alert two circles not defined", () => {
      ...
      expect(dispatch.secondCall.args[0]).to.equal("Alerting/addSingleAlert");
      expect(dispatch.secondCall.args[1].content).to.be.equal(
        "modules.draw_old.undefinedTwoCircles",
      );
      expect(event.feature.set.calledTwice).to.be.true;
      expect(event.feature.set.firstCall.args[0]).to.equal("fromDrawTool");
      expect(event.feature.set.firstCall.args[1]).to.be.true;
    ...
    it("handleDrawEvent doubleCircle: alert inner circle not defined", () => {
      ...
      expect(dispatch.secondCall.args[0]).to.equal("Alerting/addSingleAlert");
      expect(dispatch.secondCall.args[1].content).to.be.equal(
        "modules.draw_old.undefinedInnerCircle",
      );
      expect(event.feature.set.calledTwice).to.be.true;
      expect(event.feature.set.firstCall.args[0]).to.equal("fromDrawTool");
      expect(event.feature.set.firstCall.args[1]).to.be.true;
    ...
    it("handleDrawEvent doubleCircle: alert outer circle not defined", () => {
      ...
      expect(dispatch.secondCall.args[0]).to.equal("Alerting/addSingleAlert");
      expect(dispatch.secondCall.args[1].content).to.be.equal(
        "modules.draw_old.undefinedOuterCircle",
      );
      expect(event.feature.set.calledTwice).to.be.true;
      expect(event.feature.set.firstCall.args[0]).to.equal("fromDrawTool");
      expect(event.feature.set.firstCall.args[1]).to.be.true;
  ...
  describe("featureStyle", () => {
    it("featureStyle", () => {
      ...
      expect(createStyleStub.firstCall.args[0]).to.be.deep.equal(drawState);
      expect(createStyleStub.firstCall.args[1]).to.be.deep.equal(
        Object.assign({}, state.drawSymbolSettings, drawState),
      );
    });
  });
});

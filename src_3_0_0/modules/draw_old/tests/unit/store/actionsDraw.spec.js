...
describe("src_3_0_0/modules/draw/store/actionsDraw.js", () => {
  ...
  describe("createCenterPoint", () => {
    const lineFeat = new Feature({
        geometry: new LineString([
          [565647.1112172041, 5935140.711690446],
          [566028.1110114643, 5934812.628534278],
          [566546.6940647627, 5934770.295223804],
        ]),
      }),
      polygonFeat = new Feature({
        geometry: new Polygon([
          [
            [565086.1948534324, 5934664.461947621],
            [565657.6945448224, 5934738.54524095],
            [565625.9445619675, 5934357.545446689],
            [565234.3614400891, 5934346.962119071],
            [565086.1948534324, 5934664.461947621],
          ],
        ]),
      }),
      targetProjection = "EPSG:4326";
    let centerPoint, rootState;
    ...
    it("should return the center point of a polygon with the projection EPSG:4326", () => {
      centerPoint = actions.createCenterPoint(
        { rootState },
        { feature: polygonFeat, targetProjection },
      );
    ...
    it("should return the center point of a line with the projection EPSG:4326", () => {
      centerPoint = actions.createCenterPoint(
        { rootState },
        { feature: lineFeat, targetProjection },
      );
  ...
  describe("createDrawInteractionAndAddToMap", () => {
    ...
    it("commits and dispatches as expected", () => {
      ...
      expect(dispatch.firstCall.args).to.eql([
        "manipulateInteraction",
        { interaction: "draw", active: activeSymbol },
      ]);
      expect(dispatch.secondCall.args).to.eql([
    ...
    it("commits and dispatches a second set of information for drawDoubleCircle", () => {
      actions.createDrawInteractionAndAddToMap(
        { state: getState("drawDoubleCircle"), commit, dispatch, getters },
        { active: activeSymbol, maxFeatures: maxFeaturesSymbol },
      );
      ...
      expect(dispatch.args[0]).to.eql([
        "manipulateInteraction",
        { interaction: "draw", active: activeSymbol },
      ]);
      expect(dispatch.args[1]).to.eql([
        "createDrawInteractionListener",
        {
          isOuterCircle: false,
          drawInteraction: "",
      ...
      expect(dispatch.args[3]).to.eql([
        "manipulateInteraction",
        { interaction: "draw", active: activeSymbol },
      ]);
      expect(dispatch.args[4]).to.eql([
        "createDrawInteractionListener",
        {
          isOuterCircle: true,
          drawInteraction: "Two",
  ...
  describe("createDrawInteractionListener", () => {
    ...
    it("defines a drawstart and drawend function on the interaction", () => {
      actions.createDrawInteractionListener(
        { state, dispatch },
        {
          isOuterCircle: false,
          drawInteraction: "",
    ...
    it("enables the drawstart to dispatch and uses the correct parameters", () => {
      ...
      expect(dispatch.calledWithMatch("drawInteractionOnDrawEvent", "")).to.be
        .true;
    });
    ...
    it("enables the maxFeatures drawstart to dispatch, does so on maxFeatures reached", () => {
      ...
      expect(
        dispatch.calledWith(
          ...
          i18next.t("common:modules.draw_old.limitReached", { count: 5 }),
          { root: true },
        ),
      );
  ...
  describe("createModifyInteractionAndAddToMap", () => {
    it("commits and dispatches as expected", () => {
      ...
      expect(dispatch.args[0]).to.eql([
        "manipulateInteraction",
        { interaction: "modify", active: activeSymbol },
      ]);
      ...
      expect(dispatch.args[3]).to.eql([
        "createSelectInteractionModifyListener",
      ]);
  ...
  describe("createSelectInteractionAndAddToMap", () => {
    it("commits and dispatches as expected", () => {
      ...
      expect(dispatch.firstCall.args).to.eql([
        "manipulateInteraction",
        { interaction: "delete", active: activeSymbol },
      ]);
      expect(dispatch.secondCall.args).to.eql([
        "createSelectInteractionListener",
      ]);
      expect(dispatch.thirdCall.args[0]).to.eql("Maps/addInteraction");
      expect(typeof dispatch.thirdCall.args[1]).to.eql("object");
    });
  ...
  describe("deactivateDrawInteractions", () => {
    ...
    it("should push the interactions to an array if they are an instance of Draw and are not yet added", () => {
      ...
      expect(
        state.deactivatedDrawInteractions.indexOf(drawOne),
      ).to.be.greaterThan(-1);
      expect(
        state.deactivatedDrawInteractions.indexOf(drawTwo),
      ).to.be.greaterThan(-1);
  ...
  describe("redoLastStep", () => {
    ...
    it("should commit and dispatch if the redoArray contains a value", () => {
      ...
      expect(dispatch.secondCall.args).to.eql([
        "updateUndoArray",
        { remove: false, feature: featureToRestore },
      ]);
    });
  ...
  describe("resetModule", () => {
    ...
    it("should commit and dispatch as intended", () => {
      ...
      expect(commit.getCall(1).args).to.eql([
        "setDrawType",
        initialState.drawType,
      ]);
      expect(commit.getCall(2).args).to.eql([
        "setFreeHand",
        initialState.freeHand,
      ]);
      expect(commit.getCall(3).args).to.eql([
        "setPointSize",
        initialState.pointSize,
      ]);
      expect(commit.getCall(4).args).to.eql(["setSymbol", iconSymbol]);
      expect(commit.getCall(5).args).to.eql([
        "setWithoutGUI",
        initialState.withoutGUI,
      ]);
      expect(commit.getCall(6).args).to.eql([
        "setDownloadDataString",
        initialState.download.dataString,
      ]);
      expect(commit.getCall(7).args).to.eql([
        "setDownloadFeatures",
        initialState.download.features,
      ]);
      expect(commit.getCall(8).args).to.eql([
        "setDownloadFileName",
        initialState.download.fileName,
      ]);
      expect(commit.getCall(9).args).to.eql([
        "setDownloadSelectedFormat",
        initialState.download.selectedFormat,
      ]);
      ...
      expect(dispatch.getCall(1).args).to.eql([
        "manipulateInteraction",
        { interaction: "draw", active: false },
      ]);
      expect(dispatch.getCall(2).args).to.eql([
        "removeInteraction",
        drawInteraction,
      ]);
      expect(dispatch.getCall(3).args).to.eql([
        "removeInteraction",
        drawInteractionTwo,
      ]);
      expect(dispatch.getCall(4).args).to.eql([
        "removeInteraction",
        modifyInteraction,
      ]);
      expect(dispatch.getCall(5).args).to.eql([
        "removeInteraction",
        selectInteractionModify,
      ]);
      expect(dispatch.getCall(6).args).to.eql([
        "removeInteraction",
        selectInteraction,
      ]);
      expect(dispatch.getCall(7).args).to.eql([
        "removeInteraction",
        modifyAttributesInteraction,
      ]);
      expect(dispatch.getCall(8).args).to.eql([
        "removeInteraction",
        selectInteractionModifyAttributes,
      ]);
    });
  ...
  describe("toggleInteraction", () => {
    ...
    it("should enable the draw interactions and disable the other interactions if the given interaction equals 'draw'", () => {
      ...
      expect(dispatch.args[0]).to.eql([
        "manipulateInteraction",
        { interaction: "draw", active: true },
      ]);
      expect(dispatch.args[1]).to.eql([
        "manipulateInteraction",
        { interaction: "modify", active: false },
      ]);
      expect(dispatch.args[2]).to.eql([
        "manipulateInteraction",
        { interaction: "modifyAttributes", active: false },
      ]);
      expect(dispatch.args[3]).to.eql([
        "manipulateInteraction",
        { interaction: "delete", active: false },
      ]);
    ...
    it("should enable the modify interaction and disable the other interactions if the given interaction equals 'modify'", () => {
      ...
      expect(commit.calledThrice).to.be.true;
      expect(commit.firstCall.args).to.eql(["setFormerInteraction", "none"]);
      expect(commit.secondCall.args).to.eql([
      ...
      expect(dispatch.args[1]).to.eql([
        "manipulateInteraction",
        { interaction: "modify", active: true },
      ]);
      expect(dispatch.args[2]).to.eql([
        "manipulateInteraction",
        { interaction: "modifyAttributes", active: false },
    ...
    it("should enable the select interaction and disable the other interactions if the given interaction equals 'delete'", () => {
      ...
      expect(dispatch.args[3]).to.eql([
        "manipulateInteraction",
        { interaction: "delete", active: true },
      ]);
    });
  });
  ...
  describe("updateDrawInteraction", () => {
    ...
    it("should commit and dispatch as intended", () => {
      ...
      expect(dispatch.calledTwice).to.be.true;
      expect(dispatch.firstCall.args).to.be.eql([
        "removeInteraction",
        drawInteraction,
      ]);
      expect(dispatch.secondCall.args).to.be.eql([
    ...
    it("should also dispatch and commit for the second drawInteraction if its type is not 'undefined'", () => {
      ...
      expect(dispatch.thirdCall.args).to.be.eql([
        "createDrawInteractionAndAddToMap",
        { active: true },
      ]);
    });

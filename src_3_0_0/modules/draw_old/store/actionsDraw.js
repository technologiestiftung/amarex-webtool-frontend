...
import {
  ...
  handleDrawEvent,
} from "./actions/drawInteractionOnDrawEvent";
...
const initialState = JSON.parse(JSON.stringify(stateDraw)),
  actions = {
    ...
    addDrawStateToFeature({ getters }, feature) {
      ...
      const { styleSettings } = getters,
        ...
        symbol = getters.symbol
          ? JSON.parse(JSON.stringify(getters.symbol))
          : "",
        zIndex = JSON.parse(JSON.stringify(getters.zIndex)),
        imgPath = getters.imgPath
          ? JSON.parse(JSON.stringify(getters.imgPath))
          : "",
    ...
    clearTooltip({ rootState }, tooltip) {
      ...
      mapCollection
        .getMap(rootState.Maps.mode)
        .un("pointermove", tooltip.get("mapPointerMoveEvent"));
    ...
    createCenterPoint({ rootState }, { feature, targetProjection }) {
      ...
      if (featureType === "LineString") {
        if (targetProjection !== undefined) {
          centerPointCoords = crs.transform(
    ...
    createDrawInteractionAndAddToMap(
      ...
      if (
        state.selectInteractionModify?.getFeatures()?.getArray()?.length > 0
      ) {
      ...
      dispatch("manipulateInteraction", {
        interaction: "draw",
        active: active,
      });
      dispatch("createDrawInteractionListener", {
        isOuterCircle: false,
        drawInteraction: "",
        maxFeatures,
      });
    ...
    createDrawInteractionListener(
      ...
      { isOuterCircle, drawInteraction, maxFeatures },
    ) {
      ...
      interaction.on("drawstart", (event) => {
        ...
        if (
          (!tooltip && state?.drawType?.id === "drawCircle") ||
          ...
          mapCollection
            .getMap(rootState.Maps.mode)
            .on("pointermove", tooltip.get("mapPointerMoveEvent"));
          event.feature
            .getGeometry()
            .on("change", tooltip.get("featureChangeEvent"));
        }
      });
      if (maxFeatures && maxFeatures > 0) {
        interaction.on("drawstart", () => {
          const featureCount = main
            .getApp()
          ...
          if (featureCount > maxFeatures - 1) {
            const alert = {
              category: "error",
              content: i18next.t("common:modules.draw_old.limitReached", {
                count: maxFeatures,
              }),
              displayClass: "error",
              multipleAlert: true,
      ...
      interaction.on("drawend", (event) => {
        ...
        dispatch("uniqueID").then((id) => {
          ...
          if (tooltip) {
            event.feature
              .getGeometry()
              .un("change", tooltip.get("featureChangeEvent"));
          ...
          if (
            typeof Config.inputMap !== "undefined" &&
            Config.inputMap !== null
          ) {
            const { feature } = event,
            ...
            dispatch("createCenterPoint", { feature, targetProjection }).then(
              (coordinates) => {
                dispatch("downloadFeaturesWithoutGUI", {
                  prmObject: { targetProjection },
                  currentFeature: feature,
                }).then((geoJSON) =>
                  postDrawEnd({ type: "Point", coordinates }, geoJSON),
                );
              },
            );
          }
    ...
    createModifyAttributesInteractionAndAddToMap({ commit, dispatch }, active) {
      const modifyInteraction = createModifyAttributesInteraction(
          main.getApp().config.globalProperties.$layer,
        ),
        selectInteractionModify = createSelectInteraction(
          main.getApp().config.globalProperties.$layer,
          10,
        );
     ...
     * @returns {void}
     */
    createModifyAttributesInteractionListener({
      rootState,
      state,
      dispatch,
      commit,
        ...
        event.features.getArray().forEach((feature) => {
          ...
          feature.getGeometry().once("change", async () => {
            ...
            if (
              ...
              state.selectedFeature.ol_uid !== feature.ol_uid
            ) {
              await dispatch(
      ...
      state.modifyAttributesInteraction.on("modifyend", (event) => {
        ...
        if (
          ...
          dispatch("createCenterPoint", {
            feature: event.features.getArray()[0],
            targetProjection: Config.inputMap.targetProjection,
          }).then((centerPointCoords) => {
            dispatch("downloadFeaturesWithoutGUI", {
              prmObject: { targetProjection: Config.inputMap.targetProjection },
              currentFeature: event.feature,
            }).then((geoJSON) =>
              postDrawEnd(
                { type: "Point", coordinates: centerPointCoords },
                geoJSON,
              ),
            );
          });
    ...
    createModifyInteractionAndAddToMap({ commit, dispatch }, active) {
      const modifyInteraction = createModifyInteraction(
          main.getApp().config.globalProperties.$layer,
        ),
        selectInteractionModify = createSelectInteraction(
          main.getApp().config.globalProperties.$layer,
     ...
     * @returns {void}
     */
    createSelectInteractionModifyAttributesListener({
      state,
      commit,
      dispatch,
    }) {
      state.selectInteractionModifyAttributes.on("select", (event) => {
        if (
          state.currentInteraction !== "modifyAttributes" ||
        ...
        setTimeout(() => {
          state.selectInteractionModifyAttributes.getFeatures().clear();
          const textStyle = createSelectedFeatureTextStyle(
            state.selectedFeature,
          );
    ...
    createSelectInteractionModifyListener({ state, commit, dispatch }) {
      state.selectInteractionModify.on("select", (event) => {
        if (state.currentInteraction !== "modify" || !event.selected.length) {
          if (
            state.drawType.id === "writeText" ||
            state.drawType.id === "drawSymbol"
          ) {
    ...
    createSelectInteractionListener({ state, dispatch }) {
      state.selectInteraction.on("select", (event) => {
        ...
        main
          .getApp()
          .config.globalProperties.$layer.getSource()
          .removeFeature(event.selected[0]);
    ...
    manipulateInteraction({ state }, { interaction, active }) {
      if (interaction === "draw") {
        if (
          typeof state.drawInteraction !== "undefined" &&
          state.drawInteraction !== null
        ) {
          state.drawInteraction.setActive(active);
        }
        if (
          typeof state.drawInteractionTwo !== "undefined" &&
          state.drawInteractionTwo !== null
        ) {
          state.drawInteractionTwo.setActive(active);
        }
      } else if (interaction === "modify") {
        if (
          typeof state.modifyInteraction !== "undefined" &&
          state.modifyInteraction !== null
        ) {
          state.modifyInteraction.setActive(active);
        }
        if (
          typeof state.selectInteractionModify !== "undefined" &&
          state.selectInteractionModify !== null
        ) {
          state.selectInteractionModify.setActive(active);
        }
      } else if (interaction === "modifyAttributes") {
        if (
          typeof state.modifyAttributesInteraction !== "undefined" &&
          state.modifyAttributesInteraction !== null
        ) {
          state.modifyInteraction.setActive(active);
        }
        if (
          typeof state.selectInteractionModifyAttributes !== "undefined" &&
          state.selectInteractionModifyAttributes !== null
        ) {
          state.selectInteractionModifyAttributes.setActive(active);
        }
      } else if (interaction === "delete") {
        if (
          typeof state.selectInteraction !== "undefined" &&
    ...
    redoLastStep({ state, commit, dispatch }) {
      ...
      if (
        typeof featureToRestore !== "undefined" &&
        ...
        main
          .getApp()
          .config.globalProperties.$layer.getSource()
          .addFeature(featureToRestore);
        main
          .getApp()
          .config.globalProperties.$layer.getSource()
          .getFeatureById(featureId)
          .setStyle(featureToRestore.getStyle());
        dispatch("updateRedoArray", { remove: true });
        dispatch("updateUndoArray", {
          remove: false,
    ...
    resetModule({ state, commit, dispatch, getters }) {
      ...
      if (state.addFeatureListener.listener) {
        main
          .getApp()
          .config.globalProperties.$layer.getSource()
          .un("addFeature", state.addFeatureListener.listener);
      }
    },
    /**
     ...
     * @returns {void}
     */
    saveAsCurrentFeatureAndApplyStyleSettings: async (
      ...
      if (typeof drawState === "undefined") {
        ...
        if (!drawType && drawState.fontSize) {
          ...
          drawState = Object.assign(getters.styleSettings, drawState, {
            drawType: drawType,
          });
      ...
      if (
        ...
        styleSettings.colorContour === undefined
      ) {
        try {
      ...
      Object.assign(styleSettings, drawState);
    ...
    updateCircleRadiusDuringModify({ state, rootState, dispatch }, radius) {
      if (
        state.currentInteraction === "modify" &&
        ...
        circleCalculations.calculateCircle(
          { feature },
          circleCenter,
          radius,
      ...
      if (
        ...
        state.selectedFeature.setStyle(function (feature) {
          if (
            ...
            return createStyleModule.createStyle(
              feature.get("drawState"),
              styleSettings,
            );
          }
          return undefined;

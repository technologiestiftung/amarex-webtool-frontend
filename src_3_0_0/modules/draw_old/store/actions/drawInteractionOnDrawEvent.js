 ...
 * @returns {void}
 */
export function drawInteractionOnDrawEvent(
  ...
  const styleSettings = JSON.parse(
      JSON.stringify(state[state.drawType.id + "Settings"]),
    ),
  ...
  commit(
    ...
    main
      ...
      .once("addfeature", (event) => dispatch("handleDrawEvent", event)),
  );
  if (
...
export function handleDrawEvent({ state, commit, dispatch, rootState }, event) {
  ...
  if (circleMethod === "defined" && drawType.geometry === "Circle") {
    const innerRadius = !isNaN(styleSettings.circleRadius)
        ? parseFloat(styleSettings.circleRadius)
        : null,
      outerRadius = !isNaN(styleSettings.circleOuterRadius)
        ? parseFloat(styleSettings.circleOuterRadius)
        : null,
      circleRadius = event.feature.get("isOuterCircle")
        ? outerRadius
        : innerRadius,
  ...
  event.feature.set(
    "invisibleStyle",
    createStyleModule.createStyle(
      event.feature.get("drawState"),
      styleSettings,
    ),
  );
  ...
  return (feature) => {
    if (feature.get("isVisible")) {
      return createStyleModule.createStyle(
        ...
        Object.assign({}, styleSettings, feature.get("drawState")),
      );
    }
    return undefined;
  };

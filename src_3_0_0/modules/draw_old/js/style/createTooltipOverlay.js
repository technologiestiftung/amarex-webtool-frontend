...
function createTooltipOverlay({ getters, commit, dispatch }) {
  ...
  const decimalsForKilometers = 3,
    ...
    factory = {
      ...
      featureChangeEvent: (evt) => {
        if (
          (autoUnit && evt.target.getRadius() > 500) ||
          (!autoUnit && styleSettings.unit === "km")
        ) {
          tooltip.getElement().innerHTML =
            thousandsSeparator(
              Math.round(evt.target.getRadius()).toFixed(decimalsForKilometers),
            ) + " km";
        } else {
          tooltip.getElement().innerHTML =
            thousandsSeparator(Math.round(evt.target.getRadius())) + " m";
        }

        setters.setCircleRadius(
          { getters, commit, dispatch },
          Math.round(evt.target.getRadius()),
        );
      },
    };
  ...
  if (
    styleSettings?.tooltipStyle &&
    Object.keys(styleSettings.tooltipStyle).length !== 0
  ) {

...
export default {
  ...
  initialize({ commit, dispatch, state }) {
    ...
    map.on("pointermove", (evt) => {
          ...
          if (feature.getProperties().features) {
            feature.get("features").forEach((clusteredFeature) => {
              featuresAtPixel.push(
                createGfiFeature(layer, "", clusteredFeature),
              );
            });
          } else {
      ...
      map
        ...
        ) // point features are already caught by map.forEachFeatureAtPixel loop
        .forEach((layer) => {
          if (
            layer.get("gfiAttributes") &&
            layer.get("gfiAttributes") !== "ignore"
          ) {
            /**
            ...
            const hitBox = buffer(
              ...
              (layer.get("hitTolerance") || 1) *
                Math.sqrt(mapCollection.getMapView("2D").getResolution()),
            );

            if (
              layer.get("typ") === "VectorTile" &&
              layer.get("renderer") === "webgl"
            ) {
              ...
              layer
                .getSource()
                ?.forEachFeatureIntersectingExtent(hitBox, (feature) => {
                  featuresAtPixel.push(createGfiFeature(layer, "", feature));
                });
            }
          }
  ...
  setMouseHoverLayers({ commit, rootGetters }) {
    commit(
      "setMouseHoverLayers",
      rootGetters.allLayerConfigs.filter((layer) => {
  ...
  filterInfos({ commit, state }, features) {
    ...
    if (features.length > 0) {
      features.forEach((feature) => {
        const configInfosForFeature = state.mouseHoverInfos.find(
          (info) => info.id === feature.getLayerId(),
        );

        if (configInfosForFeature) {
          const featureProperties = feature.getProperties(),
            featureInfos =
              typeof configInfosForFeature.mouseHoverField === "string"
                ? configInfosForFeature.mouseHoverField
                : configInfosForFeature.mouseHoverField.filter((key) =>
                    Object.keys(featureProperties).includes(key),
                  ),
            featureDetails = [];

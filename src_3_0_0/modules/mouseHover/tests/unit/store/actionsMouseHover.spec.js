...
describe("src_3_0_0/modules/mouseHover/store/actionsMouseHover", () => {
  ...
  describe("initialize", () => {
    it("initializes the mouseHover module", () => {
      ...
      expect(commit.args[2]).to.eql([
        "setInfoText",
        "common:modules.mouseHover.infoText",
      ]);
    });
  });
  ...
  describe("filterInfos", () => {
    ...
    it("filters the infos from each feature", () => {
      state.mouseHoverInfos = [
        { id: "layerId-1", mouseHoverField: ["name", "id"] },
        { id: "layerId-2", mouseHoverField: ["name", "kategorie"] },
      ];

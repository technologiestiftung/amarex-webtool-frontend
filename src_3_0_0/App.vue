...
} from "./core/urlParams/js/urlParams";
    ...
    checkVueObservation() {
      ...
      if (process.env.NODE_ENV === "development") {
        setInterval(() => {
          ...
          if (map3d?.__ob__) {
            console.error(
              "map3d is observed by vue:",
              map3d,
              " This leads to extreme performance problems, and the cause must be eliminated. This can have several causes: the map3D is in vuex-state or is available via getter. Layers are in the state or in the getters and reference the map3D.",
            );
          }
        }, 5000);
      }
...
<template>
  ...
  >
    ...
    <MenuContainer
      v-if="
        allConfigsLoaded && addonsLoaded && mainMenu && uiStyle !== 'SIMPLE'
      "
      side="mainMenu"
    />
    <MenuToggleButton

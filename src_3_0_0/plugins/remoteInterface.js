...
export default {
  install(options) {
    if (options === undefined || typeof options.postMessageUrl !== "string") {
      console.error(
        'RemoteInterface could not been installed: Param "postMessageUrl" missing.',
      );
      return;
    }
    ...
    window.addEventListener("message", (event) => {
      ...
      if (
        event.data.namespace === undefined ||
      ...
      try {
        ...
        console.error(
          'RemoteInterface could not call "' +
            fullActionName +
            '". Please ensure this action is available.',
        );
      }
    });
    ...
    main.getApp().config.globalProperties.$remoteInterface = {
      sendMessage: (params) => {
        if (params instanceof Object === false) {
          console.error(
            "RemoteInterface sendMessage error: Given param is not an Object.",
          );
          return;
        }

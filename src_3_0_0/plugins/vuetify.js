...
export async function instantiateVuetify() {
  ...
  if (typeof Config.vuetify !== "string") {
    console.error(
      "Vuetify: Path to vuetify must be provided. Please check your config.js",
    );
    return undefined;
  }
  ...
  try {
    ...
    console.error(
      `Vuetify cannot be loaded from path ${Config.vuetify}. Please check the path set in the portal's config.js and try again.`,
      e,
    );
    return undefined;
  }

  ...
  if (config) {
    const addons = config.map(async (addonKey) => {
      try {
        ...
        if (
          addonConf &&
          Object.prototype.hasOwnProperty.call(addonConf, "type")
        ) {
        ...
        console.warn(e);
        console.warn(
          `The module ${addonKey} does not include a Vue-component and/or vuex-store-module. Please make sure the folder contains a ${addonKey}.vue and ${addonKey}.js file. Maybe it is an backbone-addon.`,
          e,
        );
      }
    });
...
async function loadControls(addonKey) {
  const addon = await loadAddon(addonKey),
    name =
      addon.component.name.charAt(0).toLowerCase() +
      addon.component.name.slice(1);
  ...
  store.commit("Controls/registerControl", {
    name: name,
    control: addon.component,
  });
...
async function loadGfiThemes(addonKey) {
  ...
  main
    .getApp()
    .config.globalProperties.$gfiThemeAddons.push(addon.component.name);
  // register the vuex store module
  if (addon.store) {
